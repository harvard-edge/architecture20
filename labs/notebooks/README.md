...
| [08](lab_08_run_the_loop.py) | Ch8, Run the Loop | simulator integration study | 45 min | Completed receipt and bounded commitment decision |
...
| **Implement** | Labs 04, 05, 06, and 08 | Replay bindings, executable configuration, tool-produced reports, and validated receipt. | The learner connects artifacts to an existing wrapper rather than building a new tool path, compiler change, or RTL artifact. |
| **Evaluate** | Labs 03 through 10, with process diagnosis in Lab 02 | Rate diagnosis, claim audit, evidence packet, gate results, provenance, and scoped evaluation record. | Most tasks use one compact workload and one simulator, so broader workload and fidelity claims remain outside scope. |
