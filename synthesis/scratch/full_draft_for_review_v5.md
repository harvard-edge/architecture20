...
> XRBench-class real-time mobile XR workload. Realize it as a vector-capable CPU, fixed-function accelerator, reconfigurable spatial array (CGRA/RDA), or Processing-in-Memory (PIM) data-centric block under a 3 W TDP target in a 3 nm-class LP
...
1. **Ensuring reproducibility across non-deterministic AI methods.**
   Building on the "loop contract" defined in the the @sec-why-the-prompt-spans-the-stack section, we must figure out how an architectural claim remains reviewable and defensible even when the underlying generative methods are inherently stochastic or frequently updated.

2. **What constitutes an adversarial stress-test for an AI-assisted design loop?**
   While the the @sec-efficiency-claims-need-rejectable-loops section argues for making efficiency multidimensional, it is unknown how to systematically design "red-team" workloads and constraints specifically engineered to trick an AI method into proposing plausible but fatally flawed hardware.

3. **Standardizing the cost of evidence.**
   The the @sec-architecture-development-spans-three-roles section outlines generation, prediction, and optimization, but we lack a standardized currency for the "feedback budget." We need to establish how to rigorously quantify and trade off the computational and financial costs of querying large models versus running cycle-accurate simulators.

4. **Failuring taxonomies become shared infrastructure.**
   The the @sec-from-architecture-10-to-architecture-20 section emphasizes designing explicit loops with rejection authority, yet we lack a universal taxonomy for *why* an AI-generated candidate was rejected. Structuring and sharing these negative traces across the community is essential to prevent different AI-assisted systems from independently repeating the same architectural mistakes.

::: {.callout-carry-forward title="What to carry forward"}
...
![**The Scissors Gap:** The gap opens when the capacity to generate candidate designs (growing exponentially with generative methods) diverges from the capacity to produce trusted, high-fidelity feedback (which remains flat or scales slowly). This visually justifies why the loop requires strict rejection gates.](images/F2-scissors-gap){#fig-scissors-gap-intro width="100%" fig-alt="Line chart showing candidate generation capacity diverging sharply from trusted feedback capacity over time."}
...
1. **Standardizing a machine-readable format for cross-organizational design loop replay.**
...
2. **Legacying EDA and simulation tools automatically enforce environment contracts.**
...
narrow action of deciding what to escalate. Crucially, generative methods are notorious for proposing "spaghetti logic"—RTL that simulates cleanly but contains massive combinational depth. While high-level trade-offs like power and area act like a continuous waterbed, physical routing congestion acts like a cliff. At high placement utilization, detours explode and the design catastrophically fails timing and design rule checks (DRCs).

To catch this early, the loop must shift-left its physical constraints by deploying a **Physical Fidelity Ladder**:
1. **Rung 1 (Seconds) — Structural Proxies:** Abstract syntax tree (AST) level checks for combinational depth and maximum fan-out.
2. **Rung 2 (Minutes) — Fast Logical Synthesis:** Zero-wireload mapping to check gate count and idealized timing.
3. **Rung 3 (Hours) — Predictive Placement:** Floorplan-aware congestion maps and early IR-drop estimation.
4. **Rung 4 (Days) — Full Sign-off:** Final routing, extraction, and DRC layout versus schematic (LVS).

An AI agent must earn the right to launch a multi-hour EDA synthesis run (Rung 3) by first passing the mathematical proofs of Rung 1. 

```python
def early_physical_rejection_gate(rtl_candidate, target_freq, pdk_proxy):
    """
    Rung 1 Commitment Gate: Rejects un-implementable RTL in milliseconds 
    before authorizing a costly EDA synthesis run.
    """
    ast = parse_rtl(rtl_candidate)
    
    # The Timing Cliff: Combinational Depth Check
    max_depth = get_max_combinational_levels(ast)
    if max_depth > pdk_proxy.max_levels_for(target_freq):
        return Reject(
            reason=f"Fails timing proxy: Logic depth {max_depth} exceeds PDK limit.",
            action="Insert pipeline registers or re-architect datapath."
        )
    return AuthorizeHigherCommitmentRung()
```

The proxy is not evidence for an implementation commitment until stronger feedback confirms the gate. Proxy
...
rankings before commitment. Its power column is frame energy divided by the frame time, an average-power check against the 3\ W envelope. (Note that average power itself is merely a higher-level proxy; a true rejection gate must eventually include thermal transients, as chips melt from peak power, not average power.) For highly regular spatial accelerators, analytical models like Timeloop are not just cheap proxies; they provide high-fidelity evidence for data movement energy, bridged to architecture-level component estimates via plugins like Accelergy. However, they are *not* physical signoff tools—they cannot capture routing congestion, IR drop, or true timing closure, which is where the physical limits of a 3 nm node will actually bite. Furthermore, evaluating any spatial architecture requires finding an optimal dataflow mapping; the loop-nest mapping space is astronomically large, meaning the AI loop must explicitly handle mapping search as a massive, decoupled obligation from hardware generation itself. It stands in for this class of model: a dataflow analysis like Timeloop [@ParasharEtAl2019Timeloop], a calibrated pre-RTL estimator like Aladdin [@ShaoEtAl2014Aladdin], or a cycle-accurate simulator such as gem5 [@BinkertEtAl2011Gem5] or FPGA-accelerated FireSim [@KarandikarEtAl2018FireSim].
...
This process is **Progressive Lowering**. When the AI agent decides to add a custom compute unit, it does not just output Verilog. It outputs *Intermediate Representation (IR) dialects*. It generates the new software dialect, the lowering rules to map high-level math into it, and the hardware dialect (using frameworks like CIRCT or MLIR) that implements it in silicon. The rejection authority is no longer just a simulator failing dynamically; it is the MLIR verifier proving statically that the lowering from software IR to hardware IR is semantically invalid.
...
An autonomous or AI-assisted system, however, expects to run end-to-end based on explicit inputs. If the rules are tacit, the AI will either stall out due to missing context or, worse, hallucinate a mathematically optimal design that completely violates the unwritten physical or software realities of the system.
...
By translating hallway conversations and senior-engineer intuition into formal **Environment Contracts** and **Rejection Gates**, the loop can finally run autonomously and safely reject bad designs *before* they waste simulation time.
...
evaluation:
  ...
  redacted_reason: 
    ...
    sub_category: "SETUP_TIME"
    relative_impact: "Critical path degraded by >15% vs baseline"
    # Note: Absolute frequency and NDA PDK details are redacted.
lessons_learned:
  - "Non-linear routing congestion in high-associativity arrays negates IPC gains in XR workloads."
```

This redacted evidence ledger acts as a small public record that
...
1. **Formally verifying redacted loop integrity.**
...
2. **Generalizing models from federated negative traces.**
...
3. **Synthesizing evidence gates dynamically under uncertainty.**
...
4. **Preserving human accountability across continuous drift.**
