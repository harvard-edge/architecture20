...
This forces a shift in the architect's daily work. Before the AI can run the loop, the human architect must act as the knowledge extractor. 
...
As shown in @fig-tacit-extraction-bridge, the architect's job is to bridge the undocumented organization and the explicit loop state. They translate hallway conversations and senior-engineer intuition into formal **Environment Contracts** and **Rejection Gates**. By codifying this tacit knowledge into the loop's tests, the loop can finally run autonomously and safely reject bad designs *before* they waste simulation time. 
