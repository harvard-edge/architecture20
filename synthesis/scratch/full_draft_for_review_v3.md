...
Just like the mobile XR prompt, this datacenter prompt forces an AI-assisted loop to carry specific architectural state: "10T-parameter mixture-of-experts" sets the memory capacity and bandwidth bounds; "XLA/MLIR mapping" defines the software contract and rejection authority for tensor placement; "bound tail latency across 10,000 optical links" requires the loop to simulate network congestion, incast, and flow control as primary physical limits; and "synchronous faults" forces the evaluation of checkpointing and reliability overheads. The evidence ledger for this prompt must record not just the winning topology, but why alternative routing schemes or fault-tolerance mechanisms failed under scale.
