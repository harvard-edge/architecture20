...
:::: {.callout-lighthouse title="The SoC choice is already a schedule"}
**Context.** The CPU, accelerator, or SoC-block decision is not one fork in a
diagram. Each option changes the interfaces, memory path, software path,
physical constraints, and evidence the loop must collect.

**In the Lighthouse prompt.** Even the narrow slice counted here crosses compute
organization, vector or accelerator interfaces, memory hierarchy, interconnect,
voltage/frequency policy, compiler/runtime policy, and verification fidelity.

**Reader takeaway.** A count such as 12,960 is not just search-space trivia. It
is a feedback schedule: which states get cheap proxies, which get simulation,
which escalate to stronger tools, and which rejected regions must be preserved.
::::
...
the boundary where hardware choices become visible to compilers, runtimes,
libraries, operating systems, and compatibility tests.

**In the Lighthouse prompt.** A vector extension, custom accelerator path, or
SoC block is credible only if the loop can explain how code reaches it, what ABI
and toolchain assumptions it preserves, and what tests reject unsupported
semantics.

**Reader takeaway.** The ISA is not a label on the hardware box. It is part of
the evidence path for whether the proposed subsystem can actually be used.
:::
...
@sec-loop-patterns-across-stack makes it precise: the design loop is
rejection-bound, its throughput set by how cheaply it can reject, not
