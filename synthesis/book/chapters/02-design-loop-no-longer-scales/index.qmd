...
*(**Author's Note**: Norbert Wiener was an American mathematician and philosopher, recognized as the founder of cybernetics. Norbert Wiener is the father of cybernetics. This chapter argues that computer architecture is fundamentally a feedback loop, and our crisis is that manual loops are now too slow to function effectively.)* To understand how to design that loop, we must first make the pressure it faces explicit.
...
the discussion on "Architectural Claims and Design Loops" (@sec-architecture-20-ontology) names the minimum loop state that makes that
