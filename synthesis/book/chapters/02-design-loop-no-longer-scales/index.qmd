...
| Action-bound | The legal action set is too narrow, or too loose, to reach the candidate that would clear the gates. | The loop can retune parameters but cannot change the accelerator interface, so it never reaches the organization that clears both gates. | The action schema: which moves are legal, and recorded. | Widen or tighten the legal action set and record it in the loop contract. |
| Feedback-bound | Trusted feedback is too scarce or too expensive to keep up with the candidates generated. | Only a few simulation-stage estimates are affordable, so most candidates are dropped or committed unjudged. | The feedback budget and each check's cost and fidelity. | Add cheaper proxies and a fidelity ladder so scarce feedback is spent where it changes a decision. |
| Rejection-bound | The loop generates faster than it can reject, so weak candidates survive to expensive stages. | Lacking a gate, the loose accelerator that leads the proxy is not rejected until an expensive check exposes its data-movement cost. | The rejection gates and the negative-trace log. | Add explicit gates and record every rejection with its reason. |
...
@sec-the-rejection-bound returns to that last scaling law, but the pressure
starts here: a loop that can generate faster than it can reject only widens the
gap.
