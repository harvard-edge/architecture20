...
::: {.callout-field-note title="the lookup-table entries that triggered a recall" icon=false}
In 1994 a mathematician found that early Intel Pentium processors returned
incorrect results for certain floating-point divisions. The cause was small:
five of the 1,066 entries in a lookup table used by the division algorithm were
wrong. The cost was not. Intel took a \$475 million charge to replace the
affected chips, and the episode pushed the industry toward formal verification
of floating-point hardware [@IntelPentiumFDIV]. For a design loop the lesson is
sharp: no cheap check rejected the error before it reached silicon, and the
missing rejection, not the missing entries, set the price.
:::
...
::: {.callout-design-principle title="Treat feedback as the bottleneck"}
More candidates do not help a loop that cannot evaluate, reject, and justify
them. Design for the rate of trusted feedback first, and add generation only as
fast as rejection can keep up.
:::
...
If a loop's throughput is set by how cheaply it can reject rather than how much
it can generate, what is the cheapest credible rejection you could add to your
own design flow, and what would it have to be trusted not to miss?
:::
