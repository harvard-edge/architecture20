...
^[**Author\'s Note**: Norbert Wiener, the founding father of cybernetics, established that true control requires feedback. In our context, this means that computer architecture is fundamentally a feedback loop, and our current crisis is simply that manual loops have become too slow to function effectively.] To understand how to design that loop, we must first make the pressure it faces explicit.
...
move quickly. AI models change. Precision formats (e.g., FP8, INT4, block-scale formats^[Block-scale formats share a single exponent across a block of low-precision mantissas to maintain dynamic range with minimal memory footprint.]) and sparsity patterns^[Sparsity patterns describe the structured or unstructured presence of zeros in neural network weights or activations, which hardware can exploit to skip computations.] mutate wildly, meaning the hardware must place bets on data types years in advance. Compiler passes change. Kernel libraries
