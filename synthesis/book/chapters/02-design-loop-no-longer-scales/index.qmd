...
Architecture 2.0 does not break the Iron Law; it uses the AI loop as an active co-designer to negotiate across the hardware-software boundary (compiler, ISA, microarchitecture), much like the early RISC teams did. There is a loop-level echo of the Iron Law's lesson: just as instruction count inflates without moving execution time, candidate count inflates without moving loop throughput, which is set instead by how cheaply the loop can reject. @sec-loop-patterns-across-stack makes that the successor law. Architecture 2.0 elevates the quantitative method from comparing candidate artifacts to designing the data, feedback, and evidence loops that make a larger, more coupled design space tractable. In that sense, the loop becomes a first-class architecture object: something to represent, instrument, test, reject, and improve.
