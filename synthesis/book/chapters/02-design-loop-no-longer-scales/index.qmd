...
::: {.callout-question}
What must an architecture loop represent, bound, evidence, reject, and keep
human-owned before AI agents can participate credibly under rising design
pressure?
:::
...
This is not the first time architecture has had to redesign its loop. Read each
row in @tbl-historical-loop-transformations as one kind of state, rule, or
rejection path that would have to be exposed before an agentic loop could help
without weakening the evidence standard. The
...
**Takeaway.** The ISA is part of the evidence path for whether the proposed
...
missing rejection, not the missing entries, set the price. An agentic loop would
need the table-generation assumptions, independent division checks, failing-case
coverage, and a named owner for accepting or rejecting the residual risk.
:::
...
Research questions for the strained-loop regime:

1. What minimum represented loop state must an agentic architecture flow preserve
   for a candidate to be reviewable under scarce feedback?
2. How can teams turn low-resource, proprietary, and missing-failure design data
   into reusable evidence without overstating what the data supports?
3. What makes a rejection cheap enough to improve loop throughput but credible
   enough not to discard the wrong candidates?
4. How should workload and data characterization be versioned when software
   stacks, benchmarks, models, and deployment policies drift faster than silicon
   cycles?
:::

Carry forward the pressure test: classical loops still work, but they strain
when candidate scale and feedback cost grow together. The bottleneck is trusted
feedback, not idea generation. Specialization, software drift, physical
constraints, and verification all add loop state, and the scissors gap is the
gap between expanding design and evidence demand and limited review capacity.
AI helps only when the loop records state, failures, evidence, rejection
authority, and the human owner of each commitment. @sec-architecture-20-ontology
names the minimum loop state needed to make that pressure inspectable.
