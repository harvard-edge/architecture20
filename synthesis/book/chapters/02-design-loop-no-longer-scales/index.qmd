# When Classical Architecture Design Loops Strain {#sec-design-loop-no-longer-scales}
...
::: {.callout-question}
When do classical architecture design loops strain because specialization, software
...
Design-loop pressure names the condition. The scissors gap names the visible
failure mode: pressure becomes a widening gap when trusted feedback cannot keep
up with the choices and evidence demands the loop has created.
...
cadence itself as a design object. Intel's tick-tock model is the cleanest
familiar example. A "tick" moved a
...
later in this chapter make the pattern concrete: autotuning^[Autotuning is the process of automatically searching for optimal program or system parameters.] treats measurements
...
:::: {.callout-field-note title="the illustrative count is already a schedule"}
...
kind of loop burden. Timeloop, an analytical framework for evaluating and mapping DNN accelerators, makes the mapping case especially concrete: for a
7D CNN on a four-level architecture, its unconstrained mapspace expression
contains $(7!)^4 \times (2^4)^3$ before co-factor choices are counted: the
$(7!)^4$ factor counts loop-order permutations of the seven tensor dimensions at
each of the four memory levels, and $(2^4)^3$ counts the level-bypass choices
that decide which tensors skip which levels [@ParasharEtAl2019Timeloop].
...
than fall, a software-era instance of Jevons' paradox.^[Jevons' paradox, from
nineteenth-century resource economics, is the observation that making a resource
cheaper to use can raise its total consumption instead of lowering it.] The
...
to repay the silicon. The loop, not the artifact, is what carries that evidence.

The scale of that target is not static either. The compute behind notable AI
systems has grown by roughly twenty orders of magnitude in six decades
(@fig-ai-compute-growth), so the workloads an architecture must serve are not
only shifting but growing far faster than any hardware cadence can follow.

```{python}
#| label: fig-ai-compute-growth
#| fig-cap: |
#|   **AI compute demand is a fast-moving target:** Training compute for notable AI systems has grown by roughly twenty orders of magnitude since the Perceptron, with a sharp acceleration in the deep-learning era after 2012 [@SevillaEtAl2022ComputeTrends]. The architecture that serves these systems is designed against a workload whose scale and shape change far faster than a silicon program can.
#| out-width: "78%"
#| fig-alt: "Log-scale plot of AI training compute by year, rising about twenty orders of magnitude from the 1958 Perceptron to GPT-4 in 2023, with a steeper slope after 2012."
...
points = [
    (1958, 7e5, "Perceptron"),
    (2012, 4.7e17, "AlexNet"),
    (2018, 2.9e20, "BERT-Large"),
    (2020, 3.1e23, "GPT-3"),
    (2023, 2.1e25, "GPT-4"),
]
years = [p[0] for p in points]
flops = [p[1] for p in points]

fig, ax = plt.subplots(figsize=(4.65, 2.4))
fig.subplots_adjust(left=0.16, right=0.96, top=0.95, bottom=0.18)
ax.plot(years, flops, color=COLORS["blue"], linewidth=1.6, zorder=1)
ax.scatter(years, flops, s=22, color=COLORS["blue"], zorder=2)
ax.set_yscale("log")
ax.set_ylim(1e4, 1e27)
...
for x, y, lab in points:
    ax.annotate(lab, (x, y), textcoords="offset points", xytext=(5, -1), fontsize=6.0, color=COLORS["ink"])
...
Drifting workloads are only one source of pressure from below; physical limits
push up on the loop just as hard. Architecture does not sit above physical
...
changed. The bottleneck is trusted feedback: preserving invalid candidates,
...
This bottleneck is not only a diagnosis. It has a bound.
@sec-loop-patterns-across-stack makes it precise as the Rejection Bound, which
states that a design loop's throughput is set by how cheaply it can reject, not
by how much it can generate. Adding proposals to a loop whose trusted feedback
cannot keep up widens the scissors gap rather than closing it.
...
Those loop requirements are precisely the conditions that most AI systems are
allowed to take for granted. Many successful AI systems are built in domains
with abundant data, cheap feedback, stable labels, and clear losses. Computer architecture violates that
...
- **Next loop state:** @sec-architecture-20-ontology names the minimum loop state
  needed to make that pressure inspectable.
:::
