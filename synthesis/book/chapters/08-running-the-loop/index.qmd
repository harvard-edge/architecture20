...
| Legal actions | Instantiate one of the three candidate organization records, run the proxy screen, escalate survivors to the simulation-stage estimate, reject only on declared deadline/power gates, and advance a survivor only to RTL-level study; changing workload scope or claiming implementation readiness is outside this turn. |
...
**In the Lighthouse prompt.** The RISC-V-compatible software path, XRBench
slice, 8\ ms deadline, and 3\ W envelope are fixed. The loop varies compute
organization and interface cost.

**Deferred evidence.** Compiler/runtime, RTL, thermal, and physical-design
evidence are deferred to the next stage.
...
estimate standing in for cycle-level and power evidence, which supports an
experimental commitment, not an implementation or a tapeout. The architect's decision is therefore bounded:
...
is a loop receipt: the surviving state, the evidence that supports it, the
...
1. What state must a design loop preserve so an escalation policy can decide,
   under a fixed feedback budget, whether a candidate should advance, be
   rejected, or receive a stronger interface- and memory-aware check?
2. How can agentic design loops using multi-fidelity feedback detect when a
   cheap proxy is likely to reverse the true ranking, especially for candidates
   whose viability depends on data movement, software path, and interface cost?
3. What is the smallest privacy-preserving loop receipt that lets other
   researchers reconstruct the task slice, workload coverage, candidate records,
   tool fidelity, uncertainty, rejection reasons, and commitment boundary without
   exposing proprietary inputs?
4. How should workload records and negative traces distinguish "failed this
   slice under this interface model" from "generally bad," and reopen an old
   rejection when workloads, interfaces, software stacks, or evidence levels
   change?
:::
