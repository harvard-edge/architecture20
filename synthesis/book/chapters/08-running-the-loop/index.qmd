...
reject it. Because the survivor is a shared-memory block, it also owes what a cycle-level simulator cannot discharge: formal memory-consistency and coherence verification (@sec-architecture-environments-tool-interfaces, @sec-feedback-verification-trust). It won partly by keeping traffic on-chip, which trades the interface data-movement cost that sank the accelerator for on-chip coherence traffic the proxy never charged, so that gate stays open until the RTL study. The other two candidates are held as recorded negative traces.
