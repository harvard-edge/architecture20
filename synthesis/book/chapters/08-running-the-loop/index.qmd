...
The AI-assisted loop records the proxy ranking and escalates. *The loop escalated because proxy uncertainty crossed the threshold for physical commit: it lacks the evidence to support the claim, and therefore lacks the authority to record alternatives rejected on physical constraints.*
...
The worked example in this chapter demonstrates a bounded, rejectable turn, but automating this process at scale exposes several unsettled research directions within the Architecture 2.0 ontology. Resolving these challenges requires moving beyond manual intervention toward robust, data-driven automation.
