...
That evidence boundary is deliberate. Until this pattern is backed by measured
receipts such as XRBench traces, gem5 runs, or synthesis estimates, the
auditable move to carry forward is the loop structure, not the specific values.
...
| `prompts-agent/` | Prompt slice, agent/model/tool versions, role assignments if several agents act, generator/search configuration, invalid proposals, selection rule, and human overrides or approvals. |
