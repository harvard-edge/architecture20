...
::: {.column-margin}
*VJ's Note:* Jan L. A. van de Snepscheut, a prominent computer scientist, humorously captured the gap between theoretical models and real-world execution. In this chapter, we see this play out as we trace a real end-to-end loop where an accelerator looks like a massive winner in a fast proxy ("theory"), but fails completely when evaluated in cycle-accurate simulation ("practice").
:::
...
rediscover it. As illustrated in @fig-proxy-mirage, the seemingly optimal proxy ranking is completely overturned once high-fidelity simulation models accurately account for the data movement overhead—demonstrating the danger of proxy mismatch.
