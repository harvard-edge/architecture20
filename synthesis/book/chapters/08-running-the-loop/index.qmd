...
```{=latex}
\abstract*{The previous chapters built the parts of an Architecture 2.0 loop: representation, environment, method roles, feedback budgets, evidence ledgers, and rejection authority. This chapter runs one loop, end to end, on the lighthouse prompt. The point is not the specific numbers, which are illustrative; it is to watch a loop instantiate candidates, predict, escalate, reject, and stop at an honest commitment boundary.}
```
