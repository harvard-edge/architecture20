...
3. **Zero-Knowledge Evidence Ledgers and Replay Receipts.** Sharing the evidence ledger (@tbl-worked-loop-ledger) and replay receipts (@tbl-loop-replay-receipt) across organizational boundaries inherently risks leaking proprietary baselines and sensitive workload distributions. We require privacy-preserving formulations—leveraging zero-knowledge proofs or secure multi-party computation—that allow external tools, reviewers, or competing vendors to cryptographically audit candidate lineage, fidelity, and rejection authority without exposing the underlying intellectual property.

4. **Ontological Resuscitation of Stale Rejections.** When a candidate is rejected on a strict physical constraint, it becomes a negative trace. However, treating these traces as permanent failures artificially restricts future search. Future work must design rigorous semantic protocols capable of differentiating localized, workload-specific failures from universally invalid architectures. This would enable automated optimizers to safely resuscitate stale negative traces when underlying physical constraints, process nodes, or software runtimes shift, achieving continuous architectural learning.
:::
