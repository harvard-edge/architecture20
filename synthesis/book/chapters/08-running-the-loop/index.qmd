...
::: {.callout-important title="Work in progress: the numbers are a deliberate scaffold" icon=false}
This worked loop is a work in progress. Its numbers are intentionally included
for the intellectual exercise: they are constructed in code to be inspectable,
not measured, so the reader can watch a loop instantiate, predict, escalate,
reject, and commit. Treat them as a scaffold for the loop's *shape*, not as
evidence for any organization. A future edition will replace them with measured
receipts, such as XRBench traces, gem5 runs, or synthesis estimates, once that
data is in hand; until then, the auditable move to carry forward is the loop
