...
Research questions for running one loop:

1. How can represented state, including task slice, provenance, uncertainty,
   workload coverage, and negative traces, tell a loop when to advance, reject,
   or escalate?
2. What is the smallest reusable loop receipt that lets low-resource
   architecture research accumulate evidence across workloads, candidates,
   failures, and tool fidelities without exposing proprietary state?
3. How should workload records distinguish "this candidate failed this slice"
   from "this candidate is generally bad," and detect when distribution shift
   invalidates a survivor or rejection?
4. How can negative traces be reused so future searches avoid rediscovering
   failed designs while still recognizing when new workloads, interfaces, or
   evidence levels make an old rejection obsolete?
:::
