...
::: {.callout-crux}
What must architecture data record before an AI agent can act on it and a human
...
scoring models; it is constraining what an agent may try next, what evidence
...
For an agentic design loop, sample cost is part of the action policy: it
...
candidate meets the 3\ W envelope, but because the estimate sits inside the
proxy's known error band of the target, the escalation trigger from
...
auditable: an agent acted on the belief, and a human could see why it was
overturned.
...
1. What task- and fidelity-conditioned loop-state schema, evaluated on a corpus
   of workload slices, candidate records, tool logs, and failed runs, lets an AI
   agent choose the next evaluation while an independent rejection authority can
   replay or reject that action?
2. How should an AI-facing scarce feedback-event record capture measurement
   cost, fidelity, calibration status, workload coverage, uncertainty, proxy
   validity, and the candidate or design-space regions it supports, so the loop
   can authorize only screening, escalation, or bounded commitment?
3. What workload-coverage record lets an AI-assisted loop detect benchmark
   overfitting: target population, scenario mix, input distribution, phase
   behavior, software stack, sampling policy, proxy metrics, coverage gaps,
   distribution shifts, and the gate that rejects unsupported generalization?
4. Under a fixed budget of scarce high-fidelity samples, what AI sample-selection
   environment should choose workloads, candidates, and fidelity levels next,
   while preserving rejected candidates, invalid actions, tool failures, human
   review decisions, and the claim boundary each sample can support?
:::
...
That is the point of representation in Architecture 2.0: not better
documentation in general, but constrained agentic action under
human-auditable authority.
