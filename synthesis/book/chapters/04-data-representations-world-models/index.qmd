...
into an actionable loop-state record. That means you can:
...
The reader move is not estimating tokens; it is identifying which missing
loop-state records prevent review.
...
and the rejected candidates six months later, compare the evidence boundaries,
identify who could reject the result, and see what commitment the evidence
supports. Once state exists, tools can become environments that act on that
state and return feedback the loop can interpret.
