...
- spot undocumented design state and the missing negative traces in a loop;
...
The token count matters only because it limits what an agent can infer about
...
records; vocabulary and retrieval gains do not grant an agent action authority.
...
Benchmark lineages such as SQuAD and GLUE[^fn-squad-and-glue-c04] teach a
related lesson. The Architecture 2.0 lesson is not to seek a cheap labeled row,
but to ask what a shared evaluation object must record before an agent can act.
Architecture needs shared evaluation objects too, but an architecture "example" is rarely just a cheap labeled row. It may
...
rules out, and how it should constrain the next agent action.
...
For an agent-facing architecture description, the minimum is not only fields but
...
screening, a rule system for invalid actions, and a human review policy for
...
For low-resource architecture data:

1. What task- and fidelity-conditioned state record lets an agent choose the
   next evaluation, lets an independent rejector replay or reject that action,
   and lets a human architect inspect the assumptions, constraints, provenance,
   and decision authority behind it?
2. How should a scarce feedback event record its decision value: measurement
   cost, fidelity, calibration status, workload coverage, uncertainty, proxy
   validity, and the candidate or design-space regions it supports or rules out?
3. What workload-characterization record is sufficient to detect benchmark
   overfitting: target population, scenario mix, input distribution, phase
   behavior, software stack, sampling policy, proxy metrics, coverage gaps, and
   distribution shifts?
4. Under a fixed budget of scarce high-fidelity samples, how should a loop choose
   which workloads, candidates, and fidelity levels to evaluate next while
   preserving rejected candidates, invalid actions, tool failures, and human
