...
XRBench gives a workload anchor [@KwonEtAl2023XRBench], but the representation
must also record the scenario, input distribution, frame deadline, software
stack, ISA and ABI assumptions, compiler choices, memory behavior, power model,
process assumptions, deployment context, and rejected alternatives.

**Reader takeaway.** Treat the prompt as an index into missing loop state.
XRBench is the workload anchor, RISC-V is the software-contract anchor, and the
