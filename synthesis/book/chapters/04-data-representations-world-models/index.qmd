...
Benchmark lineages such as SQuAD and GLUE[^fn-squad-and-glue-c04] teach a related lesson
...
free. Architecture teams accumulate representation debt[^fn-representation-debt-c04]. The debt appears whenever
...
representation. A surrogate[^fn-surrogate-model-c04] that predicts latency or energy from those
...
[^fn-agent-framework-c04]: **Agent framework**: A software structure that defines how autonomous AI agents perceive their environment, reason, and take actions to achieve goals.
...
[^fn-domain-adaptation-c04]: **Domain adaptation**: A machine learning technique where a model trained on one data distribution is adjusted to perform well on a different, specific target domain.
...
[^fn-squad-and-glue-c04]: **SQuAD and GLUE**: Standard benchmark datasets used to evaluate the performance of natural language processing models, where SQuAD is the Stanford Question Answering Dataset and GLUE is the General Language Understanding Evaluation.
[^fn-representation-debt-c04]: **Representation debt**: The term is adapted from the concept of "hidden technical debt" in machine learning systems [@SculleyEtAl2015Hidden], where models incur large maintenance costs when their complex data dependencies, undocumented constraints, and implicit assumptions are not managed as rigorously as code.
[^fn-surrogate-model-c04]: **Surrogate model**: A computationally lightweight machine learning approximation used to quickly predict the outcomes of a slower, more complex simulator or function.
[^fn-distribution-shift-c04]: **Distribution shift**: A situation where the data a model encounters during deployment differs significantly from the data it was trained on, often degrading performance.
