...
Fundamentally, this is a low-resource data problem, but the scarce resource is
not only the number of examples. Architecture does not need data in the same way
a web language model needs text. It needs structured records of architecture
work: what was intended, what was tried, which tool configuration was used,
which workload and software stack were assumed, what failed, what evidence was
accepted, and what the architect decided. The durable question is therefore not
only how to collect more architecture data. It is how to represent scarce
architecture data so that a loop can act on it and a human can audit it.
The architectural risk is not just small data; it is that an agent may infer
...
- spot undocumented design state and the missing negative traces in a workflow;
- identify who may accept evidence, reject candidates, escalate fidelity, and own residual risk;
...
::: {.callout-lighthouse title="The prompt is not the state"}
**Context.** This chapter's representation question is visible in the first noun
phrase of the lighthouse prompt. A sentence can name a workload and technology
target, but it does not yet represent the design state a loop can act on.

**In the Lighthouse prompt.** A low-power RISC-V compute subsystem for an
XRBench-class mobile XR workload cannot be answered from public text alone.
XRBench gives a workload anchor [@KwonEtAl2023XRBench], and RISC-V gives a
software-contract anchor.

**Representation.** The loop must also record the scenario, input distribution,
frame deadline, software stack, ISA and ABI assumptions, compiler choices,
memory behavior, power model, process assumptions, deployment context, and
rejected alternatives.

**Takeaway.** Treat the prompt as an index into missing loop state. The
design-space report is credible only when the representation carries enough
provenance, coverage, and negative traces for a human to audit.
:::
...
Domain adaptation is useful only when it is pulled back to the loop contract.
It can improve domain language and retrieval, but agentic architecture work
also needs action records, tool state, rejected alternatives, fidelity labels,
and decision authority. The medical AI lineage is a useful comparison because
it shows both the power and the limit of domain adaptation[^fn-domain-adaptation-c04]. BioBERT adapted a general language model
...
@tbl-representation-debt gives common sources of this undocumented
...
QuArch is a useful stress test for this chapter because it bridges two
representation layers: architecture knowledge that can be asked about in text
and architecture state that must be carried by a loop before an agent can act.
...
::: {#pri-make-legible .callout-design-principle title="Make architecture work legible"}
...
the loop cannot see, it cannot reason about or reject.
:::
...
Research questions for low-resource architecture data:

1. What is the smallest architecture-loop state record that lets an agent choose
   a next action, an independent rejector replay or reject that action, and a
   human architect inspect the assumptions behind it?
2. How can each scarce architecture sample carry its full evidentiary value:
   cost, fidelity, provenance, coverage, uncertainty, and the regions it rules
   out?
3. What workload and data-characterization records are sufficient to tell
   whether a candidate improved the target distribution rather than fitting a
   benchmark slice, proxy metric, or tool configuration?
4. How should loops preserve rejected candidates, invalid actions, tool
   failures, and human review decisions without turning proprietary or tacit
   knowledge into unusable anecdotes or unsafe disclosure?
:::

The reader test is whether another team could replay both the winning candidate
and the rejected candidates six months later. Once state exists, tools can
become environments that act on that state and return feedback the loop can
interpret.
