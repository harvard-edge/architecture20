...
As emphasized by treating sample cost as architecture data (@sec-sample-cost-is-architecture-data), workload provenance is critical loop state. Yet, we lack rigorous methods to detect when an automated optimizer has overfit to a specific execution phase or synthetic input distribution. A major research direction is establishing formal generalization bounds for architecture agents, preventing silent hardware failures under distribution shifts toward architecture world models (@sec-toward-architecture-world-models).

4. **How do we formulate representation learning over negative architecture traces?**
As established in our discussion of provenance, coverage, and negative traces (@sec-provenance-coverage-and-negative-traces), rejected candidates, missed timing constraints, and invalid parameters define the true boundary of the design space (@tbl-negative-traces). However, current generative models are primarily trained on successful artifacts. A critical problem for the ML-for-systems community is designing novel loss functions or preference-optimization pipelines that force models to learn from high-fidelity negative records, ensuring they actively avoid—rather than rediscover—known failure modes.
