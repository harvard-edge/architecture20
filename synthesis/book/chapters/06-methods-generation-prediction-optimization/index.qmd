...
- match a method role (generate, predict, optimize, critique, repair, verify,
  explain, or coordinate) to an architecture task;
- distinguish role composition from stronger evidence when several agents
  participate;
...
| Coordinate | Route state, tasks, tool calls, and evidence among role-specific methods or agents. | Shared state schema, authority boundaries, logs, and stop or escalation rules. | Hidden delegation, duplicated authority, or consensus without independent rejection. |
...
The same rule covers multi-agent implementations. Splitting work across a planner,
...
claim. The loop still has to say which agent owns which role, what shared state
they read and write, which actions are legal for each, which outputs require
independent rejection, and where authority returns to the architect. More agents
...
1. What loop-trace and tool-log corpus can validate the AI method-role view of
   ...
   edits, a verifier's failures, and a coordinator's routing decisions remain
   distinguishable, replayable, and tied to the strongest commitment each role
   can support?
