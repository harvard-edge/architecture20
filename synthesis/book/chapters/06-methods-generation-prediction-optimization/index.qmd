...
experiment, a critic that challenges assumptions, a verifier that checks
constraints, and a human architect who decides whether the evidence is strong
enough to proceed. The method question is not "Which AI system is best?" It
is "Which role is needed, what feedback can support it, and what evidence
would make its output credible?"
...
verification, provenance, negative traces, and human rejection authority inside
one explicit loop.
...
This is why environment work such as ArchGym is important: it makes method
...
![**Method roles are useful only inside a represented loop:** Generation proposes candidates, prediction estimates behavior, optimization chooses evaluations, critique and repair improve artifacts, and verification turns feedback into rejectable evidence. The roles can be implemented by different models, scripts, tools, or humans.](images/F7-method-role-map){#fig-method-role-map width="100%" fig-pos="H"}
...
invalid artifacts, and verification for independent rejection. The method
selection question is which role the current loop can support with feedback.

@tbl-method-role-discipline gives the checklist form. The table is
deliberately phrased as questions because method choice should be defended in
architecture terms, not only in machine-learning terms.
...
| Verify/explain | Invariant, interface contract, numerical tolerance, synthesis constraint, regression result, or evidence ledger. | Independent checks, replayable commands, tool logs, and escalation record. | Failed check, inconsistent evidence, weak fidelity, or architect refusal to commit. |
...
> **Hardware awareness.** In this lecture, hardware awareness means that a
...
Commitment-boundary awareness is the highest level because the method can
state what it is allowed to decide, what must be escalated, and why.
...
![**Hardware awareness is a staged capability:** The assessment is not whether a method can mention hardware terms, but what it can safely change, what feedback supports the change, and what independent mechanism can reject it.](images/F7b-hardware-awareness-ladder){#fig-hardware-awareness-ladder width="100%" fig-pos="t"}

The ladder is deliberately behavioral. A method that only uses hardware words is
at the vocabulary level. It climbs only when it can obey constraints, act in a
tool environment, preserve calibration evidence, and know where a human
commitment boundary stops delegation.
...
blocked or rejected. It has commitment-boundary awareness only if it can
connect workload intent, hardware resource limits, compiler/runtime
...
This distinction keeps Architecture 2.0 from becoming prompt-to-chip
rhetoric. A generated RTL fragment is not an architecture result. A generated
parameter set is not a design-space conclusion. A generated benchmark question
is not a validated workload. Generation earns its place when it feeds a loop
that can test, reject, compare, and revise.

::: {.callout-warning title="Failure mode: treating generation as design"}
...
version of a design-loop card. Those outputs are valuable because they give
...
There is a rigorous way to make this concrete. Conformal prediction turns any
surrogate, regardless of its internals, into one that emits calibrated
prediction sets: intervals that contain the true value with a user-chosen
probability under only an exchangeability assumption
[@AngelopoulosBates2021Conformal]. For an architecture loop this converts
"report confidence" from a slogan into an operation. The predictor returns not
...
envelope, or grows wider than the decision can tolerate. The guarantee is
distribution-free, so it survives the non-Gaussian residuals architecture
surrogates actually produce, and it degrades visibly under distribution shift,
which is exactly when a proxy should not be trusted.
...
[@MirhoseiniEtAl2021GraphPlacement]; independent baselines later challenged the
result (@sec-feedback-verification-trust) [@ChengEtAl2023AssessmentRL]. The important lesson for this
lecture is not that every architecture task should become RL. It is that method choice depends on state, action,
transition, feedback, and commitment structure.

A less contested example narrows the design space until the environment can
...
reported circuits reached the area-delay Pareto frontier of a production design
flow, and the resulting arithmetic units were adopted in shipping GPU silicon.
...
Sample efficiency is one of the reasons architecture is a hard AI domain.
...
from this book, the MAESTRO accelerator DSE scale, AutoTVM-style operator
tuning spaces, and the core Timeloop mapspace expression
...
fig.subplots_adjust(left=0.40, right=0.82, top=0.82, bottom=0.27)
...
draw_range_rows(ax, rows, low_key="count_low", high_key="count_high", label_x=-0.88, right_x=1.05, label_fontsize=6.15, right_fontsize=5.85, y_positions=y_positions, show_notes=False)
...
loop rarely needs the full apparatus, but naming it matters: sample efficiency
is a solved problem in form, and a loop that ignores it is leaving evidence on
the table.
...
will expose new interfaces, and benchmarks will evolve. A durable lecture
...
- **Principle:** Method choice is a loop decision: generation, prediction,
  optimization, critique, repair, and verification earn trust only when they
  match the task, representation, environment, feedback budget, fidelity
  ladder, evidence burden, and commitment level.
- **Reader test:** Can you write one sentence explaining why this method belongs
  in this role for this loop?
- **Next loop field:** Once methods can act, the next question is whether their
