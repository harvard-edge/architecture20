...
leaving evidence on the table. The loop still has to record the acquisition
...
Research questions for method roles:

1. What loop state must an Architecture 2.0 system represent so generation,
   prediction, optimization, critique, repair, and verification can share
   evidence rather than operate as disconnected tools?
2. How can architecture agents learn from low-resource feedback, including
   failed runs, invalid candidates, tool warnings, and expert rejections,
   without overfitting cheap proxies?
3. What tests can determine whether an architecture environment has enough
   rejection authority to support a method's claim before deployment or
   publication?
4. When should a loop use a generator, predictor, optimizer, critic, repair
   method, verifier, or explanation role, and how should those roles compose
