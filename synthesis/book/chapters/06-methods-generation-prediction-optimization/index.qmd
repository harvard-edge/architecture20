...
>
> — Lord Kelvin, *Popular Lectures and Addresses* (1891)^[**Author's Note:** This chapter covers search, reinforcement learning, and optimization. None of these AI methods work without a measurable objective function to provide a rigorous reward signal.]
:::
...
system. These roles collapse into the classic components of a closed-loop controller, mapping directly to our five-part loop:
...
To make these three core roles autonomous and robust, an agentic loop expands them with five supporting roles: **critique** (finding flaws), **repair** (fixing broken artifacts), **verification** (independent sign-off), **explanation** (making decisions legible), and **coordination** (routing tasks).
...
> **Proxy mismatch**: When a cheap stand-in metric diverges from the true objective it estimates, so optimizing the proxy stops improving the real goal; it is an instance of Goodhart's Law (when a measure becomes a target, it ceases to be a good measure), treated in full in @sec-feedback-verification-trust.
