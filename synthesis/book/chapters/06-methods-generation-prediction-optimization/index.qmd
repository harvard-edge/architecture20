...
Bayesian optimization is attractive for Architecture 2.0 because it was built
...
::: {.callout-engineer-move title="screen candidates under a power budget before committing a method"}
1. **Situation.** The loop must choose a compute organization for the 3\ W XR subsystem but cannot run cycle-level evaluation on every candidate.
2. **Architecture decision.** Which vector widths and local-memory sizes to advance, and whether each clears the 3\ W envelope.
3. **Bound the loop.** Fix the candidate set, the 3\ W power envelope, and the XRBench slice.
4. **Method role.** Use *prediction* (a calibrated surrogate) to screen broadly and *optimization* to allocate the scarce high-fidelity runs; generation is not the bottleneck.
5. **Evidence path and escalation.** Rank on the surrogate with a calibrated interval, and escalate a candidate to cycle-level simulation or synthesis whenever its interval straddles the 3\ W envelope. A point estimate never authorizes commitment.
6. **Negative trace.** Record candidates rejected for power-envelope violations and the fidelity at which a proxy win disappeared.
7. **Architect signs off.** A human decides which screened candidate has evidence strong enough to carry forward, and owns the consequence if the surrogate was miscalibrated.
:::
...
What decides whether a learning-based architecture result is contested or
trusted into shipping silicon? Can you tell, before running, whether an
environment's rejection authority is strong enough to make a method's claims
credible, rather than discovering it only in the reproducibility post-mortem?
:::
