...
**Context.** In the methods chapter, the lighthouse prompt is a method-selection
...
Use prediction to screen candidates with calibrated intervals, and use
optimization to spend scarce simulator or synthesis runs where they resolve the
decision.

**Boundary.** A surrogate may reject obviously bad points, but it cannot by
...
**Reader takeaway.** The method is credible when it says what it is allowed to
change, what feedback priced the change, which candidates were rejected at which
fidelity, and which final carry-forward decision remains with the architect.
:::
