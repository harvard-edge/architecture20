# Methods for Generation, Prediction, and Optimization {#chap-methods-generation-prediction-optimization}
...
Chapter 5 defined the environment: the place where actions are taken and
...
![**Method roles are useful only inside a represented loop:** Generation proposes candidates, prediction estimates behavior, optimization chooses evaluations, critique and repair improve artifacts, and verification turns feedback into rejectable evidence. The roles can be implemented by different models, scripts, tools, or humans.](images/F7-method-role-map){#fig-method-role-map width="100%" fig-pos="t"}
...
::: {.callout-warning}
## Fallacy: Treating Generation as Design
...
result (Chapter 7) [@ChengEtAl2023AssessmentRL]. The important lesson for this
...
rows reuse the design-space anchors from Chapter 2: the small product example
...
lower rows reuse the representative feedback budgets from Chapter 5. These
...
rows = [
    {"display_label": "small product example", "display_note": "Chapter 2 before corners/workloads", "count_low": 12_960, "count_high": 12_960, "right_label": "~1.3e4", "color": COLORS["orange"]},
...
Chapter 4 treated sample cost as data the representation must carry. Here the
...
level, then each sample teaches more. Chapter 4 and Chapter 5 are therefore
...
must match the task, representation, environment, feedback budget, evidence
standard, and commitment level. They should preserve negative traces, expose
uncertainty, and make rejection possible. They should help architects learn
the design space, not merely search it harder.
...
methods, how do we know whether its feedback is evidence? Chapter 7 answers
