...
surrogate^[Surrogate model: A cheap-to-evaluate approximation model built to mimic the behavior and predict the outputs of a complex, expensive-to-evaluate system like a hardware simulator.] may predict latency or energy well inside a calibrated accelerator
...
action space and objective are well formed. Reinforcement learning^[Reinforcement learning (RL): A machine learning paradigm where an agent learns to make decisions by performing actions in an environment to maximize a cumulative reward. This is similar to training an agent to play games like Go or navigate a maze through trial and error, but here the "moves" are placement decisions or scheduling policies.] may be
...
RTL, test, configuration, and design-report workflows. The shorthand is useful
...
This is why environment work such as ArchGym, an open-source OpenAI Gym^[OpenAI Gym is a widely used toolkit for developing and comparing reinforcement learning algorithms, providing a standard interface for agents to interact with various environments.]-based architecture environment, is important: it makes method
...
> **Object-action-evidence rule.** State every method claim as: this role
> acts on this architecture object through this interface, receives this
> feedback, and can be rejected by this evidence.
...
search^[Neural architecture search (NAS): A technique for automating the design of artificial neural networks, finding optimal model architectures for a specific task instead of relying on human design.] already shows the value of putting latency, energy, memory footprint,
...
adds calibration, uncertainty, proxy mismatch^[*Proxy mismatch* is when a cheap stand-in metric diverges from the true objective it estimates, so optimizing the proxy stops improving the real goal; it is an instance of Goodhart's Law, treated in full in @sec-feedback-verification-trust.], fidelity, and negative traces.
...
| Commitment boundary | Can it recommend or escalate without pretending to own the decision? | Evidence packet, review note, residual risk, and named decision owner. |
...
point quantitative as the Rejection Bound: candidate count does not enter the
loop's throughput bound at all, so generating more proposals cannot speed a loop
whose cheap, independent rejection coverage stays fixed.
...
the true value with a user-chosen probability under an exchangeability^[Exchangeability: A statistical property where the joint probability distribution of a sequence of random variables is invariant to their permutation; it is a weaker assumption than independent and identically distributed (i.i.d.).]
...
from a slogan into an operation. The predictor returns not
a point latency or energy but an interval, and the loop escalates to higher
fidelity whenever that interval straddles a constraint, such as the 3 W
envelope, or grows wider than the decision can tolerate. The guarantee does not
require a parametric residual model, which is useful for architecture
surrogates with awkward error shapes. But it still depends on the calibration
assumption being meaningful; under distribution shift, the interval is a
diagnostic that should trigger scrutiny rather than a promise that the proxy is
safe.
...
limited feedback. That may mean finding a Pareto region^[Pareto region: The set of optimal solutions in a multi-objective design space where no single objective can be improved without degrading at least one other objective.], identifying a
...
Bayesian optimization^[Bayesian optimization: A strategy for the global optimization of black-box functions. It builds a probabilistic surrogate model of the objective and uses it to decide where to evaluate next, minimizing expensive evaluations. This is similar to hyperparameter tuning in machine learning, where expensive training runs are minimized by modeling the loss surface, but here the "training run" is a multi-hour hardware simulation.] is attractive for Architecture 2.0 because it was built
for expensive black-box functions and sequential experimentation
...
paper reports circuits that reached the area-delay Pareto frontier of a
production design flow and arithmetic units that were adopted in shipping GPU
silicon.
...
[@ChenEtAl2018AutoTVM; @ZhengEtAl2020Ansor]. That is an active-learning^[Active learning: A machine learning approach where the algorithm interactively queries an oracle (like a simulator or hardware measurement) to obtain the true labels for new data points, typically selecting those it is most uncertain about in order to improve its model with minimal expensive feedback.] loop
...
itself the problem of sample efficiency^[Sample efficiency: A measure of how much data or how many environmental interactions an algorithm requires to reach a target level of performance. High sample efficiency means fewer expensive simulations are needed.], which is one of the reasons architecture is a hard AI domain.
...
Process Upper Confidence Bound) gives that choice a no-regret guarantee^[A *no-regret* guarantee means the loop's cumulative sampling loss grows sublinearly, so over time its choices provably approach those of the best fixed strategy under the stated assumptions.] under stated
...
loop rarely needs the full apparatus, but naming it matters: there is a
rigorous language for spending scarce samples, and a loop that ignores it is
...
: **Feedback regime should drive method choice:** Cheap proxies, moderate simulations, expensive EDA, scarce human review, and deployment signals reward different mixtures of generation, prediction, optimization, critique, and verification. {#tbl-sample-efficiency-regimes tbl-colwidths="[18,24,26,22]"}
...
- **Principle:** Method choice is a loop decision: generation, prediction,
  optimization, critique, repair, verification, and explanation earn trust only
  when they match the task, representation, environment, feedback budget,
  fidelity ladder, evidence burden, and commitment level.
- **Reader test:** Can you write one sentence explaining why this method belongs
  in this role for this loop?
- **Next loop state:** Once methods can act, the next question is whether their
