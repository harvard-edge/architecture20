...
surrogate[^fn-surrogate-model-c06] may predict latency or energy well inside a calibrated accelerator
...
action space and objective are well formed. Reinforcement learning[^fn-reinforcement-learning-c06] may be
...
Bayesian optimization[^fn-bayesian-optimization-c06] is attractive for Architecture 2.0 because it was built
...
[@ChenEtAl2018AutoTVM; @ZhengEtAl2020Ansor]. That is an active-learning[^fn-active-learning-c06] loop
...
[^fn-surrogate-model-c06]: **Surrogate model**: A cheap-to-evaluate approximation model built to mimic the behavior and predict the outputs of a complex, expensive-to-evaluate system like a hardware simulator.
[^fn-reinforcement-learning-c06]: **Reinforcement learning (RL)**: A machine learning paradigm where an agent learns to make decisions by performing actions in an environment to maximize a cumulative reward. This is similar to training an agent to play games like Go or navigate a maze through trial and error, but here the "moves" are placement decisions or scheduling policies.
[^fn-openai-gym-c06]: **OpenAI Gym**: A widely used toolkit for developing and comparing reinforcement learning algorithms, providing a standard interface for agents to interact with various environments.
[^fn-neural-architecture-search-c06]: **Neural architecture search (NAS)**: A technique for automating the design of artificial neural networks, finding optimal model architectures for a specific task instead of relying on human design.
[^fn-proxy-mismatch-c06]: **Proxy mismatch**: When a cheap stand-in metric diverges from the true objective it estimates, so optimizing the proxy stops improving the real goal; it is an instance of Goodhart's Law, treated in full in @sec-feedback-verification-trust.
...
[^fn-bayesian-optimization-c06]: **Bayesian optimization**: A strategy for the global optimization of black-box functions. It builds a probabilistic surrogate model of the objective and uses it to decide where to evaluate next, minimizing expensive evaluations. This is similar to hyperparameter tuning in machine learning, where expensive training runs are minimized by modeling the loss surface, but here the "training run" is a multi-hour hardware simulation.
[^fn-active-learning-c06]: **Active learning**: A machine learning approach where the algorithm interactively queries an oracle (like a simulator or hardware measurement) to obtain the true labels for new data points, typically selecting those it is most uncertain about in order to improve its model with minimal expensive feedback.
[^fn-sample-efficiency-c06]: **Sample efficiency**: A measure of how much data or how many environmental interactions an algorithm requires to reach a target level of performance. High sample efficiency means fewer expensive simulations are needed.
[^fn-no-regret-guarantee-c06]: **No-regret guarantee**: A guarantee that the loop's cumulative sampling loss grows sublinearly, so over time its choices provably approach those of the best fixed strategy under the stated assumptions.
