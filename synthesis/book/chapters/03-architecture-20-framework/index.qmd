...
The architecture lesson is not to build a chip-specific language model first.
...
evidence standard, rejection rule, and owner are named.
...
![**A filled design-loop card turns a prompt into reviewable state:** The lighthouse prompt becomes explicit loop state: intent, task, design space, representation, environment, method role, feedback budget, evidence, failure records, rejection authority, and human decision.](images/F4b-design-loop-card-example){#fig-design-loop-card-example width="100%" fig-alt="Filled design-loop card mapping the lighthouse prompt into fields such as intent, representation, environment, feedback, evidence, failure records, and human decision."}
...
agent participation. @sec-data-representations-world-models asks what
...
Research questions for making the ontology usable:

1. What minimal represented state must an Architecture 2.0 loop carry for an
   agent's actions, feedback, evidence, and rejections to remain replayable and
   reviewable?
2. How can low-resource architecture data capture not only successful designs,
   but also workload provenance, failed candidates, invalid actions, and
   abandoned assumptions?
3. What evidence ledger and rejection rules are strong enough to move a claim
   from proxy estimates to simulation, synthesis, post-layout analysis, or
   silicon measurement?
4. Which architectural commitments should remain human-owned even when agents
   search, verify, revise representations, and allocate feedback budgets inside
   a bounded loop?
:::

Carry forward the habit of reviewing claims through the loop that produced
them: the architectural claim, architecture environment, method roles, evidence
ledger, rejection authority, and commitment boundary. The next chapter asks what
