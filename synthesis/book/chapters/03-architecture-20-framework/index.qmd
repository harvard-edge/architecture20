...
**In the Lighthouse prompt.** If an agent claims that a vector-capable mobile XR
subsystem meets the 3\ W target, the loop must record the workload, input
distribution, power model, process assumptions, compiled software stack,
rejected alternatives, uncertainty, and rejection rule.

**Reader takeaway.** The same power number has different authority as a proxy
estimate, cycle-level simulation, synthesis result, post-layout estimate, or
silicon measurement. The design-space report must say which one it is.
:::
