...
commitment boundary are visible. @fig-architecture-tuple visualizes how these pieces form a unified design loop.
...
*We should be very clear about why we start with an ontology rather than jumping straight to a taxonomy of AI tools. An ontology answers the question: "What are the fundamental entities here, and how do they relate?" (e.g., State, Action, Environment, Rejection). A taxonomy, on the other hand, answers the question: "How do we categorize the specific instances of these entities?" (e.g., Classifying a specific AI method as a Surrogate Predictor vs. a Generative Repair tool). You cannot meaningfully classify the structural shapes of these design loops—the taxonomy—until you have firmly established the building blocks they are constructed from—the ontology.*
:::
...
1. **How can automated systems replay, verify, and compare architectural claims across different organizations, proprietary toolchains, and competing evidence standards by scaling the design-loop card to a formal, executable schema?** [See @sec-appendix-b-design-loop-card]

2. **How do we retrofit decades of existing, opaque architecture infrastructure with robust methods for legacy tools to actively expose their legal actions and invalid-action semantics, ensuring an automated optimizer cannot silently exploit simulation inaccuracies to generate false claims?** [See @sec-architecture-environments-tool-interfaces]

3. **How do we efficiently encode and curate the "dark matter" of architectural design—such as proxy wins that fail at higher fidelity or abandoned design paths—into training datasets to develop more grounded AI systems?** [See @sec-arch2-ontology]
