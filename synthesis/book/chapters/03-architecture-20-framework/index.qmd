...
*(**Author's Note**: George E. P. Box was a British statistician who made major contributions to quality control and time-series analysis. This chapter introduces proxies and surrogate models. It acknowledges that while fast proxies are inherently inaccurate ("wrong"), they are absolutely essential for accelerating an AI agent's search ("useful").)* Computer architecture has always depended on disciplined abstraction. An
...
model^[A computationally cheaper approximation used in place of an expensive simulator or complex function.], or a simulator wrapper, the analysis must first identify the exposed loop.
