...
it uses an LLM, reinforcement learning[^fn-reinforcement-learning-c03], Bayesian optimization[^fn-bayesian-optimization-c03], or a surrogate
model[^fn-surrogate-model-c03] and still leave the important state invisible: what task was bounded, what
...
pretrain-then-adapt pattern that BERT[^fn-bert-c03] established [@DevlinEtAl2019BERT] became
...
[^fn-reinforcement-learning-c03]: **Reinforcement learning**: An area of machine learning where an agent learns to make decisions by performing actions in an environment to maximize a cumulative reward.
[^fn-bayesian-optimization-c03]: **Bayesian optimization**: A statistical strategy for the global optimization of black-box functions, often used when evaluating candidates (like running a cycle-accurate simulator) is computationally expensive.
[^fn-surrogate-model-c03]: **Surrogate model**: An approximate, computationally cheap predictive model trained to mimic the behavior of a more complex and expensive simulator.
[^fn-bert-c03]: **BERT**: Bidirectional Encoder Representations from Transformers, a foundational language model architecture that popularized the pretrain-then-fine-tune paradigm in natural language processing.
