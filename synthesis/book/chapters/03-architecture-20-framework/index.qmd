...
- audit a paper, tool, or project by naming the loop state, legal actions, environment feedback, evidence ledger, rejection and escalation rules, and human-owned commitment it exposes;
...
relationships; the checklist tests whether an agent can act safely; and the
...
Architecture 2.0 loop; unanswered rows identify where an agent lacks action
...
QuArch, an architecture-centric benchmark dataset, is useful as a boundary test:
it can assess whether a model knows architecture concepts, but not whether an
agent has the represented workload, tool, action, evidence, and rejection state
...
For making the ontology executable:

1. What minimal machine-readable loop state must be carried across architecture
   actions, tool observations, evidence records, rejection decisions, and human
   commitments so a claim remains replayable and reviewable?
2. What environment contracts should simulators, compilers, EDA flows, benchmark
   harnesses, and telemetry systems expose: legal actions, invalid-action
   semantics, observation schemas, feedback cost, provenance, and escalation?
3. How can low-resource architecture datasets preserve negative information:
   workload provenance, failed candidates, invalid actions, proxy wins that fail
   at higher fidelity, and abandoned assumptions?
4. What evidence ledgers, rejection rules, and commit gates justify moving a
   claim across fidelity levels while preventing agents from crossing
   human-owned commitment boundaries?
:::
