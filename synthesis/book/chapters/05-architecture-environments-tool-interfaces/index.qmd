...
> action boundary around tools: it specifies legal actions, observations,
> feedback costs, assumptions, logged state, provenance, and rejection rules for
> an architecture design loop.
...
provenance record, and rejection rules. Once those are visible, method claims
...
move to a higher level is a human commitment, not merely a more accurate number.
...
rows = [
    ...
    {"display_label": "prototype / silicon / deployment", "display_note": "few high-commitment samples plus telemetry", "low_seconds": 604_800, "high_seconds": 31_536_000, "right_label": "commit / calibrate", "color": COLORS["brown"]},
]
...
No simulator crash is not the same as hardware validity; the environment has
to define which illegal states, unsupported configurations, and silent
out-of-model behaviors it can actually reject.

::: {.callout-tip title="Field note: when the tool becomes the target"}
