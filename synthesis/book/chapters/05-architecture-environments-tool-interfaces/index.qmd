...
integrate them. Chipyard, for example, wraps a configuration system, RTL
generators, and a compiler path together with the simulation, synthesis, and
FPGA-emulation backends the table lists, so one configuration can be carried
reproducibly from a generated design to each evaluation path [@chipyard].
...
performance, and area. Synopsys, a major EDA vendor, reported that one such reinforcement-learning
...
environments through defined interfaces [@KrishnanEtAl2023ArchGym]. The name is
...
How would you decide whether an architecture environment is valid, that gains
found inside it survive at the next fidelity level, without already paying for
the expensive high-fidelity evidence the environment exists to avoid?
:::
...
[^fn-active-learning-c05]: **Active learning**: A machine learning framework where the algorithm can query an oracle (e.g., a human or a simulator) to label new data points, deliberately selecting the most informative samples to improve its model with minimal data [@Settles2009ActiveLearning]. This is similar to how data-selection strategies in ML training curate only the most informative examples to reduce training cost, but applied here to minimize expensive simulator runs.
