...
For an agentic loop, the useful property is not the framework name; it is the
loop contract: configuration state, legal actions, backend receipts,
invalid-action logs, escalation thresholds, and the human decision points that
prevent a generated configuration from becoming an unowned commitment.
...
1. What minimal machine-checkable environment contract exposes legal actions,
   required state, validity constraints, provenance, and rejection reasons so an
   agent can act, explain, and have invalid candidates stopped before high-cost
   evaluation?
2. How can negative traces in low-resource architecture studies -- failed runs,
   invalid actions, waivers, timeouts, and rejected candidates -- become reusable
   evidence while preserving simulator, workload, tool-version, and fidelity
   context?
3. What calibrated escalation policy should govern multi-fidelity feedback: when
   may a cheap proxy prune a candidate, when must proxy uncertainty trigger
   simulation, RTL, physical-design checks, or deployment evidence, and when is
   human review the only valid rejection authority?
4. How can heterogeneous architecture, compiler, simulator, RTL, EDA, and
   ...
   contracts that trigger revalidation as workloads, PDKs, libraries, or
   deployment contexts drift?
:::
...
[^fn-active-learning-c05]: **Active learning**: In this chapter, a policy for choosing which scarce simulator, synthesis, human, or deployment feedback event to spend next, with the choice logged as part of the evidence record [@Settles2009ActiveLearning].
