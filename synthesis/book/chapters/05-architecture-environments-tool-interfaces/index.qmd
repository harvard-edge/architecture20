...
performance, and area. Synopsys, a major EDA vendor, reported that one such reinforcement-learning^[Reinforcement learning is a machine learning paradigm where an agent learns to make decisions by performing actions in an environment to maximize a reward signal.]
...
with it: Bayesian optimization^[Bayesian optimization is a strategy for the global optimization of black-box functions, using a probabilistic model to balance exploration of the design space with exploitation of known good configurations [@SnoekLarochelleAdams2012BayesianOptimization].], reinforcement learning, evolutionary search^[Evolutionary search is a family of optimization algorithms inspired by biological evolution, iteratively improving candidate designs using mechanisms like mutation, crossover, and selection.],
surrogate-guided exploration^[Surrogate-guided exploration uses a fast, approximate machine learning model (the surrogate) to predict design performance, allowing the search algorithm to evaluate many more candidates than a slow simulator would permit.], random search, heuristic search, or a human
...
quantification^[Uncertainty quantification is the science of characterizing and reducing uncertainties in computational models, critical in multi-fidelity search for deciding whether a fast model's prediction is confident enough to trust.] affordable [@PeherstorferWillcoxGunzburger2018Multifidelity].
...
needs sample efficiency, priors, surrogates, active learning^[Active learning is a machine learning framework where the algorithm can query an oracle (e.g., a human or a simulator) to label new data points, deliberately selecting the most informative samples to improve its model with minimal data. This is similar to how data-selection strategies in ML training curate only the most informative examples to reduce training cost, but applied here to minimize expensive simulator runs.], staged gates, or
...
> **ArchOps.** This book proposes *ArchOps*, by deliberate analogy to MLOps^[MLOps (Machine Learning Operations) is a set of practices that aims to deploy and maintain machine learning models in production reliably and efficiently.], for
