...
contract. "Vector-capable CPU, accelerator, or SoC block" is an action boundary
over compute organization and integration. "XRBench real-time mobile XR" fixes
the workload harness and quality-of-service target. The 3\ W, 3\ nm-class
low-power mobile envelope invokes power and physical-design constraints.

**Boundary.** A SoC block is not just an accelerator renamed. It changes memory
attachment, coherence, interrupts, DMA, runtime ownership, verification scope,
and the evidence needed to show that the subsystem works in the larger system.

**Reader takeaway.** The environment for this prompt is a bundle of coherent
interfaces. A loop can change vector width, local memory, data layout, or
...
> **The simulator case of proxy mismatch.** When the proxy is a simulator,
> proxy mismatch takes a specific form: a gap between the behavior an
> environment reports and the behavior that would matter at the next stronger
...
tools change, benchmarks gain rules, and models learn stale assumptions, so an
environment that is valid on the first run can silently mislead by the tenth.
Keeping it valid, versioning workloads and tools, recording provenance and
negative traces, monitoring drift, and maintaining the rejection gates, is the
architecture counterpart of the operational discipline MLOps
brought to machine-learning pipelines, applied to simulators, RTL, constraints,
and evidence ledgers rather than models and datasets.
:::

The distinction matters because a one-time environment and a maintained
one look identical on the first run and diverge completely by the tenth. That
maintenance burden is what separates a script that supports a single paper from
a harness that can support a field, once architecture
...
Maintaining the environment is what keeps that design system trustworthy after the first run.
