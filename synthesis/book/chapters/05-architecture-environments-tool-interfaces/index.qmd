...
: **EDA feedback has stage-specific authority:** Synthesis, placement, routing,
power closure, and signoff return different evidence and should be allowed to
reject different claims. {#tbl-eda-stage-contract tbl-colwidths="[18,26,25,21]"}

This arrangement is not hypothetical. Some commercial physical-implementation
flows already treat synthesis and place-and-route as a search environment: the
action space is a set of tool directives and floorplan parameters, the
transition is a full tool run, and the reward is the resulting power,
performance, and area. Synopsys, a major EDA vendor, reported that one such reinforcement-learning[^fn-reinforcement-learning-c05]
system reached its first hundred commercial tapeouts by 2023
[@Synopsys2023DSOai], and Cadence, another leading EDA vendor, describes a comparable loop driving the flow
from RTL to layout [@Cadence2021Cerebrus]. The durable point is not which vendor
leads, since the products will be renamed and replaced. It is that an EDA flow
...
with it: Bayesian optimization[^fn-bayesian-optimization-c05], reinforcement learning, evolutionary search[^fn-evolutionary-search-c05],
...
interpret results [@MattsonEtAl2020MLPerf]. Architecture 2.0 needs the same instinct at the loop level. A
useful environment should not merely publish a script; it should publish the
contract under which actions are legal, observations are valid, and evidence can
be compared.
Keeping those contracts valid after the first paper is the operating discipline
@sec-archops-operating-discipline later names ArchOps.
...
mismatch*: a candidate can improve a proxy by choosing an unsupported parameter
...
environment from scratch. A useful Architecture 2.0 environment can start
small. The point is not to
...
Before trusting an environment's verdict, confirm:

- the workload, action space, and observations still match the question being asked;
...
- another team could act in this harness without private knowledge.
:::
...
> **ArchOps.** This book proposes *ArchOps*, by deliberate analogy to MLOps[^fn-mlops-c05], for
...
terms. The table is intentionally prosaic because ArchOps succeeds only when
ordinary drift becomes visible before it invalidates evidence.

| **Drift source** | **What ArchOps records** | **Failure if ignored** | **Operating response** |
| --- | --- | --- | --- |
...
: **ArchOps maintains loop validity over time:** The operating discipline is to
...
- **Next loop state:** Once the environment defines what action and feedback
  mean, method selection can be judged by loop conditions rather than fashion.
:::

[^fn-reinforcement-learning-c05]: **Reinforcement learning**: A machine learning paradigm where an agent learns to make decisions by performing actions in an environment to maximize a reward signal.
[^fn-bayesian-optimization-c05]: **Bayesian optimization**: A strategy for the global optimization of black-box functions, using a probabilistic model to balance exploration of the design space with exploitation of known good configurations [@SnoekLarochelleAdams2012BayesianOptimization].
[^fn-evolutionary-search-c05]: **Evolutionary search**: A family of optimization algorithms inspired by biological evolution, iteratively improving candidate designs using mechanisms like mutation, crossover, and selection.
[^fn-surrogate-guided-exploration-c05]: **Surrogate-guided exploration**: Uses a fast, approximate machine learning model (the surrogate) to predict design performance, allowing the search algorithm to evaluate many more candidates than a slow simulator would permit.
[^fn-uncertainty-quantification-c05]: **Uncertainty quantification**: The science of characterizing and reducing uncertainties in computational models, critical in multi-fidelity search for deciding whether a fast model's prediction is confident enough to trust.
[^fn-active-learning-c05]: **Active learning**: A machine learning framework where the algorithm can query an oracle (e.g., a human or a simulator) to label new data points, deliberately selecting the most informative samples to improve its model with minimal data. This is similar to how data-selection strategies in ML training curate only the most informative examples to reduce training cost, but applied here to minimize expensive simulator runs.
[^fn-mlops-c05]: **MLOps**: Machine Learning Operations, a set of practices that aims to deploy and maintain machine learning models in production reliably and efficiently.
