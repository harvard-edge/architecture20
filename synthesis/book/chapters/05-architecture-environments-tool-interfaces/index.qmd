...
[^fn-bayesian-optimization-c05]: **Bayesian optimization**: Optimizes expensive black-box functions with a probabilistic model that balances exploring the design space against exploiting known-good configurations [@SnoekLarochelleAdams2012BayesianOptimization].
[^fn-evolutionary-search-c05]: **Evolutionary search**: Optimization inspired by biological evolution, improving candidate designs through mutation, crossover, and selection.
[^fn-surrogate-guided-exploration-c05]: **Surrogate-guided exploration**: Uses a fast surrogate model to predict design performance, so the search can screen far more candidates than a slow simulator allows.
