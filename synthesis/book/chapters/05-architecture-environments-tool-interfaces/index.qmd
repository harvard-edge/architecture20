...
> *(Note: In Architecture 2.0, Culkin's "us" is no longer just the human architect; the tool now shapes the automated loop itself.)*^[**Author's Note:** Existing EDA tools dictate human workflows. By wrapping these tools into programmatic environments for AI agents, we are fundamentally reshaping the architect's workflow and what is possible to build.]
:::
