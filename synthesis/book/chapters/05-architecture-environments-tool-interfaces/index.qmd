...
::: {.callout-crux}
What turns a tool flow into an environment an AI agent can act in and a
reviewer can trust?
:::
...
> feedback costs, assumptions, logged state, provenance, rejection authority,
> and commitment boundaries for an architecture design loop.
...
::: {.callout-field-note title="What a real EDA environment makes the wrapper do"}
Validity comes in two tiers, and honest environments separate them. *Schema
legality* is cheap and checkable up front: are the parameters, clocks, and
interfaces well formed? *Physical feasibility* is late and expensive:
routability, timing closure, congestion, and IR drop often surface only at
synthesis or place-and-route. A late failure must map back to the earlier action
that caused it, not read as a contract violation.
...
level. To stay auditable, its provenance record must carry more than a number:
tool and host version, thread count, license state, run-to-run variance, warnings
and waivers, and a hash of the report the number came from. Two "identical" runs
that differ on any of these are not comparable, and a loop that hides the
difference will trust a result it should have rejected.
:::
...
4. How can heterogeneous architecture, compiler, simulator, RTL, EDA, and
   ...
   contracts so AI agents can preserve, weaken, reject, or recertify claims as
