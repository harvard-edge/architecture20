...
> *(Note: In Architecture 2.0, Culkin's "us" is no longer just the human architect; the tool now shapes the automated loop itself.)*
:::

::: {.column-margin}
**Author's Note:** Existing EDA tools dictate human workflows. By wrapping these tools into programmatic environments for AI agents, we are fundamentally reshaping the architect's workflow and what is possible to build.
:::
...
specification gaming^[Specification gaming is when an AI exploits loopholes in its objective specification to achieve a high score through unintended behavior.] or reward hacking [@AmodeiEtAl2016ConcreteProblems]: a
...
\begin{algorithm}
\caption{Recipe for Constructing a Bounded Task Environment}
\begin{algorithmic}
\State \textbf{Define the task in architectural language:} workload characterization, cache exploration, accelerator parameter search, compiler/runtime tuning, chiplet partitioning, reliability analysis, or design-review critique.
\State \textbf{Choose the representation the loop will read and write:} configuration file, architecture description, graph, trace record, report, RTL fragment, test bench, or design-loop card.
\State \textbf{Define the action schema:} which fields can change, which are read-only, which combinations are legal, and which changes require human approval.
\State \textbf{Wrap the tool path:} simulator, compiler, profiler, RTL flow, EDA stage, runtime system, benchmark harness, or telemetry pipeline.
\State \textbf{Define observations and feedback:} metrics, traces, logs, warnings, errors, generated artifacts, cost, and fidelity level.
\State \textbf{Define invalid-action semantics:} illegal parameter, noncompilable artifact, nonsynthesizable design, violated constraint, timeout, simulator crash, or unsupported workload.
\State \textbf{Log provenance and negative traces:} tool versions, seeds, workload versions, failed candidates, rejected alternatives, and reasons for rejection.
\State \textbf{State the human decision rule:} what the architect reviews, what can be accepted automatically, and what must escalate.
\end{algorithmic}
\end{algorithm}
```
...
Operating discipline, the architecture counterpart of MLOps, is the practice of maintaining those layers over time.
...
The concepts in this chapter establish the foundation for AI-assisted architecture environments, but they also expose massive gaps in how autonomous systems interact with legacy tools. The following thesis-level challenges push beyond wrapper engineering toward self-learning, verifiable, and autonomous environment infrastructure capable of seeding high-impact research at ASPLOS, ISCA, or NeurIPS.

1. **Can agents autonomously induce formal environment contracts from unstructured tool exhaust?**
Legacy EDA flows and simulators emit opaque warnings, timeouts, and unstructured logs rather than clean APIs. A major open challenge is whether language models can observe thousands of such interactions and inductively synthesize the complete action schemas, constraint bounds, and rejection authorities defined in the environment contract (@tbl-environment-contract), effectively learning the environment boundaries without manual human specification.
...
the discussion of methods for generation, prediction, and optimization (see the discussion on "Method Roles: Generate, Predict, Optimize" in @sec-methods-generation-prediction-optimization) can ask which methods belong
inside the loop.
