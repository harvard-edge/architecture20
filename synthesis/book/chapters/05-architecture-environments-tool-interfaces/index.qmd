...
For an agentic loop, the useful property is not the framework name; it is that
configuration state, legal actions, backend receipts, and rejection paths can be
made explicit enough for a method to act without inventing hidden defaults.
...
Research questions for architecture environments:

1. What minimal represented state must an Architecture 2.0 environment expose
   for an agent to act, explain, and be rejected safely?
2. How can low-resource architecture data, including failed runs and rejected
   candidates, become reusable evidence without overfitting one simulator,
   workload slice, or tool version?
3. How can an environment decide when cheap proxy feedback is strong enough to
   prune a candidate and when a result must escalate to simulation, RTL,
   physical design, or human review?
4. How should workload and data characterization be maintained as the workload,
   compiler, benchmark, telemetry, or deployment context drifts?
:::

The reader test is whether another method could act in the same harness without
private knowledge from the original author. Once the environment defines what
action and feedback mean, method selection can be judged by loop conditions
rather than fashion.
