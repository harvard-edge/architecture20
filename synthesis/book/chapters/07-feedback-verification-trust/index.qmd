...
surrogate can rank candidates. A tool-using agent^[A tool-using agent is an AI system that can programmatically invoke external software tools or APIs to solve tasks.] can call simulators and
...
learning is possible. A Bayesian optimizer, reinforcement-learning policy^[In reinforcement learning, a policy is a function that maps an agent's state to a chosen action.],
surrogate model, critic^[A critic is an algorithmic component in reinforcement learning that evaluates a policy's actions by estimating expected future returns.], or tool-using agent should behave differently when a
...
Structuring Notation^[Goal Structuring Notation (GSN) is a graphical argumentation notation used in safety-critical engineering to formally document the elements of an assurance case.] so that each inference and the evidence under it are
...
Rejection authority is also the lever in the Rejection Bound of
@sec-loop-patterns-across-stack. A loop speeds up only as its cheap, independent
rejectors discharge a larger fraction of commitments, and generation does not
enter the bound at all. Rejection authority is therefore not a safeguard bolted
onto a loop that already works. It is the resource that sets how fast the loop
can run.
...
For the lighthouse prompt, $\mathrm{valid}(x)$ might mean that a candidate
compute organization stays within the declared ISA/software contract and does
not violate the 3\ W envelope or real-time deadline. $E_k(x)$ is the evidence at
the current stage: proxy estimate, simulation-stage estimate, RTL evidence, or
physical feedback. $\tau_k$ rises as the loop moves from exploration to
implementation, and residual risk covers what the evidence still cannot rule
out, such as thermal behavior, compiler/runtime viability, or workload drift.
...
The failure an independent rejection authority most often has to catch is proxy mismatch^[Proxy mismatch, called specification gaming or reward hacking in the machine-learning literature [@AmodeiEtAl2016ConcreteProblems], occurs when a method optimizes an imperfect proxy metric in ways that degrade the true, unmeasured objective; it is an instance of Goodhart's Law.]. A loop optimizes the measurement
...
architecture have long needed validation against held-out data^[Held-out data is a subset of a dataset reserved during model training, used exclusively to test the model's ability to generalize to unseen inputs.], higher-fidelity
...
Benchmark governance also matters. As @sec-design-loop-no-longer-scales and
...
- **Principle:** Trust is engineered by feedback budgets, fidelity regimes,
  provenance, independent rejection authority, and accountable human decision.
