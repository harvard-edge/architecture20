# Feedback, Verification, and Trust in Agentic Architecture Loops {#sec-feedback-verification-trust}

```{=latex}
\abstract*{Architecture 2.0 does not ask architects to trust an agent. It asks them to design loops in which claims become credible through feedback budgets, feedback regimes, evidence ledgers, rejection authority, security boundaries, and commitment boundaries. Trust is therefore not a property of a model's tone, confidence, or agent report. It is an engineered property of the loop.}
```

::: {.callout-crux}
When should we trust a loop's AI-assisted output, and what can reject it?
:::
...
claim.
For agentic loops, these authorities are not generic quality checks; they
...
Read the chapter as a sequence of review records rather than as separate trust
concepts. Each record answers a different question about the same claim;
@tbl-trust-record-map is the chapter's map.
...
**In the Lighthouse prompt.** A candidate for the "64-bit RISC-V-based compute
...
1. What evidence-ledger fields and invariants, evaluated on replayable
   multi-fidelity loop traces from simulators, synthesis, workload studies, or
   redacted reviews, are necessary for an AI-assisted architecture loop to
   support only the commitment level its evidence authorizes?
2. How can an AI-assisted loop represent and estimate the calibrated support
   region of its workloads, simulators, and benchmarks, then trigger escalation
   when a proxy improvement falls outside that region or exploits evaluation
   gaps?
3. What benchmark and audit protocol can measure independence between an
   ...
   modes that should disqualify the rejector?
4. How should evidence-ledger and rejection-gate mechanisms in AI-assisted
   architecture loops be evaluated using adversarial proxy tasks, hidden
   workload shifts, flawed baselines, and evidence disputes to measure false
   commitments, justified rejections, escalation cost, waiver quality, and human
   veto authority?
:::
...
The failure mode is not only leakage; it is an agent producing a claim from
