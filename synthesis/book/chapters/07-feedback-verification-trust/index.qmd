...
```{=latex}
\abstract*{Architecture 2.0 does not ask architects to trust an agent. It asks them to design loops in which claims become credible through feedback budgets, feedback regimes, evidence chains, rejection authority, security boundaries, and human commitment. Trust is therefore not a property of a model's tone or confidence. It is an engineered property of the loop.}
```
...
independent authority can reject the result, and the level of human commitment
matches the cost of being wrong.

This distinction is central to the lecture. A model can generate a plausible
...
learning is possible. A Bayesian optimizer, reinforcement-learning policy,
surrogate model, critic, or tool-using agent should behave differently when a
...
One way to make that discipline concrete is to write the feedback budget and
sample value explicitly:
...
## Fidelity Ladders and Evidence Chains
...
> **Feedback regimes and evidence chain.** Feedback regimes organize
> sources from cheap proxies to high-commitment checks. An evidence chain records
...
explicit and reviewable [@Kelly2004GSN]. An Architecture 2.0 evidence chain is
...
![**Evidence chains turn feedback into trust:** A claim becomes more credible only as it moves through staged feedback sources, records provenance and uncertainty, and gives each stage authority to reject, revise, or escalate the result before human commitment increases.](images/F9-evidence-chain-rejection-gates){#fig-evidence-chain-rejection-gates width="100%" fig-pos="t"}

Verification is used broadly in this chapter. It includes formal methods when
formal properties are available, but it also includes type checks, interface
checks, regression tests, baseline replay, simulator cross-checks, synthesis
constraints, physical-design warnings, security review, workload coverage, and
expert design review. The common requirement is independence: the check should
be able to reject the claim, not merely restate the method's output.
...
adds a loop-level requirement: the evidence chain itself must be represented
so that a compound method system and a human reviewer can inspect it.

## Commitment Levels and Reversibility

Trust requirements should rise with commitment. A loop that generates a
...
A credible loop needs something with authority to say no. The rejecting
...
\end{cases}
$$
Here, $x$ is the candidate, $k$ is the commitment level, $E_k(x)$ is the
...
the lighthouse prompt: the accelerator fails validity at the 3 W envelope, the
vector CPU fails it at the latency deadline, and only the SoC block clears every
term at the experimental commitment level.
...
AI-assisted. Production verification platforms now use machine learning to
triage failures, rank likely root causes, and direct coverage
[@Cadence2022Verisium]. Such tools are valuable, but they move part of the
...
The central failure mode is proxy mismatch. A loop optimizes the measurement
...
::: {.callout-tip title="Field note: the win that vanished at signoff"}
...
Benchmark governance also matters. MLPerf is useful not just because it names
benchmarks, but because it defines rules, versions, and submission practices
that make results more interpretable across a changing field
[@MattsonEtAl2020MLPerf]. Architecture 2.0 needs a similar instinct for
design loops: define the evaluation contract, preserve provenance, track
versions, and treat benchmark changes as part of the evidence story.

## Security, IP, and Confidentiality Boundaries

Industry readers will not trust an Architecture 2.0 workflow that ignores
security, intellectual property, and confidentiality. Architecture state is
...
Evidence disputes are inevitable. One group may claim that a learning-based
...
> counterevidence; rejection rule; and final human decision.
...
becomes tractable once the loop's evidence, baselines, and rejection rules are
made explicit.
...
: **Trust is a checklist, not a tone judgment:** A credible Architecture 2.0 claim states its task, feedback, fidelity, provenance, rejection authority, and human commitment before it asks the reader to believe the result. {#tbl-trust-checklist tbl-colwidths="[24,38,26]"}
...
simulation, rejecting on the power envelope, and stopping at an honest
commitment level. The chapter after that generalizes the single loop into the
patterns that recur across the stack, where the same ontology applies but the
...
- **Principle:** Trust is engineered by feedback budgets, fidelity regimes,
  provenance, independent rejection authority, and accountable human
  commitment.
- **Reader test:** What evidence would overturn the result, and who or what has
  authority to reject it?
- **Next loop field:** Once trust can be evaluated for one claim, the next step
