...
*(**Author's Note**: Edsger W. Dijkstra was a Dutch computer scientist, Turing Award winner, and pioneer in structured programming. This chapter focuses on verification gates—how we use tests not to prove an AI-generated design is perfect, but to aggressively reject the bad ones (the "red-teaming" approach).)* when the outputs of that loop should be believed. The answer is deliberately
...
the mechanism that pays that cost. To conceptualize this, consider the **4 P's of Rejection**, which form the primary taxonomy of rejection gates in Architecture 2.0:
...
Building on the frameworks of fidelity ladders and evidence ledgers (see the discussion on "Fidelity Ladders and Evidence Ledgers" in @sec-fidelity-ladders-and-evidence-ledgers) as well as commitment levels and reversibility (see the discussion on "Commitment Levels and Reversibility" in @sec-commitment-levels-and-reversibility), tracking provenance across fragmented simulators and proprietary physical design tools is currently an informal, error-prone process. A fundamental systems challenge is designing cryptographic or formally verified evidence ledgers that bind disparate multi-fidelity traces into a unified, irrefutable claim. Can we construct a verification architecture where an AI-generated hardware candidate inherently carries a mathematically verifiable certificate of its performance, timing, and power—guaranteeing the loop cannot exceed its authorized commitment boundary even under adversarial or buggy toolchains?

2. **Autonomous Detection and Mitigation of Proxy Exploitation.**
As highlighted in the discussion on proxy mismatch, metric gaming, and calibration (see the discussion on "Proxy Mismatch, Metric Gaming, and Calibration" in @sec-proxy-mismatch-metric-gaming-and-calibration), autonomous generators inevitably overfit and exploit the blind spots of cheap architectural proxies. Because identifying when a candidate drifts outside a simulator's calibrated support is largely manual, generative loops are fragile. A thesis-level challenge is to develop algorithms that dynamically map the epistemic uncertainty^[Epistemic uncertainty refers to uncertainty in a model's predictions caused by a lack of training data in that region, which can be reduced by gathering more data.] of architectural surrogates in high-dimensional design spaces, automatically triggering high-fidelity checks—such as RTL synthesis—exactly when proxy trust bounds are violated. How can we design compound AI agents^[Compound AI agents are systems composed of multiple interacting AI models, tools, and logical components working together to solve a complex task.] that natively co-optimize performance and calibration, structurally guaranteeing they escalate before committing to an adversarial proxy win?
