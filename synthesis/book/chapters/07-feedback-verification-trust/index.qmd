...
The same rule applies when rejection is split across agents. A generator,
critic, verifier, and summarizer may be separate agents in an implementation,
...
correlated agents to agree has created consensus, not rejection authority,
...
3. What benchmark and audit protocol can measure independence between an
   AI-assisted rejection authority and the generator, optimizer, agent, or
   multi-agent committee it judges, including shared data, objectives, tools, and
   correlated failure modes that should disqualify the rejector?
