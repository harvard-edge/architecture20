...
Who owns the escalation threshold when an agent's confidence outruns its evidence? The architect must define the exact conditions—such as a proxy win outside calibrated ranges, a security boundary, or a high-blast-radius commitment—that force an AI loop to stop relying on a generator and seek stronger evidence or human review.
:::
...
more autonomy an agent can have; the harder it is to undo, the stronger and more
...
agent is about to claim.

**In the Lighthouse prompt.** An agent's candidate for the "64-bit RISC-V-based compute
...
**Takeaway.** If any gate is unsupported, the loop should force the agent to reject, revise, or
...
winning structure creates. The agent exploited a gap in the proxy. The cheap fix is a rule written
...
Does the loop know when an agent's proxy is no longer trustworthy? A proxy win is only evidence if the loop can prove the AI-generated candidate falls within the proxy's calibrated support region. If it falls outside, the loop must escalate to a higher-fidelity check rather than trusting the uncalibrated prediction.
:::
...
Can the AI model's evidence be audited without violating security or IP boundaries? If an agent produces a claim using confidential data that cannot be exposed to the rejection authority, the loop cannot trust the claim. The architect must explicitly define which AI tools can access which data classes, and how private evidence can be verified.
:::
