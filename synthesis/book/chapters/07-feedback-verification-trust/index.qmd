...
1. What evidence-ledger fields and invariants are necessary for an agentic
   architecture loop to support an auditable commitment decision under sparse,
   proprietary, multi-fidelity feedback?
2. How can a loop estimate the calibrated support region of its workloads,
   simulators, and benchmarks, and trigger escalation when a proxy improvement
   falls outside that support or exploits evaluation gaps?
3. How can an architecture loop measure adequate independence between an
   AI-assisted rejection authority and the generator, optimizer, or agent it
   judges, including shared data, objectives, tools, and correlated failure
   modes?
4. How should trust mechanisms themselves be evaluated, using adversarial proxy
   tasks, hidden workload shifts, flawed baselines, and evidence disputes to
   measure false commitments, justified rejections, escalation cost, waiver
   quality, and human veto authority?
:::
...
Calibration is therefore an escalation contract: it defines where an agent may
...
trust an Architecture 2.0 loop that ignores security, intellectual
...
projects, and deciding whether an agentic loop is ready for a more
