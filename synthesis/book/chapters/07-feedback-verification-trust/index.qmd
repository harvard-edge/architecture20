...
Building on the frameworks of fidelity ladders and evidence ledgers (@sec-fidelity-ladders-and-evidence-ledgers) as well as commitment levels and reversibility (@sec-commitment-levels-and-reversibility), tracking provenance across fragmented simulators and proprietary physical design tools is currently an informal, error-prone process. A fundamental systems challenge is designing cryptographic or formally verified evidence ledgers that bind disparate multi-fidelity traces into a unified, irrefutable claim. Can we construct a verification architecture where an AI-generated hardware candidate inherently carries a mathematically verifiable certificate of its performance, timing, and power—guaranteeing the loop cannot exceed its authorized commitment boundary even under adversarial or buggy toolchains?

2. **Autonomous Detection and Mitigation of Proxy Exploitation.**
As highlighted in the discussion on proxy mismatch, metric gaming, and calibration (@sec-proxy-mismatch-metric-gaming-and-calibration), autonomous generators inevitably overfit and exploit the blind spots of cheap architectural proxies. Because identifying when a candidate drifts outside a simulator's calibrated support is largely manual, generative loops are fragile. A thesis-level challenge is to develop algorithms that dynamically map the epistemic uncertainty of architectural surrogates in high-dimensional design spaces, automatically triggering high-fidelity checks—such as RTL synthesis—exactly when proxy trust bounds are violated. How can we design compound AI agents that natively co-optimize performance and calibration, structurally guaranteeing they escalate before committing to an adversarial proxy win?
