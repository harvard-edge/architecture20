...
> XRBench-class real-time mobile XR workload. Realize it as a vector-capable CPU, fixed-function accelerator, reconfigurable spatial array (e.g., Coarse-Grained Reconfigurable Architecture or CGRA), or Processing-in-Memory (PIM) data-centric block under a 3 W TDP target in a 3 nm-class LP
> mobile process. The resulting compute chiplet must support seamless scale-out from a 1-die mobile package to a multi-die performance package, accounting for core-disable yield binning and standardizing the die-to-die PHY/interfaces. Return a design-space report with evidence and rejected
> alternatives.
