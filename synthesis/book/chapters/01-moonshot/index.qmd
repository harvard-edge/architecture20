...
::: {.callout-question}
How should architects represent and govern design loops so AI methods can help
turn intent into defensible system designs?
:::
...
It helps to place that act on a familiar ladder. Logic synthesis turns boolean
intent into gate structure, high-level synthesis turns algorithmic intent into
RTL, and synthesizing a system turns workload intent into a defensible system
...
**In the Lighthouse prompt.** A low-power, RISC-V-based mobile XR subsystem
under a 3\ W, 3\ nm-class envelope asks for workload scope, ISA and software
contracts, compute organization, physical assumptions, and an evidence standard.
...
Research questions for the opening loop:

1. What is the minimal represented state a lighthouse-prompt loop must carry so
   another architect or method can inspect, reproduce, reject, and extend the
   result?
2. What low-resource architecture data should be collected first when traces,
   RTL, PDKs, and signoff evidence are unavailable or proprietary?
3. How should generation, prediction, optimization, critique, and repair be
   assigned distinct method roles so no method silently changes the task,
   workload, or evidence standard?
4. Which decisions should remain human-owned even after methods improve, and
   what evidence would be required to safely delegate any one of them?
:::
