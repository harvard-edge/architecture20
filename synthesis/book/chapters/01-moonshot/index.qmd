...
- **2000s--2010s: program and domain-specific synthesis.** Specifications,
  kernels, and DSL intent became code or hardware/software artifacts inside
  specialized toolchains [@GulwaniPolozovSingh2017ProgramSynthesis].
...
A prompt or generated artifact alone is not the contribution. The durable
object is the loop that exposes state, action, feedback, rejection, and
decision, because that is what another architect can inspect, compare, reject,
and improve before anyone crosses a commitment boundary.
:::
...
::: {.callout-note title="A fill-in loop contract" icon=false}
...
For the opening loop:

1. What one-page loop-state schema, evaluated on a public XR-class workload
   packet plus shareable candidate records, lets an independent architect
   reproduce a design-space report, invalidate unsupported claims, and extend
   the next AI-assisted loop turn?
2. When traces, RTL, PDKs, and signoff evidence are unavailable or proprietary,
   which low-cost artifact bundle -- workload summary, proxy script, simulator
   configuration, invalid-action log, and negative traces -- lets an
   AI-assisted loop reject bad XR-subsystem candidates under the 3\ W envelope
   before stronger evidence is spent?
3. What permissioned AI-role schema keeps generation, prediction, optimization,
   critique, and repair from changing the task, workload, metric, power
   envelope, or evidence threshold without a logged rejection or human
   commitment decision?
4. How should two AI-assisted lighthouse loops be compared on the same workload
   packet and tool path: evidence cost, rejection quality, calibration against
   stronger feedback, human review time, replayability, and the strongest claim
   each loop may commit?
:::
