...
[@Synopsys2023DSOai; @RoyEtAl2021PrefixRL]. Reinforcement learning is a machine
learning training method based on rewarding desired behaviors and punishing
undesired ones, often used to navigate complex design spaces. @sec-loop-patterns-across-stack returns to those examples
...
XRBench, a benchmark suite for mobile extended reality (XR) workloads, gives the prompt a workload anchor rather than a vague application
...
One way to read the prompt is through the familiar foundation-model stack. A
foundation model is a large-scale AI model trained on a vast quantity of data
that can be adapted to a wide range of downstream tasks. In
...
A credible loop records what failed as well as what worked. A loop that records
only winners cannot teach the next method, reviewer, or architect which
assumptions, tools, workloads, or regions of the design space were already ruled
out. Later chapters give that record a sharper name and make it part of the
evidence packet.
