...
1. What is the smallest loop-state schema a lighthouse-prompt loop must carry so
   an independent architect or method can reproduce the design-space report,
   invalidate unsupported claims, and extend the next loop turn?
2. When traces, RTL, PDKs, and signoff evidence are unavailable or proprietary,
   which low-cost artifacts provide the highest marginal value for rejecting bad
   XR-subsystem candidates under the 3 W envelope?
3. How should generation, prediction, optimization, critique, and repair be
   separated so no method can change the task, workload, metric, power envelope,
   or evidence threshold without explicit loop-state approval?
4. How should two lighthouse loops be compared: evidence cost, rejection
   quality, fidelity calibration, human time, reproducibility, and whether bad
   candidates are caught before expensive evaluation?
5. Which commitment-boundary decisions must remain architect-owned, and what
   calibrated evidence would justify delegating one bounded decision class to a
   method?
:::
