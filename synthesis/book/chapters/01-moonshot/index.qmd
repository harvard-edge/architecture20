...
With those terms fixed, read the prompt itself. It asks for a compute subsystem
...
framework into long-horizon challenge tasks. @sec-teaching-the-loop closes the
book with what a rejection-bound field should teach, turning the design-loop
card into both a lesson and an exam. The appendices then equip the reader: a
bootstrap path for a first loop, the design-loop card and review rubric, a
loop-role resource catalog with living links, the collected design principles,
and a course kit for instructors.
