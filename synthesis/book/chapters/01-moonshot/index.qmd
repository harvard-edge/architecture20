...
How does an architectural claim remain reviewable and defensible even when the underlying generative methods are inherently stochastic or frequently updated? [See @sec-why-the-prompt-spans-the-stack]

2. **What constitutes an adversarial stress-test for an AI-assisted design loop?**
How do we systematically design "red-team" workloads and constraints specifically engineered to trick an AI method into proposing plausible but fatally flawed hardware? [See @sec-efficiency-claims-need-rejectable-loops]

3. **Standardizing the cost of evidence.**
How do we rigorously quantify and trade off the computational and financial costs of querying large models versus running cycle-accurate simulators, establishing a standardized currency for the "feedback budget"? [See @sec-architecture-development-spans-three-roles]

4. **Failuring taxonomies become shared infrastructure.**
How do we structure and share a universal taxonomy for *why* an AI-generated candidate was rejected across the community, ensuring different AI-assisted systems do not independently repeat the same architectural mistakes? [See @sec-from-architecture-10-to-architecture-20]
