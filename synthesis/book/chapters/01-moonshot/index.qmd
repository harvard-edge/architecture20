...
central act is system synthesis: turning intent into a defensible
computing-system design. The three terms below fix the discipline, name that
act, and describe the bounded role each method plays inside the loop.
...
It helps to place system synthesis on a familiar ladder. Logic synthesis turns boolean
intent into gate structure, high-level synthesis turns algorithmic intent into
RTL, and system synthesis turns workload intent into a defensible system
organization. The lower rungs are largely automated; the top rung is what
Architecture 2.0 has to learn to support.
...
A prompt or a generated artifact is not the contribution. The durable object is
the loop that exposes state, action, feedback, rejection, and decision, because
that is what another architect can inspect, reject, and improve.
:::
...
::: {#pri-system-synthesis .callout-design-principle title="System synthesis principles"}
...
for, and that target is efficiency. Efficiency is the practical north star
because architecture is the discipline
...
Having set efficiency as the loop-level target, it is worth being equally clear
about what this book is not trying to do. The goal is not to produce a paper
...
For the lighthouse prompt, which design decisions would you expose to an AI
method's control surface tomorrow, and which would you keep under human
rejection authority, and what evidence would have to exist before you moved a
decision across that line?
:::
...
[^fn-reinforcement-learning-c01]: **Reinforcement learning**: A machine learning training method based on rewarding desired behaviors and punishing undesired ones, often used to navigate complex design spaces [@SuttonBarto2018ReinforcementLearning].
...
[^fn-overfitting-c01]: **Overfitting**: Occurs when an optimization process learns the noise of a proxy metric rather than the true objective, causing poor real-world performance.
[^fn-world-model-c01]: **World model**: An agent's internal predictive model of its environment that enables it to simulate the consequences of actions [@HaSchmidhuber2018WorldModels].
