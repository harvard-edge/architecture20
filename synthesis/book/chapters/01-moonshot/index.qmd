...
The word *agent* is often singular in the prose and figures, but it names a
participant in the loop. A real implementation may use one agent, a
pipeline of methods, or several specialized agents; the Architecture 2.0
...
feedback without owning the commitment. The agent, whether singular or a
coordinated set of role-specific agents, is not the whole system. The governed
loop is the system around the artifact. The agentic parts must be embedded in
...
The rest of that vocabulary is a signpost here, not a full definition stack.
...
::: {#pri-system-synthesis .callout-design-principle title="Make synthesis obligations explicit"}
System synthesis is credible only when the loop exposes the obligations that
make a result reviewable: what state it represents, which actions are bounded,
which environment returns feedback, which method role acts, what feedback budget
...
::: {.callout-research-question title="Questions this book opens"}
...
2. What would *engineering best practices* even mean for AI-assisted design loops,
   ...
   we build today?
3. What shared venues, benchmark practices, loop cards, evidence ledgers, and
   review norms would make AI-assisted architecture claims comparable and
   reviewable rather than anecdotal?
4. What norms for publishing negative traces, failed loops, and rejected claims
   would turn scattered demonstrations into a cumulative discipline?
:::
