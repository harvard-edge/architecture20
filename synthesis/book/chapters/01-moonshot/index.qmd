...
constraints and evidence ledgers; and coordinating state, tools, feedback
...
In this book, *system synthesis* means the architecture-level act of turning
intent into defensible computing-system design. It does not mean only logic
synthesis or high-level synthesis. The short phrase is simple, but the loop is
not: credible system synthesis must coordinate constraints, representations,
tools, methods, feedback, evidence, and human judgment.
...
> **Commitment boundary.** A commitment boundary is the line between a loop
> result and the next action it is allowed to support: more exploration, a
> design-space report, an implementation step, a review decision, or a
> deployment-facing choice. The higher the cost, irreversibility, or blast
> radius, the stronger the evidence and rejection authority must be before the
> boundary moves.
...
> design request used throughout this book: a low-power, RISC-V-based mobile XR
> compute subsystem that must return a design-space report with evidence and
...
> evidence, negative traces, rejection authority, and human decision for a loop.

> **Feedback budget.** A feedback budget records which evaluations,
> measurements, tool runs, human reviews, and deployment signals are available,
> what they cost, how long they take, and what level of decision they can
> support.

> **Rejection authority.** Rejection authority is the independent test, tool,
> rule, review, or human decision that can say a candidate is invalid,
> insufficiently supported, or not ready for a stronger commitment.

> **World model.** A world model is the loop's belief about how actions change
> architecture state and outcomes, including costs, constraints, invalid-action
> rules, uncertainty, and decision policies.
...
@fig-moonshot-prompt keeps the prompt visible as an object of analysis. The
top panel states the lighthouse prompt; the middle panel names the architectural
obligations embedded in that request; and the bottom panel shows the loop turn
that would be needed before an AI system's answer deserved architectural trust:
represent the task, act through bounded tools and methods, gather evidence,
reject weak outputs, decide what to commit, and revise the next turn. The point
is not "type a prompt and get a chip." The point is to expose what a credible
loop would need to know.
...
@sec-data-representations-world-models asks what data, representations, and
world models agents would need. @sec-architecture-environments-tool-interfaces
