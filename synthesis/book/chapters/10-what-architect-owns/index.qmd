...
It looked like a request for a future hardware foundation model[^fn-hardware-foundation-model-c10]. By this
...
Together, those questions define the architecture control surface: the part of
the design space the architect exposes to methods, tools, and agents. Choosing
that surface is an architectural act. Expose too little and the loop cannot
...
owns the control surface that connects those pieces into a defensible system
claim.
...
| Abstraction and representation | Architecture schema, control surface, assumptions, hidden-state list, and redaction boundary. |
| Environment and action authority | Tool contract, legal actions, invalid-action rules, cost model, and provenance requirements. |
| Evidence standard | Fidelity ladder, calibration record, evidence packet, and escalation threshold. |
...
A useful answer is not raw design disclosure. It is a redacted evidence packet:
...
| Action-observation interface | What the loop could change, what it could observe, and which tools or environments returned feedback. | Makes the architecture control surface inspectable. |
...
The earlier benchmark examples show why this matters: community infrastructure
needs rules, versions, submissions, and governance, not only task names.
Architecture 2.0 needs a similar instinct for loops. ArchGym shows one way to
make architecture design-space exploration
...
standard is another, and that path is gradual. A research
...
| --- | --- | --- | --- |
| Practicing architects | A design-loop card attached to a design review or DSE report. | Name the workload, control surface, evidence stages, rejected alternatives, and decision owner. | Compare two candidate loops for the same design question. |
...
| Systems researchers | A workload and deployment evidence packet. | Track software stack, runtime policy, operating point, and drift. | Show when a local optimization stops supporting the end-to-end claim. |
...
For a team that wants to start immediately, the adoption path should be modest.
...
| --- | --- | --- |
| First 30 days | Attach a design-loop card to one active design review, DSE result, or paper discussion. | The team can name the task, control surface, evidence, rejected alternatives, and decision owner. |
...
| First 90 days | Define one shared evidence packet or negative-trace format for a recurring architecture decision. | Future reviews reuse prior failures instead of rediscovering them, and claims state their commitment level. |
...
Each chapter has contributed a design principle. These principles are more
useful than a list of fashionable tools because they give reviewers, students,
and builders a stable way to inspect new work.
...
argued and @sec-loop-patterns-across-stack showed at fleet scale, there is no
frozen artifact for the architect to own. A specific design ages as the workload
it served moves on. What remains stable is the loop: how intent is bounded,
evidence is demanded, candidates are rejected, and commitment is decided. The
architect's durable possession is therefore not a chip, a configuration, or a
model, but the discipline that decides what a moving target is allowed to commit
to, and on what evidence.
