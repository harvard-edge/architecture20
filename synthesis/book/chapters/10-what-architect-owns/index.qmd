...
**In the Lighthouse prompt.** "XRBench real-time mobile XR" names a workload
distribution, quality-of-experience target, benchmark version, traces, and
software stack. "64-bit RISC-V-based" names an ISA and ABI/software contract:
instruction semantics, vector behavior, memory ordering, compiler/runtime
support, operating-system interfaces, and binary compatibility. "Vector-capable
CPU, tightly coupled accelerator, or SoC block" names an integration choice:
where state is shared, how data moves, what interface software targets, and
where synchronization costs appear. "3\ W" and "3\ nm-class" name the power,
thermal, process, library, physical-design, and verification envelope.
"Design-space report with evidence and rejected alternatives" names the
