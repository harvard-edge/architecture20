...
Nor does responsibility become distributed simply because several agents divide
the work. Splitting a loop across agents can make delegation more effective, but
it also creates more places where authority can become ambiguous. The architect
still has to decide which roles may act, which agent or tool can stop another,
which evidence crosses an escalation threshold, and who owns the commitment when
the agents disagree or all agree for the wrong reason.
...
or breaks the product's safety certification. That line, between work a loop may
assist and the commitments the architect still owns, intent, abstraction,
evidence standards, rejection authority, deployment risk, and responsibility for
consequences, is the boundary the rest of this chapter defends.
...
Individual practice is not enough to move a field. The ownership test names who
...
4. How should loop artifacts name the accountable human, delegated-agent
   authority, team-level roles, escalation path, signoff, rollback path, and
   revalidation trigger for drift cases such as workload-version changes,
   tool-version changes, model updates, or deployment assumption changes?
:::
...
evidence, rejected candidates, and assigned ownership. They are more useful than a list of fashionable tools
