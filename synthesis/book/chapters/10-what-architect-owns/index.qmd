...
interface software targets, and where synchronization costs appear. "3\ W" and
"3\ nm-class" create power, thermal-design-power, process, library,
physical-design, and verification obligations; the loop must not confuse a
proxy average-power estimate with sustained thermal or signoff evidence.
"Design-space report with evidence and rejected alternatives" creates the
deliverable and its rejection burden.

**Reader takeaway.** The architect-owned work is deciding which of these
contracts are fixed, which are exposed to the loop, and which have enough
evidence to support a commitment.
:::
