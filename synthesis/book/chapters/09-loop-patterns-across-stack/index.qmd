...
## The Rejection Bound {#sec-the-rejection-bound}

These loop families look different on the surface, yet they succeed and fail for
the same reason, and that reason can be written down. The interface speedup form
...
$$
Here $f$ is the fraction of commitments the cheap rejector can discharge,
$(1-f)$ is the irreducible part that still needs high-fidelity evidence or human
judgment, $c_{\mathrm{cheap}}/c_{\mathrm{hi}}$ is the cost of a cheap check
relative to a high-fidelity one, and $\epsilon_{\mathrm{escalate}}$ is the
overhead paid each time the cheap rejector defers and the loop must climb the
fidelity ladder. The form is Amdahl's law as LogCA prices it
...
count does not appear in the bound. A loop can double, or multiply by a thousand,
the number of designs it proposes and move $S_{\mathrm{loop}}$ not at all. The
bound exposes only three levers: raise $f$, lower the cost ratio, or lower
escalation overhead.

::: {.callout-design-principle title="The Rejection Bound"}
...
What changes is the operating regime, which the Rejection Bound names exactly as
the value of $f$, the cost ratio between cheap and high-fidelity feedback, and
the escalation overhead. Feedback latency changes. Reversibility changes. Action
...
- **Principle:** Every loop obeys the same Rejection Bound, where throughput is
  set by rejection capacity, not generation. What changes across patterns is the
  operating point: the cheap-rejector coverage $f$, the cost ratio between cheap
  and high-fidelity feedback, and the escalation overhead.
