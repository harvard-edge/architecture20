...
- name the cheap independent rejection authority and the human audit point;
...
migration loop: represented state, independent rejection, preserved failures, and
...
escalating to expensive feedback or human judgment. This is the Ch7 term in its
...
::: {.callout-note title="Measuring the rejection bound" icon=false}
To estimate $f$ for a real loop rather than assert it, fix a protocol before the run.

- **Commitment unit.** State what one commitment is (a candidate accepted for the
  next fidelity stage, an RTL change merged, a signoff waiver), so $f$ is a rate
  over comparable events.
- **Outcome labels.** Label every candidate commitment `cleared`, `rejected`,
...
- **Independence.** Disclose whether the rejector shares data, model, or authorship
  with the generator; a rejector that does is not independent, and $f$ is inflated.
...
Without this protocol, $f$ is a story about the loop, not a measurement of it.
:::
...
Autonomy is a property of the loop contract, not the model. Use more autonomous
...
system-wide, shift the loop toward critique, evidence organization, rejection,
and human approval.
:::
...
1. What AI-assisted loop-record corpus should preserve workload state,
   candidate history, compiler/code-generation artifacts, deployment telemetry,
   negative traces, tool evidence, waivers, and human decisions as a design
   moves from simulation to software paths, fleet evidence, and signoff?
2. What cheap-rejection benchmark can estimate which independent authorities
   safely reject AI-assisted commitments before escalation to expensive
   simulation, compiler/runtime validation, deployment canaries, or signoff tool
   flows, and what false-pass or false-reject rates disqualify a rejector?
3. What matched task suite across loop regimes can test AI role choices --
   generator, optimizer, critic, evidence organizer, test author, telemetry
   analyst, or bounded repair tool -- by false commitments, justified
   rejections, escalation cost, and human override quality?
4. What evidence-transfer protocol decides when evidence from one AI-assisted
   loop pattern can be reused by another and when it must be invalidated because
   the simulator, compiler path, deployment regime, workload distribution, or
   signoff constraints have changed?
:::
