...
coherence assumptions. A loop may still test a looser accelerator variant, but
then the coupling assumption must be explicit.

**Reader takeaway.** The DSE loop should reject candidates that only win by
moving cost into memory traffic, compiler support, verification, or SoC
integration work that the candidate did not price.
:::
...
encoding. The migration case is not the same target, but it reveals the shape of
the obligation: build, test, sanitizers, production monitors, and application
owners become rejection authorities for the ISA choice.

**Boundary.** This does not turn the lighthouse subsystem into a fleet migration
story. It uses the migration case to explain the kind of software-contract
evidence a RISC-V subsystem claim would eventually need.

**Reader takeaway.** ISA decisions are credible only when the loop can show what
...
For a loop pattern you work in, what is the cheapest independent rejector that
could discharge a larger fraction of commitments, and what currently stops you
from trusting it enough to let it reject without escalation?
:::
