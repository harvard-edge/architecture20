...
% Global table header shading: tint the header row of every Quarto longtable.
% Pandoc emits "\toprule\noalign{}" then the header row, so a naive \rowcolor
% lands before that \noalign and errors. Instead, patch \toprule to grab the
% following \noalign{...}, re-emit it, and only then start the header rowcolor.
\usepackage{colortbl}
\definecolor{archtblhead}{HTML}{ECF1F5}
% Use \def (not \renewcommand[..][]): the LaTeX optional-arg wrapper inserts
% tokens that break the \noalign row boundary booktabs rules rely on.
\let\arxoldtoprule\toprule
\def\arxheadrow\noalign#1{\noalign{#1}\rowcolor{archtblhead}}
\def\toprule{\arxoldtoprule\arxheadrow}
