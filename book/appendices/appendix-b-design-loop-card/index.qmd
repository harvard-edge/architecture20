...
The synthetic fixture does not replace the executed AI-assisted study in @sec-running-the-loop.
...
- **3. Checks/Decision Rights (`checks`, `decision_rights`):** This is a lightweight adaptation of formal Verification and Validation (V&V) standards (like DO-178C in aerospace or ISO 26262 in automotive) applied to AI-assisted architecture, ensuring no automated action bypasses human accountability.
...
The design-loop card is a record of an AI-assisted architecture study, but the study itself is only one part of the architecture lifecycle. Regardless of how exhaustive the study is, it must eventually execute **The Architect's Pivot**.

In any AI-assisted workflow, the AI operates strictly as a search heuristic proposing candidates within a bounded design space. The human architect must remain the final judge of physical correctness. A candidate geometry, RTL block, or memory schedule that passes the proxy metrics recorded in the card must ultimately pass through a strict physical design flow, where it will be evaluated against hard physical constraints such as power, area, timing closure, and routing congestion. The card captures the context, checks, and decision rights for the proxy study, but the final implementation signoff belongs to the human architect verifying those physical realities.
