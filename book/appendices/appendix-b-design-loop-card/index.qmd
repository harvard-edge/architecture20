...
names the study boundary, records typed claims and their evidence links,
summarizes failed or rejected work, and points to deeper artifacts. Detailed
observations belong in a supporting evidence record. Executable commands,
environment bindings, inputs, and outputs belong in a replay package. The card
...
details. A form that is too long often goes unfinished. One that is too vague
hides the study boundary and evidence. Neither helps another architect make a
bounded judgment.
...
Each precedent contributes a different part of the review. Scope and use conditions
bound the claim, provenance and run rules make the evidence inspectable, and
alternatives and authority connect the evidence to a decision.
These entries make missing structure harder to hide while leaving the reviewer
to judge whether the evidence supports the claim.

## The Twelve Review Fields

The human-facing card keeps twelve review fields. Before execution, a study owner can use them to bound the work and state the proposed claim. During execution, the owner adds evidence, failed or rejected work, and rejection-check results. At review, the owner records the supported claim boundary and accountable decision. Schema 2.0 represents some fields with more than one machine key because claim support and decision authority require separate records rather than a single prose string. @tbl-design-loop-card-fields gives the review question and schema representation for each field.
...
The display label **Accountable decision** maps to the stable machine schema key
`accountable_decision`. The record names the holder, action, scope, rationale,
claim links, and conditions for reopening the decision.
...
The one-page form in @tbl-blank-design-loop-card is enough to begin. Copy it
when the study is proposed, record the initial boundary and claim, and update
its links as evidence and decisions accumulate. Keep detailed observations in
the evidence record and executable details in the replay package.
...
Use the one-page card to draft and review a study. Use
[JSON Schema version 2.0](https://arch2.mlsysbook.ai/schemas/design-loop-card.v2.schema.json),
the canonical machine contract, when software needs to validate a card, check
links among records, or exchange the card across tools.

Before reading the schema, use this mental model for its three core pillars:
...
- **3. Gates/Decision Rights (`gates`, `decision_rights`):** Who says it is okay? This is a lightweight adaptation of formal Verification and Validation (V&V) standards (like DO-178C in aerospace or ISO 26262 in automotive) applied to AI-assisted architecture, ensuring no automated action bypasses human accountability.

```mermaid
erDiagram
    CLAIM ||--o{ EVIDENCE : requires
    EVIDENCE ||--|| ENVIRONMENT : generated_in
    EVIDENCE }|--o{ GATE : evaluated_by
    GATE }|--|| ACCOUNTABLE_DECISION : authorizes
```
...
The outline abbreviates the `roles` definition. Its compact core enum is
`generate`, `predict`, `optimize`, `critique`, `verify`, `plan`, `tool_call`,
and `coordinate`. The eight roles in
@sec-methods-generation-prediction-optimization form the teaching taxonomy for
work inside a study. Mapping the machine enum to that taxonomy remains a
...
rejection-check IDs exist. These checks establish structural consistency and
reference integrity. They do not decide whether the linked evidence supports the
statement.
...
prose can add context under `notes`, but it cannot replace those fields. A tool
crash should normally be recorded as an `environment_failure`. It becomes a
candidate-rejection check only when a predeclared rule makes that failure
disqualifying for the candidate rather than evidence that the tool setup is
broken.

Decision rights are assigned by action and scope, not by a human-versus-machine
classification. A model, tool, pipeline, policy, team, or person may hold a
scoped right to propose, execute, reject, waive, or recommend. A complete
decision-right profile must assign a holder for all six actions, including
`commit`. The decision record names the holder of that commit right and the
bounded organizational action. A reviewer must assess the evidence separately
...
strengthens another. @tbl-card-profiles states what each completion claim requires and what it does not imply.
...
shows that the named replay package ran in the declared environment. Broader
reproducibility or transfer requires a separate evaluation. For proprietary EDA
flows, NDAs, and closed-source PDKs, 'Replay' may only be verifiable by authorized
internal reviewers. In these cases, the card must document the exact tool
versions, licensing dependencies, and why public replay is impossible.
...
The output is a migration draft, not a valid v2 card. It lists unresolved
semantics such as comparator, outcome, claim status, evidence status, typed
rejection checks, replay validation, and decision rights. The migration cannot
recover those meanings from a legacy card. Inventing them would misrepresent
the study.

## Review Rubric

The rubric asks whether each field is strong enough for the claim and proposed
action. It is not a numerical score. @tbl-design-loop-card-rubric separates a pass signal from the warning sign it is meant to catch.
...
adequacy. @tbl-evidence-disclosure-tiers keeps four commonly conflated facts separate.
...
These dimensions can point in different directions. A public packet may lack
the artifacts needed for replay, while a restricted packet may support an
independent review by authorized reviewers. Report each dimension separately.
...
`complete`, `partial`, or `not_claimed`. Compare what each reading reveals. Three patterns usually appear.
...
These failures affect the study itself. They can change which claim is supported
or which action may be authorized, so correcting the card alone is not enough.
...
Name who may propose, execute, reject, waive, recommend, and commit, and keep
each right within a declared scope.
...
As the study grows, add links to the evidence record and replay package rather
than expanding the card into an archive. The card then remains a reliable index
for later review.
