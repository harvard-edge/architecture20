...
## Choose a Card Over a Paper Summary
...
| **Design space** | Which choices are legal, invalid, or deferred? Does the action space restrict the agent to parameterized tuning, or does it allow arbitrary topological changes that might break LEC (Logic Equivalence Checking)? |
...
| **Evidence** | Each evidence record identifies its producer, kind, status, tool and version, input and output artifacts, scope, uncertainty or limitations, and integrity check. Does the evaluation score physical synthesis results (PPA) or only logical compilation pass rates? |
...
With these twelve fields in mind, the blank form in @tbl-blank-design-loop-card is enough to begin. Copy it when the study is proposed, record the initial boundary and claim, and update its links as evidence and decisions accumulate.
...
The public companion materials provide four starting points.
...
## Examine an Executed Study Card Sketch

While the synthetic fixture demonstrates the formatting, @tbl-executed-study-card-sketch shows how the actual systolic-array study from @sec-running-the-loop appears in the card's human-facing fields. The linked replay package contains its detailed machine records.
...
## Validate with Machine Contract 2.0
...
- **1. Context/Environment (`context`, `environment`):** This bounds the task, the workload, and the permitted toolchain.
- **2. Claims/Evidence (`claims`, `evidence`):** This links assertions to checkable execution logs.
- **3. Checks/Decision Rights (`checks`, `decision_rights`):** This is a lightweight adaptation of formal Verification and Validation (V&V) standards (like DO-178C in aerospace [@RTCA2011DO178C] or ISO 26262 in automotive [@ISO2018Iso26262]) applied to AI-assisted architecture, so no automated action bypasses a recorded human decision.
...
These structural fields are evaluated through six independent profiles, which replace the former cumulative Level 0 to 3 ladder. The profiles are non-ordinal, meaning that completing one neither requires nor strengthens another. @tbl-card-profiles states what each completion claim requires and what it does not imply.
...
When standard profiles do not capture specific proprietary workflows or novel tasks, open task identifiers and namespaced `x-...` fields allow extensions. However, an extension cannot silently redefine a claim status, evidence status, profile, rejection-check result, or decision right.
...
## The Card Records a Proxy, Not the Signoff

Regardless of the schema version, the design-loop card is a record of an AI-assisted architecture study, but the study itself is only one part of the architecture lifecycle. However exhaustive the study, its result is still a proxy that must survive the physical-design flow.

A candidate geometry, RTL block, or memory schedule that passes the proxy metrics recorded on the card must still hold up against hard constraints on power, area, timing closure, and routing congestion. The card documents the context, checks, and decision rights for the proxy study; it does not stand in for the physical signoff, which rests with the architect who verifies that result.

## Apply the Review Rubric

To determine if the proxy study justifies that verification effort, reviewers can apply a standardized rubric. The rubric asks whether each field is strong enough for the claim and proposed action rather than acting as a numerical score. @tbl-design-loop-card-rubric separates a pass signal from the warning sign it is meant to catch. This rubric reviews a single study documented on its card. When the task is reviewing a submitted paper rather than one completed study, @sec-appendix-c-reviewer-checklist gives a complementary checklist focused on artifact release, proxy-versus-oracle separation, baselines, and complete cost accounting.
...
## Use the Card in Papers and Case Studies

This review framework applies equally to academic and industrial contexts. A research paper and an industrial case study use the same card and may disclose different supporting records, but they do not need different claim or authority semantics.
...
## Run the Paper-to-Study Exercise

To see how disclosure differences and implicit structures can obscure missing evidence, try this paper-to-study exercise. Ask two readers to inspect the same paper, where one writes a conventional summary and the other fills a context-profile card while marking every other profile `complete`, `partial`, or `not_claimed`. Comparing what each reading reveals usually exposes three patterns.
...
In both reading exercises and active projects, the card proves most useful when it reveals failures early.
...
The decision holder can be a student, advisor, reviewer, or project lead, so assign these six decision rights to specific people while keeping each right within a declared scope.
:::

As the study grows, update these links rather than expanding the card into an archive so it remains a reliable, one-page index for later review.
