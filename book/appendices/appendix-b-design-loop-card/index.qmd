...
Architecture 2.0 work will often arrive as a paper or project report before it
arrives as stable infrastructure. A reported result is therefore only the
start of review. Another architect must be able to reconstruct the bounded
study, identify the claim and comparator, inspect what the evidence supports,
and tell which role may authorize the next action.

The **design-loop card** is a compact summary and index into that review packet.
It applies whether a study runs one evaluation or many iterations. The card
names the study boundary, records typed claim and evidence links, summarizes
failed or rejected work, and points to deeper artifacts. Detailed observations
belong in a supporting evidence record. Executable commands, environment
bindings, inputs, and outputs belong in a replay package. The card does not
replace either one.

The card should remain short enough to use. One page is a good first pass. A
few linked records can contain disputed evidence, raw tool output, or protected
details. A long form that nobody completes and a vague form that conceals the
study fail for the same reason. Neither helps another architect make a bounded
judgment.

## Why a Card, Not a Paper Summary

A conventional summary names the problem, method, result, and limitations. It
often omits the state the method could see, illegal actions, evaluation budget,
failed alternatives, tool versions, or the checks that could stop advancement.
Those omissions matter once models generate candidates, call tools, select
experiments, or summarize evidence.

The card borrows the compact disclosure format of several established
records. Component datasheets and datasheets for datasets record provenance and
usage limits [@GebruEtAl2021Datasheets]. Model and system cards state
intended use, evaluated conditions, and limitations
[@MitchellEtAl2019ModelCards; @MetaAI2022SystemCards]. Assurance cases connect
claims, context, evidence, and defeaters [@Kelly2004GSN]. Architecture Decision
Records preserve alternatives and rationale [@Nygard2011ADR]. Artifact-review
and reporting checklists distinguish a claim from an exercisable artifact
[@ACM2020ArtifactReview; @PineauEtAl2021Reproducibility; @PageEtAl2021PRISMA].
Benchmark and supply-chain records add versioned run rules and integrity
bindings [@MattsonEtAl2020MLPerf; @SPDX2021Standard; @SLSA2026Framework].
...
Filling the card does not make a result credible. It makes missing structure
harder to hide.

## The Twelve Review Fields

The human-facing card keeps twelve review fields. Schema 2.0 represents some
of them with more than one machine key because review and authority should not
be compressed into a single prose string.
@tbl-design-loop-card-fields gives the review question and schema representation for each
field.
...
| **Rejection checks and authority** | The schema calls each predeclared pass/fail check a `gate`. Its record includes the criterion, result, authority, waiver rule, evidence links, and outcome. `decision_rights` separately names who may propose, execute, reject, waive, recommend, and commit. |
...
| **Decision and owner** | What action is pending, authorized, rejected, deferred, or superseded; who holds the commit right; what scope is authorized; and what would reopen the decision? |

: **Twelve fields for reviewing a study.** Together they describe the study from intent through authorized action. Completing the fields does not establish that the evidence is adequate for the claim. {#tbl-design-loop-card-fields tbl-colwidths="[23,64]"}

The human-facing label **Decision and owner** corresponds to the stable machine
schema key `accountable_decision`. The key is retained for compatibility; the
record itself must name the holder, action, scope, rationale, claim links, and
conditions for reopening the decision.

The claim boundary is essential. A study may show an architecture outcome
without showing that AI helped. If authors claim both, schema 2.0 expects two
claim records. One can be `architecture_outcome` and the other
`ai_contribution`, each with its own comparator, status, and evidence links. A
better design from an AI-assisted arm does not by itself distinguish method
benefit from a different evaluation budget, candidate set, or stopping rule.

The schema defines these evidence statuses: `measured`,
`computed`, `digitized`, `illustrative`, and `author-judgment`. The status tells
the reader how the record came into being. It does not rank evidence quality.
A careful computed simulation may support a bounded architecture claim better
than a poorly controlled measurement, while an illustrative packet should
never be presented as an empirical result.

## Blank Template

The one-page form in @tbl-blank-design-loop-card is enough to begin. A study
owner can fill context and a proposed claim before execution, then complete
other profiles only as supporting records become available.
...
The synthetic fixture and deterministic course lab have different jobs. Neither
replaces the executed AI-assisted study in @sec-running-the-loop.

## Machine Contract 2.0

[JSON Schema version 2.0](https://arch2.mlsysbook.ai/schemas/design-loop-card.v2.schema.json)
is the canonical machine contract. The outline below shows the required
structure without repeating every conditional rule.
...
design_loop_card:
  ...
  failed_or_rejected:
    - { record_id, kind, candidate_id?, stage, reason, gate_ref?, evidence_refs }
  gates:
    - { gate_id, category, criterion, result, authority_id, waiver_rule,
...
The schema requires at least one typed claim. A proposed or untested claim may
have an empty `evidence_refs` array. Every stronger status requires at least one
evidence link. The validator also checks that referenced claim, evidence, and
rejection-check IDs exist. A status remains an author assertion, however. Validation
cannot decide whether the cited evidence really supports the statement.

The machine schema retains the key `gates` for compatibility. Here it means only
a predeclared pass/fail check; it never means a design change, workflow stage,
reviewer, or general quality goal. Every entry has a typed category, a decidable
criterion, result, authority, waiver rule, evidence links, and outcome. Free
prose can add context under `notes`, but it cannot replace those fields. A tool crash should normally be
recorded as an `environment_failure`. It becomes a candidate-rejection check
only when a predeclared rule makes that failure disqualifying for the candidate
rather than evidence that the tool setup is broken.

The decision-right model is deliberately not a human-versus-machine switch. A
model, tool, pipeline, policy, team, or person may hold a scoped propose,
execute, reject, waive, or recommend right. A complete decision-right profile
must assign all six actions, including `commit`. The decision record names
the holder of that commit right and the bounded organizational action. Evidence
does not authorize itself, and organizational authority cannot strengthen weak
evidence.

### Independent Profiles, Not a Maturity Ladder

Schema 2.0 replaces the former cumulative Level 0–3 ladder with six independent
profiles. The profiles are non-ordinal. Completing one neither requires nor
strengthens another.
@tbl-card-profiles states what each completion claim requires and what it does
not imply.
...
A `partial` profile must list its missing bindings in `profile_gaps`. Use
`not_claimed` when the profile is outside the current record. Do not mark a
profile complete merely because some related prose exists.

Replay has the strictest mechanical rule. A card may claim the replay profile
as complete only when commands, an environment binding, inputs, outputs,
expected status, observed status, hashes, and a verified result are present.
The command-line validator checks hashes for local card-relative artifacts. It
cannot fetch and revalidate remote or protected artifacts. Successful replay
shows that the named packet ran in the declared environment. Broader
reproducibility or transfer requires a separate evaluation.

### Extensions and Compatibility

Open task identifiers and namespaced `x-...` fields allow new task-specific
information without changing the versioned core. An
extension cannot silently redefine a claim status, evidence status, profile,
rejection-check result, or decision right.

Schemas 1.0 and 1.1 remain readable for one public compatibility release.
Their cumulative levels and legacy keys are frozen. New cards should use 2.0.
The migration command maps only content with a direct meaning.

```console
./arch2 migrate card old-card.yaml --output old-card.v2-migration-draft.yaml
```

The output is a migration draft, not a valid v2 card. It lists missing
semantics such as comparator, outcome, claim status, evidence status, typed
rejection checks, replay validation, and decision rights. This is intentional. A migration
that invented those meanings would produce a cleaner file and a less truthful
record.

## Review Rubric

The rubric asks whether each field is strong enough for the claim and proposed
action. It is not a numerical score.
@tbl-design-loop-card-rubric separates a pass signal from the warning sign it
is meant to catch.
...
| **Claims** | Statement, comparator, outcome, scope, non-claims, status, and evidence links agree. AI contribution is separate from architecture outcome. | A broad claim rests on a narrow benchmark, or AI receives credit without a matched no-AI or conventional comparison. |
...
| **Rejection checks and decision rights** | Criteria, results, authority, waivers, evidence, outcomes, and all six rights are stated. | A plausible but wrong result has no stopping path, or a recommender is treated as the decision owner. |
| **Decision and owner** | Status, holder, action, rationale, scope, claim links, and reopen conditions match the evidence. | Evidence is self-authorizing, the authorized scope exceeds support, or no role is responsible for the action. |
...
If those questions cannot be answered, the work may still be promising, but
another architect cannot yet evaluate its architectural claim.

## Using the Card in Papers and Case Studies

A research paper and an industrial case study use the same card. They may
disclose different supporting records, but they do not need different claim or
authority semantics.

For a paper, focus on the precise claim and comparator, evidence pointers,
matched AI and conventional arms when method benefit is claimed, mechanism
tests, failed alternatives, and the check that limits the conclusion. A paper
may be technically interesting while leaving the study underdescribed. The
review question is whether a reader can tell what was tried, what failed, what
was measured, and which action the result supports.

For a case study, focus on the action space, tool and workload provenance,
decision-relevant regressions and cross-checks, disclosure boundary, and decision owner.
Confidentiality does not imply absence, and public release does not imply
adequacy.
@tbl-evidence-disclosure-tiers keeps four commonly conflated facts separate.
...
| **Replay result** | Whether replay was not attempted, failed, or succeeded in a named environment, and who performed it. | Successful replay applies to that packet and environment only. |
...
Ask two readers to inspect the same paper. One writes a conventional summary.
The other fills a context-profile card and marks every other profile
`complete`, `partial`, or `not_claimed`. Compare what each reading reveals.
...
For the debrief, record which omission changed the claim or authorization
judgment. If a replay profile is complete, attempt the recorded command and
report the result separately. If the card changed no judgment and identified no
actionable omission, ask whether it earned its review cost.

## Executed Study Card Sketch

@tbl-executed-study-card-sketch condenses the executed study into the card's
human-facing fields. The linked replay package contains the detailed machine records.
...
| **Claims and evidence** | The architecture claim requires a nonbaseline candidate to beat the baseline by at least 1 percent. The separate AI-contribution claim requires the model arm to beat the conventional arm under equal budgets. Neither claim is supported; the mechanism is falsified. |
...
| **Rejection checks and decision rights** | Geometry and deadline criteria can reject candidates. The model may propose but may not execute, reject, waive, recommend, or commit; those rights remain separately assigned. |
| **Decision and owner** | The evidence supports retaining 32 by 32 and stopping. The recorded status remains `awaiting_author_confirmation`, keeping recommendation separate from the commit decision. |
...
The card is most useful when it reveals failures early.
...
These are study-design failures with consequences beyond documentation.

Solo researchers can address them without building a large review process.

::: {.callout-field-note title="Practical workflow tips for solo researchers"}
A useful first record can be small. A local CSV of attempted parameters and
tool results is enough to begin preserving failed work, but it does not prove
adequate exploration. Record the command, tool version, candidate identity,
and reason for failure. A segfault or compile failure is an environment failure
to diagnose. It becomes a candidate-rejection check only when a predeclared
rule makes that outcome disqualifying.

The decision holder can be a student, advisor, reviewer, or project lead.
The important step is naming who may reject, waive, recommend, and commit, and
keeping each right within a declared scope.
...
The small record is a starting point, not evidence that the study is complete.
Its value is that it gives later review, replay, and expansion a reliable place
to begin.
