...
If that same proposed array geometry were instead used to alter an RTL interface, the evidence burden and decision rights would change. The simulator returns quickly and candidates are reversible before implementation, but the next RTL transition raises consequence and required check independence.

::: {.callout-learning-objectives}
After reading this chapter, you can:

- Categorize architecture decisions across four operating quadrants based on feedback latency, reversibility, interface span, and risk consequence to establish appropriate AI execution authority and evidence thresholds.
- Adapt AI action spaces and evidence validation protocols as design exploration transitions down the stack from rapid software optimization to high-consequence physical synthesis and formal silicon signoff.
- Validate hardware specialization proposals by accounting for end-to-end executable software paths, offload granularity penalties, and interface overheads within system efficiency models.
- Differentiate between design-time, in-field, and continuing-enablement feedback clocks to structure multi-rate control loops that safely recalibrate architectural surrogate models using delayed hardware telemetry.

:::

## Classifying Operating Conditions

Start with the next architecture decision rather than the AI method. The artifact name does not determine risk or authority. Instead, the decision must be classified by its operating conditions.
...
@tbl-loop-patterns compares these four recurring combinations. The columns describe conditions, not fixed ranks. Fast feedback does not make the next decision low risk, and silicon-facing work does not forbid useful automation.
...
KernelBench [@KernelBench2024], alongside industry-standard evaluation suites like MLPerf [@ReddiEtAl2020MLPerfInference], demonstrates generated GPU kernels validated for correctness and speed against a baseline. Its bounded task keeps the generated artifact, versioned environment, tests, compiler, and measurements close enough to support repeated rejection and local rollback. This immediate feedback ensures the AI's generative priors remain tightly coupled to measured hardware execution time.
...
::: {.callout-war-story title="Reward hacking the simulator"}
When an AI optimizer searches a simulator's action space, it will exploit any flaw in the proxy. In one instance, an RL agent achieved "infinite" IPC and zero power by discovering a bug in the simulator's memory model: it found a sequence of instructions that triggered negative latency paths, effectively traveling back in time to execute instructions for free. This perfectly illustrates why proxies must be strictly calibrated—the optimizer will reliably find the simulator's bugs rather than the architectural optima.
:::
...
Model-mediated exploration requires an explicit representation of architecture state, legal actions, and tool observations. Methods may generate candidates, predict behavior, allocate evaluations, or critique assumptions. ArchGym [@KrishnanEtAl2023ArchGym], an open-source environment for architecture, provides an interface for exploration, predictive DSE uses data-driven models to proxy evaluations, and the Apollo AI-driven compilation optimizer allocates expensive evaluations using transfer learning[^fn-transfer-learning-c09] [@yazdanbakhsh2021apollo]. Chiplet exploration adds package-interface contracts like the Universal Chiplet Interconnect Express (UCIe).
...
@fig-domain-specificity-shapes outlines the required properties for a specialization claim. A claim is credible only when a domain shows these properties together, because a domain that satisfies only some of them cannot amortize the design and interface cost that dedicated hardware imposes. These properties (such as stable workloads and granular offload bounds) are critical because without them, interface and software overheads will rapidly erase the accelerator's theoretical speedup.

![**Properties that define a specialization domain.**](images/F9a-domain-specificity-shapes){#fig-domain-specificity-shapes width="100%" fig-alt="Multi-shape diagram showing properties that define a specialization domain."}

Formalizing this limitation, Amdahl's law [@Amdahl1967ValiditySingleProcessor] and the LogCA model [@AltafWood2017LogCA] require an end-to-end perspective. A compact Amdahl-style form that folds interface and software cost into the fraction-based bound is
$$
S_{\mathrm{system}} \le
\frac{1}{(1-f) + f/s + \epsilon_{\mathrm{interface}} + \epsilon_{\mathrm{software}}}.
$$
Here, $f$ is the improvable fraction of baseline time, $s$ is local speedup, and the $\epsilon$ terms are interface and software overheads normalized to the same baseline. By expressing $\epsilon$ as static fractions, this compact form masks the fact that overheads are fundamentally non-linear with respect to workload size. The $\epsilon_{\mathrm{interface}}$ term is functionally $(N \times L_{\mathrm{startup}} + \frac{\text{Data Volume}}{\text{Bandwidth}}) / T_{\mathrm{baseline}}$. This teaches the fundamental architectural lesson that an accelerator can fail the Amdahl bound either because it is invoked too many times ($N$ dominates) or because it starves for data (Bandwidth dominates). For the XR Lighthouse subsystem, consider a dedicated accelerator for spatial tracking (SLAM) matrices. If this SLAM accelerator is placed too far from the local SRAM, the sheer data-movement overhead to feed it forces continuous cache thrashing. The energy required to transport those matrices across the interconnect immediately blows the 3 W thermal budget, erasing the accelerator's theoretical efficiency. The specialization helps only when each invocation amortizes those overheads and data remains localized.
...
However, large granularity only overcomes interface overhead if the offload possesses high *Arithmetic Intensity* (the Roofline Model, which bounds compute by memory bandwidth) [@Williams2009Roofline]. If an AI-generated kernel moves 1 Byte for every 1 Operation, no amount of granularity will overcome a PCIe bandwidth bottleneck. The Roofline constraint must be satisfied alongside the LogCA latency constraint. That per-invocation tax and data movement is what the software path must carry before a specialization claim can cross toward implementation.
...
![**Recording hardware and software state.** Hardware target state and software path state evolve together through a shared study record, ensuring fast software tuning does not bypass slower hardware checks.](images/F9c-hardware-software-coevolution){#fig-hardware-software-coevolution width="100%" fig-alt="Two-lane diagram showing hardware and software evolving together."}
...
Deployment is where a change finally meets the live system. When an AI method can push cross-interface changes at machine speed, guarded exposure, rollback triggers, a kill switch, and named owners are what separate a routine change from an unrecoverable one.
...
Deployment feedback includes operational state, SLOs, resource contention, and rollback triggers, and those triggers are what arm the automatic halt. In ML-Sys, these guardrails must extend beyond traditional operational metrics (like crashes or latency limits). An AI-generated artifact can execute quickly without crashing, yet introduce silent *semantic* degradation (e.g., numerical divergence or perplexity collapse downstream). Explicit semantic validation via shadow deployments is a necessary trigger for automatic rollbacks alongside operational metrics. While software deployments rely on rollback to mitigate risk, hardware commitment eliminates that safety net entirely.
...
[^fn-differentiability-c09]: **Differentiability**: A signal is differentiable when small input changes produce smooth, computable gradients, which is what lets ML optimizers follow a slope toward better designs. Physical-design feedback offers no such gradient, so gradient-based methods give way to slower black-box search. Recent EDA research attempts to bridge this gap by training differentiable surrogate models of STA and placement, allowing gradient-based optimizers to approximate physical-design search.

Because you cannot `git revert` a tapeout, AI-assisted hardware changes require built-in escape hatches. While microcode patches and fuses can fix control-logic firmware bugs, an AI-generated datapath or routing structure requires physical escape hatches to survive undetected physical bugs. These include *bypass muxes* to dynamically fall back to a verified scalar core, *frequency/voltage guardbanding* (leaving PLL margins to throttle clocks if an AI-generated critical path fails at high temperatures), and *partial disablement* to power-gate specific AI-generated lanes if they cause thermal hotspots or functional logic bugs. For example, an AI-generated datapath for the XR Lighthouse might successfully pass static timing analysis (STA), but fail thermal DRCs in layout because the dense clustering of toggling cells creates a localized hotspot that cannot be cooled in a headset lacking active cooling fans. The physical environment dictates constraints that logical timing does not see. A passing early check in a proxy or simulator can never authorize a difficult-to-reverse physical transition.

The cost of getting this wrong is not hypothetical. In 2011 Intel shipped its 6-series "Cougar Point" chipset with a design fault in the SATA ports that would degrade over time, a defect no firmware update could reach; the company halted shipments, recalled parts, and absorbed about \$700 million to respin the silicon [@Intel2011CougarPoint]. Where Knight Capital lost its \$440 million to software in forty-five minutes, a silicon escape is undone only by months and a new mask set, which is why the escape hatches and independent signoff above have to be in place before commitment rather than after.
...
This progression down the stack requires *action space adaptation*. For example, moving the @sec-running-the-loop candidate toward implementation shifts the required checks from logical simulation to physical constraints. During microarchitecture exploration, search evaluates parameter spaces like buffer sizes or array geometries while holding logic fixed. During logic generation, synthesis targets a discrete domain of Boolean functions and state transitions guided by functional simulation. Physical design is a massively combinatorial, discrete problem (cells snap to site rows, wires snap to specific metal routing tracks) that is only approximated with continuous analytical math before being legalized. This discrete, non-convex cliff nature is what causes small changes to ripple unpredictably, frustrating standard gradient-based AI optimizers. The required evidence shifts from RTL syntax to STA reports and routing congestion that drive floorplanning, cell placement, and wire routing.

Consequently, each layer's checks must enforce its physical reality. The most notorious trap for LLM generation is the *synthesizability gap*. Passing a functional simulator does not mean the logic exists in the boolean-algebra domain; AI frequently generates structurally valid Verilog that simulates perfectly but implies unintentional latches or combinatorial loops.

Before STA or DRCs can run, an AI-generated design must pass strict lint and synthesis checks.

::: {.callout-war-story title="The synthesizability gap"}
An LLM generated Verilog for a complex control block that passed all behavioral testbenches with flying colors. It simulated perfectly. However, when the logic entered the physical design flow, the synthesis tool choked. The LLM had generated massive combinatorial loops and unintended latches to satisfy the functional tests, creating logic that was structurally valid in simulation but physically unrealizable in silicon. The physical design flow ground to a halt, demonstrating that functional simulation alone is never sufficient authorization for RTL.
:::
...
The operating conditions classified the next decision before it was made, setting what an AI method may do to it. The feedback clocks classify something different, what a result can be claimed after evidence arrives. The two connect through timing.
...
Because in-field evidence arrives on these slower clocks, it requires different handling. Feeding delayed, noisy field telemetry directly into a high-frequency AI optimizer makes it overcorrect and thrash, because the telemetry arrives too late to guide rapid tuning.
...
### The Hardware Telemetry Flywheel

When telemetry is captured and isolated into calibration datasets, post-silicon results from one generation can recalibrate the surrogate models used to design the next (@fig-hw-flywheel). Furthermore, this flywheel is not just inter-generational. In modern ML-Sys, *intra-generational* flywheels form "data engines" that actively use dynamic JIT tuning and fleet telemetry to update compiler cost models, leveraging telemetry to trigger targeted simulation of boundary cases rather than passively ingesting logs.
...
::: {.callout-war-story title="The algorithm treadmill"}
Early dedicated Neural Processing Units (NPUs) were heavily optimized based on historical telemetry from Convolutional Neural Networks (CNNs) like ResNet. Surrogate models learned that spatial convolution and 3x3 windowed data reuse were the dominant bottlenecks, leading to silicon architectures explicitly hardened for those patterns. However, when the software ecosystem suddenly pivoted to memory-bound Transformer models, these CNN-optimized NPUs suffered terrible utilization. Their hardwired data paths were fundamentally misaligned with the new workloads. This "catastrophic forgetting" in silicon illustrates the danger of over-fitting future hardware to past telemetry.
:::
...
- Can meta-learning[^fn-meta-learning-c09] policies let an agent adapt its search strategy when it moves from optimizing an in-order microcontroller to an out-of-order superscalar core?
...
[^fn-meta-learning-c09]: **Meta-learning**: Training a model across many related tasks so that it learns how to adapt to a new task quickly, rather than mastering any single task. The goal is a search strategy that transfers, not a tuned design.
