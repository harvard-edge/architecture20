...
An XR accelerator claim needs more than "XRBench" to be credible. While XRBench supplies an architecture anchor [@KwonEtAl2023XRBench], the architect must still define the scenario labels, device class, trace provenance, input distributions, excluded cases, and rejection conditions. Otherwise, an optimizer tuned to one fast slice may fail the distribution the real XR subsystem serves.
...
Architecture exploration often evaluates candidate parameters that are cheap to propose, but feedback comes from proxies, simulators, surrogates, or staged tools. Feedback is slower and more abstract than software tests. Actions can violate constraints, and proxies can fail outside their calibrated regions.
...
Software feedback supports an architectural claim only when the run record ties each code path to the hardware snapshot it targeted. We can view this through an Evolutionary Biology analogy: software tuning (the fast loop) is like behavioral adaptation or phenotypic plasticity—it is rapid and reversible. In contrast, a silicon tapeout (the slow loop) is like morphological lock-in or genetic fixation. If a fast software loop is accidentally granted the authority to make a catastrophic morphological decision without independent checks, it leads to system extinction. @fig-hardware-software-coevolution illustrates this hardware-software coevolution, showing how these two radically different adaptive timescales must remain structurally joined by one claim without bypassing the slower layer's checks.
