...
AI optimizers often exploit a basic principal-agent problem: they can post massive local throughput gains by dumping technical debt onto the compiler interface, taking advantage of information asymmetry. Independent rejection checks are formally required here as "costly state verification." When an optimizer tunes against a local proxy such as core area, check whether the harness includes checks for candidates that win only by pushing complexity into memory traffic, compiler support, verification, or SoC integration.
:::

### Specialization and Executable Software Paths

A specialization claim needs an executable software path and an end-to-end interface-cost argument before a proxy win can cross toward implementation.
...
Software feedback supports an architectural claim only when the run record ties each code path to the hardware snapshot it targeted. We can view this through an Evolutionary Biology analogy: software tuning (the fast loop) is like behavioral adaptation or phenotypic plasticity—it is rapid and reversible. In contrast, a silicon tapeout (the slow loop) is like morphological lock-in or genetic fixation. If a fast software loop is accidentally granted the authority to make a catastrophic morphological decision without independent checks, it leads to system extinction. 

This directly maps to the AI Spectrum and the Cost/Efficiency of AI. Fast, reversible loops (plasticity) should be driven by cheap, highly efficient models—Support Vector Machines (SVMs) or specialized Small Language Models (SLMs)—that can handle high-frequency, low-latency search space exploration at low inference cost. Conversely, slow, irreversible morphological decisions warrant the high inference cost of frontier Large Language Models (LLMs). The compute cost of a frontier LLM is infinitesimally small compared to the CapEx of pouring silicon concrete. A rigorous methodology matches the cognitive cost of the AI to the physical cost of the decision.

@fig-hardware-software-coevolution illustrates this hardware-software coevolution, showing how these two radically different adaptive timescales must remain structurally joined by one claim without bypassing the slower layer's checks.

![**Recording hardware-software coevolution.** Hardware target state and software path state evolve together through a shared study record, balancing behavioral plasticity and morphological lock-in.](images/F10c-hardware-software-coevolution.svg){#fig-hardware-software-coevolution width="100%" fig-alt="Two-lane diagram showing hardware and software evolving together."}
...
High-consequence work includes RTL changes, physical design, timing closure, power analysis, and formal signoff. Feedback is delayed and reversal is slow and expensive. In a Macro-Economics analogy, software patches are like adjusting overnight interest rates (high liquidity, easily reverted market signals). Conversely, committing silicon is like pouring concrete for a deep-water port: a massive CapEx endeavor defined by sunk costs and multi-year horizons where timescales strictly dictate reversibility. AI heuristics frequently break down when hitting the physical realities of multi-day placement and routing runs, severe Design Rule Check (DRC) violations, and highly non-linear timing closure loops where physical design feedback is notoriously non-differentiable. Because you cannot `git revert` a tapeout, architecture must borrow the concept of *sagas* (compensating transactions) from distributed systems. Hardware must be designed with escape hatches—like microcode patches, degraded fallback modes, or fuses—to compensate for irreversible AI flaws once they reach silicon. Consequently, a passing early check cannot authorize a difficult-to-reverse transition.
...
**In-field evidence** tests the deployment context. Google and Meta discovered "mercurial" cores that silently returned wrong results after passing manufacturing tests [@HochschildEtAl2021Cores; @DixitEtAl2021SDC]. This observation rejected the sufficiency of earlier test regimes under fleet-scale operation, even though it did not identify a unique architectural cause. It showed that deployment can expose failure classes missed by prior checks.
...
![Cascade Control and the Smith Predictor illustrates nested feedback loops where an inner SLM handles fast tuning while an outer LLM processes delayed telemetry through a predictive review gate.](images/F9-cascade-control.svg){#fig-cascade-control}

Feeding delayed observations, like field telemetry, directly into a high-frequency AI optimizer causes the same violent oscillations seen in naive control systems. Instead of static placement, use Control Theory (specifically, *Cascade Control*) to view the system as nested feedback loops (@fig-cascade-control). Fast inner loops handle immediate stabilization and local tuning, while slow outer loops handle semantic planning and architectural policy. This requires the equivalent of a Smith Predictor: a mechanism that simulates the system's delayed physical response to prevent the AI controller from wildly overcorrecting while waiting for real telemetry.
...
![Amdahl's Law for Human Review contrasts the high proposal arrival rate of an AI generator with the constrained settlement rate of human verification.](images/F9-review-bottleneck.svg){#fig-review-bottleneck}
