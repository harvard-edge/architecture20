...
exemplar; no measured cross-layer hardware co-design loop is offered. The reported effort analyzed more than 38,000
...
fast trusted, independent dispositions can be applied, not by how fast
candidates can be produced. This is the compact hypothesis the preceding
vocabulary makes measurable. The interface speedup form
...
If a loop is not yet instrumented enough to estimate these terms, use the bound
qualitatively first. Name the commitment unit, the cheap independent check, and
the cases that must still escalate.
...
For instance, if a fast compiler pass (the cheap rejector) can confidently discard 90% of invalid generated kernels ($f=0.9$) at 1/1000th the cost of full execution on silicon ($c_{\mathrm{cheap}}/c_{\mathrm{hi}} = 0.001$), the loop's throughput accelerates dramatically, provided the pass's overhead for escalating borderline cases ($\epsilon_{\mathrm{escalate}}$) does not wipe out those gains.
...
Software testing gives the intuition. A fast unit test that misses bugs later
caught by integration tests cannot earn a cheap clear just because it is cheap,
and a flaky test that discards good patches does not make the pipeline more
reliable. Cheap architecture rejectors need the same calibration against
stronger checks before their clears can count.
...
credited. It is an Amdahl-like lens, not a new law. What is new is what the
variables name, what they rule out, and the protocol that would falsify them.
...
For a fixed commitment target, an AI-assisted design loop becomes
rejection-bound when candidate supply exceeds the capacity of trusted,
independent adjudication. In that regime, adding candidates does not raise
per-commitment throughput. The loop speeds up when a cheap, independent,
trusted rejector discharges a larger fraction of commitments, when trusted
feedback gets cheaper relative to high-fidelity evidence, or when escalation
gets cheaper. Search loops can remain generation-bound when a better generator
reduces the evaluations needed to reach a fixed-quality target.
:::
...
The fleet ISA migration earlier in this chapter gives one reading. Applied to the
...
critique, evidence organization, and rejection. The bound gives that advice a
testable form rather than leaving it as an assertion.
...
The rejection bound in @sec-the-rejection-bound makes cheap rejection
authorities central to loop throughput, but the field needs to know when those
