...
The chapter's core spectrum categorizes the loop patterns by their feedback latency and rollback cost (@fig-loop-pattern-spectrum). The
...
Read the method column as an agency contract for what an assistant, optimizer,
...
1. **The Surrogate Prediction Loop:** The loop uses a fast proxy to approximate or triage expensive feedback, such as full-system simulation or physical-design exploration. The failure mode here is proxy mismatch.
2. **The Constrained Optimization Loop:** The loop searches within legal actions under explicit rejection gates. Reinforcement learning[^fn-rl-environment-contract] or another optimizer may help explore the space, but the environment contract decides which paths are legal and which evidence can reject them.
3. **The Generator-Critic Repair Loop:** A generator, such as an LLM[^fn-ml-llm-generator-critic], proposes a bounded artifact, a compiler, simulator, formal tool, or synthesis check rejects it, and the loop uses the error trace to repair the next candidate.
...
preserve negative traces. ArchGym, an open-source framework connecting machine learning algorithms to architecture simulators, is one example of making such loops more
...
returns at the end of the chapter as a scoped model of one design loop's
adjudication stage (@sec-the-rejection-bound).
...
A simplified LogCA-style calculation [@AltafWood2017LogCA] formalizes this interface tax (@fig-logca-breakeven). End-to-end speedup is not a property of the accelerator
...
Domain specificity is multidimensional (@fig-domain-specificity-shapes). The figure should be read as a rigorous checklist
...
compiler representations, which all offer structured ways to describe computation and its mapping to hardware, matter here because they turn software reachability
...
This software path makes code generation the narrow waist of specialization (@fig-codegen-narrow-waist). Above the waist are
...
applications must follow, and it is the book's measured deployment-facing
exemplar; no measured cross-layer hardware co-design loop is offered. The
reported effort analyzed more than 38,000 migration commits. It also evaluated
an AI-assisted repair tool on a selected replay benchmark of 245 historical
commits with identifiable build and test targets. The tool produced a passing
repair for 30 percent of that benchmark, a directional result rather than a live
fleet automation rate [@ChristopherEtAl2025ISAMigration]. Separately, build,
test, sanitizers that expose memory-model differences between the two ISAs, and
a production monitor can reject regressions or evict jobs that crash-loop or run
slow on the new target. Authority sits with those instruments, not the
generator, while application owners keep the cases the automated repair path
cannot settle. The model behind the loop will age; the represented migration
state, instrument-held rejection authority, and human-owned exceptions are the
parts worth keeping.
...
These loop families look different on the surface, yet many enter the same
...
method posture toward critique, evidence organization, and rejection.
...
baseline, objective vector, or negative trace carries the same authority in a
...
loop outcomes while keeping posture changes inside human-defined bounds.
