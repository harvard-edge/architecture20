...
Model-mediated exploration requires an explicit representation of architecture state, legal actions, and tool observations. Methods may generate candidates, predict behavior, allocate evaluations, or critique assumptions. ArchGym, an open-source environment for architecture, provides an interface for exploration, predictive DSE uses data-driven models to proxy evaluations, and the Apollo AI-driven compilation optimizer allocates expensive evaluations using transfer learning[^fn-transfer-learning-c09] [@yazdanbakhsh2021apollo]. Chiplet exploration adds package-interface contracts like the Universal Chiplet Interconnect Express (UCIe).
...
$$
Here, $f$ is the improvable fraction of baseline time, $s$ is local speedup, and the $\epsilon$ terms are interface and software overheads normalized to the same baseline. By expressing $\epsilon$ as static fractions, this compact form masks the fact that overheads are fundamentally non-linear with respect to workload size. The $\epsilon_{\mathrm{interface}}$ term is functionally $(N \times L_{\mathrm{startup}} + \frac{\text{Data Volume}}{\text{Bandwidth}}) / T_{\mathrm{baseline}}$. This teaches the fundamental architectural lesson that an accelerator can fail the Amdahl bound either because it is invoked too many times ($N$ dominates) or because it starves for data (Bandwidth dominates). The specialization helps only when each invocation amortizes those overheads.
