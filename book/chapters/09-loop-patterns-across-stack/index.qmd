...
Software feedback supports an architectural claim only when the run record ties each code path to the hardware snapshot it targeted. @fig-hardware-software-coevolution shows those two changing histories joined by one claim.

![**Recording hardware and software changes together.** Hardware target state and software path state evolve together through a shared study record.](images/F10c-hardware-software-coevolution){#fig-hardware-software-coevolution width="100%" fig-alt="Two-lane diagram showing hardware and software evolving together."}
...
High-consequence work includes RTL changes, physical design, timing closure, power analysis, and formal signoff. Feedback is delayed and reversal is slow and expensive. Because you cannot `git revert` a tapeout, architecture must borrow the concept of *sagas* (compensating transactions) from distributed systems. Hardware must be designed with escape hatches—like microcode patches, degraded fallback modes, or fuses—to compensate for irreversible AI flaws once they reach silicon. Consequently, a passing early check cannot authorize a difficult-to-reverse transition.
...
Feeding delayed observations, like field telemetry, directly into a high-frequency AI optimizer causes the same violent oscillations seen in naive control systems. Instead, architecture needs the equivalent of *cascade control* or a Smith Predictor: a mechanism that predicts the delayed response and carefully gates the update.
...
If proposals arrive faster than reliable evaluation can settle them, more generation grows a queue rather than useful progress. A measurable bottleneck occurs when proposal arrival rate $\lambda_{\mathrm{propose}}$ exceeds trusted settlement rate $\mu_{\mathrm{review}}$. In this case, proposal supply is not limiting progress; trusted evaluation and review are the bottleneck. Improving throughput requires validating cheaper checks, reducing review cost, and cutting avoidable routing and escalation overhead. For quantitative limits on service-cost speedups, see the service-cost model in @sec-additional-framework-context (@fig-rejection-bound-ceiling).
