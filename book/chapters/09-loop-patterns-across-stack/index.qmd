...
2. **The Constrained Optimization Loop:** The loop searches within legal actions under explicit rejection gates. Reinforcement learning^[A machine learning method where an agent learns to make decisions by performing actions and receiving rewards or penalties.] or another optimizer may help explore the space, but the environment contract decides which paths are legal and which evidence can reject them.
3. **The Generator-Critic Repair Loop:** A generator, such as an LLM^[Large Language Model, a neural network trained on vast amounts of text to understand and generate human-like language.], proposes a bounded artifact, a compiler, simulator, formal tool, or synthesis check rejects it, and the loop uses the error trace to repair the next candidate.
...
count. In active ML-serving domains, workload drift can be rapid, for example shifts from dense Transformers to Mixture-of-Experts^[A neural network architecture where different parts of the model (experts) are activated conditionally based on the input, increasing capacity without a proportional increase in compute.] in months. A static benchmark can become a liability, so an Architecture 2.0 loop needs an explicit refresh path for new graphs, traces, and coverage claims. A workload loop should therefore record benchmark version, workload
...
Because the hardware side and software side may both change, the evidence ledger
...
## Deployment-Facing Architecture Loops: Runtime, Serving, and Datacenter Policy
...
> **Service-level objectives (SLOs):** target values for system reliability and performance, commonly used to manage operational tradeoffs [@BeyerEtAl2016SRE].
