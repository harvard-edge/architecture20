...
Model-mediated exploration requires an explicit representation of architecture state, legal actions, and tool observations. Methods may generate candidates, predict behavior, allocate evaluations, or critique assumptions. ArchGym, an open-source environment for architecture, provides an interface for exploration, predictive DSE uses data-driven models to proxy evaluations, and Apollo allocates expensive evaluations using transfer learning[^fn-transfer-learning-c10] [@yazdanbakhsh2021apollo]. Chiplet exploration adds package-interface contracts like the Universal Chiplet Interconnect Express (UCIe).

[^fn-transfer-learning-c10]: **Transfer learning**: The ML practice of reusing what a model learned on one task to start a related task, rather than training from scratch, much as an architect seeds a new design-space search with the tuned parameters of the previous generation.

AlphaChip is a concrete instance of model-mediated exploration [@MirhoseiniEtAl2021GraphPlacement], with reported results that remain publicly contested [@ChengEtAl2023AssessmentRL]. Its reinforcement-learning agent was guided by fast wirelength and congestion estimators rather than full routing, because RTL-to-GDS placement is too slow to supply the millions of feedback cycles a learned policy[^fn-rl-policy-c10] needs. The proxy helps only while it tracks the final routed design. Once it diverges, the agent optimizes a target that does not exist, which is why the architect, not the optimizer, must own proxy calibration.

[^fn-rl-policy-c10]: **Policy (reinforcement learning)**: The learned mapping from an observed state to the next action, trained by trial and error against a reward signal. In placement, the policy maps the current partial layout to the next macro position.
...
A single-layer optimization can be locally correct and still make the complete system worse. For example, the first-generation TPU's 8-bit datapath was a cross-layer decision co-designed with quantization software, and its serving objective included a 99th-percentile latency bound [@JouppiEtAl2017TPU]. When an AI method tunes architectural parameters for ML workloads, it must co-optimize hardware datapath precision alongside software quantization schedules; treating either as a fixed baseline artificially truncates the Pareto frontier. A throughput improvement that violated the latency target was unacceptable.
...
Deployment feedback includes operational state, SLOs, resource contention, and rollback triggers, and those triggers are what arm the automatic halt. While software deployments rely on rollback to mitigate risk, hardware commitment eliminates that safety net entirely.
...
AI heuristics that work well in software often break down against the physical realities of multi-day placement and routing runs, Design Rule Check (DRC) violations, and non-linear timing-closure loops where physical-design feedback is noisy and non-differentiable.[^fn-differentiability-c10]

[^fn-differentiability-c10]: **Differentiability**: A signal is differentiable when small input changes produce smooth, computable gradients, which is what lets ML optimizers follow a slope toward better designs. Physical-design feedback offers no such gradient, so gradient-based methods give way to slower black-box search. Recent EDA research attempts to bridge this gap by training differentiable surrogate models of STA and placement, allowing gradient-based optimizers to approximate physical-design search.

Because you cannot `git revert` a tapeout, AI-assisted hardware changes require built-in escape hatches, such as microcode patches, fuses, or degraded fallback modes, to compensate for unexpected failures once they reach silicon. A passing early check in a proxy or simulator can never authorize a difficult-to-reverse physical transition.
...
- Can meta-learning[^fn-meta-learning-c10] policies let an agent adapt its search strategy when it moves from optimizing an in-order microcontroller to an out-of-order superscalar core?

**Theme 2: Post-Silicon Recalibration.** Data from shipped silicon could correct the pre-silicon models that guided the design.

- How can causal inference[^fn-causal-inference-c10] methods isolate hardware-specific failure signals from noisy, application-level fleet telemetry to recalibrate pre-silicon surrogate models?

[^fn-meta-learning-c10]: **Meta-learning**: Training a model across many related tasks so that it learns how to adapt to a new task quickly, rather than mastering any single task. The goal is a search strategy that transfers, not a tuned design.

[^fn-causal-inference-c10]: **Causal inference**: Statistical methods for estimating cause-and-effect from observational data when a controlled experiment is impossible, distinguishing a true hardware fault signal from a correlated software or workload change.
...
- **Up next:** This chapter set out what must be revalidated when a decision crosses a boundary. @sec-what-architect-owns examines the responsibility and governance that the architect must retain when delegating work to AI.
:::
