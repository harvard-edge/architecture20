...
A common failure mode in AI-assisted architecture is treating surrogate models—such as power or timing predictors—as static artifacts that are trained once and deployed forever. In reality, models decay. As the underlying software compiler is updated, or as the process node shrinks from 5nm to 3nm, the training distribution drifts. Drawing inspiration from MLOps, architecture loops must implement **Continuous Calibration** for their AI tools. Every time a candidate reaches the bottom of the funnel (true RTL or silicon), that ground-truth data must automatically loop back to retrain the fast top-of-funnel surrogate models. The loop never stops learning, and drift detection becomes a first-class metric in the environment.

Before a new, continuously calibrated surrogate is trusted to steer a multi-million-dollar chip design, it can be deployed in **Shadow Mode**—running silently alongside the traditional human-led EDA flow. By predicting outcomes without actively controlling the project, architects can measure its calibration against ground-truth tools over thousands of commits until it earns the authority to actively optimize.
