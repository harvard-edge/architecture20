...
Ansor, two TVM tensor-program autotuners, generate candidate schedules, predict
...
[^fn-math-pareto-region]: A Pareto region is the set of solutions in a multi-objective design space where no single objective can be improved without degrading at least one other. The chapter names it as one defensible target for architecture optimization under limited feedback, alongside a constraint boundary or a ruled-out class of candidates, rather than collapsing the search to a single best point.
...
**The claim.** AlphaChip is remembered as the reinforcement-learning result, and the learning is the part that drew the attention.
