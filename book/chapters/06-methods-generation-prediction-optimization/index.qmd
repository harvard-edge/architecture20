...
Demonstrating one capability does not authorize a method to widen its scope, waive a constraint, or make the final architecture decision. Hardware grounding and feedback cost test every role. Cheap checks can screen broad sets of candidates, while expensive tool runs should be reserved for questions that can change the study.
