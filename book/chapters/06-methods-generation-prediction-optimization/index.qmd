...
search, in the ML model-design sense rather than computer architecture proper,
...
::: {.callout-war-story}
**The method everyone remembers.** AlphaChip is remembered as the reinforcement-learning result, and the learning is the part that drew the attention. The beat this chapter cares about is quieter. When independent researchers reran the comparison, a strong simulated-annealing baseline, a classical optimizer with no learning at all, matched or beat the learned method on its own proxy cost across most modern testcases [@ChengEtAl2023AssessmentRL].

**Reputation is not a reason.** That does not make reinforcement learning the wrong tool for sequential placement. It makes the case that a method should be chosen for the loop bottleneck it relieves, not for how modern it sounds. A learned policy earns its place only when it beats a well-tuned classical baseline under the same rejection authority, and here that margin was exactly what could not be shown.

**The real dividing line.** The contrast with PrefixRL just above is the whole lesson. A learned method is worth its added complexity only when the environment can verify it beats the simpler alternative. When it cannot, the honest move is to prefer the method a high-fidelity instrument can already reject. Method choice is inseparable from what the environment can verify, and from whether a simpler method would have done as well.
:::
