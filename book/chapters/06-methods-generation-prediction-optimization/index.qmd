...
The discipline of assigning specific roles to AI methods exposes several unsettled research directions. At this point in the progression, the question is not whether a model can generate, predict, or optimize in isolation. The research problem is whether a method performs its assigned role inside a bounded, evidence-constrained architecture loop.

1. **What formalisms can rigorously enforce and audit AI role contracts across heterogeneous execution environments?**
The decision matrix for method selection (@sec-choosing-a-method-under-constraints) assigns generators, predictors, critics, and optimizers to different obligations, but runtime enforcement remains weak. A strong contribution would define a role-contract language plus a monitor that detects when a method silently escalates its authority, such as a predictor acting as a decision maker. The evaluation should use public loop traces or synthetic environments to measure role-violation detection, false alarms, and the cost of preserving the commitment boundary.

2. **How can active learning models mathematically synthesize heterogeneous negative traces?**
Sample efficiency under expensive feedback (@sec-sample-efficiency-under-expensive-feedback) depends on learning from a non-i.i.d. mixture of cheap warnings, proxy mismatches, synthesis timeouts, and rare silicon failures. A thesis-level paper would introduce acquisition functions that ingest heterogeneous negative traces while preserving their fidelity labels. The result should be tested on a design-space exploration task where the method improves sample efficiency without allowing cheap negative evidence to masquerade as high-fidelity rejection.

3. **Can we build bounded rejection oracles for hardware-aware generative models?**
The architecture community lacks standardized adversarial tests designed to falsify unsupported predictions or trap invalid generative actions. The right target is not a definitive oracle for all hardware awareness. A stronger paper would build a bounded rejection suite for a named environment and state the maximum commitment that passing the suite can authorize. The evaluation should include intentionally invalid candidates, near-miss candidates, and valid candidates, so the oracle's false-pass and false-reject behavior is visible.

4. **What control-theoretic mechanisms should govern the dynamic reallocation of method roles?**
The decision-value heuristic for expensive feedback (@sec-sample-efficiency-under-expensive-feedback) assumes a mostly static policy for invoking the next method role. A compelling thesis direction is a meta-controller, perhaps based on optimal control or meta-reinforcement learning[^fn-rl-meta-reinforcement-learning], that reallocates effort among generation, prediction, critique, and repair. The paper should report whether the controller improves evidence use under a fixed budget while obeying the role contracts and commitment boundaries defined earlier in the chapter.

[^fn-rl-meta-reinforcement-learning]: Meta-reinforcement learning is a paradigm in which an agent learns to learn, adapting rapidly to new tasks by leveraging past experience. The open question raises it as a candidate meta-controller that could reallocate effort among generation, prediction, critique, and repair while still obeying the chapter's role contracts and commitment boundaries.
