...
**The gap.** Early reinforcement learning placement models optimized strictly for half-perimeter wirelength and macro non-overlap. The agents quickly learned to pack all macros into a dense central cluster. This minimized the proxy metric but created a placement with unacceptable routing congestion and catastrophic thermal density. The proxy metric disconnected from the physical reality of routing limits.
...
Feedback, retry, and stopping behavior.
...
A composition is useful when it reduces the limiting work without hiding which role produced each result or which check can reject it. The core thesis of Architecture 2.0 is that generation without independent verification produces physically ungrounded artifacts. To prevent these hallucinations from corrupting the design loop, methods must be composable but strictly sequenced: **Generate** to explore the candidate space, **Predict** via fast surrogates to filter unpromising options, and **Verify** against a ground-truth physical oracle.
