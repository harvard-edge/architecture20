...
malformed artifacts, and disconnected tool work remains useful. A rigorous architecture study relies on Bayesian optimization for scarce sampling, reinforcement learning for sequential state manipulation, conformal prediction for uncertainty bounds, and deterministic formal solvers.

To make sense of these options, we must draw a strict line: **A role is the *job description*; the method is the *tool* we hire for the job.** While roles stay the same, the methods we use span a broad **AI Spectrum**, depending on the specific hardware or software problem. This spectrum ranges from simple, low-cost Support Vector Machines (SVMs) and decision trees for basic classification, to Small Language Models (SLMs) for local code generation, all the way up to frontier-class Large Language Models (LLMs) and massive diffusion models for complex reasoning or spatial layout.

Equally important is the **Cost and Efficiency of AI**. AI assistance is not free. A frontier LLM query might cost fractions of a cent and take seconds, but fine-tuning a surrogate on millions of EDA runs or deploying a massive model cluster for search consumes massive amounts of compute, energy, and time. An architect must treat AI inference cost just like any other design budget. We cannot assume that AI assistance will infinitely scale without hitting its own thermal, financial, and latency walls.
...
: **The 8 AI Roles in Architecture 2.0:** Complex tasks are decomposed into these specialized jobs. {#tbl-ai-roles} (@fig-architecture-development-triad).

![**The 8 AI Roles in Architecture 2.0:** Generation proposes artifacts and candidates, prediction estimates behavior and risk, and optimization searches tradeoffs under constraints. The architect still needs evidence, a mechanism-level explanation, and judgment about the result.](images/ch6_ai_roles.svg){#fig-architecture-development-triad width="100%" fig-alt="The 8 AI Roles in Architecture 2.0 diagram"}
...
All of those outputs go through evidence review on the right. Before comparing the mechanics of these roles, we must establish their *levels of trust*. Treating all AI outputs as evidence is a fundamental error. If an architect treats a generated guess as a final result, the entire design process breaks down. These roles are not just components in a software stack; they represent different levels of reliability:
...
2. **Predictive Roles (Estimates):** Prediction and optimization produce probabilities. They guide the search, but are not evidence.
3. **Evaluative Roles (Evidence):** Verification and critique produce hard facts by checking artifacts against independent ground truth (like a cycle-accurate simulator). This is the only tier the architect can trust.
...
The choice of generative method depends on the type of artifact. For text—like RTL, C++ models, or compiler scripts—Large Language Models (LLMs) are the obvious choice. However, when the artifact is spatial—such as macro placement or floorplanning—architects use **Diffusion Models**. While the math under the hood differs, their reliability is exactly the same: both produce guesses, not evidence. Passing unchecked AI-generated RTL or a malformed configuration to a multi-day cycle-accurate simulation wastes the project's most expensive resource, causing 72-hour formal equivalence timeouts, overnight place-and-route crashes, or stalled compute clusters. To catch these errors early, generation must be paired with fast, local checks (like syntax parsers or linters).

Returning to our Lighthouse prompt—*designing a low-power, 3W 64-bit RISC-V-based compute subsystem for an XR platform*—generation might propose three legal pipeline depths or L1 cache configurations, alongside a qualitative behavior, a failure mode, and a directional mechanism that can be tested. Schema and semantic checks act as these initial contracts to establish that the response is a legal proposal within the declared constraints (e.g., valid RISC-V instruction extensions and compatible power-domain definitions). They do not establish that the proposal is faster or that its mechanism is correct, but they ensure it will not crash the machinery downstream.
...
Unlike generative brainstorming, prediction requires models that can be calibrated against real data. Large Language Models are poor fits here. Instead, to predict routing congestion or critical path delays from a netlist, **Graph Neural Networks (GNNs)** are a better choice because they map directly to the circuit's wiring. For power and thermal estimation, **Physics-Informed Neural Networks** can enforce basic thermodynamic rules so the model doesn't predict impossible cooling rates.
...
## Optimization Chooses What to Evaluate Next (@fig-bayesian-optimization).

![**Bayesian Optimization:** The surrogate mean predicts the objective, the uncertainty bands capture epistemic unknowns, and the acquisition function guides the tradeoff between exploration and exploitation.](images/ch6_bayesian_optimization.png){#fig-bayesian-optimization width="100%" fig-alt="1D BO plot showing surrogate mean, uncertainty bands, and acquisition function"}
...
To spend our simulation budget efficiently, we use algorithms built to handle uncertainty. We apply **Bayesian Optimization** when cycle-accurate simulations are extremely expensive and we must find good designs with very few runs. We apply **Reinforcement Learning (RL)** when the problem involves a sequence of steps, like ordering compiler passes or routing wires. These methods are our resource managers—they do not invent the design, they decide *where* to spend the next simulation run.
...
Bayesian optimization was developed for sequential experiments on expensive black-box functions [@JonesSchonlauWelch1998EGO; @SnoekLarochelleAdams2012BayesianOptimization]. Its surrogate and acquisition rule balance exploration against exploitation and make uncertainty part of the choice of what to evaluate next. Crucially, Bayesian Optimization explicitly models epistemic uncertainty (uncertainty due to a lack of data in a region) versus aleatoric uncertainty (inherent system noise). By targeting regions of high epistemic uncertainty, the optimizer systematically maps the unknown design space rather than getting stuck in a local minimum. Recording the acquisition rule, score, and current uncertainty lets a reviewer see why the optimizer selected a candidate. The run record should also include evaluation fidelity, expected benefit, and the stopping condition.
...
Reinforcement learning can fit sequential problems such as placement, scheduling, adaptive control, or multi-stage design flows.

::: {.callout-war-story title="AlphaChip and RL for Placement"}
**The claim.** AlphaChip demonstrated that Reinforcement Learning can successfully tackle the highly complex, sequential problem of macro placement in TPUs [@MirhoseiniEtAl2021GraphPlacement].

**The gap.** Traditional simulated annealing and analytical placers struggle with the immense scale and discrete constraints of modern macro placement.

**The lesson.** By treating the canvas as a Go board and placing one macro at a time, the RL agent learned to optimize for proxy rewards like wirelength and congestion. It highlighted how RL excels when the problem is fundamentally sequential and there is a clear, fast proxy reward to learn from, drastically reducing the time required to generate candidate floorplans.
:::

PrefixRL shows what a tightly defined design space makes possible. The hardware-aware RL framework casts parallel-prefix arithmetic circuits, such as adders, as a reinforcement-learning problem with logic synthesis in the evaluation path [@RoyEtAl2021PrefixRL]. Each candidate comes from a declared circuit space and receives a score from an actual synthesis run rather than a hand-built proxy. A synthesis run can reject candidates because the action space is restricted and the output is synthesizable. That environment does not remove the need for a baseline. A learned optimizer must still be compared with a well-tuned simpler method under the same action space, tool flow, and evaluation budget.
...
Architecture often has a few high-fidelity runs, more medium-fidelity simulations, many cheap proxies, and hidden costs such as tool setup, debugging, license availability, and review. The useful method role changes with that budget. Architecture design spaces suffer from the state space explosion—a simple processor has more possible configurations than there are atoms in 10^3,700 universes. These spaces hold orders of magnitude more candidates than any real evaluation budget can ever reach. We must explicitly balance the compute cost of the AI against the compute cost of the simulator: running an LLM agent in a loop might be cheap, but if it hallucinates and triggers 1,000 invalid RTL synthesis runs, it will bankrupt the project's compute budget. @fig-evidence-gap places representative candidate counts and affordable feedback counts on one logarithmic axis.
```{python}
...
Why not simply use one giant AI model for everything? The first reason is the **Separation of Powers**. If the exact same neural network generates the RTL, predicts its performance, and verifies its own logic, the system lacks an independent check. This does not produce a verified design; it produces a *shared delusion*. The generator and the verifier cannot be the same model.

The second reason is physical reality. Silicon design is a rigid funnel: finding a configuration that increases IPC is easy compared to closing timing, preventing voltage drop (IR drop), and meeting power limits. A single model cannot juggle all of these physical constraints simultaneously without making things up. To successfully tape out a chip, the system must break the problem down into specialized, independent tools.

The third reason is **inference speed**. The debate between massive models and small, focused models comes down to where they sit in the loop. Massive models are great for "outer-loop" tasks—like brainstorming ideas or explaining a weird simulation result—where you only ask them a question once an hour. But if an "inner-loop" optimizer needs to score 10,000 cache configurations to find the best tradeoffs, waiting seconds for a massive model to reply to each query will stall the whole search. For tight, high-volume loops, small, fast-executing models are required.
