...
system. These roles can be read through a closed-loop-control lens and related
to the five-part execution state:
...
Here, verification is only a method-selection role; @sec-feedback-verification-trust
treats verification more broadly as the rejection machinery that turns feedback
into evidence.
...
search[^fn-ml-nas], in the ML model-design sense rather than computer architecture proper,
already shows the value of putting latency, energy, memory footprint, and
target-device cost into the search problem
...
profilers, simulators, synthesis reports, and failed runs; gem5, a cycle-accurate architectural simulator, can itself be a
tool environment when the loop invokes it, records
...
The practical reviewer version in @tbl-hardware-awareness-tests gives each level an observable test. A method should not
...
important receipt is narrower than a leaderboard. Each candidate was a bounded
...
cost outside the ledger and can make an already expensive loop look cheaper than
it is. When energy or carbon exposure is material, the ledger should also state
...
[^fn-ml-multi-fidelity-bo]: Multi-fidelity Bayesian optimization jointly chooses which design to evaluate and at which fidelity level, trading information gain against cost. That is exactly the fidelity-ladder decision this section poses, whether to spend a cheap proxy or an expensive simulation on a candidate.
...
> feedback, deployment, or silicon evidence. Higher levels usually cost more and
> expose different failure modes. Their rejection authority is assigned
> separately through a declared scope, validation basis, independence claim, and
> decision contract.
...
acting as a decision maker. Public loop traces or synthetic environments should
...
training objectives that ingest heterogeneous negative traces while preserving
