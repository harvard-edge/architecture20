...
An accelerator study can stall for several different reasons. The architect may
lack viable array organizations, have too few simulator runs to evaluate the
ones already proposed, or face a review backlog of inconsistent results. Those
conditions call for different assistance. A generator can propose legal array
geometries, a predictor can estimate which geometry might reduce cycles, an
...
connect a result to a proposed architectural mechanism. In an accelerator
...
::: {.callout-design-principle title="Keep each role's output and check visible"}
Compose roles only when another architect can identify what each role read or
changed, what it returned, what it consumed, and which independent check or
reviewer received the result. If those boundaries disappear, the composition
cannot show which method helped or where an error entered the study.
:::

The accelerator study follows this separation. The model proposes candidates,
a fixed aspect-ratio heuristic supplies the comparison arm, SCALE-Sim produces
...
An architect should choose an AI method by the work limiting the bounded study.
Generation, prediction, optimization, critique, repair, verification,
explanation, and coordination read or change different objects, return
different results, consume different resources, and create different handoffs.
Demonstrating one of those capabilities does not authorize the method to widen
its scope, waive a constraint, or make the final architecture decision.

Hardware grounding and feedback cost test every role. Cheap checks can screen
broad sets of candidates, while expensive tool runs should be reserved for
questions that can change the study. For every method, the architect should
state its job, feedback budget, independent check, stopping condition, and
authority limit. A composition is useful when it reduces the limiting work
without hiding which role produced each result or which check can reject it.
