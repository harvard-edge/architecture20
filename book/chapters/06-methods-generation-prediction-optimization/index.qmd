...
[^fn-rl-reinforcement-learning]: Reinforcement learning is a machine learning paradigm in which an agent learns to make sequential decisions by receiving rewards or penalties from its environment. It fits sequential architecture tasks such as placement, scheduling, and adaptive control, but the loop must treat it cautiously when invalid actions are common and reward feedback is delayed.
...
**The gap.** The beat this chapter cares about is quieter. When independent researchers reran the comparison, a strong simulated-annealing baseline, a classical optimizer with no learning at all, matched or beat the learned method on its own proxy cost across most modern testcases [@ChengEtAl2023AssessmentRL]. That does not make reinforcement learning the wrong tool for sequential placement, but it means the learned method's advantage over a well-tuned classical baseline was exactly what could not be shown.

**The lesson.** A method should be chosen for the loop bottleneck it relieves, not for how modern it sounds. A learned policy earns its added complexity only when the environment can verify it beats a simpler, well-tuned baseline. When it cannot, the honest move is to prefer that baseline, or to narrow the claim until higher-fidelity evidence can adjudicate it.
:::
...
[^fn-ml-multi-fidelity-bo]: Multi-fidelity Bayesian optimization extends Bayesian optimization by jointly choosing which design to evaluate and at which level of fidelity, trading information gain against computational cost. That is exactly the fidelity-ladder decision this section poses, namely whether to spend a cheap proxy or an expensive simulation on a given candidate.
