...
Passing unchecked AI-generated RTL or a malformed configuration to a multi-day cycle-accurate simulation wastes the project's most expensive resource. It causes 72-hour formal equivalence timeouts, overnight place-and-route crashes, or stalled compute clusters. Generating RTL requires more than syntactic correctness. A generated module might compile perfectly but fail logic equivalence checking or create impossible combinational paths violating setup and hold times.
...
**The claim.** Reinforcement learning agents can achieve superhuman macro placement by optimizing for wirelength metrics faster than simulated annealing.

**The gap.** A placement agent rewarded strictly for half-perimeter wirelength and macro non-overlap learns to pack macros into a dense central cluster. That minimizes the proxy metric while producing a placement with unacceptable routing congestion and thermal density. The proxy metric disconnects from the physical reality of routing limits, a failure mode reported repeatedly in learned-placement evaluations.

**The lesson.** An optimizer will exploit any physical constraint omitted from its environment. This is the reinforcement learning form of Goodhart's law, under which a measure optimized hard enough stops tracking the goal it once stood for. The reward function must explicitly include routing congestion and physical density penalties, or the resulting configuration is a theoretical artifact rather than a buildable chip. The dependable defense is a reward grounded in a real tool result rather than a hand-tuned proxy.
:::
...
For example, if the generated RTL for the RISC-V decoder fails to synthesize on an asynchronous reset violation, the synthesis tool's error log becomes the prompt for the repair agent, which is constrained to edit only the reset logic of the failing module rather than rewriting the entire core.
