...
Do not apply generative search when the study requires constraint solving, and do not use deep reinforcement learning when an analytical surrogate suffices. The chosen AI method must be algorithmically matched to the specific failure mode of the current bounded study, balancing sample efficiency against the ruggedness of the design space.
:::
...
1. **Sample-Efficient Reinforcement Learning in Non-Convex Hardware Spaces:** How can RL algorithms be adapted to explore the highly discontinuous, non-convex topologies of hardware design spaces where a single parameter change can cause complete unroutability?
2. **Integrating Formal Methods with Generative LLMs:** How do we constrain the token-generation process of Large Language Models using real-time solvers (e.g., SMT solvers) to guarantee that emitted RTL is syntactically correct and formally equivalent to a reference specification by construction?
3. **Multi-Fidelity Bayesian Optimization over Graph Structures:** What are the most effective acquisition functions for Bayesian Optimization when the action space is not a continuous vector, but a complex graph of hardware modules and interconnects evaluated across discrete simulation rungs?
