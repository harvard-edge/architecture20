...
Overfitting the cheap metric is not a hypothetical, and a neighboring field watched it happen at national scale.

::: {.callout-field-note title="The predictor that tracked winter, not flu"}
Google Flu Trends estimated influenza prevalence from search queries and, for a time, tracked the official surveillance data closely. Then it drifted, over-predicting flu in a hundred of a hundred and eight weeks, because the cheap proxy had quietly learned a seasonal winter signal rather than a flu signal, and Google's own changing search behavior moved the ground under it [@LazerEtAl2014ParableFlu]. The proxy was never re-checked against the higher-fidelity surveillance data often enough to catch the drift before it was trusted.
