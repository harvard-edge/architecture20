...
To navigate this heterogeneous toolkit, we must draw a strict line: **A role is the *job description*; the method is the *candidate* we apply to the job.** While roles are conceptually fixed, the methods we select to fulfill them vary wildly based on the modality and physics of the task.
...
All of those outputs go through evidence review on the right. Before comparing the mechanics of these roles, we must establish their *epistemological hierarchy*. Treating all AI outputs as evidence is a fundamental error. If an architect treats a generated hypothesis as ground truth, the entire design loop collapses into an unverified state. These roles are not just components in a software stack; they represent different tiers of truth:
...
To allocate this feedback budget efficiently, we use algorithms that formalize uncertainty and sequential decision-making. We apply **Bayesian Optimization** when cycle-accurate simulations are extremely expensive and we need sample-efficient search. We apply **Reinforcement Learning (RL)** when the problem is sequential, like ordering compiler passes or routing step-by-step. These methods act as our "Value of Information" engines—they do not generate the design, they decide *where* to search next.
...
![Artifact Lifecycle: Verification, Critique, and Repair route candidates through syntax checks and oracles. Failures dictate whether the loop repairs the artifact or escalates to the architect.](images/F6-verification-lifecycle.svg){#fig-verification-lifecycle}
...
The second reason lies in the strict physical constraints of the design flow. Silicon design faces a rigid funnel: finding a configuration that increases IPC is trivial compared to closing timing, preventing EMIR (Electromigration and IR Drop) violations, and meeting power density limits. A single monolithic model cannot manage all of these coupled, nonlinear physical constraints simultaneously without hallucinating or losing calibration. To survive the tapeout process, the system must separate concerns into heterogeneous, specialized tools.

The third reason is governed by **Critical Path Inference Latency**. The debate between "Big" models (hundreds of billions of parameters) versus "Small" models (highly distilled, task-specific networks) resolves perfectly across the epistemological hierarchy. Big models excel at outer-loop tasks—like generative brainstorming or explaining simulation anomalies—where queries are infrequent and high latency is tolerable. However, if an inner-loop optimizer needs to evaluate 10,000 architectural candidates to find a Pareto frontier, the inference latency of a massive foundation model becomes the bottleneck, destroying the value of the AI assistant. For tight, continuous feedback loops, small, fast-inference models (like distilled GNNs or light RL policies) are non-negotiable.
