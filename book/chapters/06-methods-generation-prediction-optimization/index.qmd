...
::: {.callout-design-principle title="The action space must not exceed the oracle's fidelity"}
Never allow a generative model or optimizer to propose architectural changes that the available simulator or formal tool cannot verify. If the AI can mutate a thermal property but the current EDA loop only runs logic simulation, the AI will silently exploit the unchecked dimension. Constrain the method's action space strictly to the dimensions the feedback budget can measure.
:::
