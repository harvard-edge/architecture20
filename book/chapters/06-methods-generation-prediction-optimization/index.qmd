...
An accelerator study can stall for several reasons. The architect may lack viable array organizations, have too few simulator runs to evaluate proposed ones, or face a review backlog of inconsistent results. These conditions call for different assistance. A generator can propose legal array geometries, a predictor can estimate which geometry might reduce cycles, an optimizer can choose the next configuration to evaluate, and a critic can find a missing comparison, after which a cycle-level simulator can test the selected configurations. The first method question is therefore not which model performs best overall, but rather what work prevents the bounded study from reaching its next defensible result. Current model rankings will age, while the distinction among missing alternatives, expensive evaluation, poor experiment selection, weak review, malformed artifacts, and disconnected tool work remains useful. Consequently, a rigorous architecture study relies on Bayesian optimization for scarce sampling, reinforcement learning for sequential state manipulation, conformal prediction for uncertainty bounds, and deterministic formal solvers.

To make sense of these options, we must draw a strict line. **A role is the *job description*, and the method is the *tool* we hire for the job.** While roles stay the same, the methods we use span a broad **AI Spectrum** depending on the specific hardware or software problem. This spectrum ranges from simple, low-cost Support Vector Machines (SVMs) and decision trees for basic classification, to Small Language Models (SLMs) for local code generation, and all the way up to frontier-class Large Language Models (LLMs) and massive diffusion models for complex reasoning or spatial layout.

Equally important is the **Cost and Efficiency of AI**. AI assistance is not free. A frontier LLM query might cost fractions of a cent and take seconds, but fine-tuning a surrogate on millions of EDA runs or deploying a massive model cluster for search consumes massive amounts of compute, energy, and time. An architect must treat AI inference cost just like any other design budget, because we cannot assume that AI assistance will infinitely scale without hitting its own thermal, financial, and latency walls.
...
: **The 8 AI Roles in Architecture 2.0:** Complex tasks are decomposed into these specialized jobs (@fig-architecture-development-triad). {#tbl-ai-roles}
...
Architecture often has a few high-fidelity runs, more medium-fidelity simulations, many cheap proxies, and hidden costs such as tool setup, debugging, license availability, and review. The useful method role changes with that budget. Architecture design spaces suffer from the state space explosion, as a simple processor has more possible configurations than there are atoms in 10^3,700 universes. These spaces hold orders of magnitude more candidates than any real evaluation budget can ever reach. We must explicitly balance the compute cost of the AI against the compute cost of the simulator. Running an LLM agent in a loop might be cheap, but if it hallucinates and triggers 1,000 invalid RTL synthesis runs, it will bankrupt the project's compute budget. @fig-evidence-gap places representative candidate counts and affordable feedback counts on one logarithmic axis.
```{python}
