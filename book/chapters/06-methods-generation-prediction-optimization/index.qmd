...
Building a highly accurate surrogate model for architecture—one that can bypass slow simulation—requires adopting the methodologies that succeeded in other physics-based domains. As @sec-data-representations-world-models introduced, systems like AlphaFold and GraphCast achieved their breakthroughs not just by scaling model size, but by combining massive unsupervised structural data with highly precise physical constraints. AlphaFold learned the fundamental spatial relationships of amino acids from vast, unlabeled sequence databases, then constrained its predictions using known thermodynamic and stereochemical rules. Similarly, GraphCast modeled global weather patterns by learning fluid dynamics from decades of historical reanalysis data, structured through a mesh-graph representation that inherently respected spatial physics.
