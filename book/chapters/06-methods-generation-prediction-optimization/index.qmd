...
When the artifact is spatial, such as macro placement or floorplanning, architects use **Diffusion Models**. While the math under the hood differs, their reliability is the same. Both produce guesses, not evidence.
...
Running an LLM agent in a loop might be cheap, but if it hallucinates and triggers 1,000 invalid RTL synthesis runs, it will bankrupt the project's compute budget. The architect must constrain the agent's actions to prevent this waste. @fig-evidence-gap places representative candidate counts and affordable feedback counts on one logarithmic axis.
```{python}
