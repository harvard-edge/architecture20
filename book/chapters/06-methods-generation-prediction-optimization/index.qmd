...
configurations.The first method question is therefore not which model performs best overall.
...
Crucially, Architecture 2.0 is not synonymous with Large Language Models (LLMs). While generative foundation models are powerful tools for code synthesis and unstructured reasoning, they are merely one class of components. A rigorous architecture study relies equally on Bayesian optimization for scarce sampling, reinforcement learning for sequential state manipulation, conformal prediction for uncertainty bounds, and deterministic formal solvers. Treating AI-assisted architecture as solely an "LLM problem" obscures the vast, heterogeneous toolkit required to navigate the physical realities of silicon.

To navigate this heterogeneous toolkit, we must draw a strict line: **A role is the *job description*; the method is the *candidate* we hire for the job.** While roles are conceptually fixed, the methods we select to fulfill them vary wildly based on the modality and physics of the task.

## Comparing Method Roles: The Epistemological Hierarchy
...
All of those outputs go through evidence review on the right. Before comparing the mechanics of these roles, we must establish their *epistemological hierarchy*. The cardinal sin of Architecture 2.0 is treating all AI outputs equally. If an architect treats a generated hypothesis as verified ground truth, the entire design loop collapses into hallucination. These roles are not just components in a software stack; they represent different tiers of truth:

1. **Generative Roles (Hypotheses):** Generation and repair produce plausible artifacts. They hold zero authority and act merely as an automated brainstorming engine.
2. **Predictive Roles (Heuristics):** Prediction and optimization produce probabilities. They are navigators guiding the search, not evidence.
3. **Evaluative Roles (Evidence):** Verification and critique produce ground truth by checking artifacts against independent oracles (like a simulator). This is the only tier the architect can trust.
...
The choice of generative method depends entirely on the modality of the artifact. For textual artifacts—like RTL, C++ models, or compiler scripts—Large Language Models (LLMs) are the natural candidates. However, when the artifact is spatial—such as macro placement, floorplanning, or layout generation—architects increasingly turn to **Diffusion Models**. While the underlying mathematics differ wildly, their epistemological status remains identical: both LLMs and diffusion models produce hypotheses, not evidence.

Think of generation not as a solo artist, but as a station on a manufacturing assembly line. In a physical factory, pushing an uninspected part down the line is fatal; it jams the expensive machinery downstream. Similarly, passing an unchecked RTL fragment or malformed configuration to a multi-day cycle-accurate simulation wastes the project's most expensive resource. To isolate faults, generation must be paired with localized *rework stations*.

We formalize these rework stations using the distributed systems concept of *consumer-driven contracts*. The downstream "consumers" (e.g., the compiler, physical-design tools, the thermal simulator) publish executable expectations—rules about syntax, interface limits, and memory bounds.
...
that can be tested. Schema and semantic checks act as these initial contracts to establish that the response is a
legal proposal within the declared constraints. They do not establish that the
proposal is faster or that its mechanism is correct, but they ensure it will not crash the machinery downstream.
...
Unlike generative brainstorming, prediction requires calibratable surrogates. Here, the candidates diverge from sequence-to-sequence foundation models. To predict routing congestion or critical path delays from a netlist, **Graph Neural Networks (GNNs)** are increasingly the preferred candidate, as they natively capture the topological structure of the circuit. For power and thermal estimation, **Physics-Informed Neural Networks** can embed thermodynamic constraints directly into the loss function.
...
This relationship mirrors a hospital's tiered triage system. A fast predictive model acts as a triage nurse taking vitals: it can rapidly flag anomalies, bound uncertainty, and rule out obviously disastrous configurations. However, the triage nurse cannot prescribe surgery. The cycle-accurate simulator acts as the attending physician, providing the ground truth necessary to authorize a major design commitment. This separation of powers protects the design loop from the *sim-to-silicon gap*—the danger of optimizing so aggressively for a cheap proxy's score that the resulting hardware becomes fragile to physical variation.
...
interval, the architecture system should trigger **Active Learning**. It stops using the surrogate for that region, requests a high-fidelity evaluation (like a cycle-accurate simulation), and uses that new ground-truth observation to *retrain and update* the surrogate on the fly. Whether that new observation supports the architectural claim remains a separate question.
...
Optimization does not design the architecture. It allocates candidate and fidelity evaluations under a declared action space, objective, constraints, stopping condition, and feedback budget. The right method depends on whether the problem is expensive and black-box, discrete and combinatorial, sequential with delayed consequences, smooth enough for a surrogate, parallelizable, or small enough for enumeration and simple heuristics.

To allocate this feedback budget efficiently, we use algorithms that formalize uncertainty and sequential decision-making. We "hire" **Bayesian Optimization** when cycle-accurate simulations are extremely expensive and we need sample-efficient search. We hire **Reinforcement Learning (RL)** when the problem is sequential, like ordering compiler passes or routing step-by-step. These methods act as our "Value of Information" engines—they do not generate the design, they decide *where* to search next.
...
In a DRAM-controller or accelerator environment, an architect might select a cheap heuristic as a baseline, Bayesian optimization for scarce evaluations, evolutionary search for parallel discrete exploration, or RL only when sequential state and repeated feedback justify its setup cost.



The lesson is not that one named method wins. It is that method, budget, and tuning procedure form one comparison.

Bayesian optimization was developed for sequential experiments on expensive black-box functions [@JonesSchonlauWelch1998EGO; @SnoekLarochelleAdams2012BayesianOptimization]. Its surrogate and acquisition rule balance exploration against exploitation and make uncertainty part of the choice of what to evaluate next. Recording the acquisition rule, score, and current uncertainty lets a reviewer see why the optimizer selected a candidate. The run record should also include evaluation fidelity, expected benefit, and the stopping condition.
...
Architecture often has a few high-fidelity runs, more medium-fidelity simulations, many cheap proxies, and hidden costs such as tool setup, debugging, license availability, and review. The useful method role changes with that budget. Architecture design spaces hold orders of magnitude more candidates than any real evaluation budget can reach. @fig-evidence-gap places representative candidate counts and affordable feedback counts on one logarithmic axis.
```{python}
...
## Explanation and Coordination Connect the Work
...
Coordination routes state, tasks, and artifacts when work crosses several tools or roles. Rather than viewing the coordinator as a simple shell script, architects should treat it as an *architectural control plane*, functioning much like Kubernetes does for cloud microservices.

This routing naturally implements a macro-level **Mixture of Experts (MoE)** methodology [@JacobsEtAl1991MoE; @ShazeerEtAl2017MoE]. In a classic neural MoE, a gating network assesses an input and delegates it to the most qualified sub-network. In Architecture 2.0, the *Coordinator* acts as the gating network. The implementation specifics of this interface require **contract-based design** [@BenvenisteEtAl2012Contracts]. The coordinator does not need to understand the internal weights of the routing GNN or the prompt structure of the RTL generator; it only needs to interface with their *contracts*—their stated support regions, uncertainties, and resource costs. For the human acting as the ultimate "Manager of Experts," this intersection requires **mixed-initiative interfaces** [@Horvitz1999MixedInitiative], where the human specifies intent and bounds, and the experts bid for execution.
...
Why not simply use one monolithic AI oracle for everything? The first reason is the **Separation of Powers**. If the exact same neural network generates the RTL, predicts its performance, and verifies its own logic, the system lacks an independent oracle. This does not produce a verified design; it produces a *correlated hallucination*. The generator and the verifier must share no assumptions.

The second reason lies in the **Silicon ADMET** funnel. In molecular biology, discovering a molecule that binds to a target (performance) is relatively easy, but passing toxicity, metabolism, and excretion constraints (ADMET) is where most drugs fail. Silicon design faces an identical funnel: finding a configuration that increases IPC is trivial compared to the "toxicity" of routing congestion, thermal limits, and physical data movement constraints. A single monolithic model cannot manage all of these coupled, nonlinear physical constraints simultaneously without hallucinating or losing calibration. To survive the Silicon ADMET funnel, the system must separate concerns into heterogeneous, specialized tools.

The third reason is governed by **Amdahl's Law for Inference**. The debate between "Big" models (hundreds of billions of parameters) versus "Small" models (highly distilled, task-specific networks) resolves perfectly across the epistemological hierarchy. Big models excel at outer-loop tasks—like generative brainstorming or explaining simulation anomalies—where queries are infrequent and high latency is tolerable. However, if an inner-loop optimizer needs to evaluate 10,000 architectural candidates to find a Pareto frontier, the inference latency of a massive foundation model becomes the bottleneck, destroying the value of the AI assistant. For tight, continuous feedback loops, small, fast-inference models (like distilled GNNs or light RL policies) are non-negotiable.
...
Do not celebrate a multi-agent arrangement for having more named participants. If none differs in data, representation, or permissions, the split may only convert local context into a coordination problem.
...
Compose roles only when another architect can identify what each role read or changed, what it returned, what it consumed, and which independent check or reviewer received the result. If those boundaries disappear, the composition cannot show which method helped or where an error entered the study.
:::
...
1. **How can architecture systems compare one shared model with specialized and hybrid organizations while charging context construction, inference, tool time, review, synchronization, and recovery on the same cost basis?**

2. **What empirical tests distinguish useful architectural reasoning from memorized heuristics, black-box search skill, or exploitation of simulator artifacts?**

3. **How should a system detect when role reuse has created correlated failure and request a genuinely different model, tool, oracle, workload, or reviewer?**

4. **How can role compositions change as software, workloads, tools, and deployed hardware evolve without invalidating old traces or silently shifting the study objective?**
...
Crucially, this framework redefines the human architect's core responsibility. As AI methods automate the generation of code and the optimization of parameters, the architect transitions from a *creator of artifacts* to an *auditor of evidence and specifier of constraints*. They become the **Manager of Experts**. Demonstrating one capability does not authorize a method to widen its scope, waive a constraint, or make the final architecture decision. Hardware grounding and feedback cost test every role. Cheap checks can screen broad sets of candidates, while expensive tool runs should be reserved for questions that can change the study.
