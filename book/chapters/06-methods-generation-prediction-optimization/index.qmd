...
design region, but fail outside the sampled space. Bayesian optimization[^fn-ml-bayesian-optimization] may
spend scarce simulator or synthesis evaluations carefully, but only if the
action space and objective are well formed. Reinforcement learning[^fn-rl-reinforcement-learning] may be
...
[^fn-ml-bayesian-optimization]: Bayesian optimization is a strategy for the global optimization of expensive black-box functions that uses a probabilistic surrogate model to balance exploration and exploitation. In an architecture loop it serves as the acquisition policy that decides which scarce simulator or synthesis evaluation to spend next, so its value depends on a well-formed action space and objective.

[^fn-rl-reinforcement-learning]: Reinforcement learning is a paradigm in which an agent learns sequential decisions from environmental rewards and penalties. It suits architecture tasks such as placement and scheduling, but the loop must treat it cautiously when invalid actions are common and reward feedback is delayed.
...
[^fn-ml-foundation-models]: Foundation models are large-scale machine learning models trained on vast quantities of data that can be adapted to a wide range of downstream tasks. The chapter invokes them only to date the prediction role, whose lineage of regression models and learned surrogates for architecture long predates them.
...
**The gap.** The point this chapter needs is quieter. When independent researchers reran the comparison, a strong simulated-annealing baseline, a classical optimizer with no learning at all, matched or beat the learned method on its own proxy cost across most modern testcases [@ChengEtAl2023AssessmentRL]. That does not make reinforcement learning the wrong tool for sequential placement, but it means the learned method's advantage over a well-tuned classical baseline could not be shown in that public evaluation.
...
[^fn-ml-gp-ucb]: GP-UCB (Gaussian Process Upper Confidence Bound) is an acquisition function that selects the next point to evaluate by weighing both the expected reward and the uncertainty of a Gaussian Process model. It matters here because it gives the sample choice a no-regret guarantee under stated assumptions, letting the loop defend where it spent scarce feedback instead of appealing to intuition.
