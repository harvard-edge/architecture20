...
**Author's Note:** The aphorism fits this study, which compares a model proposal against a fixed aspect-ratio heuristic in a physical simulator. The model's explanation fails a declared distinguishing contrast. This study proves useful precisely because the recorded results are less exciting than the initial hypothesis.
...
::: {.callout-design-principle title="Preregister the rule and make every deviation auditable"}
A preregistration does not make a metric incorruptible. It makes any later change to the workload, score, schema, or budget visible and checkable against the rule recorded before results were known. A mid-study deviation is legitimate only when it is declared before results, scoped to a named defect, recorded by hash, and marked non-substantive. The schema repair here met all four conditions, so it fixed an environment failure without loosening the comparison.
:::
...
the AI-contribution comparison is underpowered by construction. A tie was the
...
1. **Comparing an AI proposal arm with a fixed heuristic.** Equal simulator calls may control one scarce resource while leaving model inference, human setup, repair, prior knowledge, and total labor unmatched. The comparison can also be underpowered when the two proposal sets nearly coincide, as they did here. Future work must determine which costs matter for the architecture decision, which comparisons require them to be matched, and how to give a proposer contrast enough resolution to separate a model from a strong heuristic.
...
5. **Standardizing agentic benchmarks.** This chapter runs one bounded study, but the wider use of generative models raises a second problem, how to evaluate the agents themselves on realistic, long-horizon architecture workflows. It remains open what a standardized agent benchmark, commonly called an eval, should measure so that it stays meaningful across model generations rather than rewarding one model's quirks. Candidate properties include pass rate and bounds adherence, trajectory efficiency measured as simulator calls to convergence, robustness across error streams, and operational cost. Without such benchmarks, a hardware organization has no principled way to compare competing models or to decide which agent is fit to drive its own architecture environment.
