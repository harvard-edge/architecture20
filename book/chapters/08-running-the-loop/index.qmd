...
- Execute targeted contrasts to prove exactly why an AI-proposed mechanism succeeds.
...
**Theme 1: The Memory-Blind Proxy**

The local score combined only cycles and average layer utilization, so it rated the 16 by 64 and 32 by 32 shapes identical while their off-chip write traffic differed by a factor of two.
...
**Theme 2: The Unpinned Mapping**

The record named the GEMM (M, N, K) dimensions and fixed weight-stationary dataflow, but never stated how those axes bind to array height and width, so the transposition test distinguished the model's prediction without causally isolating orientation.
...
**Theme 3: The Sample of One**

One recorded model call, replayed under its own retained tooling, cannot speak for a model family or stand in for an independent reproduction.
