...
| **Design space** | Vector CPU extension, intentionally separated accelerator stress case, or shared-memory SoC block; no cache, chiplet, voltage, compiler-policy search, or full tightly coupled accelerator, CGRA, or PIM search yet. PIM is the natural low-data-movement contender, deferred to a later turn. |
...
signoff thermal design power (TDP) verdict. Peak and transient power (di/dt, IR drop) and sustained
...
[@ParasharEtAl2019Timeloop], calibrated pre-RTL estimation coupled to a
full-system model [@ShaoEtAl2014Aladdin; @ShaoEtAl2016Gem5Aladdin],
cycle-accurate simulation [@BinkertEtAl2011Gem5], and FPGA-accelerated
simulation [@KarandikarEtAl2018FireSim].
...
against the deadline. This is proxy mismatch made concrete (@fig-proxy-mirage), a phenomenon the Roofline model also captures for ML systems [@WilliamsWatermanPatterson2009Roofline]. The loop was
...
consistency model would need litmus tests escalated to formal memory-model
...
what remains non-public, and what can be independently tested. This is the kind
of public-audit structure that would have helped the AlphaChip war story. Its
public artifacts did not allow independent reconstruction of every disputed run,
especially the private pretraining and production context behind the strongest
claim [@ChengEtAl2023MacroPlacement; @GoldieEtAl2024ThatChipHasSailed]. A
receipt would not make proprietary inputs public. It would show which parts of
the loop another team could rerun, which parts were redacted, and which claims
therefore had to remain bounded. The receipt's minimum shape is a small set of directories and files (@tbl-loop-replay-receipt).
