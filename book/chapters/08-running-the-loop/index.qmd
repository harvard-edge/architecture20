...
This crux addresses baseline shopping, a practical trap in AI-assisted architecture. Generative AI accelerates the creation of architectural candidates [@Thakur2023LLM4EDA], but because simulators are configurable along many axes and models are sensitive to phrasing, it is easy to subconsciously baseline shop by tweaking simulator parameters after the fact to validate a plausible AI candidate. It is equally easy to prompt shop by silently discarding dozens of failed prompts until one produces the desired architecture.

::: {.callout-war-story title="The Shifting Baseline"}
**The pitfall.** A design team celebrated a 20% architectural speedup from a novel accelerator proposal, only to discover weeks later that the baseline software was accidentally compiled without standard optimization flags (`-O3`). When the baseline was properly optimized, the entire architectural gain vanished.

**The lesson.** Unfixed baselines create illusions of progress. If the baseline is allowed to shift or remain untuned during an architectural study, any measured "improvement" is meaningless.
:::
...
To make the problem computationally tractable for a bounded loop, we decompose that top-level system intent. We narrow the scope from the full SoC down to the XR platform's spatial tracking (SLAM) subsystem, which relies heavily on dense matrix multiplication. This partitioning is how large architectural constraints are mapped into localized AI search loops. Instead of brute-forcing all geometries, the AI proposes candidates that must satisfy physical constraints like die area and wiring congestion before simulation.
...
A 32x32 matrix unit was selected as the baseline because it perfectly aligns with the standard tensor shapes prevalent in our target mobile XR vision models [@Howard2017MobileNets]. The workload contains three GEMM layers extracted from the XR platform's spatial tracking (SLAM) pipeline, serving as a compact test slice rather than a claim about a full XR application. This specific workload characterization is preregistered upfront, ensuring the AI model does not silently overfit to an unrepresentative trace. Only the datapath's array dimensions vary, while dataflow, on-chip scratchpad capacity, interface bandwidth, layout, simulator version, workload, deadline, and scoring rule remain fixed. This narrowness isolates the effect of proposal source and array shape by holding most of the environment still.
...
![**The bounded study workflow.** The preregistration, bounded generation, deterministic evaluation, and judgment phases of a bounded Architecture 2.0 study.](images/ch8_design_loop_workflow){#fig-bounded-study-workflow width="100%" fig-alt="Bounded Study Workflow diagram"}
...
with lower values preferred. While this naive composite score mathematically double-counts the penalty for underutilization compared to a true Area-Delay Product (ADP), it serves as a predeclared selection rule for this study, not a universal measure of accelerator quality or a physical measurement. This specific scoring formula is used to prevent the search from collapsing to degenerate solutions like massively oversized arrays that remain mostly idle, enforcing a strict penalty for wasted silicon area. Reporting cycles and utilization separately keeps the composite score from hiding the two ingredients it combines, though it cannot reveal an axis the score leaves out, such as memory traffic.
...
arm from receiving extra candidates after results appear. In scaled physical design, bounding by a raw count of simulator calls is dangerous because runtimes are highly non-linear; the budget must evolve to a compute effort (CPU-hours) limit with multi-stage short-circuiting to fast-fail candidates at earlier stages (e.g., floorplanning estimates) before committing to detailed routing. The repair and decision rules fix how invalid model output and small score differences will be treated. The mechanism contrast challenges the generated explanation, while
...
That premise was not grounded in the retained study context. The transposition still distinguishes the model's exact directional prediction, but it is not a causally isolated test of orientation. Because the spatial mapping (e.g., which loop indices map to spatial X/Y dimensions in the weight-stationary setup) was not explicitly defined in the AI prompt before inference, the test probes the prompt's completeness rather than the AI's architectural reasoning. A stronger follow-up must pin the mapping semantics explicitly in the prompt and choose contrasts that causally isolate them.
...
While this schema error was simple API friction, real hardware environments involve undocumented EDA tool segfaults, FlexLM license server timeouts, or simulation deadlocks. Fixing a broken schema or a transient environment timeout is a necessary infrastructure repair, not a concession to the model or a change in the evaluation protocol. However, treating all EDA timeouts or deadlocks purely as infrastructure issues is an oversimplification. Automated harness scripts must classify EDA errors into two categories: *Transient/Infrastructure* (e.g., FlexLM denial, random tool segfaults) which trigger transparent retries, and *Design-Induced/Systematic* (e.g., DRC violation, place-and-route timeout due to congestion) which must feed back into the AI model as valid architectural constraint violations. The AI needs this negative feedback to learn physical boundaries, rather than the harness simply discarding the run.

::: {.callout-war-story title="The License Server Hallucination"}
**The incident.** An automated AI design agent was tasked with optimizing a block's floorplan area. During a sweeping exploration, the EDA tool's `FlexLM` license server temporarily went down, causing synthesis jobs to fail. The agent misinterpreted the generic tool timeouts as physical routing congestion failures, assuming its proposed designs were too dense. In response, it endlessly and incorrectly shrank the design area until it proposed an unbuildable, zero-area degenerate layout.

**The lesson.** Agents must precisely distinguish between transient infrastructure faults and true architectural violations; feeding toolchain noise back as a physical constraint will systematically derail AI-driven search.
:::
...
| **Execution environment** | In production EDA, requires recording PDK version, standard cell library PVT corners, binary patches, and Tcl setup scripts (e.g., via Nix or containers) to prevent environmental drift. |
...
The 16 by 64 candidate exactly matches the baseline's cycles, average layer utilization, and local score. Calling it the new winner would improperly convert a tie into an improvement by changing the rule after seeing the output. The tie also hides a difference the score never registers.[^fn-dram-traffic] Because the local score combines only cycles and utilization, it reads the two shapes as equal while their off-chip write traffic differs by a factor of two. For a 3W mobile XR chip, data movement energy typically dominates compute [@Horowitz2014Energy]. This means the 16 by 64 candidate is realistically worse, being Pareto-dominated on the unmeasured memory axis (@fig-proxy-mismatch).

This massive spike in off-chip DRAM writes is not just an isolated power metric failure. At the system level, the 16 by 64 SLAM subsystem aggressively steals limited, shared memory bandwidth away from the RISC-V host cores and the XR display engine, risking system-wide contention. Furthermore, under the thermal realities of wearables, blowing the 3W limit on data movement translates directly to rapid battery drain and unacceptable thermal throttling on a user's face.

::: {.callout-war-story title="The Space Heater"}
**The incident.** A hardware team optimized a mobile vision accelerator solely for a compute-bound proxy metric (utilization and cycle count) during their architectural exploration. The design taped out successfully, but when integrated into the final device, the unmeasured off-chip data-movement energy was so massive that the chip instantly thermally throttled in production, rendering the performance gains unusable.

**The lesson.** A 1D proxy score hides multi-objective physical realities. Optimizing for compute utilization without accounting for data-movement energy will produce a chip that functions perfectly in simulation but acts as a space heater in reality.
:::

While strict preregistration prevents moving the goalposts to declare a false 'win,' an architect discovering this severe proxy mismatch in production (@sec-feedback-verification-trust) would explicitly abort the preregistered study, amend the objective function to incorporate multi-objective physical constraints directly into the rejection criteria (e.g., an Energy-Delay Product $E_{total} = E_{compute} + E_{SRAM} + E_{DRAM}$), and restart the bounded loop to co-optimize Power, Performance, and Area (PPA) simultaneously.

```{python}
#| label: fig-proxy-mismatch
#| fig-cap: |
#|   **The proxy mismatch hides a Pareto-dominated failure.** A 1D surrogate score (such as cycles and utilization) ranks the 16 by 64 candidate exactly equal to the 32 by 32 baseline. However, plotting the candidates across two physical axes (Compute Cycles vs. Off-Chip DRAM Writes) reveals the 16 by 64 candidate is severely Pareto-dominated by the baseline when unmeasured memory traffic is accounted for.
#| out-width: "80%"
...
for shape, color, marker in [("16×64", COLORS["blue"], "o"), ("64×16", COLORS["orange"], "s")]:
    ...
    ax2.plot(x, marker=marker, markersize=4.2, linewidth=1.5, color=color, label=shape)
...
To distinguish them, an architecture outcome could be a "Success" if an AI candidate beats the baseline by 2%, but the stopping decision could still be "Stop" if the compute budget is exhausted or the 2% margin isn't worth the engineering risk to verify.
