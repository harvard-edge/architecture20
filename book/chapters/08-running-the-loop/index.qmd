...
As established in @sec-data-representations-world-models, general knowledge is meaningless without explicit project constraints. The representation for this study is small, serving as a specific record rather than a general world model of the accelerator. The fifteen legal shapes, baseline, mutable array dimensions, fixed simulator conditions, local score, and workload matrices form the explicit design state. By formalizing these variables, the architect retains authority over the design boundaries. In this instance, the candidate representation changes only the datapath's `ArrayHeight` and `ArrayWidth`, while the weight-stationary dataflow [@ChenEtAl2016Eyeriss], SRAM capacity, interface bandwidth, layout, workload, and simulator version remain fixed.
