...
**Author's Note:** The aphorism fits this study, which compares a model proposal against a fixed aspect-ratio heuristic in a physical simulator. The model's explanation ultimately fails a declared distinguishing contrast. This study proves useful precisely because the recorded results are less exciting than the initial hypothesis.

```{=latex}
\abstract*{This chapter presents one bounded integration study using a recorded model response and SCALE-Sim, a cycle-accurate systolic array simulator. The model and a fixed aspect-ratio heuristic each select three candidates under identical simulator-call budgets. Across five unique evaluated shapes, no nonbaseline candidate clears the declared decision margin. The two proposal arms reach an equal-call tie, while the model's directional mechanism prediction contradicts its declared contrast. This study demonstrates how to preserve a no-change architecture outcome, an equal-call tie, a contradicted directional prediction, an unsupported mechanism explanation, and a stopping decision without claiming that one small experiment validates the complete Architecture 2.0 discipline.}
```

::: {.callout-crux}
An architect must carefully fix, execute, and preserve study elements to ensure a bounded AI-assisted search reaches no conclusion stronger than its recorded results.
:::

This crux addresses metric shopping, a practical trap in AI-assisted architecture. Generative AI accelerates the creation of architectural candidates, but because simulators are highly configurable and models are highly sensitive to phrasing, it is easy to subconsciously baseline shop by tweaking simulator parameters after the fact to validate a plausible AI candidate. It is equally easy to prompt shop by silently discarding dozens of failed prompts until one produces the desired architecture. In a real EDA environment, where a single iteration takes days of RTL simulation and fighting for limited flexlm license checkouts, this trial-and-error hides massive physical and calendar costs. The design loop must be strictly bounded to prevent these hidden expenses.
...
The workload contains three GEMM layers extracted from the XR platform's spatial tracking (SLAM) pipeline, serving as a compact test slice rather than a claim about a full XR application. This slice nevertheless hints at a broader industry shift. As generative AI workloads move from fixed-size computation (like ResNet-50) to dynamic, iterative generation (like LLMs with growing KV-caches), the underlying hardware must become highly adaptive. We focus purely on the static dimensions for this experiment, acknowledging that a complete adaptive system study would need to model dynamic generation behavior.
Only the datapath's array dimensions vary, while dataflow, SRAM capacity (our cache subsystem), interface bandwidth, layout, simulator version, workload, deadline, and scoring rule remain fixed. This narrowness isolates the effect of proposal source and array shape by holding most of the environment still.

## Separate Outcomes from AI Claims

The preregistration keeps the architecture-outcome and AI-contribution claims
separate. This deliberate decoupling enforces a simple rule: the AI proposes candidates, and the deterministic simulator evaluates them. The model must never be allowed to grade its own homework. This ensures that a "successful" model invocation—one that generates valid JSON or a plausible explanation—is not automatically conflated with a successful architectural design. This structured cycle of preregistration, evaluation, and judgment is illustrated in @fig-bounded-study-workflow.
...
1. **Comparing an AI proposal arm with a fixed heuristic.** Equal simulator calls may control one scarce resource while leaving model inference, human setup, repair, prior knowledge, and total labor unmatched. Future work must determine which costs matter for the architecture decision and which comparisons require them to be matched.

2. **Testing an AI-generated explanation without rewarding a plausible story.** The directional prediction here preceded the workload transformation, but the record omitted the mapping semantics needed for a causal interpretation. It remains an open problem what representation a tool must expose and what contrast would distinguish the proposed mechanism from competing effects.

3. **Preserving state when an AI model call cannot be replayed exactly.** An archived response and hash pin the recorded output rather than the hosted generator. Architecture studies must establish which claims require a provider revision or digest, repeated calls that characterize variation, or an independently reproducible proposal method.

4. **Stopping an AI-assisted search after a no-change result.** Stopping was appropriate under the declared rule even though ten legal shapes remained unevaluated. Future guidance must address how an architect should value another AI proposal or simulation when the workload, local score, simulator abstraction, or missing mapping semantics may matter more than another point in the same search.

5. **Standardizing Agentic Benchmarks.** While this chapter demonstrates a single bounded study, the integration of generative models introduces a parallel necessity: evaluating the agents themselves. Future work must establish standardized benchmarks—commonly called "evals"—to measure model capabilities against realistic, long-horizon architecture workflows. To remain robust across generations of AI models, these evaluations must track timeless algorithmic properties: pass-rate and bounds adherence, trajectory efficiency (simulator calls to convergence), contextual awareness across error streams, and operational cost. By standardizing these agentic evaluations, hardware organizations can objectively rank competing models and safely determine which algorithmic agent is best suited to drive their specific Architecture Environment.

## Conclusion

This bounded integration study ends with a tie against the baseline, where the model and fixed aspect-ratio heuristic selected five unique datapath shapes from fifteen legal shapes. Because no evaluated nonbaseline shape cleared the 1 percent decision margin over 32 by 32, the record retains the baseline and demonstrates the preregistration methodology actively rejecting a flawed candidate.
