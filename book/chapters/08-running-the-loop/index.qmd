...
In a real EDA environment, the penalty for guessing wrong is severe. Waiting eight hours for a physical routing failure consumes vastly more calendar time and limited FlexLM license checkouts than waiting ten milliseconds for a Python syntax error. This trial-and-error hides massive physical costs. To combat this, the design loop must be strictly bounded.
...
Executing a multi-agent architectural search requires far more than zero-shot generation; it demands robust frameworks for trajectory management and error recovery. The open questions here involve designing representations that allow an agent to version its search tree and backtrack from physical synthesis failures, as well as reward structures capable of balancing competing power, performance, and area constraints without collapsing into oscillatory loops.


**Theme 1: Autonomous Trajectory Management**
* What latent-space representations enable an agent to semantically version physical synthesis failures and execute precise backtracking across non-linear hardware search trees?
* How do we build distributed version control systems that track non-linear execution histories, failed branches, and multi-fidelity evidence from autonomous searches?
* When an agent detects a local optimum, what algorithms dictate whether it should incrementally mutate the current state or completely restart the topology generation?

**Theme 2: Multi-Objective Co-Adaptation**
* How can we construct a unified, differentiable state space that safely bounds divergent oscillations when an agent simultaneously optimizes compiler passes and microarchitectural parameters?
* What reward structures successfully balance inherently competing constraints (e.g., power, timing, area) without collapsing into single-variable optimization?
* Can hierarchical multi-agent reinforcement learning (MARL) stably decouple the optimization of layout from the optimization of logic without losing Pareto efficiency?

**Theme 3: Error Localization and Recovery**
* How can an agent parse a 10,000-line synthesis failure log to pinpoint the exact architectural parameter or line of RTL responsible for the violation?
* Can we build "recovery policies" that automatically patch syntax and DRC errors in generated RTL without requiring a full re-generation from the LLM?
* What mechanisms allow an agent to isolate and quarantine buggy sub-modules while continuing to optimize the rest of the SoC?
...
Furthermore, catching schema errors as simple API friction before simulation proves the necessity of validating structured output and parsing AI intent into strictly legal simulator actions. Architecture must treat AI as a candidate generator, but strictly leave the simulator as the final judge. Reporting a null result cleanly, without recasting an equal-call tie or a contradicted directional prediction as an AI success, is the discipline this chapter set out to demonstrate. One small experiment does not validate the complete Architecture 2.0 discipline, but it shows that an adopted proposal can be tied to measured performance rather than to a persuasive model output.
