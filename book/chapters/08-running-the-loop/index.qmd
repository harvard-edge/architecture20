...
rankings before commitment. Its power column is frame energy divided by the candidate's active compute time, a conservative average-power screen against the 3\ W budget. It is a screen, not a signoff TDP verdict; peak and transient power (di/dt, IR drop) and sustained thermal behavior are deferred to later stages. This toy stage is not a claim that Timeloop, Aladdin, gem5, or FireSim was run here. Those tools are examples of the stronger model class it stands in for, including dataflow analysis [@ParasharEtAl2019Timeloop], calibrated pre-RTL estimation coupled to a full-system model [@ShaoEtAl2014Aladdin; @ShaoEtAl2016Gem5Aladdin], cycle-accurate simulation [@BinkertEtAl2011Gem5], and FPGA-accelerated simulation [@KarandikarEtAl2018FireSim].
...
what remains non-public, and what can be independently tested. This is the
public-audit structure the AlphaChip war story needed. A widely reported
placement result whose proprietary pretraining inputs and full replay path were
not released left no complete public residue a reviewer could replay, so the
strongest run could not be reconstructed in public and the dispute never fully
closed [@ChengEtAl2023MacroPlacement; @GoldieEtAl2024ThatChipHasSailed]. The
