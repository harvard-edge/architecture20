...
Because of these structural differences, the choice of generative model must match the artifact geometry. For sequential text like RTL or compiler scripts, Large Language Models (LLMs) are used. To ensure syntactic legality and schema compliance, ML-Sys practitioners employ *Constrained Decoding* (using Finite State Machines or context-free grammars during token generation) to restrict the model's output to valid syntax by construction, solving the syntax hallucination problem [@Poesia2022Synchromesh; @Willard2023Outlines]. For spatial artifacts like macro placement, *Diffusion Models* are used.[^fn-ml-diffusion-c06] However, while constrained decoding can guarantee a generated RISC-V decoder is syntactically valid, it cannot guarantee it is physically realizable. Unlike analytical solvers that guarantee routability, a diffusion model's spatial layout is merely a statistical prediction and will blindly produce overlapping macros if not subsequently validated by a physical routing tool.
...
Unlike generative brainstorming, prediction requires models that can be calibrated against real data. Large Language Models are poor fits here. Instead, to predict routing congestion or critical path delays from a netlist, architects have turned to *Graph Neural Networks (GNNs)* [@Wang2022CircuitNet]. Because chips are physical graphs, GNNs pass messages over the circuit's wiring graph and align with its structure. For power and thermal estimation, *Physics-Informed Neural Networks* (PINNs) penalize violations of physical laws during training to keep estimates within thermodynamic limits [@RaissiEtAl2019PINNs].
...
For discrete search spaces that require looking ahead, *Monte Carlo Tree Search (MCTS)* and *Tree-of-Thoughts* [@Yao2023TreeOfThoughts] allow an agent to plan multiple steps into the future, simulating rollouts of potential architecture configurations before committing to an expensive evaluation. Treating the design space as a game tree allows these methods to explore promising branches while pruning unviable configurations, ensuring the search doesn't waste runs on dead-ends.
...
When the objective is a power, performance, and area vector rather than a single scalar, the fitting methods become multi-objective. Multi-objective Bayesian optimization[^fn-ml-parego-c06] and multi-objective evolutionary search such as NSGA-II [@Deb2002NSGAII] and related dominance-sorting algorithms return a Pareto set rather than one best point. The architect then prunes that set against the Lighthouse power and performance frontier, since the 3\ W thermal budget rules out configurations a single scalar objective would have hidden.
