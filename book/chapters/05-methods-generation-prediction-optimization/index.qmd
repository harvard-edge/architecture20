...
To spend our simulation budget efficiently, we use algorithms built to handle uncertainty. We apply *Bayesian Optimization* when tuning continuous parameters or hyperparameters, such as sizing caches or out-of-order window dimensions under the 3\ W XR thermal constraints. When Bayesian optimization hits a nonlinear physical constraint, such as an unroutable configuration where the simulator crashes, the model stalls unless the constraint boundary is mapped.
