...
To mature from experimental demos to production deployment, the hardware community must establish rigorous methodologies for evaluating autonomous agents. This requires building standardized benchmark suites that test end-to-end EDA navigation, developing formal metrics that accurately quantify true reductions in tape-out burden, and calculating the exact mathematical penalty incurred when we force an AI to keep its generated topologies human-readable.


**Theme 1: The "SWE-bench" of Hardware**
* How do we build rigorous, standardized evaluation suites that measure an agent's ability to navigate multi-tool design loops, beyond just zero-shot coding fluency?
* What metrics accurately capture an agent's ability to recover from errors, handle out-of-distribution constraints, and navigate asynchronous EDA failures?
* Can we establish a global leaderboard that evaluates not just the performance of the generated architecture, but the compute efficiency of the agent's search trajectory?

**Theme 2: Quantifying Engineering Burden**
* What formal evaluation metrics reliably isolate an agent’s true reduction in end-to-end tape-out time by directly penalizing the "Simulator Tax" of unroutable candidates?
* How do we measure the cognitive load shifted to human verification engineers when they are forced to review hyper-optimized, non-intuitive AI-generated RTL?
* Can we build models that predict the total NRE cost savings of deploying a specific agentic workflow for a given chip design project?

**Theme 3: The Explainability Penalty**
* How much optimization performance must be mathematically sacrificed to guarantee that the resulting microarchitecture remains human-readable and debuggable?
* Can we quantify the "alienness" of an AI-generated topology to enforce constraints that keep the design within the bounds of human comprehension?
* How do we engineer interpretability tooling that automatically reverse-engineers a natural-language explanation of *why* the AI chose a specific, non-intuitive data path?
