...
1. **Benchmarking Agentic Capacity in Hardware Design:** How do we develop the "SWE-bench" or "ImageNet" equivalent for autonomous hardware agents—a rigorous, open-source evaluation suite that measures the agent's ability to navigate complex, multi-tool design loops rather than just its zero-shot coding fluency?
2. **Quantifying Total Engineering Burden:** What formal metrics can accurately measure whether an AI tool has genuinely reduced the end-to-end wall-clock time and cognitive load of a tape-out, rather than simply shifting the burden to the verification and physical design stages?
3. **Evaluating Multi-Agent Collaboration Topologies:** In a multi-agent architecture team (e.g., a Generator agent, a Verifier agent, and a Power Analysis agent), what communication topologies and consensus algorithms yield the highest rate of timing-clean, functionally correct designs?
