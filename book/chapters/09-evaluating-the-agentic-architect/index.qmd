...
| **Standard Benchmarks** | SPEC CPU, MLPerf, PARSEC [@PARSEC]. | ArchEval, VerilogEval [@VerilogEval], RTLLM [@RTLLM]. |
...
**Why we measure it (causal reasoning versus random search).** A failing STA trace pinpoints the exact critical path. A strong agent parses the timing report, identifies the bottleneck, and fixes the logic depth or the congested route.[^fn-sta-correction] A weak agent treats the timing log as random noise and simply guesses a new configuration. The Self-Correction Rate measures the agent's capacity to resolve physical constraints rather than brute-forcing the simulator.

[^fn-sta-correction]: For example, if an agent routes a 128-bit bus across the die, failing STA with a massive setup violation, a capable agent inserts pipeline registers to break the path.
...
**What to measure.** Whether the score survives contact with purely synthetic, zero-day tasks the model could not have possibly seen. If the performance collapses when an open-source task is swapped for a novel, procedurally generated microarchitectural challenge, the benchmark was merely measuring memory. Frameworks like ArchEval mitigate test leakage by parameterizing the environment: rather than asking the agent to implement a static specification (which might be memorized), they dynamically synthesize the constraints[^fn-dynamic-constraints] and measure the agent's real-time trajectory to optimize around them. This creates an un-gameable pass/fail criterion where success requires generalizing physics-bound logic rather than regurgitating leaked training data.

[^fn-dynamic-constraints]: For example, generating randomized but physically valid memory hierarchies.
...
### Navigating the Cost-Quality Pareto
