# Evaluate the AI Architect {#sec-evaluating-agentic-architect}
...
## Shift Measurement for AI-Driven Design
...
- **Measure** the search itself using **physics-bound benchmarks**: pass-rate, proxy calibration to avoid the **Proxy Mirage**, trajectory efficiency, autonomous recovery, tool use, and long-horizon context retention;
- **Trust** a score by mitigating **data contamination** and **test leakage**, reporting variance across seeds, and comparing against a baseline at an equal simulator-call budget;
...
## Measure the Search, Not the Chip
...
**Why we measure it (the Proxy Mirage).** A poorly calibrated proxy sends the optimizer confidently toward a Proxy Mirage, a region where the proxy predicts large gains but physical synthesis returns poor results. Scoring rank fidelity precisely where the search concentrates is what catches the mirage—preventing the agent from exploiting the surrogate's blind spots. For the field to mature, it requires standardized, physics-bound benchmarks that rigorously punish this kind of metric-gaming and validate true engineering value.

### Track Trajectory Efficiency and the Simulator Tax
...
**Why we measure it (causal reasoning versus random search).** A failing STA trace pinpoints the exact critical path. A strong agent parses the timing report, identifies the bottleneck, and fixes the logic depth or the congested route. For example, if an agent routes a 128-bit bus across the die, failing STA with a massive setup violation, a capable agent inserts pipeline registers to break the path. A weak agent treats the timing log as random noise and simply guesses a new configuration. The Self-Correction Rate measures the agent's capacity to resolve physical constraints rather than brute-forcing the simulator.

### Verify Tool Use and Rationale Faithfulness
...
### Isolate Held-Out Tasks to Prevent Contamination

The public corpus of open-source hardware, such as RISC-V cores like BOOM or Rocket, is exceedingly small compared to software repositories. Because modern LLMs aggressively scrape GitHub and open-source codebases during pre-training, the probability of **data contamination**—where the benchmark's test data leaks into the model's training corpus—is critically high for standard hardware tasks. When this leakage occurs, a high pass-rate measures **memorization** (data retrieval of existing RTL) rather than **generalization** (the capability to reason about novel microarchitectural trade-offs).

**What to measure.** Whether the score survives contact with purely synthetic, zero-day tasks the model could not have possibly seen. If the performance collapses when an open-source task is swapped for a novel, procedurally generated microarchitectural challenge, the benchmark was merely measuring memory. Frameworks like ArchEval mitigate test leakage by parameterizing the environment: rather than asking the agent to implement a static specification (which might be memorized), they dynamically synthesize the constraints (e.g., generating randomized but physically valid memory hierarchies) and measure the agent's real-time trajectory to optimize around them. This creates an un-gameable pass/fail criterion where success requires generalizing physics-bound logic rather than regurgitating leaked training data.
...
### Control Variance, Seeds, and Replay
...
### Compare Baselines at Equal Budgets
...
### Navigate the Cost-Quality Pareto
...
## Address Open Research Questions
...
## Conclude and Carry Forward {#sec-conclusion-and-carry-forward}

Evaluating an AI architect is a different discipline from evaluating a chip. We must rely on **physics-bound benchmarks** to measure the search rather than only the artifact, tracking metrics like trajectory efficiency and self-correction, while avoiding the **Proxy Mirage** by grounding validation in physical hardware realities. We make the score trustworthy by aggressively mitigating **data contamination** and **test leakage** in open-source designs, reporting variance, and comparing against a baseline at an equal EDA tool budget. Finally, we turn the score into a decision, reading reliability and cost to judge how much of the loop an agent may autonomously run.
