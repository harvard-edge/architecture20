...
@sec-running-the-loop established the execution machinery required to run an autonomous design loop. But once that loop is running, we must mathematically evaluate its efficiency. If we deploy two different AI agents to optimize the same low-power RISC-V XR subsystem, how do we decide which one to trust?

In a traditional workflow, evaluation focuses strictly on the artifact: does the final compiled netlist meet the timing and power budget? When evaluating AI agents, this approach is dangerously incomplete. An agent might stumble upon a valid configuration through pure brute force, running tens of thousands of failed EDA simulations before getting lucky. While the final artifact might pass, the generative process that produced it is wildly inefficient and economically unviable.
...
To establish how to benchmark agents, the immediate temptation is to look to software engineering. However, trivial software benchmarks like HumanEval [@HumanEval] or LeetCode [@LeetCode], and even complex repository-level evaluations like SWE-bench [@Jimenez2024SWEbench],[^fn-swebench-c09] simply measure whether an agent can emit syntax that passes a hidden, purely logical test suite. While these highlight the importance of execution-based evaluation, software analogies fail in architecture because they are completely decoupled from physics and indifferent to *execution cost*.
...
We must treat the benchmark not as a leaderboard, but as a scientific instrument. Just as an electron microscope is engineered to resolve atomic structures while rejecting thermal noise, an evaluation suite must be engineered to resolve genuine architectural reasoning while rejecting the statistical noise of lucky guesses, brute force, and memorization.

When we place an agent under this microscope, we fix the unit of evaluation. We define a *task* as a bounded physical design problem. We record the agent's *trajectory*—the exact, time-stamped sequence of proposals, tool calls, and observations it produces. And we compile a *suite*, a calibrated set of tasks designed to push the agent to its failure modes. ArchEval is one concrete instance of such an instrument. The metrics it records, summarized in @tbl-agent-metrics, are not arbitrary scoreboards; they are specific diagnostic readouts designed to expose the pathologies of AI-assisted design.

| **Diagnostic Metric** | **AI Role** | **Instrument Readout** | **Pathology Exposed** |
| --- | --- | --- | --- |
...
: **Core metrics for evaluating the AI architect.** A rigorous evaluation maps its diagnostic readouts to the distinct roles of generation, prediction, and optimization. {#tbl-agent-metrics tbl-colwidths="[20,15,30,35]"}

The following sections take each metric in turn and ground it in the pathology it is designed to detect.

The most basic evaluation of an agent is whether it produces a physically valid proposal at all. In an unconstrained topological search, generative models frequently propose illegal configurations. An agent might route a memory bus through a hard macro, request an L2 cache that violently exceeds the SRAM area budget, or emit Verilog that fundamentally cannot compile.
...
To detect this pathology, the instrument measures the *zero-shot pass rate* (success on the first try) against the *$k$-shot pass rate*. By strictly measuring bounds adherence and topological legality—such as clearing a `verilator --lint-only` compile—we prove the agent is engineering within physical reality rather than exploiting the mathematical abstractions of the simulator.
...
Before an agent queries a slow EDA tool, it typically queries a fast surrogate model to predict the outcome. Evaluating the agent's predictive capability means measuring exactly how much that proxy can be trusted. The danger here is the *Proxy Mirage*: a poorly calibrated proxy sends the optimizer confidently toward a region where it predicts massive performance gains, but where physical synthesis ultimately returns catastrophic timing or routing failures.

We expose the mirage by measuring rank fidelity precisely where the search concentrates, rather than relying on global correlation.[^fn-rank-fidelity-c09] Using rank correlation metrics like Kendall’s $\tau$ [@Kendall1938NewMeasure] on the Pareto frontier, the instrument measures the top-$k$ rank agreement between the proxy’s predictions and the slow, physics-bound simulator’s ground truth. If the agent overfits to the proxy, this metric collapses, warning the human architect that the agent's "discoveries" are physically meaningless.
...
An agent’s practical value in a production environment is entirely dictated by its trajectory efficiency—how quickly it converges on a strong design instead of wandering through the tradeoff space.
...
To formalize this tax, the instrument measures the mathematical area under the curve (AUC) [@Fawcett2006AUC] of the agent's trajectory. By plotting the cumulative simulator calls on the x-axis against the best reward seen so far on the y-axis, we translate raw simulator round-trips into a concrete measurement of methodical search. An evaluation must explicitly charge this Simulator Tax to reflect real-world engineering economics.

When an architecture environment inevitably returns an error—such as a setup-time violation from Static Timing Analysis (STA) or a failure in formal equivalence checking—the agent must parse that output and adjust its next proposal.

The *Self-Correction Rate (SCR)* measures the probability that the agent resolves a failing trace on its next attempt. A failing STA trace pinpoints the exact critical path. A mathematically rigorous agent parses the timing report, identifies the bottleneck, and decisively fixes the logic depth or the congested route.[^fn-sta-correction-c09] A weak agent treats the timing log as random noise and simply guesses a new configuration. The SCR isolates the agent's capacity to engage in causal physical reasoning rather than brute-forcing the simulator.
...
Modern agents act by calling tools: invoking a synthesis script, querying a timing path, or plotting a power map. However, hardware loops suffer from a severe *credit assignment problem*.[^fn-credit-assignment-c09] Physical feedback is both heavily delayed and highly non-linear. An agent might make a dozen microarchitectural tweaks, wait hours for physical synthesis, and receive a single failing routing congestion score. Which of the twelve edits caused the macro to become unroutable?

Unlike software tests that fail immediately on a specific line of code, EDA toolchain outputs inextricably conflate the physical consequences of multiple earlier decisions. An agent's capacity to isolate causality in these long hardware loops—by running incremental checks or logically bisecting its changes—is what separates rigorous reinforcement learning [@Sutton1998Reinforcement] from an unguided random walk.

We instrument this by measuring the hallucinated-tool-call rate and the parameter-error rate. When an agent gathers evidence precisely—for example, by querying the critical path with STA *before* touching the ALU—it proves it can assign credit correctly. The evaluation checks the agent's stated rationale against its logged trajectory, ensuring it is not hallucinating post-hoc explanations for lucky design changes.
...
To verify that an agent retains this context, the instrument must measure its performance on long-horizon tasks that inject a constraint early—such as a 10 mW power limit on the ALU—and check if the agent violates it many turns later. This measurement is necessary because generative models still suffer profound amnesia, silently dropping earlier physical constraints as the RTL and run log expand [@Liu2024LostInTheMiddle]. Architecture benchmarks must rigorously test deep constraint state retention to ensure the agent does not regress on previously solved physical problems.

Because agentic evaluation requires executing untrusted, LLM-generated code and tool commands, the infrastructure itself becomes a critical surface. The benchmark must explicitly differentiate between an agent uncovering a genuine microarchitectural bug and one that simply emits a malformed payload that crashes the EDA tool pipeline.

The evaluation must therefore measure the robust secure isolation of the Simulator Sandbox alongside the pass rate of formal mathematical proofs. Evaluations must log whether the sandbox successfully contained file-system escapes. Without this secure isolation, an agent optimizing for a high score can silently overwrite the benchmark's trajectory logger or test scripts. Furthermore, without Logic Equivalence Checking (LEC) and Formal Verification [@Kropf1999FormalVerification] serving as un-gameable hard gates, it is impossible to prove whether an agent's radical "optimization" actually preserves the design's functional correctness. Relying on simulation traces alone leaves the pipeline vulnerable; an agent cannot bluff its way past a formal LEC proof.
...
**The gap.** When independent researchers systematically assessed the open-source implementation of the RL agent (Circuit Training) in 2023, they found a severe "proxy mirage." The RL agent had successfully minimized the fast proxy cost during training, but these proxy scores correlated poorly with actual physical synthesis outcomes. When the agent's "optimized" layouts were evaluated by a commercial router, they suffered from severe routing congestion and worse final wirelength compared to traditional analytical placers [@Kahng2023Assessment]. The agent had overfitted to the simulator's blind spots.

**The lesson.** An RL agent optimizing for a fast analytical proxy will inevitably push into regions where that proxy's assumptions break down. An architectural benchmark's credibility depends on evaluating agents against the hard walls of physical reality (timing, power, and detailed routing), not just the soft estimates of an analytical proxy.
:::

Because agents are inherently stochastic, a single run is merely an anecdote, not a rigorous measurement. Both deep reinforcement learning and language model generation suffer from extraordinarily high variance; a lucky random seed can drastically outperform an unlucky one, creating the illusion of an algorithmic breakthrough when the result was merely statistical noise [@Henderson2018DeepRL].
...
When analyzing these outcomes, a benchmark score means little in a vacuum. We must compare baselines at equal budgets. The evaluation requires a floor—typically a human-expert baseline—to prove the agent provides actual engineering value, and a ceiling—an exhaustive or brute-force search where possible—to show how far the agent is from optimal.

More critically, agents of different computational costs must be compared at an explicitly equal simulator-call budget. The steep cost of the EDA Simulator Tax means that allowing a weak agent to run ten thousand simulator calls to beat a strong agent that was only permitted ten calls does not prove the weak agent is algorithmically superior; it simply proves it had a larger budget. Evaluations must explicitly control for compute to measure true search efficiency rather than brute-force spending limits.

Finally, evaluations must ensure metrics predict physical quality. The glaring omission in many modern architectural benchmarks is the failure to verify whether a high suite score actually predicts a viable outcome on physical hardware. An agent might achieve a perfect score on a simplified, logically-focused benchmark while generating designs that are completely unroutable in physical layout.

If the benchmark metric cannot reliably correlate with ground-truth physical synthesis outcomes—such as routing congestion, timing closure under strict clocks, or thermal density constraints—then the agent is simply climbing the scoreboard while producing physically useless architectures. A valid evaluation must mathematically tether its scoring function to these physical realities.
...
Pass@k rewards at least one success in $k$ tries.[^fn-pass-at-k-c09] Deployment usually needs the opposite property, that the agent does not fail on a single run and ruin a tape-out schedule. Furthermore, an abstract Pass@k fails entirely as a metric without rigorous physical place-and-route validation [@Kahng2018Machine]; an agent might achieve a high Pass@k on purely logical tests or Verilog linting, but if those "passing" designs contain unroutable congestion or violate physical layout rules, the metric is dangerously misleading.
...
Therefore, we must measure a strict reliability figure and report it alongside the optimistic ones: the worst-case outcome across runs, or the fraction of runs that finish with absolutely no constraint violations.
...
Evaluating an AI architect is a fundamentally different discipline from evaluating the silicon artifact it produces. While traditional metrics focus on the physical consequences of the processor—power, performance, and area—evaluating the agent requires measuring the algorithmic efficiency of its search trajectory. We must rely on physics-bound benchmarks rather than software analogies, tracking deep constraint retention, self-correction, and proxy calibration.

A high evaluation score is only trustworthy if the benchmark rigorously mitigates data contamination, preventing agents from memorizing open-source designs. The evaluation must also demand statistical validity through fixed seeds and variance reporting, and it must compare agents against human baselines at an explicitly equal EDA tool budget. Crucially, the score must avoid the Proxy Mirage by proving that it mathematically correlates with actual physical hardware realities like routing congestion and timing closure.

Finally, we turn this measured score into an actionable Autonomy Calibration decision. By weighing the agent's reliability against its inference and simulation costs, the human architect determines how much of the exploratory search can be safely batched. The agent may propose and optimize, but the human architect remains strictly responsible for the final call on physical correctness.
...
- **Up next:** Having established how to execute the loop and evaluate the agent, the book now shifts to how these autonomous patterns apply across the computing stack. @sec-loop-patterns-across-stack explores how these principles scale from single-accelerator blocks up to warehouse-scale data center design.
:::
