...
This need for explicit evidence introduces a hierarchy of commitment. As an AI-assisted design loop explores candidates, it must navigate different levels of fidelity, from fast, imprecise proxies to cycle-accurate simulation, Register Transfer Level (RTL) verification, emulation, synthesis, place-and-route, and signoff checks. The architect must define the escalation path, deciding exactly what level of evidence is required to advance a design to the next stage without incurring unacceptable risk. This rising standard of evidence, mapped against the increasing cost of being wrong, forms the commitment ladder (@fig-commitment-ladder).
...
This forces a shift in the architect's daily work. Before the AI can run the loop, the human architect must act as the knowledge extractor, bridging the gap between undocumented organizational wisdom and explicit loop state (@fig-tacit-extraction-bridge).

![**The Extraction Bridge:** AI-assisted loops stall or hallucinate when they encounter the undocumented, tacit constraints held by human experts. The human architect must bridge this gap by extracting tacit wisdom and codifying it into explicit, machine-readable loop state (such as environment contracts and rejection gates) that the AI can safely act upon.](images/F10-tacit-extraction-bridge){#fig-tacit-extraction-bridge width="100%" fig-alt="Diagram showing an architect extracting undocumented constraints from human experts and codifying them into explicit loop state for an AI agent to use."}
...
The automated participant cannot query unwritten wisdom directly; it relies on
the architect to explicitly bound the environment. In Architecture 2.0,
extracting tacit knowledge is no longer just good communication. It is the
prerequisite engineering step for bounded automation.
...
[^fn-ml-multi-fidelity]: Here multi-fidelity modeling is the ancestor of the book's fidelity ladder, the architect-owned escalation from cheap proxies to higher-cost evidence, with the commitment boundary rising as fidelity rises.
...
```{.yaml #lst-redacted-trace lst-cap="**Redacted Negative Trace:** An illustrative YAML sketch for sharing rejected design paths without disclosing proprietary physical design rules."}
# Illustrative sketch: a redacted negative trace (RNT)
...
context:
  ...
  proxy_environment: "open_or_synthetic_physical_proxy"
action:
...
evaluation:
  status: "REJECTED"
  rejection_authority: "PhysicalDesign_ReviewGate"
  disclosure_boundary:
    data_classes: ["physical_design_report", "PDK_rules"]
    redactions: ["absolute_dimensions", "NDA_design_rules"]
    reviewer_roles: ["artifact_committee_or_independent_auditor"]
    release_boundary_or_compliance_review_id: "internal_review:..."
  integrity_manifest:
    artifact_hashes: ["sha256:..."]
    attestations: ["redacted_report_attestation"]
  redacted_reason:
    category: "PHYSICAL_VIOLATION"
    sub_category: "ROUTING_CONGESTION"
    relative_impact: >
      Exceeds internal congestion threshold in L2 array macros;
      unroutable at target utilization
    # Note: Absolute cell dimensions and NDA Process Design Kit (PDK) Design Rule Check (DRC) rules are redacted.
lessons_learned:
  - >
    Non-linear routing congestion in high-associativity arrays
    negates Instructions Per Cycle (IPC) gains in XR workloads.
```

This projected proxy pattern (@lst-redacted-trace) sketches how a loop could
capture the essential context of an architectural exploration while redacting
absolute dimensions and proprietary design rules. The redacted evidence ledger
acts as a small public record. It names the loop well enough for another group
to understand what was attempted, what was observed, what was rejected, and what
kind of commitment the evidence can support. The minimum useful version appears in
@tbl-public-evidence-ledger. Anything less makes the claim hard to compare; anything
more may be impossible to share.
The point is field-level accountability, not full disclosure. A public ledger
...
**The path forward.** The goal of Architecture 2.0 is not about winning that old argument, but preventing the next one. A public evidence ledger and a redacted negative trace (@lst-redacted-trace, @tbl-public-evidence-ledger) let a team share the boundaries of their loop without exposing proprietary IP. By adopting these tools, the entire architecture community can celebrate and build upon future industry breakthroughs with rigorous, auditable trust.
:::
...
produced it. In that regime, the field becomes rejection-bound. Progress depends
on how much unsupported work can be rejected, escalated, or bounded before it
hardens into a commitment.
...
lets an ambitious result become a claim the field can inspect, reproduce within
its limits, and trust. That bridge is not a capability anyone waits to be handed.
