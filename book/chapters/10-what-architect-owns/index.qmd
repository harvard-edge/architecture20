...
This apparatus has a specific case in mind. The AlphaChip war story returns here
because the designs the method trained on were never released, for the legitimate
reason that they are Google's own chips, so the strongest public claim boundary
remained hard to reconstruct from public artifacts. A redacted negative trace and
a public evidence ledger (@lst-redacted-trace, @tbl-public-evidence-ledger) could
let a future result share more of its loop without sharing its chips. Such
records would bound public auditability rather than fully settling proprietary
claims. The point is not to reopen that case, but to give the field a way to keep
the next one from staying open.
