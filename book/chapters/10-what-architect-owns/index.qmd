...
evidence ledger (@lst-redacted-trace, @tbl-public-evidence-ledger) are what would
let a future result like it share its loop without sharing its chips. The point
is not to reopen that case, but to give the field a way to keep the next one from
staying open.
...
Building shared infrastructure and navigating the transition to AI-assisted architecture leaves several unsettled research directions. At the end of the book, the questions return to ownership. The technical challenge is to make loop integrity, shared failures, evidence gates, and human accountability durable enough for community-scale practice.

1. **How can redacted loop integrity be verified in competitive, federated ecosystems?**
The public evidence ledger (@tbl-public-evidence-ledger) assumes that some fields can be shared while others remain proprietary. A thesis-level systems contribution would define a redacted audit protocol that verifies constraints, proxy runs, evidence levels, and escalation events without exposing physical-design IP. Cryptographic hash commitments or zero-knowledge proofs may be part of the answer, but the paper should be judged by whether a reviewer can detect missing or inconsistent evidence across organizational boundaries.

2. **What can federated negative traces teach architecture models?**
This chapter argues for treating rejected alternatives as first-class evidence, but learning from design failure at discipline scale remains unsolved. A strong paper would build representation-learning methods for redacted, cross-organizational negative traces and test whether they improve early rejection in unseen but related design environments. The contribution should avoid claiming a complete architecture foundation model unless the evaluation shows broad transfer. A more credible first target is zero-shot[^fn-ml-zero-shot] anticipation of specific classes of integration faults.

3. **How can environments dynamically synthesize context-aware evidence gates under uncertainty?**
Escalation rules and review artifacts (@tbl-architect-owned-artifacts) are currently static, but future environments will face out-of-distribution workloads, unmodeled nodes, and changing software assumptions. A major research contribution would synthesize evidence gates from uncertainty estimates, workload provenance, and rollback cost, then enforce those gates before a claim can advance. The evaluation should show when the synthesized gate correctly stops, escalates, or permits a candidate under conditions the original loop did not anticipate.

4. **How do we preserve continuous human accountability across asynchronous, multi-agent drift?**
Long-horizon architecture tasks require decisions to survive workload changes, compiler updates, surrogate-model drift, and personnel changes. A concrete thesis could define a commitment ledger with ownership transfer, expiration, revalidation triggers, and human stop authority for asynchronous multi-agent loops. The evaluation should ask whether a later reviewer can identify who owns a stale claim, what evidence still supports it, and what event should force re-approval when the original decision maker is unavailable.

[^fn-ml-zero-shot]: The ability of a machine learning model to correctly perform a task or make predictions on data it has never seen during training, without requiring any specific fine-tuning. Here it sets a deliberately modest research target, anticipating classes of integration faults in unseen design environments rather than claiming a complete architecture foundation model.
...
::: {.callout-carry-forward title="The Final Loop State"}
...
a real and celebrated result the field could not replay, compare, or adjudicate,
not because the method was ever shown wrong but because the standard that could
settle it was never built. Set side by side, they mark the distance this book set out to close. The
moonshot names the reach and the war story names the debt, and the bridge
between them is the discipline these chapters describe, the explicit,
evidence-bearing, redactable loop that lets an ambitious result become a claim
the field can inspect, reproduce within its limits, and trust. That bridge is
not a capability anyone waits to be handed. It is infrastructure the community
builds.
