...
[^fn-arch-hardware-foundation-model]: A foundation model is a large-scale machine learning model trained on a vast quantity of data that can be adapted to a wide range of downstream tasks. A hardware foundation model would apply that approach to hardware design representations, which is exactly what the lighthouse prompt appears to demand before this chapter reframes it as a missing design loop.
...
This prompt may seem straightforward, but beneath the surface it binds together workload, software, architecture, and physical constraints. For an AI-assisted loop to act on this prompt, these implicit contracts must be explicitly surfaced as machine-readable constraints and rejection criteria.
...
[^fn-ml-held-out-validation]: In machine learning, the practice of reserving a portion of the dataset during training exclusively for evaluating a model's performance on unseen data. It is one of the established rigor practices Architecture 2.0 concedes as old, then requires to be recorded explicitly once a method rather than a human runs the loop.

[^fn-ml-multi-fidelity]: An optimization technique that combines cheap, low-accuracy simulations with expensive, high-accuracy simulations to explore a design space efficiently. The book borrows it deliberately as the fidelity ladder, the architect-owned escalation from fast proxies to high-cost signoff evidence.
...
disclosure. If the only way an AI can learn is by reading proprietary physical EDA failure logs, Architecture 2.0 risks becoming a closed oligopoly. One open mechanism would be a shared evidence-exchange format for redacted negative traces. The ISA analogy is useful only in a limited sense: the format would separate proprietary implementation details from the architecture lesson a reviewer can inspect.

The community could explore this mechanism through *Projected Proxies and Redacted Traces*. When a corporate AI loop rejects a candidate due to a proprietary NDA constraint (e.g., TSMC 3nm wire congestion), the loop might map that failure to an open-source proxy (e.g., ASAP7 or FreePDK45). A projected proxy could publish a relative failure when the projection preserves the rejection condition; it should not be treated as equivalent to the proprietary physical-design result.
...
This projected proxy pattern (@lst-redacted-trace) captures the essential context of the architectural exploration while strictly redacting the absolute cell dimensions and proprietary design rules that triggered the rejection. This redacted evidence ledger acts as a small public record that
...
This apparatus has a specific case in mind. The AlphaChip war story that has run
alongside the lighthouse throughout this book stayed open because the designs the
method trained on were never released, for the legitimate reason that they are
Google's own chips, so the strongest form of the claim could be neither
reproduced nor refuted in public. A redacted negative trace and a public
evidence ledger (@lst-redacted-trace, @tbl-public-evidence-ledger) are what could
let a future result like it share its loop without sharing its chips. Such
...
book, the questions return to ownership and community infrastructure. The
technical challenge is to make loop integrity, shared failures, evidence gates,
and human accountability durable enough for field-scale practice.

1. **How can redacted loop ledgers be audited without revealing design state?**
The public evidence ledger in @tbl-public-evidence-ledger assumes that some
fields can be shared while others remain proprietary. A thesis-level systems
contribution would define a redacted audit protocol that verifies constraints,
proxy runs, evidence levels, rejection events, and escalation decisions without
exposing physical-design IP. Cryptographic commitments may be part of the
answer, but the paper should be judged by whether reviewers can detect missing
or inconsistent evidence across organizational boundaries.

2. **How can negative traces become shared infrastructure?**
This chapter argues for treating rejected alternatives as first-class evidence,
but learning from design failure at discipline scale remains unsolved. A strong
paper would define schemas, redaction rules, review norms, and reuse tests for
negative traces across organizations. The evaluation should ask whether future
loops avoid known failure modes, reopen stale failures correctly, and preserve
the boundary between local evidence and general architectural knowledge.

3. **How can tacit physical and integration constraints become loop state?**
Many architecture decisions depend on physical-design judgment, integration
experience, workload context, and review instincts that are rarely captured in
machine-readable form. A major research contribution would turn selected tacit
constraints into loop state without pretending that all expertise can be
automated. The evaluation should ask whether the representation improves
rejection, review, or escalation while keeping ownership with the architect.
The relevant artifacts are summarized in @tbl-architect-owned-artifacts.

4. **How can human stop authority survive long-horizon multi-agent loops?**
Long-horizon architecture tasks require decisions to survive workload changes,
compiler updates, surrogate-model drift, and personnel changes. A concrete
thesis could define a commitment ledger with ownership transfer, expiration,
revalidation triggers, and human stop authority for asynchronous multi-agent
loops. The evaluation should ask whether a later reviewer can identify who owns
a stale claim, what evidence still supports it, and what event should force
re-approval when the original decision maker is unavailable.
...
These benchmarks share a target shape (@fig-long-horizon-task-timeline). The task is defined as long horizon
...
challenge is not prompt-to-chip. It is prompt-to-loop. @tbl-long-horizon-challenges sketches a starting set of tasks.
...
interfaces, cards, tasks, and evidence conventions that make claims reviewable and
contrastable without pretending all architecture work is the same. Full
comparability, in MLPerf's sense, is a stronger and later goal. It needs a fixed
task, workload, metric, and submission rules, and a neutral referee that can
reject a published claim, none of which a self-attested card supplies on its own. A credible standard should make visible the same fields the public evidence
ledger already names (@tbl-public-evidence-ledger), from task and workload state
through to the accountable human or gate that can stop or accept a commitment.

SPEC CPU and MLPerf show one maturation path from local measurements to shared
benchmark practice. They did not make every result identical, but they did make
a class of claims harder to state without versions, rules, measurements, and
reviewable reporting [@SPEC2017CPU; @MattsonEtAl2020MLPerf]. Architecture 2.0
needs analogous field infrastructure for loop-level evidence packets before it
can support stronger cross-paper comparability.
...
Different communities can start at different leverage points. Each group can
contribute a first useful artifact without waiting for a complete field standard
(@tbl-role-agenda).
...
credible loop packet, not a process program. That packet turns the field agenda
into three artifacts a reviewer can inspect (@tbl-minimum-credible-loop-packet).
...
The tools, models, and automated systems driving this shift are changing at
breakneck speed, and any catalog of today's capabilities will quickly become
obsolete. Because this transition is a massive work in progress, the one durable
anchor is engineering discipline. We are crossing the threshold from *artifact
