...
| --- | --- | --- | --- |
| **Pass-Rate ($Pass@k$)** | Generation | The fraction of proposals that clear validation without errors. | Detects reward hacking, catching illegal topologies or interconnect protocol deadlocks before they score. |
...
If we reward an agent solely for maximizing IPC, we invite Goodhart’s Law.[^fn-goodharts-law-c10] The agent will learn to "reward hack" the simulator. For instance, it might exploit a known boundary bug in a McPAT power model to artificially deflate estimated energy usage by gating clocks that are structurally required to remain active.

To detect this, we measure the *zero-shot pass rate* (success on the first try) against the *$k$-shot pass rate*. A high zero-shot pass rate indicates the agent relies on internalized, physics-bound reasoning, whereas a high k-shot pass rate might merely indicate the agent iteratively probed the simulator until it found an exploitable blind spot. Measuring bounds adherence and topological legality, such as clearing a `verilator --lint-only` compile, shows the agent is engineering within physical reality rather than exploiting the abstractions of the simulator.
...
### Proxy Calibration and the Mirage
...
### Self-Correction and Credit Assignment
...
[^fn-credit-assignment-c10]: **Credit assignment**: The reinforcement learning problem of determining which earlier action caused a later outcome. An agent that queries the critical path before editing shows it can attribute a timing gain to the change that produced it.

### Context Retention and Sandbox Security
...
Just as an agent must respect the historical context of its own design decisions, it must also be constrained by the security context of the evaluation environment. Because agentic evaluation requires executing untrusted, LLM-generated code and tool commands, the infrastructure itself becomes a critical surface. The benchmark must differentiate between an agent uncovering a genuine microarchitectural bug and one that emits a malformed payload that crashes the EDA tool pipeline.

The evaluation must therefore measure the secure isolation of the Simulator Sandbox alongside the pass rate of formal proofs, logging whether the sandbox contained file-system escapes. Without that isolation, an agent optimizing for a high score can overwrite the benchmark's trajectory logger or test scripts. And without Logic Equivalence Checking (LEC) for synthesis integrity and SystemVerilog Assertions (SVA) for bounded model checking as hard gates, there is no way to show whether an agent's aggressive optimization preserves functional correctness. Simulation traces alone leave the pipeline vulnerable; an agent cannot bluff its way past a formal LEC proof.

## Measurement Validation

Even if an evaluation successfully isolates the sandbox and records a comprehensive set of metrics, those metrics are only as trustworthy as the underlying benchmark data. Four failures make a good-looking score a lie. The tasks were contaminated, the number was noise, there was nothing to compare it against, or the metric was never shown to predict the capability it stands in for.

### Distinguishing Harness from Data
...
The public corpus of open-source hardware, such as RISC-V cores like BOOM [@BOOM] or Rocket, is exceedingly small compared to software repositories. Because modern LLMs aggressively scrape GitHub and open-source codebases during pre-training, the probability of *data contamination*,[^fn-data-contamination-c10] where the benchmark's test data leaks into the model's training corpus, is very high for standard hardware tasks [@Balloccu2024Leakage]. Standard ML mitigations, such as n-gram overlap filtering and embedding-based decontamination of the pre-training corpus, are necessary but insufficient because RTL semantics can vary wildly with small syntactic changes. When this leakage occurs, a high pass-rate measures *memorization* (data retrieval of existing RTL) rather than *generalization* (the capability to reason about novel microarchitectural trade-offs).

[^fn-data-contamination-c10]: **Data Contamination**: When the answers to a benchmark are inadvertently included in an AI's pre-training data. If an agent has already "read" the solution to a specific RISC-V challenge, a high score measures rote memorization rather than actual architectural reasoning.
...
### Controlling Variance and Budgets
...
More critically, evaluations must compare agents at an explicitly equal simulator-call budget. Because the EDA Simulator Tax dominates the cost of search, allowing a weak agent more tool calls than a strong agent measures spending limits rather than true algorithmic efficiency.

Finally, a valid evaluation must ensure its metrics reliably correlate with physical synthesis outcomes, such as routing congestion and timing closure. If an agent can achieve a perfect score on a logic-focused benchmark while generating unroutable designs, the metric has fallen prey to the Proxy Mirage, allowing the agent to climb the scoreboard while producing physically useless architectures.

## Scoring and Decisions

Assuming the benchmark is valid and free of the Proxy Mirage, the resulting evaluation exists to change what an architect does next. Applying it requires reading the scores through reliability and cost, not just peak capability.

### Reliability Over Optimism

Pass@k rewards at least one success in $k$ tries.[^fn-pass-at-k-c10] Deployment usually needs the opposite property, that the agent does not fail on a single run and ruin a tape-out schedule. Furthermore, an abstract Pass@k fails as a metric without rigorous physical place-and-route validation [@Kahng2018Machine]; an agent might achieve a high Pass@k on logic-only tests or Verilog linting, but if those passing designs contain unroutable congestion or violate physical layout rules, the metric is misleading.
...
Therefore, we must measure a reliability figure and report it alongside the optimistic ones. This includes the worst-case outcome across runs, or the fraction of runs that finish with no constraint violations.
...
![**Comparing agents on a cost-quality plane.** Each agent is a point: total evaluation compute against the best design quality it reaches.](images/F10-agent-pareto){#fig-agent-pareto width="80%" fig-alt="A scatter plot of agents on axes of total evaluation compute versus design quality, with a Pareto frontier of non-dominated agents."}

@fig-compute-economics shows why the cost axis cannot be waved away. Total evaluation compute is the sum of a *falling* EDA tool simulation cost and a *rising* Transformer inference cost. A weak agent queries the simulator thousands of times; a frontier agent[^fn-frontier-model-c10] reasons more but pays a large inference cost.

[^fn-frontier-model-c10]: **Frontier model**: Industry shorthand for the most capable current generation of large models, which spend heavily on inference per step. The term is unrelated to the Pareto frontier on the cost-quality plane.
...
Measuring the agent rather than the artifact requires benchmarks, burden metrics, and faithfulness checks. Three gaps remain open.
...
Weighing the agent's reliability against its inference and simulation costs, the human architect determines how much of the exploratory search can be safely batched. The agent may propose and optimize, but the human architect remains responsible for the final call on physical correctness.
...
- **Up next:** This chapter set out what must be revalidated when measuring the search trajectory. @sec-what-architect-owns examines the responsibility and governance that the architect must retain when delegating work to AI.
:::
