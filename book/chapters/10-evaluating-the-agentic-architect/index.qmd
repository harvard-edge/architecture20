...
- When evaluating an agent, the total cost of search is as important as the final score. An agent that finds an optimal 3 W RISC-V core after consuming 10,000 core-hours of cycle-accurate simulation is not 'better' than a fixed heuristic that finds a 3.1 W core in 10 minutes. Frontier agents can intentionally scale test-time compute to improve search quality, explicitly trading increased Transformer inference for a reduced EDA Simulator Tax. To prevent reporting illusions, evaluations must fix a **simulation budget** (e.g., maximum clock cycles or wall-clock time) and plot the cost-quality U-curve: the trade-off between the AI's inference cost (API tokens) and the environment's execution cost (simulation hours).
:::
