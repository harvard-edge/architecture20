...
[^fn-sta-correction-c09]: **Self-correction (example)**: An agent that routed a 128-bit bus across the die and drew a large setup violation from STA demonstrates recovery when its next proposal swaps for lower-Vt cells, eases localized placement density, or inserts pipeline registers to break the failing path.
