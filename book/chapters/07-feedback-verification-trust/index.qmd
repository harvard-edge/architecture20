...
Architecture studies need a typed epistemic path. Feedback says that something returned. A tool observation records what the declared path actually produced. Evidence connects that observation to a bounded claim. A review result judges the claim. A knowledge-update decision selects what, if anything, should change in project memory or a model. An architecture decision selects the next design action. These stages can stop, branch, or disagree.Three nonautomatic transitions keep these stages distinct, preventing raw data from bypassing the review process. In control theory terms, bypassing the review step is feeding raw sensor noise directly into the steering actuator. You must update your internal model of the world (state estimation) before applying a design change (actuation), avoiding oscillatory instability. Ultimately, evidence must act like a cryptographic epistemic ledger. The AI assumes the role of an Untrusted Prover presenting a generated artifact and an accompanying trace, while the human Architect acts as the Verifier who validates the physical proof without ever trusting the AI's internal reasoning or persuasive text. This establishes an unbroken chain of custody from intent to hardware execution.

First, a tool return does not automatically become a valid architecture observation, as a broken environment, malformed request, timeout, flexlm license server hang, OOM crash during elaboration, or unsupported action may report only the state of the tool path.h.
...
![The Compact Claim Frame diagram visualizes the relational graph linking the artifact candidate to its measurement metrics and environmental constraints.](images/F7-claim-frame.svg){#fig-claim-frame}
...
The transition from a returned tool signal to a trusted architectural decision requires structured human judgment. As automated generators enter the design loop, the volume of data grows, but the requirements for evidence do not. We must actively guard against proxy mismatch, correlated errors, and uncalibrated models. By bounding claims, demanding independent physical checks, and treating waivers as accepted risks rather than passes, architects maintain control over the integrity of the system. The purpose of verification is to build trust, ensuring that every transition in the architecture is anchored by defensible, reproducible, and context-aware evidence.
