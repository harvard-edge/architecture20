...
learning is possible. A Bayesian optimizer[^fn-ml-bayesian-optimizer], reinforcement-learning policy[^fn-rl-policy],
surrogate model[^fn-ml-surrogate-model], critic[^fn-rl-critic], or tool-using agent should behave differently when a
...
[^fn-ml-bayesian-optimizer]: A Bayesian optimizer builds a probabilistic model of an objective function so it can locate strong parameters in relatively few evaluations. The feedback budget ledger shapes how aggressively it may sample, since each evaluation carries a cost and produces evidence of a stated fidelity.
...
[^fn-ml-surrogate-model]: A surrogate model is a cheaper, learned approximation of a complex and expensive simulation. Its rankings are proxy feedback, useful for exploration but only trustworthy up to the point where an independent, higher-fidelity check can still reject a proxy win.
