...
1. **Self-Verifying Autonomous Agents:** What is the minimal set of self-generated verification collateral (SystemVerilog assertions, constrained-random tests) an agent must produce alongside its RTL to mathematically prove its proposed logic is functionally equivalent to the human intent?
2. **Quantifying the Sim-to-Silicon Gap for Agentic Policies:** How do we mathematically quantify the "sim-to-silicon" gap—the divergence between simulation proxies and physical realization—and incorporate this uncertainty directly into the agent's reward function?
3. **Adversarial Resilience of Architecture Oracles:** How vulnerable are current architectural simulators and power models to adversarial exploitation by reward-maximizing agents, and how can we harden these oracles against "metric hacking"?
