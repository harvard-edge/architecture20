...
risk unpriced. Architecture 2.0 requires a non-interference screen, a cheap
...
contention, transient-execution, and simultaneous multithreading (SMT) channels. A complete security screen also covers memory-integrity and disturbance faults, not only information-flow side channels. A design that trims DRAM refresh to meet a power budget can open a RowHammer path, so read-disturbance is a rejection gate in its own right.
