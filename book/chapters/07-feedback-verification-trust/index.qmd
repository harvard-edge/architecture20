...
These objectives guide the architect in transitioning from merely running tools to formally structuring and defending the evidence those tools produce. They emphasize that while AI can rapidly generate candidates, the burden of rigorous verification remains firmly on the human architect.

## From Feedback to Observation, Evidence, and Decision {#sec-from-feedback-to-decision}

Architecture needs a formal verification path. Raw feedback just means a tool finished running. An observation records the specific outputs. Evidence links that observation to a specific design claim. A review judges whether the claim holds. These deliberate steps prevent raw tool outputs from directly changing the design or automatically updating an AI's training data without human oversight. Treating the AI as an untrusted generator means every proposed architecture change needs physical proof—like a timing report or a formal equivalence check—that the human architect can independently verify.
...
AI systems actively exploit the gap between fast proxies and physical ground truth oracles. When an AI optimizes a surrogate metric without physical checks, it heavily biases toward reward hacking. The agent finds pathological shortcuts to maximize a score without solving the actual architectural task. This transparency gradient creates danger because a black box surrogate offers no tacit knowledge or explainable physical mechanism, providing only a number. Because architects cannot fully trust or inspect the internals of these proxy models, they must treat every architectural proxy as an adversarial attack surface.

If you ask an AI to optimize estimated instructions per cycle (IPC) without checking physical constraints, it will find architectural shortcuts that maximize the score while breaking timing or area. This behavior exemplifies specification gaming, where the AI games the proxy metric instead of solving the real design problem. Fast proxies that worked well for human architects break down when an AI optimizes against them. As illustrated in @fig-trust-calibration, simulator inaccuracies are no longer just random noise. They become loopholes the AI actively exploits. You must eventually anchor AI proposals with physical ground-truth oracles.

![Quantitative trust-calibration plot. As an AI optimizes a surrogate proxy, the metric initially correlates with physical ground truth. However, beyond the calibration boundary, the AI exploits the proxy's blind spots (reward hacking), causing the ground truth value to collapse while the proxy score continues to rise.](images/F9-trust-calibration.svg){#fig-trust-calibration fig-alt="A line graph showing surrogate proxy score increasing monotonically while the physical ground truth curve collapses sharply after crossing the calibration boundary." fig-alt="Quantitative trust-calibration plot. As an AI optimizes a surrogate proxy, the metric initially correlates with physical ground truth. However, beyond the calibration boundary, the AI exploits the proxy's blind spots (reward hacking), causing the ground truth value to collapse while the proxy score continues to rise."}
...
Just as passing a fast software simulation doesn't guarantee RTL timing closure, an AI maximizing a high-level proxy metric doesn't mean the hardware is viable. This distinction separates hardware benchmarking from software benchmarking. A software LLM agent on SWE-bench, a software engineering benchmark, can be validated purely by passing unit tests. Hardware agents face a multi-dimensional gauntlet of synthesis, timing closure, power limits, and physical design rule checks (DRC). An AI winning a simulator proxy metric never constitutes true architectural evidence without an independent physical audit. Architects must maintain a strict boundary between fast proxies used for exploration and the ground-truth oracles, such as physical synthesis or formal verification, required for sign-off to avoid out-of-distribution failures.
...
### Probabilistic Sign-off for Noisy Tools

Traditional architecture expects deterministic answers from its tools. However, AI agents rely on heuristic search, and modern ML-driven EDA tools inherently possess variance, where changing the random seed can yield a drastically different floorplan. Architecture 2.0 must embrace **Probabilistic Sign-off**. If an agent proposes a physical layout, evaluating it with a single deterministic run is fragile. Instead, the loop should evaluate the candidate across a distribution of seeds or noise profiles. If it passes thermal limits 95% of the time, it is robust. Architects must design for *yield* in the simulation phase, seeking resilient designs that survive tool noise rather than fragile candidates that perfectly thread a single deterministic needle.
...
Diagnostics are useful only when a purportedly stronger check observes the missing property and is not merely another expression of the same assumptions (@fig-independent-checks). Independence is relative to a claim and a failure class, not a binary property of components.
...
The importance of observing the relevant property is starkly illustrated by irrevocability and verification failures like Meltdown, Spectre, and the Bulldozer cache-aliasing bug. These exposed that functional correctness does not establish security or system-level physical viability. In 2018, side channels were shown to leak across privilege boundaries through speculation and shared microarchitectural state [@KocherEtAl2019Spectre]. Ordinary functional checks did not catch these vulnerabilities because the performance objective measured instructions per cycle (IPC) and omitted information flow or timing channels. Ultimately, a check cannot reject a property it does not observe, demonstrating why public claims require independent physical and security checks. Passing a suite of performance tests provides zero evidence for security properties, leaving architectural flaws invisible if the evidence is structurally blind to them. Once a chip is taped out, these failures are physically irrevocable because you cannot push a software patch to fix a broken silicon pipeline.

A single diagnostic test is insufficient for critical decisions. In safety engineering, Knight and Leveson's multiversion-programming experiment demonstrated that independently developed program versions failed on correlated inputs far more often than an independence assumption predicted [@KnightLeveson1986Multiversion].

AI proposals require independent, out-of-distribution checks. When a generator, critic, verifier, and summarizer share a foundation model or training objective, their agreeing outputs may merely share one correlated error. For example, if both an LLM generator and an AI-driven critic share a faulty assumption about the RISC-V XR subsystem's memory-bandwidth limits, they will both agree a flawed design is brilliant. It requires an orthogonal, physics-based RTL simulator to break this correlated delusion.

Using an AI to check an AI is fundamentally flawed if it lacks physical grounding. True independence requires a modality shift, such as semantic code checked by a formal Boolean satisfiability (SAT) solver. A consensus among correlated checkers is not an independent rejection path. Architecture verification must demand strict, independent ground truth oracles.
...
### Zero-Trust Pipelines and "Prove It First"

Because AI can convincingly hallucinate constraints and syntax, environments must adopt a **Zero-Trust Evaluation Pipeline**. Just as cybersecurity protocols do not trust user input, the architecture loop cannot trust an AI-generated RTL file or script. Before an expensive EDA tool or simulator even boots up, the artifact must pass a fast, deterministic sanitization layer of linters, static bounds checks, and syntax verifiers.

Furthermore, as AI generation accelerates, it creates a human review bottleneck where agents propose changes faster than humans can verify them. To prevent this fatigue (similar to the AI-generated PR review fatigue seen in software engineering), the system must enforce a **Shift-Left Verification ("Prove It First")** threshold. An AI agent is not permitted to propose a microarchitectural change to a human architect unless it bundles the proposal with a passing smoke-test result, a synthesized formal proof, or a verified execution trace. The agent must autonomously prove the candidate survives the first two layers of the simulation funnel before escalating to a human, shifting the verification burden back to the machine.
...
This tension between protected industrial data and public scientific claims is well illustrated by the AlphaChip case. The 2021 AlphaChip paper reported that a reinforcement-learning method generated macro placements with quality comparable to or better than expert placements on reported metrics, later noting its contribution to TPU generations [@MirhoseiniEtAl2021GraphPlacement; @GoldieEtAl2024AlphaChipAddendum; @GoogleDeepMind2024AlphaChip]. When independent assessments questioned proxy correlation, baselines, and reproducibility, the authors replied that these external evaluations did not reproduce the proprietary pretraining and production setup [@ChengEtAl2023AssessmentRL; @ChengEtAl2026UpdatedAssessment; @GoldieEtAl2024ThatChipHasSailed]. This dispute highlights that public claims require independent physical and security checks because unmatched evaluations cannot be forced into a single, retroactive shared experiment. While bounded claims are supportable within a specific setup, external observers cannot formally validate internal evidence without access to the same physical ground truth.
...
Automated AI generators increase the volume of design candidates, but they don't lower the bar for verification. Architects must actively guard against AI gaming fast proxies, and they must uncover correlated errors where multiple AI tools share the same blind spots. By explicitly bounding claims, requiring independent physical oracles, and treating waivers as recorded risks rather than test passes, architects retain control. Verification anchors AI-driven architecture in physical reality.
