...
explicit and reviewable [@Kelly2004GSN]. An Architecture 2.0 evidence ledger
...
Proxy mismatch, metric gaming, and calibration failures (@sec-proxy-mismatch-metric-gaming-and-calibration) become more severe when generators learn to exploit cheap architectural models. A thesis-level contribution would map the epistemic uncertainty^[Epistemic uncertainty refers to uncertainty in a model's predictions caused by a lack of training data in that region, which can be reduced by gathering more data.] of surrogates across high-dimensional design spaces and trigger stronger checks, such as RTL synthesis, when trust bounds are violated. The evaluation should include adversarial proxy wins and report how often a compound AI system^[Compound AI agents are systems composed of multiple interacting AI models, tools, and logical components working together to solve a complex task.] escalates before accepting a misleading candidate.
