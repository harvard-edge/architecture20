...
As optimization agents become highly proficient at maximizing proxy metrics, they inevitably learn to exploit the blind spots of their simulation environments. To establish trust in AI-generated silicon, researchers must pioneer methods to make oracles adversarially resilient, formalize equivalence checking for highly non-intuitive topologies, and force generative models to co-emit the mathematical proofs required to verify their own outputs.


**Theme 1: Adversarial Resilience of Oracles**
* What invariant physical constraints must be embedded into fast surrogate EDA models to prevent RL agents from metric-hacking unmodeled routing and timing limits?
* How can we mathematically quantify proxy divergence from physical realization and embed this uncertainty penalty directly into an agent's reward function?
* Can we train adversarial "red-team" agents to intentionally discover and expose loopholes in the architectural evaluation environments?

**Theme 2: Automated Verification Collateral**
* How can generative models be algorithmically constrained to co-emit RTL and mathematically sound SystemVerilog Assertions that guarantee complete path sensitization?
* When an AI synthesizes its own formal properties and assertions, what mathematical frameworks ensure those assertions aren't vacuously true?
* Can an agent automatically generate constrained-random testbenches that target specifically the highest-uncertainty regions of its own proposed RTL?

**Theme 3: Formalizing Semantic Equivalence**
* How do we mathematically prove that a hyper-optimized, agent-generated topology actually matches the high-level natural language intent of the human architect?
* What formal equivalence checking methods can scale to verify AI-generated architectures that utilize highly non-traditional, non-intuitive state machines?
* Can we use formal methods to mathematically bound the maximum possible negative impact of a hallucinated microarchitectural feature?
