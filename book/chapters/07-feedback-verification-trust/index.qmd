...
AI will actively exploit the gap between Proxies vs. Ground Truth Oracles. When an AI optimizes a surrogate metric without physical checks, it heavily biases toward reward hacking, finding pathological shortcuts to maximize a score without solving the actual architectural task. This is the danger of the transparency gradient—a black box surrogate offers no tacit knowledge or explainable physical mechanism, only a number. Because we cannot fully trust or inspect the internals of these proxy models, architects must treat every architectural proxy as an adversarial attack surface.

AI naturally exploits the gap between fast proxies and physical ground truth. If you ask an AI to optimize estimated IPC without checking physical constraints, it will find architectural shortcuts that maximize the score while breaking timing or area. This is classic "specification gaming"—the AI games the proxy metric instead of solving the real design problem. Fast proxies that worked well for human architects break down when an AI optimizes against them. As illustrated in @fig-trust-calibration, simulator inaccuracies are no longer just random noise; they become loopholes the AI actively exploits. You must eventually anchor AI proposals with physical ground-truth oracles.
...
Just as passing a fast software simulation doesn't guarantee RTL timing closure, an AI maximizing a high-level proxy metric doesn't mean the hardware is viable. This is the core distinction between hardware vs software benchmarking: while a software LLM agent on SWE-bench can be validated purely by passing unit tests, hardware agents face a multi-dimensional gauntlet of synthesis, timing closure, power limits, and physical DRC rules. An AI winning a simulator proxy metric never constitutes true architectural evidence without an independent physical audit. Architects must maintain a strict boundary between fast proxies used for exploration and the ground-truth oracles (like physical synthesis or formal verification) required for signoff to avoid out-of-distribution failures.
...
Traditional architecture expects deterministic answers from its tools. However, AI agents rely on heuristic search, and modern ML-driven EDA tools inherently possess variance—changing the random seed can yield a drastically different floorplan. Architecture 2.0 must embrace **Probabilistic Sign-off**. If an agent proposes a physical layout, evaluating it with a single deterministic run is fragile. Instead, the loop should evaluate the candidate across a distribution of seeds or noise profiles. If it passes thermal limits 95% of the time, it is robust. Architects must design for *yield* in the simulation phase, seeking resilient designs that survive tool noise rather than fragile candidates that perfectly thread a single deterministic needle.

## Independent Checks and Correlated Errors {#sec-independent-checks-and-correlated-errors} (@fig-independent-checks).
...
Using an AI to check an AI is fundamentally flawed if it lacks physical grounding. True independence requires a modality shift (e.g., semantic code checked by a formal SAT solver). Therefore, a consensus among correlated checkers is not an independent rejection path; architecture verification must demand strict, independent Ground Truth Oracles.
...
Because AI can convincingly hallucinate constraints and syntax, environments must adopt a **Zero-Trust Evaluation Pipeline**. Just as cybersecurity protocols do not trust user input, the architecture loop cannot trust an AI-generated RTL file or script. Before an expensive EDA tool or simulator even boots up, the artifact must pass a fast, deterministic sanitization layer—linters, static bounds checks, and syntax verifiers.
