...
[^fn-ml-tool-using-agent]: A tool-using agent is an AI system that can programmatically invoke external software tools or APIs, such as simulators or synthesis flows, to carry out a task. In this chapter it is one of the actors whose outputs count as feedback rather than trust, since the loop still has to record provenance and give an independent check the authority to reject what the agent reports.
...
category (@tbl-verification-authorities). Inside AI-assisted loops, these authorities are not generic quality checks; they
...
together they form the chapter's map (@tbl-trust-record-map). Two further sections, on proxy
mismatch and on security, confidentiality, and policy boundaries, defend the
evidence ledger and the trust checklist against gamed metrics and hidden
evidence.
...
| **Trust checklist** | Combines claim, feedback, fidelity, provenance, security and policy boundaries, rejection, and decision ownership. | Is the loop asking for a level of belief it has earned? |
...
The design-loop card is the summary and index for these views. Detailed
observations remain in the evidence ledger, and a replay receipt bundles the
card, ledger, inputs, runs, and decision records when the loop claims
replayability. Card conformance, evidence fidelity, and commitment strength are
separate claims; one does not advance automatically when another does.
...
learning is possible. A Bayesian optimizer[^fn-ml-bayesian-optimizer-budget], reinforcement-learning policy[^fn-rl-policy],
...
[^fn-rl-policy]: In reinforcement learning, a policy is a function that maps an agent's observed state to a chosen action. Treated here as a method inside the loop, it earns trust the same way any other generator does, through the evidence its actions leave and the gates that can reject them.
...
As used in @sec-architecture-environments-tool-interfaces, feedback regimes
classify checks by fidelity and cost. This chapter reuses the term for trust,
while keeping the commitment at stake as a separate decision property.
...
Structuring Notation[^fn-safety-gsn] so that each inference and the evidence under it are
explicit and reviewable. The Architecture 2.0 evidence ledger introduced in @sec-moonshot
borrows this discipline for design decisions that move across feedback regimes.
It is not necessarily a full assurance case, but it serves the same auditing
role for a design claim. It
records the candidate (*actions allowed*), the feedback and fidelity (*state
seen*), the provenance and uncertainty (*evidence*), what could block the claim
(*alternatives rejected*), and the required escalation boundary (*ownership*).
It is also a decision-rights record. The ledger should show which rejection
events, waivers, redactions, and human authorities allowed a candidate to move
forward, stop, or reopen later.
Naming the lineage helps, because that field already catalogs how such
arguments fail, through unstated assumptions, evidence that does not support the
claimed scope, and confidence
that outruns the proof.

[^fn-safety-gsn]: Goal Structuring Notation (GSN) is a graphical argumentation notation used in safety-critical engineering to document the elements of an assurance case, making each inference and its supporting evidence explicit and reviewable [@Kelly2004GSN]. The evidence ledger borrows this discipline so a design claim carries its support, assumptions, and rejected alternatives rather than a bare score.
...
rejection gate and evidence ledger entry at each stage (@fig-evidence-chain-rejection-gates).

![**Evidence ledgers connect feedback to decisions:** Staged checks record provenance and uncertainty while declaring what each stage may reject, revise, or escalate. Fidelity, cost, rejection authority, commitment, and allowed autonomy remain separate judgments; moving to a stronger check does not authorize a stronger commitment by itself.](images/F9-evidence-chain-rejection-gates){#fig-evidence-chain-rejection-gates width="100%" fig-alt="Staged evidence-ledger diagram showing feedback sources, provenance, uncertainty, declared rejection gates, revision, and escalation, with a note that rejection authority, commitment, and allowed autonomy are governed separately from evidence fidelity and cost."}

The figure changes the meaning of trust. Trust is not a model property or a
single score. It is a staged loop property.
...
the mechanism that pays that cost. The verification authorities in @tbl-verification-authorities are its operational catalog.
...
A compact way to remember *why* a candidate is rejected is the *four Ps of
rejection*:

1. **Proxy mismatches:** Rejection occurs because the system optimized a stand-in that did not track the declared objective.
2. **Protocol violations:** Rejection occurs because the candidate breaks the ISA, hardware/software contract, memory model, or another declared interface.
3. **Physical limits:** Rejection occurs because the candidate fails timing closure, violates power or area bounds, or fails a physical-design check.
4. **Policy boundaries:** Rejection occurs because the candidate violates security, privacy, license, disclosure, release, or other organizational constraints.
...
When a generative model or optimizer proposes a "winning" candidate, does the loop name at least one independent authority that can say no? Trusting an AI-assisted architecture loop requires showing that the generator is not the sole grader of its own work. The loop must expose a simulator, constraint checker, formal tool, benchmark rule, or expert review path with the authority to reject a candidate before treating it as an architectural commitment.
:::
...
the loop must record the disposition and supporting observation. An invalid
constraint, timing miss, benchmark violation, or expert objection can expose a
design-space boundary. A simulator crash or failed build is recorded first as
an environment failure and becomes a candidate rejection only when a declared
validity or constraint rule makes the candidate responsible for that outcome.
...
**The gap.** An independent 2023 reassessment found that proxy had weak correlation with the post-route chip-quality metrics that decide a real chip, and little useful correlation with timing [@ChengEtAl2023AssessmentRL]. A candidate could look excellent on the proxy the loop trusted and only ordinary once a full flow charged what the proxy ignored.

**The lesson.** For this chapter, the useful part of the dispute is proxy mismatch at full-flow scale. Trust is a staged property earned up a fidelity ladder, not a single score. An unverified proxy win is a reason to escalate, never a reason to commit. The broader dispute also involved pretraining, compute, implementation, and testcase choices, but the proxy question shows why evidence must mean what everyone assumes before the loop can ask for commitment.
:::
...
[^fn-ml-held-out-data]: Held-out data is a subset of a dataset reserved during training and used only to test whether a model generalizes to unseen inputs. Here it is one way a proxy earns calibrated trust, since a prediction validated against held-out cases carries more evidence than one fitted and reported on the same data.
...
they do not settle trust if the evidence cannot be audited by the relevant
...
rejection authority, the loop cannot trust the claim. The architect must
explicitly define which AI tools can access which data classes, how private
evidence can be verified, and who can authorize redaction, compliance review, or
release.
:::
...
Even after accuracy and policy boundaries are stated, trust still has to survive
...
baseline receipts, not just model weights.
...
: **Trust is a checklist, not a tone judgment:** A credible Architecture 2.0 claim states its task, feedback, fidelity, provenance, security or policy boundary, rejection authority, and commitment boundary before it asks the reader to believe the result. {#tbl-trust-checklist tbl-colwidths="[24,38,26]"}
...
The mechanisms of trust and verification presented in this chapter establish a
...
Fidelity ladders, evidence ledgers, commitment levels, and reversibility all
assume that evidence remains attached to the claim it supports. A fundamental
systems challenge is to design a ledger format and checker that bind proxy
traces, simulation runs, synthesis results, human waivers, rejection events,
...
The trust checklist in @sec-evidence-disputes-and-the-trust-checklist audits a
...
The report should state false commitment rates, escalation cost, and whether
human rejection authority remains effective when the loop's evidence sources
become unreliable.
...
no. A policy boundary keeps private or restricted evidence auditable. A
...
of this. Output supports a claim only after the declared acceptance criteria are
met with relevant, scoped, and traceable evidence. A gate merely declining to
reject is not sufficient. That is why the chapter treats verification as a
family of rejection authorities rather than a single tool, and why the fidelity
ladder and evidence ledger exist at all. They let a claim carry an honest account
of what it has shown and what it still cannot.

The durable move is to escalate with commitment. Cheap feedback screens, and
stronger feedback can support higher commitment, but only when the evidence
ledger names the gate, the uncertainty, and the authority that can still say no.
The loop should obtain the evidence appropriate to the decision's consequences
and stop at an honest commitment boundary, rather than dressing a proxy win as
silicon-grade proof. A result that cannot
...
- **Carry forward:** A claim is trustworthy only when you can name the evidence that would overturn it and the authority that can reject it.
