...
The importance of observing the relevant property is illustrated by verification failures like Meltdown, Spectre, and the Bulldozer cache-aliasing bug. These exposed that functional correctness does not establish security or system-level physical viability. In 2018, side channels were shown to leak across privilege boundaries through speculation and shared microarchitectural state [@KocherEtAl2019Spectre]. Ordinary functional checks did not catch these vulnerabilities because the performance objective measured instructions per cycle (IPC) and omitted information flow or timing channels. A check cannot reject a property it does not observe, demonstrating why public claims require independent physical and security checks. Passing a suite of performance tests provides zero evidence for security properties, leaving architectural flaws invisible if the evidence is structurally blind to them. Speculative side channels were later reduced through software and microcode mitigations at a performance cost. A functional defect already committed to silicon, such as the Bulldozer cache-aliasing bug, is harder to undo, since it can force a respin rather than a patch.
...
The AlphaChip dispute shows how this boundary holds when an internal result meets outside scrutiny.

::: {.callout-war-story title="A proxy under dispute"}
**The claim.** The 2021 AlphaChip paper reported that a reinforcement-learning method generated macro placements with quality comparable to or better than expert placements on reported metrics. Later reports noted its contribution to TPU generations [@MirhoseiniEtAl2021GraphPlacement; @GoldieEtAl2024AlphaChipAddendum; @GoogleDeepMind2024AlphaChip].

**The gap.** Independent assessments questioned proxy correlation, baselines, and reproducibility. The authors replied that the assessments did not reproduce the proprietary pretraining and production setup and used different cases and budgets [@ChengEtAl2023AssessmentRL; @ChengEtAl2026UpdatedAssessment; @GoldieEtAl2024ThatChipHasSailed].

**The lesson.** The public records test related placement claims under different cases and conditions. Bounded claims are supportable within each specific setup, but unmatched evaluations cannot be forced into a single, retroactive shared experiment.
:::
