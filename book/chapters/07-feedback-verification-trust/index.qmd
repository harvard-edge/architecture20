...
To measure the completeness of this statistical sampling, architects rely on *coverage models* (statement, branch, toggle, condition, FSM, and functional coverage). A coverage model quantifies what parts of the vast state space the testbench actually reached. Furthermore, AI-generated RTL is highly susceptible to X-propagation bugs (where uninitialized states simulate favorably as deterministic values but synthesize to unpredictable hardware) [@Liu2024RTLLM], making explicit X-optimism and X-pessimism EDA checks mandatory.
...
**The gap.** A missing entry in a programmable logic array (PLA) used for the radix-4 SRT division algorithm caused the processor to return incorrect results for specific floating-point division operations. Because the flaw only manifested on rare operand pairs, constrained-random simulation missed it. The bug was discovered post-silicon by a mathematician (Thomas Nicely) who noticed inconsistencies in twin prime calculations.

**The lesson.** Statistical sampling and dynamic simulation cannot guarantee the absence of bugs in vast state spaces. The ensuing $475 million recall catalyzed the industry's shift toward formal verification. Today, complex datapaths are mathematically proven correct using theorem provers and equivalence checkers rather than relying solely on simulation coverage, proving that formal oracles are necessary to establish ground truth.
:::
...
This leads to the ML-Sys challenge of **Reward Shaping**. Simply relying on physical oracles is insufficient for training, because they are too slow. Developers must carefully shape reward functions to gently penalize PPA constraint violations without causing the model's learning process to collapse. The fundamental limit is that finding a continuous reward function that perfectly aligns with discontinuous physical constraints (like a strict 3W thermal envelope or a boolean DRC violation) is mathematically intractable. This is especially true when an agent trades one physical metric for another—such as adhering to a peak Power constraint by stretching out execution time, inadvertently inflating total Energy (Joules) consumed and destroying the Energy-Delay Product (EDP).
