...
The mechanisms of trust and verification presented in this chapter establish a foundation, but robust automation still needs concrete artifacts. The questions here ask how evidence is bound, how proxy exploitation is detected, and how the trust machinery itself can be stress-tested.

1. **How should multi-fidelity evidence ledgers be bound to claims?**
Fidelity ladders and evidence ledgers (@sec-fidelity-ladders-and-evidence-ledgers), commitment levels, and reversibility (@sec-commitment-levels-and-reversibility) all assume that evidence remains attached to the claim it supports. A fundamental systems challenge is to design a ledger format and checker that bind proxy traces, simulation runs, synthesis results, and human waivers into a unified audit object. A strong paper would evaluate tamper detection, replayability, and whether the checker prevents a candidate from exceeding the commitment level supported by its evidence.

2. **Can proxy exploitation be detected before it becomes a commitment?**
Proxy mismatch, metric gaming, and calibration failures (@sec-proxy-mismatch-metric-gaming-and-calibration) become more severe when generators learn to exploit cheap architectural models. A thesis-level contribution would map the epistemic uncertainty[^fn-stats-epistemic-uncertainty] of surrogates across high-dimensional design spaces and trigger stronger checks, such as RTL synthesis, when trust bounds are violated. The evaluation should include adversarial proxy wins and report how often a compound AI system[^fn-ml-compound-ai-system] escalates before accepting a misleading candidate.

3. **How do we stress-test the trust machinery itself?**
The trust checklist for evidence disputes (@sec-evidence-disputes-and-the-trust-checklist) audits a single claim, but the field also needs evaluation methodology for whole closed-loop systems. A strong research artifact would be an adversarial environment that injects hidden workload drift, compromised baselines, microarchitectural side channels, and deceptive proxies into the design process. The paper should report false commitment rates, escalation cost, and whether human rejection authority remains effective when the loop is intentionally pressured.

[^fn-stats-epistemic-uncertainty]: Epistemic uncertainty is uncertainty in a model's predictions that comes from a lack of training data in that region, and it shrinks as more data is gathered. Mapping it across the design space is what tells the loop when a surrogate has left its calibrated support and a stronger check should be triggered rather than trusted.

[^fn-ml-compound-ai-system]: A compound AI system is composed of multiple interacting AI models, tools, and logical components working together on a complex task. Whether such a system escalates before accepting a misleading candidate is exactly the trust property this chapter asks the loop to demonstrate.
