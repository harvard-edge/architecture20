...
In the previous chapters, we constructed the first three pillars of the autonomous design loop: **The State** (@sec-data-representations-world-models), **The Policy** (@sec-methods-generation-prediction-optimization), and **The Environment** (@sec-architecture-environments-tool-interfaces). When an agent executes an action within that environment, it produces a raw result. This chapter focuses on the final pillar: **The Reward**—the process of converting that raw result into verified feedback, bounded claims, and architectural trust.

This core dilemma separates gathering data from validating an architectural decision. A decision requires evidence, not merely feedback, so a tool return must be typed and bound to a specific claim. The bottleneck is proxy mismatch and correlated error, where fast surrogate metrics diverge from physical reality. AI sharpens that bottleneck by generating plausible but invalid results faster, which demands independent oracles[^fn-independent-oracles-c07] rather than consensus among correlated models. Trusting an AI's stated intent in place of that evidence is the trap, and the defenses run from independent physical oracles up to formal equivalence checks.

[^fn-independent-oracles-c07]: **Test oracle**: In software testing, the trusted mechanism that decides whether an output is correct. In hardware flows the role falls to physics-grounded EDA tools, for example SymbiYosys proving safety properties formally or Synopsys Formality [@Formality] confirming the synthesized netlist matches the RTL.
...
Consequently, architects must structurally separate the AI's *intent* (the natural language rationale, prompt, or ungrounded objective function) from its *implementation* (the emitted RTL, script, or layout). The rationale is not the evidence. An agent might output a well-reasoned explanation for why its proposed cache hierarchy avoids deadlocks, but that probabilistic rationale carries no formal verification weight. Epistemic trust requires passing the generated artifact across the boundary into deterministic, physics-bound solvers that evaluate the structure of the implementation independently of how or why the AI generated it.

The architect enforces this separation through Equivalence Checking (EC) and Bounded Property Checking (BPC). Bounded Property Checking transforms high-level architectural constraints, such as a guarantee that a cache controller never deadlocks under a specific coherency race, into mathematically exhaustive proofs over a bounded number of clock cycles using SystemVerilog Assertions (SVA) and SAT solvers. BPC searches for edge cases in the AI's logic that heuristic simulation misses.

Once the RTL is proven, Logical Equivalence Checking (LEC) ensures the elaborated Boolean logic perfectly matches the downstream synthesized netlist. This guarantees that optimizations, whether proposed by the AI during structural generation or by the EDA tool during synthesis, do not mutate the golden semantic behavior. Without EC and BPC, the architect is trusting that a probabilistic text generator understood silicon concurrency, risking physical bugs that surface only after tapeout.

### Dynamic Simulation and Coverage

While formal methods (BPC/EC) prove specific assertions, full-system functional validation requires robust dynamic simulation environments. Generating constrained-random stimuli in a *UVM (Universal Verification Methodology)* testbench forces the AI-generated RTL through corner cases it never explicitly trained on. To measure the completeness of these tests, architects rely on rigorous *coverage models* (such as statement, branch, toggle, and functional coverage). A coverage model quantifies exactly what parts of the design space the testbench actually reached. If an AI generates both the RTL and the testbench, the coverage model provides the independent metric proving the testbench didn't just vacuously pass by avoiding complex state transitions. Furthermore, AI-generated RTL is highly susceptible to X-propagation bugs (where uninitialized states simulate favorably as deterministic 0s or 1s but synthesize to unpredictable hardware) requiring explicit X-optimism and X-pessimism EDA checks.
...
While fast deterministic checks can filter out malformed candidates, generative AI also introduces structural security risks that standard functional checks easily miss, such as the hallucinated vulnerability and the maliciously injected hardware trojan. In traditional architecture, a backdoor required a rogue engineer or compromised EDA tool to intentionally alter the netlist. In AI-driven architecture, a generator trained on unvetted, scraped open-source RTL can inadvertently memorize and reproduce buried trojans. An adversary could also poison the foundation model's training data, teaching the agent to subtly weaken cryptographic isolation boundaries or insert data-leaking side-channels under specific, rare workload triggers.

Because these malicious structures, such as a finite state machine that bypasses a privilege check after a specific sequence of branch mispredicts, are structurally valid and functionally silent during standard regressions, they can evade traditional code coverage metrics. To defend against these poisoned models, the architect must orchestrate verification beyond checking that the generated RTL meets the design specification, employing formal path-sensitization and information flow tracking (IFT) to prove the absence of undocumented state transitions and data-exfiltration paths.
