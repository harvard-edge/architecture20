...
A synthesis report says that an AI-proposed wider issue queue misses the declared clock target. What exactly has happened, what does the report establish, what should be recorded, and which action, if any, should follow?
...
## 1. From Feedback to Observation, Evidence, and Decision {#sec-from-feedback-to-decision}
...
Returning to the issue queue, the synthesis report is the observation. The evidence relation connects that report to the claim about clock speed. The review result states the claim is unsupported. The knowledge-update branch decides to store the failure in the project trajectory, while the architecture decision stops the transition to RTL.
...
## 2. Evidence Is Specific to the Claim and Scope {#sec-evidence-specific-to-claim-and-scope}

Evidence has no useful strength independent of a claim. An observation bears on a property for a candidate relative to a comparator under a stated workload, design state, tool path, metric, assumptions, validity region, uncertainty, and proposed action. A cycle estimate can support a bounded performance comparison while remaining irrelevant to power, timing, isolation, software usability, or product readiness.
...
For the issue queue, a structural check can reject an invalid parameterization. A regression suite can support functional behavior for covered cases. Workload experiments can support a bounded performance comparison. A synthesis report can address timing and area under its constraints. None of those checks silently establishes the other properties.
...
## 3. Proxy Mismatch, Calibration, Confounding, and Nondeterminism {#sec-proxy-mismatch-metric-gaming-and-calibration}
...
For the issue queue, an early analytic or learned estimate might predict performance improvements for a wider queue. However, synthesis under declared constraints might reverse the apparent preference due to severe timing degradation. Or, if the performance check omits thermal considerations, moving a compute block might create a thermal hotspot.
...
## 4. Independent Checks and Correlated Errors {#sec-independent-checks-and-correlated-errors}
...
## 5. Protected Evidence and Public Claims {#sec-protected-evidence-and-public-claims}
...
## 6. Evidence Cost and Consequence {#sec-evidence-cost-and-consequence}
...
Returning to the issue-queue prototype, a cheap structural check rejects illegal parameters quickly. Simulation determines if a performance hypothesis deserves scarce synthesis time. A synthesis flow exposes a timing failure. Security review might be more valuable than an additional cycle estimate if it reveals a shared-state flaw. Each check has a different cost and a different decision job.

## 7. Disputes, Waivers, and Reopening {#sec-disputes-waivers-and-reopening}
...
A waiver does not turn a failed check into a pass. It accepts a bounded exception and residual risk for a named action and period. For the issue-queue timing miss, accepting a clock shortfall for a local prototype does not establish timing closure. A waiver requires stating the exact scope, compensating check, expiration, and residual uncertainty.

Reopening is required when new evidence or changed conditions make the old evidence bear on a different claim. For the issue-queue, a revised target, stronger flow, or shared-interface transition reopens the review. (@sec-loop-patterns-across-stack discusses reopening caused by software evolution, workload drift, and deployment telemetry.)
...
A returned signal counts as feedback. A declared tool path may turn it into a typed observation. The observation becomes evidence only through a bounded claim, comparator, scope, validity region, provenance, and uncertainty. A review records whether the claim is supported, unsupported, or unresolved. Separate decisions determine whether the result updates project or model knowledge and whether the architecture advances.

Proxy mismatch, calibration error, confounding, nondeterminism, and correlated checks limit what apparently favorable results establish. Protected evidence may support an internal decision when authorized review remains possible, but public claims must stay within the public or audited record. Evidence cost limits available checks without determining their strength. A waiver records a bounded accepted failure rather than a pass, and changed evidence or conditions reopen the claim.
