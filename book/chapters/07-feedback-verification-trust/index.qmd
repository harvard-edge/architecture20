...
Each question below grows out of a verification difficulty this chapter examined. Some ask what an AI system must observe, detect, or leave to a human before a returned result counts as evidence. Others address correlated error, protected evidence, and the separate decision to fold a result into project knowledge. In every case, the comparison is with current verification practice, and success is judged by the effect on architecture decisions rather than the model's output alone.

1. **How can AI tell when a stored result no longer bears on the claim it once supported?** This chapter bound every observation to a specific comparator, workload, design state, tool path, validity region, and proposed action, and it showed the 3W RISC-V XR subsystem's timing report supporting the clock claim only under one named flow and corner. When the workload, compiler, floorplan, or target moves, the same number can silently attach to a different claim, which is why the chapter requires changed conditions to reopen the review. The open problem is whether an AI system can track the claim frame and flag when a new observation or a shift in design state has carried a result outside the region where it was evidence, instead of forwarding a stale pass. Manual reopening triggers and expert judgment about what invalidates a prior result are today's practice, and success is measured by stale reuses caught before a transition, valid results needlessly reopened, and whether the reopened claims are the ones a later physical check would have overturned.
2. **How can AI predict where a surrogate metric will be gamed before the expensive check reveals it?** The trust-calibration curve in this chapter showed a proxy tracking physical ground truth up to a calibration boundary and then diverging, while the optimizer keeps climbing a surrogate that no longer models timing, area, or power. The chapter tied this to Goodhart's law and to a search that surfaces the best of thousands of candidates on a noisy proxy, so the winner's proxy value is biased upward. The open problem is whether an AI system can locate the calibration boundary for a given proxy and design region and name the omitted cost a strong optimizer will exploit, such as the thermal hotspot that would violate the 3W limit, before scarce synthesis or place-and-route time confirms it. Held-out validation, sensitivity studies around the candidate, and expert intuition about where a fast model breaks are the current defenses, and success is measured by reward-hacking cases caught before the ground-truth oracle, high-fidelity runs saved, and whether the flagged candidates' physical results collapse as predicted.
3. **How can AI find the property that no current check observes?** The chapter's most dangerous failure was not a check that agreed wrongly but a property no check watched at all, illustrated by processors that passed every functional and performance suite while nothing observed the cross-privilege timing channel behind Meltdown and Spectre. Because no assembled check covered that property, more independent functional checking could not have rejected the flaw. The open problem is whether an AI system can enumerate the properties a claim depends on, compare them against what the assembled checks actually observe, and name the missing oracle before sign-off, rather than reporting a green suite as demonstrated coverage. Expert threat modeling, coverage metrics, and mutation testing that injects faults to confirm a suite catches them are the baseline, each limited to failure classes someone already encoded, and success is measured by decision-relevant unobserved properties surfaced before a transition, costly false alarms raised, and whether the newly proposed checks reject candidates the prior suite passed.
4. **How can AI measure whether two checks fail independently rather than sharing a blind spot?** This chapter argued that independence is relative to a claim and a failure class, that a generator, critic, and verifier sharing a foundation model can agree while sharing one correlated error, and that separate prompts or agent names create no independence. It drew on the safety-engineering finding that independently developed program versions still failed together on the same hard inputs far more often than an independence assumption predicts. The open problem is whether an AI system can estimate how much shared model, training data, objective, workload, or tool path a set of checks holds in common, and whether a proposed additional check is a genuine modality shift, such as a formal SAT solver over the same RTL, or only a restatement of the generator's assumptions. N-version diversity, manual review of what tools share, and hand-added physical oracles are the baseline, and success is measured by whether checks it certifies as independent actually catch each other's escapes on held-out failures and by the reduction in correlated passes a later physical audit overturns.
5. **How can AI decide when a nondeterministic result is stable enough to support a bounded decision?** The chapter separated nondeterminism from proxy mismatch and showed an ML-driven place-and-route tool returning a different floorplan under a new seed, so a single deterministic run is fragile evidence, and it showed that selecting the best of thousands of candidates biases the winner's proxy value upward so the choice must be re-measured on fresh held-out seeds and workloads. The open problem is whether an AI system can decide how many repetitions, seeds, and held-out workloads a given decision margin requires, and spend that scarce re-measurement on the candidates whose ranking is least stable, rather than reporting a mean without dispersion. Fixed repetition counts, recorded seeds, and expert judgment about when a margin is safe are the current practice, and success is measured by how often a re-measured margin holds under independent seeds, tool time spent per resolved decision, and lucky winners caught before they enter an implementation.
6. **How can AI keep a public architecture claim within what an outside reviewer can inspect while a stronger internal result stays protected?** The AlphaChip dispute in this chapter showed a placement result whose strongest evidence depended on a proprietary pretraining and production setup that public cases could not match, so independent assessments and the authors' response could not be collapsed into one shared experiment. The chapter's rule is that a public claim must stay within what the public record or a credible protected audit can establish, and that missing public information is not proof the internal evidence was absent. The open problem is whether an AI system, working from restricted workloads and flows it is cleared to touch, can construct a public claim with explicit nonclaims that an outside reviewer can actually check, along with the redaction limits and the authorized-audit path, without leaking protected data or overstating a redacted result. Manual protected-evidence framing, end-to-end public benchmarks that score final metrics, and human disclosure review are the baseline, and success is measured by whether its public claims survive independent reproduction on public cases, whether its stated nonclaims hold, and whether any protected data leaks.
7. **How can AI decide which results should update project or model knowledge and which should not?** The six-part branch in this chapter separated reviewing a claim from two independent choices, the knowledge-update branch and the architecture-action branch, and warned against treating every result as training data or a parameter change. It showed a failed 3W RISC-V XR timing candidate that could be stored in the project trajectory or a timing-surrogate calibration set but should not be automatically fine-tuned into a model. The open problem is whether an AI system can screen an observation before it reaches a shared knowledge surface, admitting results whose scope and provenance make them safe to generalize while rejecting gamed, confounded, or correlated evidence that would poison a downstream predictor or world model. Human-curated dataset construction and logging every run for later training without a scope check are the current baseline, and success is measured by whether models updated only from screened evidence make better-calibrated architecture predictions than models trained on all feedback, and by contamination caught before it degrades a later decision.
