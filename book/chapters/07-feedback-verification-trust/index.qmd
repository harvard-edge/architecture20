...
or disclosure boundaries? If a generative method produces a claim using
confidential or restricted data that cannot be exposed to the rejection
authority, the loop cannot trust the claim. The architect must explicitly define
which AI tools can access which data classes, how private evidence can be
verified, and who can authorize redaction or release.
:::
...
information-flow side channels. A design that trims DRAM refresh to meet a power
budget can open a RowHammer path, so read-disturbance is a rejection gate in its
own right.
...
**Lesson:** Spectre and Meltdown are a reminder that functional correctness is not the same as security. If non-interference and timing-channel evidence sit outside the loop, performance wins can accumulate risk that ordinary correctness tests never see.
:::
...
Even after accuracy and confidentiality boundaries are stated, trust still has to survive
...
| What is the security, confidentiality, or policy boundary? | Data classes, access rules, redaction limits, license or disclosure class, public proxy limits, authorized reviewer role, and an audit path. | Private or restricted evidence is treated as invisible or unverifiable. |
