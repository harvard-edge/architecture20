...
An extreme request has a way of becoming routine. Provisioning a datacenter once
meant a capital project and a year of lead time, and today it is an API call.
...
[^fn-logistics-amazon]: The pattern is not unique to computing. Ordering a book
once meant a trip to a store, and now a single click sets off an orchestrated
chain of inventory, robotics, packing, routing, and delivery that no one in the
bookstore era would have believed. What made it ordinary was engineering every
step to be reliable and composable, the same move this book asks of the design
loop.
...
question uses the shift from artifact design to loop design in
@sec-from-architecture-10-to-architecture-20.
...
framing in @sec-moonshot makes this a community problem, not a matter of
...
prompt in @sec-why-the-prompt-spans-the-stack and the efficiency claims in
@sec-efficiency-claims-need-rejectable-loops show where this record would
matter.
...
field-level lesson of the moonshot examples and the AlphaChip war story in
@sec-moonshot.
