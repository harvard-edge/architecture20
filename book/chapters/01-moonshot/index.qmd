...
These open research questions set the field-level agenda for the rest of the book. They are intentionally broad, but each one should still be readable as the seed of a serious paper, thesis, benchmark, or shared research artifact. Subsequent chapters make the loop state, environment, methods, evidence, and ownership obligations more concrete.

1. **What provenance record is sufficient for stochastic architecture generators?**
AI-generated architecture claims may depend on prompts, model versions, tool versions, random seeds, private workloads, and later rejection decisions. A thesis-level contribution would define the minimal provenance packet that lets another architect replay, weaken, or reject a generated claim even when the original model is non-deterministic or proprietary. The paper object could be a provenance schema plus a replay benchmark that measures whether independent reviewers can reconstruct why a candidate was accepted or rejected (see the discussion on "Why the Prompt Spans the Stack" in @sec-why-the-prompt-spans-the-stack).

2. **How should the field red-team autonomous hardware generation loops?**
Red-teaming[^fn-security-red-teaming] should test the loop's rejection machinery, not merely the surface quality of generated designs. A strong research contribution would build adversarial workloads, interface constraints, and physical-design traps that induce plausible but invalid candidates across the hardware-software stack. The evaluation would report false-accept rates, escalation cost, and the kinds of evidence that successfully stopped the loop before an unsupported commitment (see the discussion on "Efficiency Claims Need Rejectable Loops" in @sec-efficiency-claims-need-rejectable-loops).

3. **What is the right currency for multi-fidelity evidence budgets?**
Architecture agents will spend a mixture of cheap proxy evaluations, expensive simulation runs, synthesis checks, and human review time. A top-tier paper here would define an information-theoretic or decision-theoretic unit of evidence cost that can compare those expenditures without pretending they have the same fidelity. The result should be testable through a controller or benchmark that improves the quality of accepted claims under a fixed simulator-hour, latency, or review budget (see the discussion on "Architecture Development Spans Three Roles" in @sec-architecture-development-spans-three-roles).

4. **Can rejection reasons become reusable architecture knowledge?**
Rejected candidates are often the clearest record of the design space boundary, but each tool currently emits failures in its own language. A concrete research artifact would be a machine-readable taxonomy of simulator, compiler, and physical-design rejection reasons, paired with translation tools that convert raw failures into reusable negative evidence. The evaluation should show that a later loop avoids repeated invalid regions, explains why a candidate failed, or transfers a rejection lesson across related workloads or tools (see the discussion on "From Architecture 1.0 to Architecture 2.0" in @sec-from-architecture-10-to-architecture-20).

[^fn-security-red-teaming]: Red-teaming in AI means proactively probing a system to surface vulnerabilities, biases, or unexpected failure modes before deployment. Here the target is the loop's rejection machinery, so a red team succeeds when it induces plausible but invalid candidates the loop should have rejected, not when it merely dents the surface quality of a generated design.
