...
::: {.callout-war-story}
**The claim.** In 2021 a Google team reported a reinforcement-learning method that generates a chip's macro placement in under six hours, at quality comparable to or better than human experts on key power, performance, and area metrics [@MirhoseiniEtAl2021GraphPlacement]. Named AlphaChip in a 2024 Nature addendum [@GoldieEtAl2024AlphaChipAddendum], and, Google later said, used across several TPU generations [@GoogleDeepMind2024AlphaChip], it is one of the most influential demonstrations that AI can do real architectural work.
...
**The response.** The original authors argued the critics did not run the method as published. They skipped the pre-training that is central to the method, did not train to convergence, used far less compute, and tested on designs the authors considered unrepresentative [@GoldieEtAl2024ThatChipHasSailed].

**Why it stays open.** The proprietary designs used for pre-training were never released, for a legitimate reason. They are Google's own chips. Google later released code and a pre-trained checkpoint, but not those training inputs, so the strongest form of the claim cannot be independently reproduced in public. In September 2024 Nature removed an earlier editor's note, said the concerns were resolved to its satisfaction, and published a clarifying addendum. That closed the journal's process. It did not produce an independent public reproduction of the strongest deployment and pre-training claims.
