...
Just like the mobile XR prompt, this datacenter prompt forces an AI-assisted loop to carry specific architectural state across memory capacity and bandwidth, the tensor-compiler software contract, deadlock-free routing and virtual channels, and cloud multi-tenancy overheads. Treating ecosystem compatibility as a first-class rejection condition keeps the loop tied to a deployable datacenter platform, not only to a topology that is attractive under an abstract network objective. The evidence ledger for this prompt must record not just the winning topology, but why alternative routing schemes or fault-tolerance mechanisms failed under scale.
...
**Why it stays open.** The proprietary designs used for pretraining were never released, for a legitimate reason. They are Google's own chips. Google later released code and a pretrained checkpoint, but not those training inputs, so the strongest form of the claim cannot be independently reproduced in public. In September 2024 Nature removed an earlier editor's note, said the concerns were resolved to its satisfaction, and published a clarifying addendum. That closed the journal's process. It did not produce an independent public reproduction of the strongest deployment and pretraining claims.

**Status (2026).** Contested. Reported as deployed and apparently usable, but with no public, independent, apples-to-apples benchmark that settles whether it beats the strong classical baselines. This book takes no side on who is right. The point is that the field had no shared way to release, replay, and adjudicate the claim, and closing that gap is what Architecture 2.0 is for.
:::
