...
This chapter opens those fields: what state the loop records, what consequences
...
ImageNet is the useful foil. Its value was not that images happened to exist on
the web. The challenge made a large public image collection comparable through
...
**The gap.** The designs the method learned from were proprietary, and while some code was later released, the pretraining inputs needed to rerun the strongest version of the claim were not [@ChengEtAl2023MacroPlacement; @Markov2023FalseDawn]. The authors answered that the critics had not run the method as described [@GoldieEtAl2024ThatChipHasSailed]. Whatever placement priors or transferable representation the method learned from those chips, its coverage, assumptions, and blind spots cannot be inspected, so a reviewer acting in good faith cannot audit the data behind the claim.
