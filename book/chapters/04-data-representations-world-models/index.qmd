...
For an architect, this separation of timescales is a fundamental safety property, not just a machine learning implementation detail. If an AI attempted to learn daily project updates the same way it learns basic physics, the compute cost would be prohibitive. Instead, knowledge is stratified.
...
By stratifying knowledge into these distinct timescales, the AI's generalized statistical reasoning is strictly tethered to the physical reality of the current project state.
...
A common flaw in many early predictive surrogates is that they fall into the correlative trap. A black-box surrogate might learn a correlation (for example, that higher chip area often equals higher power) through curve-fitting on its training data, entirely without understanding the underlying physics or capacitance equations.
...
Only causal representations allow the AI to safely generalize to novel, un-simulated architectures.[^fn-arch-3-0] Correlative models tend to collapse outside their training distribution when the optimizer pushes them into Out-of-Distribution (OOD) spaces, generating confident but physically impossible predictions.
...
A learned cost model performs this estimation job. Given the current constrained design state and a proposed mutation, it emits a predicted consequence (such as cycle count or power). This prediction is *not* a tool observation. It is an estimate fraught with epistemic and aleatoric uncertainty.[^fn-uncertainty-types]
...
A later simulator execution tests the prediction and returns a high-fidelity observation. Detecting when a candidate has left the calibrated region is the job of uncertainty quantification, through deep ensembles, Gaussian-process predictive variance, or conformal prediction [@AngelopoulosBates2021Conformal]. The execution history must keep the AI's prediction and the simulator's observation distinct to catch these OOD failures. Predicting one consequence reliably is already hard. Predicting a next design state faithfully enough to plan several moves ahead remains largely out of reach, so this chapter treats it as a target rather than a tool.
