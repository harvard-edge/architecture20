...
If an AI method is tasked with optimizing against a Lighthouse comparison, the representation might declare that the SRAM capacity and MAC array dimensions are mutable. The peak power limit and the target workload mix are fixed constraints. If these boundaries are not explicit, an AI method might propose a candidate that achieves high performance by silently violating a power constraint or altering the workload mix, invalidating the comparison. The state space is astronomically large. A simple processor has more reachable configurations than there are atoms in the observable universe. Without explicit constraints on the search, an optimizer wanders through useless regions of that space.
...
An architecture world model performs a transition job. Given the current constrained design state and a proposed mutation, it emits a predicted consequence (such as cycle count or power). This prediction is *not* a tool observation. It is an estimate fraught with epistemic uncertainty (uncertainty due to a lack of knowledge) and aleatoric uncertainty (inherent system noise). When an optimizer pushes these models into untested design spaces, they suffer from Out-of-Distribution (OOD) failures, making confident but wrong predictions. A later simulator execution tests the prediction and returns a high-fidelity observation. The execution history must keep the AI's prediction and the simulator's observation distinct to catch these OOD failures.
...
Private data was not the only requirement. The results also depended on how the placement problem was presented to the agent.

::: {.callout-war-story title="AlphaChip's Explicit Graph Representation"}
**The claim.** When Google developed AlphaChip for TPU macro placement [@MirhoseiniEtAl2021GraphPlacement], the reinforcement learning agent could not output text or $(x,y)$ coordinates in a vacuum. It needed a machine-readable view of physical reality.

**The gap.** Generating numbers without structural context ignores the physical adjacency rules that hardware design requires.

**The lesson.** The project represented the chip as a graph, with macros and standard cells as nodes and the connecting wires as edges. That graph, combined with a grid-based canvas, forced the agent to operate within physical rules. Without this translation from RTL to a structural graph, the agent would have had no legal action space to explore.
:::
...
2. **Fast Surrogate Models:** Can an AI method screen candidates with a fast predictor of cycle-level performance and power from abstract structural descriptions, and can it detect when a candidate has moved outside the region where that predictor was calibrated?
...
An architecture world model maps the represented state and a semantic action to a bounded predicted consequence. That prediction is an estimate, distinct from the high-fidelity tool observation that later tests it. Keeping observations, design failures, and decisions as separate, dated records is what lets the next change start from current state rather than a stale one, and lets a reviewer tell a prediction from ground truth.
