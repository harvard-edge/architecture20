...
Every team has reported a strong result and then failed to reproduce it six
months later because a simulator default shifted or a script was lost on an old
laptop. The fix is not human heroics; it is enforcing tool and environment
provenance as the run happens, so the loop leaves a replayable receipt rather
than an anecdote.
:::
