...
be neither confirmed nor rejected. The lesson is not who is right; it is that a
result no one can rerun leaves the loop with an anecdote instead of evidence. The
fix is not human heroics; it is enforcing tool and environment provenance as the
run happens, so the loop leaves a replayable receipt rather than a dispute.
:::
