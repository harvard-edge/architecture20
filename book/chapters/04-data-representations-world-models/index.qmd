...
@sec-architecture-20-ontology introduced the 6-stage Architecture Design Loop. In this chapter, we focus on the machinery that powers the *Explore* and *Evaluate* stages. When an AI agent generates a candidate in the *Explore* stage, it relies on a **Representation**—a formal, explicit encoding of architectural knowledge and project state. When it evaluates that candidate in the *Evaluate* stage, it queries a **Cost Model** (a predictive proxy).
...
Real systems architecture is dominated by data movement, not isolated compute blocks. An AI's representation must structurally encode system-level topologies like Network-on-Chip (NoC), memory hierarchies, and bandwidth constraints. However, NoCs are fundamentally different from single-block dataflow; they involve distributed state, flow control, and dynamic protocol abstractions (e.g., virtual channels) that require complex state-machine encodings to avoid deadlock. Furthermore, an AI must understand the "meta-graph" of Clock Domain Crossing (CDC)[^fn-cdc] and reset trees. An AI that mutates a pipeline without explicitly representing asynchronous boundaries will confidently generate untraceable metastability failures. Finally, generating RTL without a testbench is useless. The AI must also represent verification intent and coverage models (such as Universal Verification Methodology, or UVM). @sec-feedback-verification-trust deeply covers the mechanics of verification and testbench representations.

[^fn-cdc]: **Clock Domain Crossing (CDC)**: When signals pass between logic blocks operating on different clock frequencies. Without proper synchronization logic (like FIFOs or double-flops), the destination registers can become metastable, settling on unpredictable voltage levels and corrupting data.

The representation problem extends beyond logic to physical realization. Power and timing bounds are codified in side-channel constraints like UPF (Unified Power Format) and SDC (Synopsys Design Constraints). These constraints govern power domains and timing boundaries alongside the RTL. For AI models, they are uniquely hostile because of *topological coupling*. SDC and UPF rely on exact hierarchical paths and pin names (specific string references to internal RTL signals). Any structural mutation by the AI—such as flattening a hierarchy or renaming a buffer to improve performance—requires a synchronized co-mutation of the constraint files, otherwise the EDA tool will instantly fail with "path not found" errors. An agent must co-generate these formats to operate within a realistic physical envelope, transforming an abstract concept into a manufacturable netlist (@tbl-representation-formats).
...
@tbl-negative-traces gives examples of negative knowledge, including manual waivers for physical constraints like Static Timing Analysis (STA)[^fn-sta].

[^fn-sta]: **Static Timing Analysis (STA)**: A method of computing the expected timing of a digital circuit without requiring simulation, ensuring that data arrives at registers within the required setup and hold windows.
...
For example, a cost model fit to earlier cycle-accurate results maps a represented workload trace and a proposed pipeline configuration to a predicted performance metric far faster than rerunning the simulator. @tbl-executed-study-world-model shows exactly what such a predictor consumes and what its predictions leave out.
...
This is fundamentally different from a true *World Model*. In artificial intelligence, a world model predicts the *next state* of an environment, allowing an agent to plan several steps ahead without querying a simulator. Because hardware environments are deterministic once a mutation is chosen, true architecture world models are rarely needed. Instead, the field relies on highly accurate, multi-fidelity cost models. We emphasize this distinction to set baseline expectations. The distinction between generative models (producing candidate designs) and predictive models (estimating PPA) must be crystal clear. Current agents do not hallucinate entire physical worlds; they search structural representations guided by learned cost proxies, with physical synthesis (PPA) serving as the ultimate arbiter for these predictive models.
