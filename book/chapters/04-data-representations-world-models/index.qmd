...
[^fn-ml-benchmarks-squad-glue]: SQuAD and GLUE are standard benchmark datasets used to evaluate natural language processing models, where SQuAD is the Stanford Question Answering Dataset [@RajpurkarEtAl2016SQuAD] and GLUE is the General Language Understanding Evaluation [@WangEtAl2018GLUE]. They show how a shared, scoreable object can move a field, and the chapter's point is that an architecture evaluation object must record far more than a labeled row before an optimizer can safely act on it.
...
[^fn-systems-negative-results-venues]: The wider computing-systems community has begun building places to publish exactly these records, such as the NOPE workshop (Negative results, Opportunities, Perspectives, and Experiences) held with ASPLOS. The field needs more of them so that failed runs, rejected candidates, and the assumptions behind them accumulate as shared evidence instead of being discarded before publication.
...
[^fn-stats-generalization-bounds]: Theoretical limits on how well a model's performance on its training data will carry over to unseen, out-of-distribution data. The chapter wants such bounds for architecture agents so a reviewer can tell when a claimed design-space lesson transfers to a new workload and when it should be rejected.
