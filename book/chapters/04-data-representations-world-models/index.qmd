...
@sec-architecture-20-ontology introduced the 6-stage Architecture Design Loop and the four pillars of an autonomous design agent. In this chapter, we focus on the first pillar: **The State**. When an AI agent generates a candidate in the *Explore stage*, it relies on a **Representation**—a formal, explicit encoding of architectural knowledge and project state that tells the agent what it can see and change. When it evaluates that candidate in the *Evaluate stage*, it queries a **World Model** (a predictive proxy).

To do this safely, a representation must do more than store text or an embedding. It has to capture structural constraints, Register Transfer Level (RTL) boundaries, configuration parameters, and fixed physical limits. By enforcing "safe-by-construction" intermediate representations, we tame the *Curse of the Action Space* introduced in the previous chapter, preventing the agent from exploring structurally impossible designs.
...
- Distinguish general parametric architectural priors from explicit, versioned project constraints to establish legal design action spaces.
- Evaluate the trade-offs along the multi-fidelity simulation pyramid to optimize training sample acquisition and leverage negative execution traces for cost-model calibration.
- Formulate structural hardware encodings and intermediate representations to preserve topological invariants and physical connectivity during automated design state mutations.
- Decouple fast predictive surrogate outputs from authoritative physical synthesis observations within versioned execution histories to prevent knowledge drift and out-of-distribution inference failures.
...
### Design History and Episodic Memory

When a human architect tries a cache coherence protocol and it deadlocks, they remember it. For an AI architect, this requires Retrieval-Augmented Generation (RAG) over past simulation logs, failed RTL patches, and git commit history. This grounds the ML concept of continuous learning in the practical need to retain architectural state and prevent repeating the same costly simulator errors across design iterations. It functions as the agent's episodic memory.

::: {.callout-war-story title="The Cost of Forgetting a Deadlock"}
A notoriously difficult cache coherence deadlock in a legacy processor required a costly microcode patch post-silicon to resolve. The architectural root cause had actually been encountered and dismissed as a testbench artifact months earlier during simulation. Retaining *failed* execution traces as episodic memory is orders of magnitude cheaper than re-discovering those same deadlocks in physical silicon.
:::
...
![**The architectural data fidelity pyramid mirrors a memory hierarchy.** High-fidelity data is expensive and scarce, while low-fidelity data is cheap and abundant.](images/F4-data-pyramid){#fig-data-fidelity-pyramid fig-alt="The architectural data fidelity pyramid drawn as tiers. High-fidelity data at the apex is expensive and scarce, while low-fidelity data at the base is cheap and abundant." width="100%"}
...
To bridge generalized statistical reasoning and the physical reality of a project, an AI system uses different objects to store different kinds of knowledge. Because this information appears in various forms, it is imprecise to collapse them all into the single term *representation*. @tbl-knowledge-objects compares what these objects preserve, what they lose, and what authority they have in an architecture study.
...
To bridge this gap, agents increasingly rely on Intermediate Representations (IRs) like CIRCT [@CIRCT] and FIRRTL [@Izraelevitz2017FIRRTL]. These IRs shift the verification burden from slow, late-stage simulation to fast, upfront compilation. Instead of waiting hours for an EDA tool to fail on a mismatched bit-width or an unassigned wire, a typed, composable IR acts as a safe-by-construction API that catches illegal mutations instantly. Constraining the AI to emit structurally valid IR rather than raw text bounds the agent's action space, so expensive compute cycles go to meaningful semantic tradeoffs rather than debugging trivial syntax hallucinations.

Beyond static IRs, real architects explore design spaces using *Parameterized Generators* (such as Chisel, Python frameworks, or SystemVerilog parameters). A useful AI representation must encode these generator boundaries and configuration schemas, allowing the agent to tune parameters rather than rewrite static graphs from scratch.

Real systems architecture is dominated by data movement, not isolated compute blocks. An AI's representation must structurally encode system-level topologies like Network-on-Chip (NoC), memory hierarchies, and bandwidth constraints. However, NoCs are fundamentally different from single-block dataflow; they involve distributed state, flow control, and dynamic protocol abstractions (e.g., virtual channels) that require complex state-machine encodings to avoid deadlock. For example, connecting the Lighthouse's RISC-V compute tile to its vision accelerators using a Ring or Mesh NoC requires explicit virtual channel representations to guarantee that high-bandwidth rendering traffic does not starve latency-sensitive eye-tracking. Furthermore, an AI must understand the "meta-graph" of Clock Domain Crossing (CDC)[^fn-cdc-c04] and reset trees. An AI that mutates a pipeline without explicitly representing asynchronous boundaries will confidently generate untraceable metastability failures. Finally, generating RTL without a testbench is useless. The AI must also represent verification intent and coverage models (such as Universal Verification Methodology, or UVM). @sec-feedback-verification-trust deeply covers the mechanics of verification and testbench representations.
...
The representation problem extends beyond logic to physical realization. Power and timing bounds are codified in side-channel constraints like UPF (Unified Power Format) and SDC (Synopsys Design Constraints). These constraints govern power domains and timing boundaries alongside the RTL. In the Lighthouse SoC, for example, the need to cross clock domains between a high-frequency rendering pipeline and a low-frequency, always-on sensor hub requires strict synchronization logic governed by UPF and SDC. For AI models, they are uniquely hostile because of *topological coupling*. SDC and UPF rely on exact hierarchical paths and pin names (specific string references to internal RTL signals). Any structural mutation by the AI—such as flattening a hierarchy or renaming a buffer to improve performance—requires a synchronized co-mutation of the constraint files, otherwise the EDA tool will instantly fail with "path not found" errors. An agent must co-generate these formats to operate within a realistic physical envelope, transforming an abstract concept into a manufacturable netlist (@tbl-representation-formats).
...
In one instance, a text-based LLM hallucinated a wire connection spanning two different voltage domains without a level shifter. It passed syntax checks but would have fried the chip, perfectly illustrating why 1D sequence tokens fail and structural or graph encodings are required to capture the physical invariants of the design.
:::

To reason about hardware natively, an AI system must move beyond text sequences to structural embeddings. First, a specialized encoder parses a raw text snippet (like SystemVerilog) and lowers it into an Abstract Syntax Tree (AST), a Data Flow Graph (DFG), or an And-Inverter Graph (AIG). Once lowered into a graph, Graph Neural Networks (GNNs) can ingest it. Because GNNs offer permutation invariance and explicit edge-based message passing, they inherently capture physical locality, fan-out, and combinational depth—properties that a 1D sequence model struggles to infer through attention mechanisms alone.
...
Whether using sequence tokens or graph embeddings, for an AI to *work* on a design, it does not just generate text. It mutates a constrained state machine.
...
### Workload Representations and Phase Behavior

Because architecture is a co-design problem, static RTL code is not enough. The AI agent must understand runtime execution. Traces, phase classifications (like SimPoint), and bottleneck graphs serve as the fundamental data representations of the software target. For example, explicitly using a Lighthouse SLAM (Simultaneous Localization and Mapping) trace illustrates bursty, phase-based execution that analytical cost models often miss. However, while generating execution traces is easy, translating those into cycle-accurate switching activity for power and thermal analysis requires deep pipeline semantics that simple LLMs lack. The AI must be able to ingest and reason about workload state just as fluently as it does design state, matching the hardware mechanisms to the dynamic phase behavior of the target application without losing the physical context of the switching activity.

### State Mutation and Tradeoffs

An explicit design state must distinguish the fields that an AI method may mutate from the invariants and constraints that remain fixed to preserve system intent. Crucially, "Project State" in physical design is not a single environment, but a Multi-Corner Multi-Mode (MCMM)[^fn-mcmm-c04] matrix. An AI's proposed structural mutation must be evaluated against dozens of concurrent physical scenarios—improving setup timing in a typical corner might instantly violate hold timing in a fast-corner/cold-temperature mode. This severely multiplies the cost of the tool observation loop.
...
@tbl-negative-traces gives examples of negative knowledge, including manual waivers for physical constraints like Static Timing Analysis (STA)[^fn-sta-c04].
...
Once a representation properly bounds the AI to legal, bootable changes, the system still must select which valid mutation to pursue. To choose the next change, an AI method needs an estimate of what that change may do. Predictive design-space exploration has long used analytical models, learned surrogates, and regression modeling to predict configuration performance without running full simulations [@LeeBrooks2006RegressionModeling]. More recent learned cost models predict throughput or performance directly from a structural representation of the program, as in AutoTVM's learned tensor-program cost model and the Halide learned autoscheduler [@Chen2018AutoTVM; @Adams2019Halide]. What these predictors consume is a learned graph or structural representation of the design, the same move that later lets the AlphaChip placement agent operate on a chip graph.

### Cost Models vs. World Models

Before evaluating consequences across tools, we must clarify what the AI is building with these representations. In current AI-assisted architecture, systems rely heavily on *Cost Models* (surrogates). A cost model takes a proposed design state and predicts its physical consequence (e.g., predicting that an expanded cache will consume 3 W of power).

This is fundamentally different from a true *World Model*. In artificial intelligence, a world model predicts the *next state* of an environment, allowing an agent to plan several steps ahead without querying a simulator. Because hardware environments are deterministic once a mutation is chosen, true architecture world models are rarely needed. Instead, the field relies on highly accurate, multi-fidelity cost models. We introduce this distinction here to set baseline expectations. Current agents do not hallucinate entire physical worlds; they search structural representations guided by learned cost proxies, with physical synthesis (PPA) serving as the ultimate arbiter for these predictive models.

### Legacy Compatibility as a Boundary Condition

When an AI predicts the consequences of an architectural mutation, it often drifts toward structures that are most efficient in isolation. A mathematically optimal design is still useless if it breaks the software ecosystem it has to run inside. The cost model has to carry backward compatibility as an explicit part of the environment it reasons about.

Design does not happen in a vacuum. It sits beneath billions of lines of existing code. A generative model might propose a memory interleaving scheme that improves average-case latency by 15%, yet if the scheme quietly violates the memory consistency models that legacy software assumes, it breaks every multithreaded operating-system kernel running on it. A non-standard interrupt controller might save area while violating platform architecture specifications (e.g., ARM SBSA) or OS Hardware Abstraction Layer (HAL) assumptions, forcing rewrites across millions of lines of legacy OS drivers.

::: {.callout-war-story title="When Hardware Savings Eclipse Software Costs"}
In one industry case, a clever hardware optimization saved 5% of die area but subtly broke a strict memory consistency model and OS Hardware Abstraction Layer assumption. The resulting system instability took months of complex software driver rewrites to fix, completely eclipsing the financial benefit of the initial hardware savings.
:::

These boundaries are hard architectural constraints, not suggestions. A useful cost model cannot optimize for IPC (Instructions Per Cycle) or PPA (Power, Performance, Area) alone. It has to stay bounded by the backward-compatibility contracts, ISA semantics, and memory consistency models that define the environment the chip will inhabit. Strip those constraints out of the representation and the AI will propose optimized but unbootable silicon.
...
To overcome this, an AI requires a mechanistic or causal cost model (or causal surrogate). Such a model explicitly encodes structural causality; it knows that adding a cache level forces an increase in access latency due to wire physics and multiplexing, not just because the two variables co-occurred in a dataset.
...
A later simulator execution tests the prediction and returns a high-fidelity observation. Detecting when a candidate has left the calibrated region is the job of uncertainty quantification,[^fn-uncertainty-quantification-c04] through deep ensembles, Gaussian-process predictive variance, or conformal prediction [@AngelopoulosBates2021Conformal]. But UQ is not just a defensive guardrail against OOD failures; it is also the offensive engine for active exploration. *Epistemic uncertainty* (predictive variance) drives the acquisition function in multi-fidelity Bayesian Optimization, providing the exact signal that tells the agent when to pay the high compute cost of an RTL simulation to acquire a new ground-truth sample. The execution history must keep the AI's prediction and the simulator's observation distinct to catch these failures. Predicting one consequence reliably is already hard. Predicting a next design state faithfully enough to plan several moves ahead remains largely out of reach, so this chapter treats it as a target rather than a tool.
...
@fig-architecture-cost-model separates the model prediction from the tool observation. The proposal records an anticipated effect. The represented state and action feed a performance model or simulator that returns a prediction; the same state and action feed a declared tool path that returns an observation with its cost, fidelity, and provenance.
...
For example, a cost model fit to earlier cycle-accurate results maps a represented workload trace and a proposed pipeline configuration to a predicted performance metric far faster than rerunning the simulator. @tbl-executed-study-cost-model shows exactly what such a predictor consumes and what its predictions leave out.
...
![**State inconsistency.** When the AI retrieves stale manuals or training data while the project state advances, it hallucinates fixes that violate current constraints.](images/F4b-split-brain-causality){#fig-split-brain-causality width="100%" fig-alt="State Inconsistency: When the AI retrieves stale manuals or training data while the project state advances, it hallucinates fixes that violate current constraints."}
...
- How can the industry develop standardized APIs or Intermediate Representations (akin to MLIR for compilers) to allow seamless bi-directional data flow between ML agents and commercial EDA tools?
...
- **Up next:** We have successfully established the first pillar of the agentic loop: **The State**. But a state is only useful if an agent can explore it. In the next chapter, @sec-methods-generation-prediction-optimization, we introduce **The Policy**—the search, generation, and optimization algorithms that navigate this state to propose new architectures.
:::
