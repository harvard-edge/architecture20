...
**The lesson.** A surrogate is trustworthy only in the region and under the conditions it was calibrated on, and a system that keeps acting on it can move those conditions itself. An architecture world model carries the same exposure. Its predicted consequence is an estimate, and an optimizer searching for the best-scoring candidate pushes it toward the untested corners where that estimate is least reliable. The safeguard is to keep the prediction distinct from the tool observation that later tests it, and to detect when a candidate has left the calibrated region before an automated loop commits to it.
:::
