...
> **Architecture representation.** An architecture representation structures the loop into three state schemas that a method can read, compare, update, replay, and audit:
>
...
| Post-silicon bring-up or validation | Weeks to months and high commitment. | Real hardware exposes integration, timing, power, reliability, and validation failures that earlier evidence missed. | Hardware stepping, firmware or microcode version, test context, failure mode, waiver, and owner. |
...
Collecting this architecture data, from high-fidelity negative traces to structured constraints and world-model escalation triggers, ultimately serves a narrower goal. It makes bounded automated action reviewable, rejectable, and replayable. A representation becomes actionable when it can safely support loop operations.
...
provenance, support replay, preserve uncertainty, separate feedback from
...
learning problem, or benchmark would make architecture state replayable,
rejectable, and useful inside an AI-assisted loop.
...
The evaluation should show that proposals can be replayed, checked, compared,
and safely rejected using attached provenance rather than private author
memory.
...
actions, feedback, rejection, replay, and review can actually happen.
...
replay, compare, and revise.

::: {.callout-carry-forward}
- **Carry forward:** Represent enough loop state that a result can be replayed, its rejected alternatives compared, and its commitment read long after the run.
