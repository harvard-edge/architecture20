...
Real systems architecture is dominated by data movement, not isolated compute blocks. An AI's representation must structurally encode system-level topologies like Network-on-Chip (NoC), memory hierarchies, and bandwidth constraints (e.g., CXL). Finally, generating RTL without a testbench is useless. The AI must also represent verification intent and coverage models (such as UVM). @sec-feedback-verification-trust deeply covers the mechanics of verification and testbench representations.
...
Beyond graphs, bridging the semantic gap requires multi-modal representations. Visual Language Models (VLMs) and spatial encodings move past text tokens, letting agents reason directly over 2D floorplans and physical layouts. This allows the AI searcher to comprehend density, congestion, and spatial topologies visually. When projecting these structures into continuous latent spaces (embeddings), a central challenge is maintaining structural validity—for example, preserving graph invariants for RTL or Network-on-Chip (NoC) topologies so that decoded candidates remain physically realizable. This shift from 1D sequence tokenization to structurally-valid graph embeddings and multi-modal spatial awareness is the core data representation problem in AI-assisted architecture.
