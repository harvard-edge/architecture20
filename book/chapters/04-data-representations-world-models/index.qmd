...
As emphasized by treating sample cost as architecture data (see @sec-sample-cost-is-architecture-data), workload provenance is critical loop state. Yet, we lack rigorous methods to detect when an automated optimizer has overfit to a specific execution phase or synthetic input distribution. A major research direction is establishing formal generalization bounds^[Theoretical limits on how well a machine learning model's performance on its training data will apply to unseen, out-of-distribution data.] for architecture agents, helping prevent silent hardware failures under distribution shifts toward architecture world models (see @sec-toward-architecture-world-models).

4. **How do we formulate representation learning over negative architecture traces?**
As established in our discussion of provenance, coverage, and negative traces (see @sec-provenance-coverage-and-negative-traces), rejected candidates, missed timing constraints, and invalid parameters define the true boundary of the design space (@tbl-negative-traces). However, current generative models are primarily trained on successful artifacts. A critical problem for the ML-for-systems community is designing novel loss functions^[Mathematical functions used to measure the difference between an AI model's predicted output and the actual target, which the training process seeks to minimize.] or preference-optimization pipelines^[Techniques like Reinforcement Learning from Human Feedback (RLHF) or Direct Preference Optimization (DPO) used to align a model's outputs with human preferences.] that force models to learn from high-fidelity negative records, ensuring they actively avoid, rather than rediscover, known failure modes.
