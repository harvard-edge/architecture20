...
```{=latex}
\abstract*{Representation is one of the first hard problems in Architecture 2.0. Automated optimizers cannot act credibly unless architecture work is made legible. The representation must structure three state schemas for loop state, execution history, and meta-state. Together, those schemas make the five-part execution state readable, comparable, and revisable, supply records for audit, and provide state that a runnable receipt can bind for replay.}
```
...
@sec-architecture-20-ontology ended with the design-loop card, where
representation and world model are the fields that make a claim inspectable.
This chapter opens those fields. It examines what state the loop records, what consequences
...
[^fn-ml-language-model-data-contrast]: The relevant contrast is data shape. A web language model can learn from abundant public text; an architecture loop needs recorded state, provenance, tool outcomes, and negative traces that final prose usually omits.
...
design has two halves. The representation records that state, and the world
model that predicts what acting on it will do.

> **Architecture representation.** An architecture representation structures the loop into three state schemas that a method can read, compare, and update. Those records support audit, while a runnable receipt can additionally bind them to replay:
>
> - **Loop State:** Workload, design state, constraints, and action-relevant fields (the system inputs and outputs defining the *state seen* and the fields an environment can validate).
...
> model, and reports uncertainty and invalid-action conditions. Human review is
> not part of the world model; it is an external rejection authority that acts on
> the model's output.
...
The receipt in @tbl-architecture-data-receipt is a scale check that separates the easy
...
| **Receipt** | **July 2026 release snapshot** | **What it supports** | **Caveat** |
| --- | --- | --- | --- |
...
Domain adaptation[^fn-ml-domain-adaptation] matters here only if it helps construct or retrieve loop
...
pretraining[^fn-ml-domain-continued-pretraining], and retrieval over internal corpora, then applies them to
...
it cannot by itself represent the design loop. These examples are adjacent tool-facing cases, not claims that EDA scripting or macro
placement is itself the architectural decision. They matter here because
architecture loops must carry EDA and software feedback when those flows
constrain a candidate.

[^fn-ml-domain-adaptation]: Domain adaptation is a machine learning technique where a model trained on one data distribution is adjusted to perform well on a different, specific target domain [@PanYang2010TransferLearning]. The chapter's caution is that adaptation sharpens vocabulary and retrieval over architecture text, yet it does not by itself grant an optimizer the authority to act inside a design loop.

[^fn-ml-domain-continued-pretraining]: Domain-continued pretraining further trains a foundational model on a large corpus of domain-specific text to strengthen its specialized vocabulary and understanding. ChipNeMo uses it to fit hardware-design text, which helps text-facing tasks but still leaves the executable tool state and negative traces a design loop needs unrepresented.

Moving from training text to evaluation, benchmark lineages such as SQuAD and GLUE[^fn-ml-benchmarks-squad-glue] offer a
...
[^fn-ml-benchmarks-squad-glue]: SQuAD and GLUE are standard evaluation objects for natural language processing, where SQuAD is the Stanford Question Answering Dataset [@RajpurkarEtAl2016SQuAD] and GLUE is the General Language Understanding Evaluation, a multi-task benchmark suite [@WangEtAl2018GLUE]. They show how a shared, scoreable object can move a field, and the chapter's point is that an architecture evaluation object must record far more than a labeled row before an optimizer can safely act on it.
...
that can change what the loop believes. It might be a cycle-level simulation, a
...
fidelity, and provenance, and it may support or reject a claim or proposed
decision.

> **Architecture sample.** An architecture sample is a feedback event that can
> change the loop's belief about a design candidate. Its record includes cost,
> fidelity, provenance, assumptions, rejected-space coverage, and the declared
> claims or decisions it can support or reject.
...
It is a reminder that an architecture sample carries hidden state. A
...
: **Architecture samples carry cost, fidelity, and decision scope.** A feedback event is useful only when the representation records what it cost, what assumptions it used, and what future decision it can support or reject. {#tbl-sample-cost-regimes tbl-colwidths="[20,24,24,22]"}
...
the pairing, is what turns that record into predicted or constrained consequences. The world-model idea becomes architecture-specific when it is tied to tool
behavior, cost, constraints, invalid-action rules, and uncertainty, much like reinforcement learning agents use latent world models[^fn-rl-latent-world-model] to plan and simulate outcomes before acting.

[^fn-rl-latent-world-model]: Internal, compressed representations of an environment that an agent uses to simulate and plan actions without touching the real world. The architecture world model in this chapter plays the same predictive role, but it must stay scoped and auditable so the loop knows when to trust it, escalate fidelity, or reject its advice.
...
modes are part of the world model. A set of design rules, expert heuristics, or
physical constraints can also act as a world model.

A representation record contains workload traces, architecture descriptions,
tool configurations, logs, constraints, and objectives. A world model is the
predictive or constraint layer over that represented state and the legal actions
defined by the environment. It estimates consequences, uncertainty, costs,
failure modes, and invalid-action conditions. It does not own the action schema,
sampling policy, or human decision policy. The basic structure, shown in @fig-architecture-world-model, has tools return
feedback while evidence updates both the representation and the world model.

![**A world model connects artifacts to valid architectural action:** A representation record captures what the loop can read, compare, and update; a world model captures what the loop believes actions will change. Feedback becomes reusable only when provenance, coverage, and negative traces are recorded, while replay additionally requires runnable inputs and environment bindings.](images/F5-architecture-world-model){#fig-architecture-world-model width="100%" fig-alt="Loop diagram showing representation records, world models, tool feedback, evidence, provenance, coverage, and negative traces connected to valid architectural action, with replay requiring a separate runnable receipt."}
...
The type of world model matters only through the loop contract it supports. It dictates what
...
these pieces. It uses a simulator for selected candidates, a learned predictor for cheap
...
escalation trigger, and the resulting evidence are auditable, but they belong to
different parts of the loop. The prediction and escalation trigger belong to the
world model; the recorded rejection belongs to execution history and the
evidence ledger, with the architect as rejection authority. An automated
optimizer acted on the belief, and a human could see why it was overturned.
...
When an optimizer, acting through a world model, proposes a candidate whose margin of safety is smaller than the model's known error band, the loop must pause. The architect decides whether to escalate to a higher-fidelity tool (e.g., a cycle-level simulator) or reject the candidate outright based on risk. This gate prevents the optimizer from treating an uncalibrated proxy estimate as commitment evidence.
:::
...
At this point the definition of negative traces introduced in @sec-moonshot
becomes operational. They matter because a
loop that records only winners cannot inform the next method, reviewer, or
architect which assumptions, tools, workloads, or regions of the design space
...
The data schema in @tbl-negative-traces records what failed, what boundary it
...
Negative traces require provenance detailed enough to interpret the environment
that produced the result. A replay claim additionally requires the runnable
environment and inputs. A failed result without context is not very useful. The minimum reusable record includes the candidate identifier, workload
...
A negative trace also needs a reason to survive ordinary deadline pressure. It
...
Collecting this architecture data, from high-fidelity negative traces to structured constraints and world-model escalation triggers, ultimately serves a narrower goal. It makes bounded automated action reviewable and rejectable, and supplies state for replay when a runnable receipt is also present. A representation becomes actionable when it can safely support loop operations.
It should expose the action-relevant fields that an environment can validate,
expose relevant observations, carry constraints and objectives, record
provenance, preserve uncertainty, separate feedback from
evidence, and record what rejects a candidate. It should also say what remains a
human decision.
...
surface where valid actions, feedback, rejection, or review can happen earlier,
cheaper, or with clearer authority. Replay additionally depends on an attached
runnable receipt. An abstraction that cannot return
...
ledger, evidence standard, negative traces, and rejection authority. Only then can the loop ask a
...
scarce measurement of the design space. The needed artifact is a sample record
that preserves workload provenance, tool versions, fidelity, assumptions,
coverage labels, uncertainty, and negative outcomes. Its value should be tested
...
The world model sketch in @tbl-xr-world-model-sketch uses explicit escalation
triggers, but a stronger research result would learn or verify where those
triggers should be. A concrete paper could build a calibration-boundary model
...
introduce a negative-trace dataset or task suite and test whether models learn
...
the wrong direction. Get it right and the loop gains a surface where valid
actions, feedback, rejection, and review can happen, and where a runnable receipt
can support replay.
...
which means recording provenance, assumptions, costs, failures, and negative
traces, not only the successful endpoints.
World models, structured design data, and negative-trace corpora all serve that
one end, turning scattered architecture experience into state a loop can read,
compare, and revise, and that a suitable receipt can make replayable.

::: {.callout-carry-forward}
- **Carry forward:** Represent enough loop state that rejected alternatives can be compared and the commitment read long after the run; attach a runnable receipt when replay is a requirement.
- **Reader test:** Could an automated optimizer read both the winning and rejected candidates six months later, compare their evidence boundaries, see who could reject the result, and find the runnable environment and inputs if the record claims replayability?
