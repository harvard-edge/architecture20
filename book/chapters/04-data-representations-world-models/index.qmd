...
1. **World Models for Microarchitectural State Transitions:** Can we train foundation models to predict the cross-layer consequence of a structural mutation (e.g., changing cache associativity) with sufficient fidelity to enable multi-step reinforcement learning planning without invoking a cycle-accurate simulator at every step?
2. **Representation of Negative Evidence in Combinatorial Spaces:** What is the optimal embedding strategy for representing failed synthesis runs and DRC violations such that an agent can generalize from a single routing failure to avoid the entire infeasible region of the design space?
3. **Detecting Constraint Drift in Continual Learning:** How can an agentic system detect when its learned parametric priors (e.g., timing delays) have drifted out of alignment with the current authoritative design state (e.g., a new process node) before it proposes physically invalid architectures?
