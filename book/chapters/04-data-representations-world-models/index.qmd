...
::: {.column-margin}
**Author's Note:** Ludwig Wittgenstein, one of the most important philosophers of the 20th century, argued that language constrains what we can possibly conceive. For this chapter, the architectural lesson is narrower: a loop can only optimize and audit the state its representations, tools, and rejection rules expose. If an HDL or architecture description has no field for a design concept, a generator may still mention it, but the loop cannot test or own that concept until the representation is extended.
:::

```{=latex}
\abstract*{Representation is one of the first hard problems in Architecture 2.0. Automated optimizers cannot act credibly unless architecture work is made legible. The representation must structure three state schemas to directly implement the five-part execution state: Loop State (workload, architecture, constraints), Execution History (provenance, feedback, rejected alternatives), and Meta-State (assumptions and tools) must be captured in forms the loop can read, replay, compare, and revise.}
```
...
[^fn-sw-agent-framework]: An agent framework is a software structure that defines how an autonomous system perceives its environment, reasons, and takes actions to achieve goals. No framework can act on state the representation never recorded, so the choice of framework is secondary to designing the loop state it will read.
...
[^fn-ml-language-model]: A statistical model trained to predict the next word or token in a sequence, forming the basis of modern AI text generation. Such a model is fed by abundant public text, whereas an architecture loop is starved of the recorded state, provenance, and negative traces that make a design decision meaningful.
...
The transparent literature calculation and the architecture-relevant Wikipedia corpus land at the same $10^9$-token scale.
...
**The gap.** The designs the method learned from were proprietary, and while some code was later released, the pretraining inputs needed to rerun the strongest version of the claim were not [@ChengEtAl2023MacroPlacement; @Markov2023FalseDawn]. The authors answered that the critics had not run the method as described [@GoldieEtAl2024ThatChipHasSailed]. Whatever world model the method built from those chips, its coverage, assumptions, and blind spots cannot be inspected, so a reviewer acting in good faith cannot audit the data behind the claim.

**The lesson.** As a public, independently replayable claim, the strongest version can still be neither confirmed nor rejected, and the lesson is not who is right. It is that a result whose data cannot be inspected leaves the loop with an anecdote instead of evidence. Enforce provenance for data and environment as the run happens, so the loop leaves a record a reviewer can audit rather than a dispute.
:::
...
policies, much like reinforcement learning agents use latent world models[^fn-rl-latent-world-model] to plan and simulate outcomes before acting.
...
uncertainty, and decision policies. @fig-architecture-world-model gives the
basic structure, where tools return feedback and evidence updates both the
...
Collecting this architecture data (from high-fidelity negative traces to structured constraints and world-model escalation triggers) ultimately serves a narrower goal: making bounded automated action reviewable, rejectable, and replayable. A representation becomes actionable when it can safely support loop operations.
