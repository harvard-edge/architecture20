...
Once we understand the cost and structure of architectural data, we must consider how an AI agent consumes it. As shown in @tbl-ingestion-timescales, an AI system does not ingest all information in the same way because the knowledge it needs operates on vastly different timescales .
...
[^fn-hw-sw-examples]: For example, adding a new vector instruction requires updating the ISA, memory consistency models, or Control and Status Registers (CSRs).
...
When an AI explores the vast combinatorial space of architectural mutations, it naturally gravitates toward the most mathematically efficient structures. However, a mathematically optimal hardware design is useless if it breaks the surrounding software ecosystem. The AI's world model must explicitly include the inflexible realities of backwards compatibility—what we call the **Chains of Legacy**.

Architectural design does not happen in a vacuum; it sits beneath billions of lines of existing code. A generative model might propose a novel memory interleaving scheme that improves average-case latency by 15%, but if that scheme silently violates the strict memory consistency models assumed by legacy software, it will break every multithreaded operating system kernel that runs on it. Similarly, proposing a highly optimized, non-standard interrupt controller might save area, but it will shatter POSIX contracts and require rewriting millions of lines of legacy OS drivers.

These legacy boundaries are not suggestions; they are immovable architectural constraints. For an AI to be useful, its internal world model cannot simply optimize for IPC (Instructions Per Cycle) or PPA (Power, Performance, Area). It must be bounded by the Chains of Legacy—the strict backward compatibility contracts, ISA semantics, and memory consistency models that define the environment the chip will inhabit. Without an explicit representation of these legacy constraints, an AI will propose brilliant but unbootable silicon.
...
Only causal representations allow the AI to safely generalize to novel, un-simulated architectures.[^fn-arch-3-0] Correlative models inevitably collapse outside their training distribution when the optimizer pushes them into Out-of-Distribution (OOD) spaces, generating confident but physically impossible predictions.

[^fn-arch-3-0]: Safely generalizing to novel, un-simulated architectures is the hallmark of Architecture 3.0.
...
A learned cost model performs this estimation job. Given the current constrained design state and a proposed mutation, it emits a predicted consequence (such as cycle count or power). This prediction is *not* a tool observation. It is an estimate fraught with epistemic and aleatoric uncertainty.[^fn-uncertainty-types] When an optimizer pushes these models into untested design spaces, they suffer from Out-of-Distribution (OOD) failures, making confident but wrong predictions.

[^fn-uncertainty-types]: Epistemic uncertainty refers to uncertainty due to a lack of knowledge, while aleatoric uncertainty refers to inherent system noise.
