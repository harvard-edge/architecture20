...
We must build verifiable frameworks where the representation tracks what the AI consumes and returns. The AI accelerates the search legwork, but the human architect remains the final judge who orchestrates the Evaluation Harness to validate against physical silicon constraints like PPA, thermal budgets, and DRC rules.
...
The QuArch benchmark [@PrakashEtAl2025QuArch] begins bridging this gap by forcing models to calculate quantitative tradeoffs rather than just defining terms. However, AI acts only as a searcher; true architectural exploration requires datasets that move beyond factual recall. To be useful to a hardware team, the training data must force the model to confront the architect's reality: PPA (Power, Performance, Area), STA (Static Timing Analysis), and DRC rules. If a model does not learn the brutal physics of placement and congestion from its data, it cannot be steered by the architect, and its generated RTL will remain a theoretical exercise rather than a tape-out candidate that passes the Evaluation Harness.
...
Beyond graphs, bridging the semantic gap requires multi-modal representations. By moving beyond text tokens to Visual Language Models (VLMs) and spatial encodings, agents can reason directly over 2D floorplans and physical layouts. This allows the AI searcher to comprehend density, congestion, and spatial topologies visually. This shift from 1D sequence tokenization to graph embeddings and multi-modal spatial awareness is the core data representation problem in AI-assisted architecture. However, no matter how advanced the encoding, the AI remains an explorer; the human architect orchestrates the Evaluation Harness to verify DRC and PPA compliance.
...
A later simulator execution tests the prediction and returns a high-fidelity observation, serving as the Evaluation Harness orchestrated by the architect. Detecting when a candidate has left the calibrated region is the job of uncertainty quantification, through deep ensembles, Gaussian-process predictive variance, or conformal prediction [@AngelopoulosBates2021Conformal]. The execution history must keep the AI's prediction and the simulator's observation distinct to catch these OOD failures. Predicting one consequence reliably is already hard. Predicting a next design state faithfully enough to plan several moves ahead remains largely out of reach, so this chapter treats it as a target rather than a tool.
