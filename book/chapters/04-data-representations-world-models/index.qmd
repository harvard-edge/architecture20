...
> **Architecture world model.** An architecture world model is a scoped belief
> model for a bounded design loop. It estimates or constrains what a candidate
...
$$
For a 1 GHz target, @tbl-simulation-time turns the multiplication into wall-clock
intuition. Read each row as a span of target execution time, each column as a
...
sampling policy, or human decision policy. @fig-architecture-world-model gives
the basic structure, where tools return feedback and evidence updates both the
...
At this point the definition introduced in @sec-moonshot becomes operational:
negative traces are recorded failed, rejected, invalid, or abandoned candidates
together with the reason they did not support the claim. They matter because a
...
This point becomes a data schema in @tbl-negative-traces.
