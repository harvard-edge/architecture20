...
## Hardware Encoding Mechanics: Tokenization and Graphs
...
Representing a tool like Yosys or OpenROAD for an agent involves defining its input schema (e.g., accepted flags, valid file formats, constraint objects), its execution bounds, and its structured outputs (e.g., JSON-formatted area and timing reports rather than raw text stdout). By formalizing tools as typed APIs with explicit failure modes, the agent can programmatically construct valid toolchain requests, parse the resulting metrics directly into its internal state, and execute deterministic recovery actions when a simulation or synthesis run fails.

## Working on a Design, Not Just Generating Text
