...
## Define Architectural Knowledge for Design
...
## Structure Sources and Datasets for Architectural Knowledge
...
Because architectural tasks range from semantic reasoning to precise tool execution, the capability an AI develops is fundamentally bounded by the geometry of its training data:
...
As shown in @tbl-dataset-capability, a flat document corpus like conference papers can instill general conceptual fluency, but it cannot teach a model how to act. For that, the model must consume tool-use traces that demonstrate exact API syntax and sequence execution. Moving beyond mere syntax to true optimization requires full state-action-consequence simulation logs. These logs explicitly map a proposed architectural mutation to its downstream physical costs, allowing the model to learn not just *how* to invoke a tool, but *why* certain designs fail routing or violate thermal limits. The QuArch benchmark [@PrakashEtAl2025QuArch] begins bridging this gap by forcing models to calculate quantitative tradeoffs rather than just defining terms. However, true architectural agency requires datasets that move beyond factual recall. To be useful to a hardware team, the training data must force the model to confront the architect's reality: timing closure, power integrity, and physical routing. If a model does not learn the brutal physics of placement and congestion from its data, it cannot be steered by the architect, and its generated RTL will remain a theoretical exercise rather than a tape-out candidate.
...
## Ingest Architectural Knowledge into a Model

Once we understand the cost and structure of architectural data, we must consider how an AI agent consumes it. As shown in @tbl-ingestion-timescales, an AI system does not ingest all information in the same way because the knowledge it needs operates on vastly different timescales [@LiuEtAl2023ChipNeMo].
...
For an architect, this separation of timescales is a fundamental safety property, not just a machine learning implementation detail. If an AI attempted to learn daily project updates the same way it learns basic physics, the compute cost would be catastrophic. Instead, knowledge is stratified.
...
Instead, the system relies on instruction fine-tuning to shape how the model acts. Fine-tuning teaches the agent how to interact with EDA tools, format API calls, and orchestrate workflows. It does not teach the model the current state of your chip; it simply teaches it how to hold the tools.

To bridge the gap between static weights and live projects, the system utilizes dynamic retrieval to fetch versioned manuals, ISA definitions, and reference designs at query time. These retrieved facts are then combined with in-context conditioning—the transient working memory that holds the immediate area constraints, failing timing paths, and workload settings of the current inference step.
...
By stratifying knowledge into these distinct timescales, the AI's generalized statistical reasoning is strictly tethered to the physical reality of the current project state, ensuring the human architect remains in total command of the power, area, and timing boundaries.

## Preserve Context Across Different Representations
...
## Generate Concrete Industry-Standard Representations

When transitioning from abstract reasoning to concrete execution, we must ask: what does the AI actually output to the toolchain? Generating abstract pseudo-code is insufficient. Generating raw SystemVerilog, while common, is notoriously brittle; it requires exact, rigid syntax and is highly susceptible to hallucinated wires and logic flaws that break synthesis.

To bridge this gap, agents increasingly rely on Intermediate Representations (IRs) like CIRCT and FIRRTL. These IRs fundamentally shift the verification burden from slow, late-stage simulation to fast, upfront compilation. Instead of waiting hours for an EDA tool to fail on a mismatched bit-width or an unassigned wire, a typed, composable IR acts as a safe-by-construction API that catches illegal mutations instantly. By constraining the AI to emit structurally valid IR rather than raw text, the architecture team bounds the agent's action space, ensuring it spends expensive compute cycles exploring meaningful semantic tradeoffs rather than debugging trivial syntax hallucinations.

Furthermore, the representation problem extends beyond logic to physical realization. Power and timing bounds are codified in syntax-heavy constraints like UPF (Unified Power Format) and SDC (Synopsys Design Constraints). An agent must parse and generate these formats to operate within a realistic physical envelope, transforming an abstract concept into a manufacturable netlist (@tbl-representation-formats).
...
## Encode Hardware: Tokenization, Graphs, and Vision

Beneath these high-level knowledge objects lies the physical reality of how an AI system encodes hardware. General-purpose large language models (LLMs) rely on 1D sequence tokenizers (like Byte-Pair Encoding) optimized for natural language. When applied naively to hardware description languages like Verilog or Chisel, these tokenizers chop signal names and structural code into meaningless fragments, destroying the implicit connectivity of the design.

To reason about hardware natively, an AI system must move beyond text sequences to structural embeddings. Instead of reading Verilog as a string, a specialized encoder parses the RTL into an Abstract Syntax Tree (AST) or a Data Flow Graph (DFG). For placed netlists and floorplans, Graph Neural Networks (GNNs) are far superior to sequence models. A GNN represents macros and standard cells as nodes and wires as edges, inherently capturing physical locality, fan-out, and combinational depth—properties that a 1D sequence model must struggle to infer through attention mechanisms. Beyond graphs, bridging the semantic gap requires multi-modal representations. By moving beyond text tokens to Visual Language Models (VLMs) and spatial encodings, agents can reason directly over 2D floorplans and physical layouts, comprehending density, congestion, and spatial topologies visually. This shift from 1D sequence tokenization to graph embeddings and multi-modal spatial awareness is the core data representation problem in AI-assisted architecture.

## Expose EDA to Agents via Tool APIs

Just as the hardware itself must be encoded, the tools that manipulate it require formal representations. An AI agent cannot reliably interact with legacy EDA tools through fragile, unstructured shell scripts or screen-scraping text logs. Instead, the design loop demands structured tool API representations—schemas that expose tool capabilities as typed endpoints.

Representing a tool like Yosys or OpenROAD for an agent involves defining its input schema (e.g., accepted flags, valid file formats, constraint objects), its execution bounds, and its structured outputs (e.g., JSON-formatted area and timing reports rather than raw text stdout). However, agents inevitably hit the **Telemetry Wall**: the physics of context windows restrict how much output they can observe. When an API serializes a massive physical layout into text, it risks lossy serialization. Truncating a million-line timing trace to fit a context window creates a permanent verification blind spot for the AI, causing it to hallucinate success simply because the error was paginated away. By formalizing tools as typed APIs with explicit failure modes and hierarchical summarization, the agent can programmatically construct valid toolchain requests, parse the resulting metrics directly into its internal state, and execute deterministic recovery actions without hitting the telemetry wall.

## Work on a Design, Do Not Just Generate Text
...
- **Current-state view:** The authoritative explicit design state. It identifies the exact design version, the workload, the mutable fields available for change, and the fixed constraints. Crucially, it must also track the **Hardware/Software Contract**. Architecture is a co-design problem; if the AI mutates the hardware state (e.g., adding a new vector instruction), the representation must explicitly track the required update to the software stack (ISA, memory consistency, CSRs). A hardware mutation is strictly invalid if the representation lacks compiler coherence and cannot map a software workload to the new topology.
...
## Mutate State and Track Tradeoffs
...
Within these boundaries, we must explicitly distinguish two types of legal moves. The first is *parametric tuning*, such as resizing a buffer, changing queue depths, or adjusting cache associativity. This represents a continuous or bounded discrete search where the fundamental pipeline topology remains unchanged, and it requires only a simple scalar or JSON schema to govern the action space.

The second type is *structural mutation*. This involves fundamental topological changes, such as adding a new pipeline stage, swapping a mesh interconnect for a ring, or inserting a bespoke accelerator block. Governing this action space is far more complex. Structural mutation requires a graph-based representation—such as an AST or a netlist graph—to enforce physical connectivity, validate interface protocols, and maintain coherence rules during the transformation.

Regardless of the mutation type, the state space is astronomically large. The number of reachable configurations grows combinatorially and quickly exceeds any space that could be searched exhaustively. Without explicit constraints on the search, an optimizer wanders through useless regions of that space.
...
## Predict Architectural Consequences
...
### Distinguish Causal vs. Correlative World Models

The fundamental flaw in many early predictive surrogates is that they fall into the correlative trap. A black-box surrogate might learn a correlation—for example, that higher chip area often equals higher power—through curve-fitting on its training data, entirely without understanding the underlying physics or capacitance equations.
...
Only causal representations allow the AI to safely generalize to novel, un-simulated architectures (the hallmark of Architecture 3.0). Correlative models inevitably collapse outside their training distribution when the optimizer pushes them into Out-of-Distribution (OOD) spaces, generating confident but physically impossible predictions.
...
## Track Missing, Private, Stale, and Changing Knowledge
...
Each question below grows out of a difficulty this chapter examined. Some ask what an AI system must represent so that a proposed change stays legal and checkable. Others ask what it must predict or preserve as evidence ages and the design version advances. Throughout, the baseline is current architecture practice, and success is measured by the effect on the architecture study rather than by the fluency of the model's output.

1. **Can AI read and safely mutate a single design state that links structural boundaries, behavioral traces, and physical limits?**
This chapter showed that working on a design means mutating a highly constrained state machine, a different act from producing text, and that the current-state, execution-history, and context views must stay consistent for a change to remain legal. The Lighthouse subsystem makes the difficulty concrete, since its SRAM capacity and MAC-array dimensions are mutable while the peak-power limit and eye-tracking workload mix are fixed, and a candidate that raises throughput by silently relaxing the power limit invalidates the comparison. As the chapter noted, a constraint that a representation has no way to express is one that a tool cannot check. The open problem is whether structural boundaries in RTL, behavioral properties in execution traces, and physical limits such as power, thermal, and routing can be linked into one executable state that an AI method edits without breaking a constraint that lives in another view. Current practice stitches this together by hand across a configuration schema, a workload manifest, and separate constraint files, with human review catching cross-view violations, and the earlier translation of a netlist into a chip graph for reinforcement-learning placement shows how much a structural representation rather than plain text widens the legal space an AI method can search. Progress would show up as fewer violations introduced silently across views, fewer candidates that pass one view while breaking another, and more area, thermal, and timing failures caught before an expensive synthesis run instead of after it.

2. **How can AI detect when its learned and retrieved knowledge no longer matches the current design version?**
This chapter showed that model parameters, a retrieval index, explicit design state, and an execution history each preserve different things and are updated on different schedules, so they drift out of alignment. Its split-brain example captured the failure, where the AI retrieves stale manuals while the project advances and proposes fixes that violate the constraints of the new version, as when the Lighthouse architect restricts L2 capacity or swaps the eye-tracking trace and the model still reasons from the old bounds. The open problem is whether an AI system can recognize that its parametric priors or retrieved context have aged out of alignment with the authoritative design state before it commits to an expensive synthesis run. Current practice leans on manually maintained version tags, document timestamps, and provenance metadata, with a human noticing the mismatch during review. Success would appear as more stale-constraint violations caught before a costly run instead of after it, fewer reruns triggered by false alarms, and fewer constraints that reach silicon because a mismatch went undetected.

3. **How should AI decide which candidates are worth an expensive high-fidelity sample?**
This chapter's data fidelity pyramid set cheap analytical and trace-driven data, which can be generated by the million, against scarce cycle-accurate, RTL, and silicon observations that can cost days of compute or the non-recurring engineering charge of a silicon run, because a single valid architectural sample requires executing a formal model of a machine. The open problem is whether an AI method can spend a limited high-fidelity budget on the candidates that most improve what a cost model learns about the design space, mapping broadly with cheap approximations and reserving expensive runs for optimal and boundary regions. Where the expensive oracle is spent bounds what the model can ever learn about those boundaries, so a cheap sampling choice made early propagates into every later prediction. Active learning and multi-fidelity Bayesian optimization, along with an expert's intuition about which corners are worth simulating, are the methods to beat, and uniform or hand-chosen sampling is the weaker baseline. The comparison should count coverage and prediction accuracy gained per hour of high-fidelity compute, boundary cases such as performance cliffs and unroutable configurations found per expensive sample, and the total oracle cost to reach a usable cost model.

4. **How can AI turn failed and rejected candidates into evidence that reshapes later search?**
This chapter argued that architecture projects usually keep the winning design and discard the work that did not advance, which hides which assumptions, tools, and regions of the space were already ruled out. It drew the compiler analogy, where a discovered bug becomes a regression test, and asked that a rejected pipeline configuration leave a trace naming the candidate, the area or timing condition it violated, and the tool and physical context. The harder half is that an old failure does not automatically rule out a new candidate, because the workload, tool version, or surrounding design may have changed. The open problem is whether an AI method can capture failed synthesis runs, unroutable netlists, proxy wins that a higher-fidelity check overturns, and rejected rationales, represent them with enough context to avoid known dead ends, and surface a past failure for an architect to judge when its conditions may no longer hold. Against the common baseline of keeping only winners, or at best a hand-curated regression suite a human decides when to trust, success would appear as fewer dead ends rediscovered at high cost, fewer synthesis and routing failures re-encountered, and fewer viable candidates wrongly excluded because a stale failure was applied without checking.

5. **Can AI tell when a fast surrogate has left the region where its predictions can be trusted?**
This chapter kept a cost model's prediction distinct from the tool observation that later tests it, and warned that an optimizer chasing the best-scoring candidate pushes a surrogate toward the untested corners where its estimate is least reliable and it makes confident but wrong out-of-distribution predictions. The Google Flu Trends case examined in this chapter made the exposure concrete, since a proxy tracked ground truth until the behavior it measured shifted, and a system that keeps acting on a surrogate can move the very conditions it was calibrated on. The open problem is whether uncertainty quantification, through deep ensembles, predictive variance, or conformal prediction, can flag that a candidate has crossed outside the calibrated region reliably enough for an automated loop to stop before it commits. The alternatives to beat are running the full high-fidelity simulator every time to be sure and relying on an expert's sense of when a fast model is out of its depth. The comparison should count how often predictions flagged reliable survive a high-fidelity check, how many out-of-distribution candidates the loop commits to before a tool overturns them, and how many expensive runs are avoided without discarding a design the surrogate wrongly dismissed.

6. **Can AI predict a sequence of design states well enough to plan several changes ahead?**
This chapter drew a careful line between the learned cost model in routine use, which predicts the consequence of one fixed mutation, and a genuine architecture world model, which would predict the next design state faithfully enough to roll out and plan a multi-step sequence of changes. It borrowed the world-model idea from reinforcement learning, where a model predicts the next state of an environment so an agent can plan without acting, and observed that architecture usually needs less generality because the next state is applied deterministically once a mutation is fixed, leaving the consequence as the uncertain quantity. The open problem is whether an AI method can predict a trajectory of design states and their consequences accurately enough to plan several architectural moves ahead, given that errors compound across steps and that architecture feedback is cross-layer, multi-fidelity, slow, and costly. The baseline is the single-step cost models and surrogates already in routine use, together with human architects who plan a sequence of changes from experience and re-simulate each step. Success would appear as more moves a planned sequence holds before a tool observation diverges from it, fewer decisions reversed after a multi-step rollout proves wrong, and fewer high-fidelity runs than evaluating each candidate change on its own. Whether architecture's slow, cross-layer, and costly feedback can support state prediction at all, unlike the game and robotics settings where world models arose, is the question this chapter marks and leaves open.
