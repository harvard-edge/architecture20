...
The gap between public architecture knowledge and actionable loop state is significant. The questions here focus on representations, not yet on full tool environments or governance. Each one asks what data structure, learning problem, or benchmark would make architecture state more usable inside an AI-assisted loop.

1. **What are the formal semantics of an auditable architecture proposal?**
Representations must act as boundary objects between human intent and tool execution (see @sec-architecture-descriptions-as-boundary-objects), yet optimizer proposals are still described informally. A thesis-level contribution would define a machine-readable proposal language with operational semantics for task intent, design-space boundaries, evidence, and rejection. The evaluation should show that proposals can be replayed, checked, compared, and safely rejected using their attached provenance rather than private author memory.

2. **Can world models discover and navigate fidelity frontiers?**
The world model sketch (@tbl-xr-world-model-sketch) uses explicit escalation triggers, but a stronger research result would learn where those triggers should be. A concrete paper could build a calibration-boundary model that predicts when a proxy is leaving its support region and should route a candidate to cycle-level simulation or physical synthesis. The success metric is not only prediction accuracy. It is lower evidence cost at the same false-accept risk, or lower false-accept risk at the same evidence cost.

3. **What is the information-theoretic limit of generalization in AI-driven design?**
Sample cost and workload provenance are architecture data (see @sec-sample-cost-is-architecture-data), but the field lacks rigorous methods for detecting when an optimizer has overfit to a phase, trace slice, or synthetic input distribution. A strong paper would define a shifted-workload benchmark plus generalization bounds[^fn-stats-generalization-bounds] or empirical tests for architecture agents. The point is to tell reviewers when a claimed design-space lesson transfers, when it weakens, and when it should be rejected (see @sec-toward-architecture-world-models).

4. **How do we formulate representation learning over negative architecture traces?**
Rejected candidates, missed timing constraints, and invalid parameters define much of the design-space boundary (@tbl-negative-traces), but most generative systems still learn mainly from successful artifacts. A paper-quality question would introduce a negative-trace dataset or task suite, then test loss functions[^fn-ml-loss-function] or preference-optimization pipelines[^fn-ml-preference-optimization] that teach a model what not to propose. The evaluation should measure avoided repeated failures, transfer to nearby design spaces, and the risk of overgeneralizing one failure into a false universal rule (see @sec-provenance-coverage-and-negative-traces).

[^fn-stats-generalization-bounds]: Theoretical limits on how well a model's performance on its training data will carry over to new samples drawn from the same distribution, under stated assumptions. The chapter wants such bounds for architecture agents so a reviewer can tell when a claimed design-space lesson transfers to a new workload and when it should be rejected.

[^fn-ml-loss-function]: A mathematical function that measures the gap between a model's predicted output and the intended target, which training seeks to minimize. The open question here is whether a loss can be shaped over negative traces so a generative model learns what not to propose, not only which artifacts succeeded.

[^fn-ml-preference-optimization]: Techniques such as Reinforcement Learning from Human Feedback (RLHF) or Direct Preference Optimization (DPO) that align a model's outputs with human preferences. This chapter asks whether the same pipelines, trained on rejected candidates, could teach an architecture model to avoid proposing moves the loop has already ruled out.
