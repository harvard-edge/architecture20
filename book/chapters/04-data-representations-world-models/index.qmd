...
**The lesson.** As a public, independently replayable claim, the strongest version can still be neither confirmed nor rejected, and the lesson is not who is right. It is that a result whose data cannot be inspected leaves reported evidence that public reviewers cannot fully audit or replay. Enforce provenance for data and environment as the run happens, so the loop leaves a record a reviewer can audit rather than a dispute.
:::
