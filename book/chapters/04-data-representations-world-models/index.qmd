...
Architecture does not need data in the same way a web language model[^fn-ml-language-model] needs text.
...
[^fn-ml-language-model]: A statistical model trained to predict the next token in a sequence, the basis of modern AI text generation. It is fed by abundant public text, whereas an architecture loop is starved of the recorded state, provenance, and negative traces a design decision needs.
...
[^fn-systems-negative-results-venues]: The wider computing-systems community has begun building places to publish exactly these records, where a rigorous experiment that failed to confirm its hypothesis still enters the shared record instead of being discarded. The field needs more of them so that failed runs, rejected candidates, and the assumptions behind them accumulate as shared evidence instead of being discarded before publication.
