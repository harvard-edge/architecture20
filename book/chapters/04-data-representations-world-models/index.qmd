...
framework, or optimization method to use. But a method can only act on what the
...
Sample cost and workload provenance are architecture data (see @sec-sample-cost-is-architecture-data), but the field lacks rigorous methods for detecting when an optimizer has overfit to a phase, trace slice, or synthetic input distribution. A strong paper would define a shifted-workload benchmark plus generalization bounds^[Theoretical limits on how well a machine learning model's performance on its training data will apply to unseen, out-of-distribution data.] or empirical tests for architecture agents. The point is to tell reviewers when a claimed design-space lesson transfers, when it weakens, and when it should be rejected (see @sec-toward-architecture-world-models).

4. **How do we formulate representation learning over negative architecture traces?**
Rejected candidates, missed timing constraints, and invalid parameters define much of the design-space boundary (@tbl-negative-traces), but most generative systems still learn mainly from successful artifacts. A paper-quality question would introduce a negative-trace dataset or task suite, then test loss functions^[Mathematical functions used to measure the difference between an AI model's predicted output and the actual target, which the training process seeks to minimize.] or preference-optimization pipelines^[Techniques like Reinforcement Learning from Human Feedback (RLHF) or Direct Preference Optimization (DPO) used to align a model's outputs with human preferences.] that teach a model what not to propose. The evaluation should measure avoided repeated failures, transfer to nearby design spaces, and the risk of overgeneralizing one failure into a false universal rule (see @sec-provenance-coverage-and-negative-traces).
