...
Integrating autonomous agents with legacy Electronic Design Automation (EDA) workflows presents massive systems engineering challenges. Research in this area must address how to build robust, secure abstraction layers over proprietary foundry tools, how to distill days-long synthesis logs into actionable state observations, and how to accurately backtrace physical sign-off violations to upstream generative architectural mutations.


**Theme 1: The EDA Abstraction Layer**
* How do we build unified, robust programmatic wrappers around fragmented, legacy vendor tools to provide a seamless interface for RL environments?
* What standardized observation spaces can capture the full, complex state of a proprietary synthesis run for ingestion by an open-source agent?
* Can we develop environment APIs that natively handle the non-deterministic crashes and licensing failures inherent to massive EDA deployments?

**Theme 2: Secure and Asynchronous Execution**
* How can cryptographic environment wrappers expose dense state observations to an optimizer while rigorously shielding proprietary foundry DRC rules?
* How must agents be architected to maintain state and context when physical synthesis feedback takes multiple days to compute?
* What protocols ensure that an agent cannot exfiltrate confidential PDK parameters by intentionally failing synthesis runs and observing the error logs?

**Theme 3: Bridging the Sim-to-Silicon Gap**
* What probabilistic models can accurately backtrace asynchronous, late-stage physical sign-off violations to the exact upstream generative RTL mutation responsible?
* How can massive, unstructured EDA textual logs be distilled into dense, actionable architectural state observations while preserving the exact anomalies dictating sign-off failure?
* Can we train diagnostic agents specifically to interpret routing congestion heatmaps and translate them into architectural parameter adjustments?
