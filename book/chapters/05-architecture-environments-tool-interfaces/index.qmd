...
Fundamentally, this abstraction can be described as a contract. The environment is literal. It is the toolchain exposed as an executable sandbox. The environment does not compute a scalar reinforcement-learning reward. It emits raw observations and cost telemetry, such as cycle counts, gate area, and test failures. The loop's rejection authority evaluates this multi-dimensional telemetry against our claims. If we let tools compute a scalar reward directly, we invite reward hacking^[Reward hacking occurs when an AI agent finds an unintended shortcut to maximize its reward signal without solving the underlying problem.] where the loop optimizes for simulator bugs instead of physical reality. The contract requires that the loop make a small set of obligations explicit (@tbl-environment-contract).
...
invalid-action, escalation, and human-review rules, which @tbl-archgym-mini-contract reads as an ArchGym-style mini-contract.
...
**Lesson:** An architecture environment that hands a tool a scalar reward invites the same move on a simulator's blind spots, so the environment, not the optimizer, must own the invalid-action and baseline checks that keep the proxy honest.
:::
