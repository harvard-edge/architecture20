...
```{=latex}
\abstract*{Architecture 2.0 requires tools placed inside AI-assisted architecture loops to become environments, the tool-facing part of the loop contract made executable. This chapter treats ISA boundaries, compiler IRs, memory models, simulator APIs, profilers, RTL flows, EDA handoffs, runtimes, benchmarks, and telemetry systems as explicit places where automated systems and human reviewers can take design actions, observe feedback, log costs and constraints, and reject invalid or unsupported candidates.}
```
...
environment cannot see. For example, an automated memory partitioner might find a layout that perfectly hides latency in the simulator's memory model, only to fail logic synthesis because it routed thousands of wires across a single chiplet boundary. A workload harness may reward one benchmark version
...
integrity record, provenance scope, and stop or review paths. A strong
