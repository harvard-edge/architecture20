...
::: {.callout-design-principle title="Separate the read, action, and return paths"}
Give the environment three explicit interfaces and keep them distinct. The read path supplies state, permissions, and budget before an action. The action path accepts only typed, legal transformations. The return path carries typed observations, provenance, and terminal status. When one wrapper blurs these paths, a study cannot tell which inputs the tool saw, which action ran, or what a result means. Keep the environment reporting observations, and leave the architecture decision to the architect who reviews them.
:::
