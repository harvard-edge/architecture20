...
In modern machine learning, an agent typically interacts with an environment (such as an OpenAI Gym simulation) via a clean, synchronous loop: the agent takes an action, and the environment instantly returns a new state and a dense scalar reward. For decades, architecture tools such as CACTI for memory modeling [@MuralimanoharEtAl2009CACTI], DRAMSys for memory controllers [@WeisEtAl2021DRAMSys], or gem5 for full-system simulation [@BinkertEtAl2011Gem5] were engineered to support human intuition and manual workflows, not programmatic APIs. They assume an architect who can read dense log files, debug build errors interactively, or interpret color-coded waveforms in an RTL viewer. Inserting a custom vector block into an XR subsystem requires modifying Chisel, a hardware construction language used to generate RTL, changing linker scripts, and hacking a Device Tree Blob (DTB), all disconnected from a clean API.
...
For a complex System-on-Chip (SoC) fulfilling the **Lighthouse prompt**, orchestrating the design intent requires managing the Chipyard generator framework through a directed acyclic graph (DAG) of dependencies. Integrating the low-power RISC-V compute subsystem with custom XR accelerators requires explicit lowering from MLIR, a reusable multi-level compiler intermediate representation, to RTL, along with related compiler IR transformations. The dataflow must orchestrate several precise stages: an agent's high-level intent first triggers an MLIR lowering to emit new spatial-tracking instructions, which then informs a hardware target feature declaration within the Chipyard configuration. This configuration generates RTL, which in turn feeds both a rebuilt custom Linux binary for a functional correctness run in Verilator, an open-source RTL simulator, and a physical logic synthesis flow to check routing congestion and verify the power budget. The environment acts as the arbiter of this DAG, ensuring that some branches execute in parallel while others block until earlier artifacts exist, and exposing exactly where the agent is allowed to intervene.
...
7. **How can the AI method behind an architecture environment be swapped without invalidating its prior evidence or leaking a protected parameter?**
   ...
   This chapter's claim that the harness, not the agent, is the deliverable is tested by time. A three-year silicon program cannot have its optimization interface deprecated, rate-limited, or re-priced under it, and it cannot re-run finished evaluations every time the tools, schemas, or AI method change. What is not yet settled is which interface guarantees let an environment swap the AI method behind it, including a strictly on-premise model, while keeping prior evidence comparable and keeping every RTL, PDK, and foundry-rule parameter inside the harness boundary. Today's brittle script wrappers and one-off environment bindings, together with vendor-hosted APIs that can change under a program, are the baseline. The comparison should count evaluations that stay valid across a model swap, interface changes that force earlier runs to be repeated, and protected parameters that ever leave the boundary, judged over the length of a chip program rather than a single study.
