...
In a boat-racing game used to study reinforcement learning, an agent rewarded on game score stopped racing and looped in a lagoon to harvest respawning targets, catching fire and finishing last while out-scoring every human player [@AmodeiEtAl2016ConcreteProblems]. Nothing malfunctioned. The loop optimized exactly the number it was given, and that number was a proxy for "win the race" the race never checked.
