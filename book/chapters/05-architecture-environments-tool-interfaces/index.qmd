...
> authority, and commitment boundaries for the tools a design loop acts through.
...
feedback budgets, invalid-action rules, provenance records, and commitment
boundaries. Otherwise a result may compare private worlds rather than
...
| **Feedback receipt** | Tool/version hashes, seed records, queue or license state, elapsed time, and artifact pointers. | License-server behavior, internal tool telemetry, and nonpublic debug artifacts. |
| **Rejection authority** | Gate type, reviewer role, escalation condition, waiver status, and commitment level blocked. | Product review details, customer commitments, waiver rationale, and named approvers. |
...
: **Closed flows can still expose review contracts:** Architecture 2.0 should converge on receipts, field names, and review semantics around commercial EDA and foundry flows where possible, without prescribing vendor implementations, foundry-rule semantics, signoff meaning, or protected design state. {#tbl-standardizable-proprietary-contract tbl-colwidths="[18,34,38]"}
...
Once an environment is structured to support these composable agents, the
hardest design choice is often the feedback regime, not the action schema.

> **Feedback regime.** A feedback regime is a class of checks at a particular
> fidelity and cost, from cheap proxies to implementation, deployment, or
> silicon evidence. The commitment under review is recorded separately.

Architecture feedback ranges from cheap and weak to slow
...
This dynamic creates an economy of evidence. Cheap feedback buys breadth, pruning, and parallel scaling.
Expensive feedback may expose behavior that cheaper checks miss, but cost or
fidelity does not confer authority. Authority comes from the declared contract,
scope, validation, and independence of the check. As visualized in @fig-fidelity-cost-confidence,
this economy dictates the fundamental pressure of the design loop. As feedback moves toward implementation and
deployment, the loop usually spends orders of magnitude more time per sample, gains less freedom to
explore, and needs dramatically clearer justification for every action it takes. The ranges
...
quantification affordable[^fn-stats-uncertainty-quantification] [@PeherstorferWillcoxGunzburger2018Multifidelity].
Architecture 2.0 inherits that economy and adds two architecture-specific
requirements. Each feedback level must declare what its checks may reject, under
which validation and independence assumptions, and when escalation should spend
a different evidence budget. Higher-fidelity feedback may be required before a
stronger commitment, but obtaining it does not authorize that commitment by
itself.

[^fn-stats-uncertainty-quantification]: In this chapter, recording where an environment's feedback is uncertain enough to force escalation rather than commitment.
...
that extends the sample-cost regimes of
...
economics permits (@tbl-feedback-latency-fidelity). A loop that confuses those regimes can search quickly and
...
| Cycle-level simulation | Minutes to days depending on model detail and target workload. | Tens to hundreds of scoped architecture comparisons. | Declared candidate-state violations, unsupported candidate states, calibration failures, or sensitivity failures; setup and configuration errors remain environment failures. | Use staged design-space exploration with replay and escalation. |
...
: **Feedback regimes create evidence economics:** A method should match the latency and sample budget of the feedback it receives, while the loop separately declares each check's scope, rejection authority, and relationship to the proposed commitment. Representative ranges are project-dependent and should be replaced by local source receipts in a real loop. {#tbl-feedback-latency-fidelity tbl-colwidths="[17,19,17,22,17]"}
...
This fidelity regime structure therefore drives method choice. If feedback is cheap, broad search
or online adaptation may be reasonable. If feedback is expensive, the loop
needs sample efficiency, priors, surrogates, active learning[^fn-ml-active-learning], staged gates, or
stronger human filtering. If the proposed decision is high consequence or hard
to reverse, the loop should become more conservative. Use AI to organize
evidence, critique assumptions, and narrow the search, not to make unsupported
final decisions.

[^fn-ml-active-learning]: In this chapter, a policy for choosing which scarce simulator, synthesis, human, or deployment feedback event to spend next, with the choice logged as part of the evidence ledger [@Settles2009ActiveLearning].
...
decision that requires, foreshadows @sec-feedback-verification-trust. First,
@sec-methods-generation-prediction-optimization uses the same feedback budget to
decide which method roles belong inside the loop.
...
7. Log provenance, environment failures, and negative traces, preserving tool versions, seeds, workload versions, failed runs, rejected alternatives, and reasons for each disposition.
8. State the human decision rule, defining what the architect reviews, what can be accepted automatically, and what must escalate.
...
Operating discipline, the architecture counterpart of MLOps[^fn-systems-mlops], is the practice of maintaining those layers over time. A one-time environment can
...
[^fn-systems-mlops]: MLOps is the operating discipline for deployed learning systems, including versioning, monitoring, drift detection, rollback, and ownership records [@SculleyEtAl2015Hidden; @BeyerEtAl2016SRE]. This chapter borrows that discipline for architecture loops, where workloads, tools, evidence, and rejection gates also have to be maintained after the first run.

> **ArchOps.** ArchOps is the operating discipline for Architecture 2.0
> environments. It versions workloads, tools, action schemas, evidence rules,
> negative traces, and decision owners so drift does not silently change what a
> loop result means.
...
The maintenance work can be stated in operational
terms (@tbl-archops-operating-discipline). The table is intentionally prosaic because the discipline succeeds only when
...
| Evidence and rejection rules | Fidelity labels, escalation thresholds, signoff gates, waivers, and reviewer decisions. | A proxy keeps authority after the commitment level has risen. | Recalibrate the evidence rule and require review whose declared scope, validation, and independence support the new commitment. |
...
engineering. They ask how tools become valid action spaces, evidence sources,
and rejection authorities for architecture work.
...
integrity record, provenance scope, and rejection authority. A strong
...
invalid-action labels, and rejection authorities. The evaluation should measure
...
needed for a field. A concrete systems paper would build an ArchOps monitor
...
operating discipline in @sec-archops-operating-discipline.
...
provenance, and let a rejection authority stop an illegal, unsupported, or
under-evidenced candidate. That is the whole content of turning a tool into an
environment.
...
layer by layer, across workload, action, observation, fidelity, provenance, and
rejection, before an environment's verdict is worth believing.
...
completely by the tenth. This architecture-specific operating discipline is what
versions workloads and tools, preserves provenance and negative traces, monitors
drift, and keeps architect-owned rejection gates alive.
Building the environment earns a paper. Maintaining it is what lets a loop, and
a field, keep trusting the evidence.

::: {.callout-carry-forward}
- **Carry forward:** An environment is a contract over legal actions, returned feedback, and rejection conditions. That contract lets another method act in the same harness.
