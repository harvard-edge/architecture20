...
1. **Zero-Shot Translation from Opaque Tool Telemetry:** What methods allow an AI system to distill the unstructured, tens-of-thousands-of-lines output from proprietary EDA tools into dense, actionable state observations without losing the single anomalous line that dictates physical failure?
2. **Causal Attribution in Asynchronous Feedback Loops:** When a design fails late in the physical sign-off process, what algorithms can probabilistically backtrace the failure to the specific upstream generative action (e.g., a specific compiler transform or RTL parameter) that caused it?
3. **Secure Agentic Interfaces for Confidential PDKs:** How can we design cryptographic or privacy-preserving environment interfaces that allow an AI model to optimize against foundry-specific constraints without the proprietary parameters ever leaving the secure harness boundary?
