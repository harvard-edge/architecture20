...
For decades, architecture tools such as CACTI for memory modeling [@MuralimanoharEtAl2009CACTI], DRAMSys for memory controllers [@WeisEtAl2021DRAMSys], or gem5 for full-system simulation [@BinkertEtAl2011Gem5] were painstakingly engineered to support human intuition. They assume an architect sitting at a workstation, interacting through Graphical User Interfaces (GUIs), interpreting color-coded waveforms, and entering commands into an interactive shell. Inserting a custom vector block into an XR subsystem requires modifying Chisel code, changing linker scripts, and hacking a Device Tree Blob (DTB)---tasks completely disconnected from a clean API.
...
Hardware design is bound by severe economic constraints. Unlike RL agents playing Atari at thousands of frames per second, architecture tools are locked behind expensive licenses and days-long Slurm queues. The compute required for a single physical design signoff can be *orders of magnitude* slower and more expensive than an AI model's inference pass. As fidelity increases toward silicon, the sample budget shrinks exponentially from millions of cheap functional checks to handfuls of expensive layout runs.uns.
...
The machine learning community has long warned of **Reward Hacking** (Goodhart's Law). If an AI is given a simple scalar reward, it will find loopholes to maximize that number while violating physical intent. In architecture, if an environment merely reports IPC as a reward, the Generator will "hack" the World Model by hallucinating a physically impossible 10-Terabyte L1 cache. Physical ground truth from traditional tools remains indispensable to enforce laws like timing closure and power limits, preventing such hacking.
...
When interfacing Large Language Models (LLMs) with traditional architecture environments, a fundamental impedance mismatch occurs at the observation layer. A commercial EDA synthesis tool or a detailed cycle-accurate simulator like gem5 generates exhaustive text output containing tens of thousands of lines. If an environment wrapper naively pipes raw stdout directly back to an LLM, the agent suffers from an attention bottleneck. Even with million-token context windows, flooding the prompt with gigabytes of raw trace data leads to "lost in the middle" attention failures, causing the model to hallucinate fixes based on spurious correlations.

A critical requirement of the Return Path is **Data Distillation**^[Not to be confused with *knowledge distillation* in machine learning [@HintonEtAl2015Distilling], where a small "student" neural network is trained to mimic a large "teacher" model. In our context, data distillation is the programmatic filtering of massive, unstructured tool logs into dense semantic observations before they reach the AI agent.]. Because traditional architecture and EDA tools are excessively verbose, the architect must build rigid **log-to-semantic parsers** into the environment wrapper. Much like DevOps engineers use monitoring tools to distill millions of server logs into a single red or green dashboard light, architecture environments must translate sprawling, human-targeted outputs into a single, unified mathematical observation vector. If the AI is trying to solve routing congestion, the environment wrapper must parse the EDA log, ignore the 9,900 irrelevant setup warnings, extract the 10 specific design-rule violations, and return *only* that dense semantic observation.
...
![Artifact Lineage and Cryptographic Provenance. A single Lighthouse intent fans out into software binaries and hardware RTL, each hashed to ensure reproducible evaluations.](images/F5-artifact-lineage.svg){#fig-artifact-lineage}
