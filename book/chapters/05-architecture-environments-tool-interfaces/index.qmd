...
Fundamentally, this abstraction can be described as a contract. The environment is literal. It is the toolchain exposed as an executable sandbox. An environment may expose a scalar objective to a method, but it should not collapse the architecture claim into that scalar. The contract must preserve raw observations and cost telemetry, such as cycle counts, gate area, test failures, invalid actions, and stronger rejection checks. Otherwise the loop invites reward hacking^[Reward hacking occurs when an AI agent finds an unintended shortcut to maximize its reward signal without solving the underlying problem.], where a method optimizes for simulator bugs instead of physical reality. The contract requires that the loop make a small set of obligations explicit (@tbl-environment-contract).
...
::: {.callout-war-story}
**Two private scripts.** The AlphaChip dispute is, at bottom, a story about missing shared conditions. The original team and its critics did not disagree only about a number. They ran the method in different setups, on different testcases, with different baselines and compute budgets, and each read the result through its own script [@ChengEtAl2023AssessmentRL; @GoldieEtAl2024ThatChipHasSailed].

**Why it could not be settled.** When two parties compare under private, non-shared setups, there is no way to separate a real difference from a difference in conditions. The authors could argue the critics had not run the method as published, and the critics could argue the method did not beat a strong baseline, and both could be right about their own environment. The environment, not the algorithm, was the ambiguity.

**What an environment fixes.** This is why a shared environment changes the research question. Fix the task, the action space, the feedback, the workloads, and the baseline, and the comparison becomes a question about the method rather than the setup. A claim is only as credible as the conditions under which it can be replayed.
:::
...
> **Active learning:** In this chapter, a policy for choosing which scarce simulator, synthesis, human, or deployment feedback event to spend next, with the choice logged as part of the evidence ledger [@Settles2009ActiveLearning].
...
Environment design should therefore include red-team checks^[Red-teaming involves intentionally attacking a system to expose vulnerabilities, flaws, and failure modes before deployment.] for the feedback
...
Operating discipline, the architecture counterpart of MLOps, is the practice of maintaining those layers over time. A one-time environment can
