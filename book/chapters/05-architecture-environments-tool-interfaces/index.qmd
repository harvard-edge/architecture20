...
**The gap.** When two parties compare under private, non-shared setups, there is no way to separate a real difference from a difference in conditions. The authors could argue the critics had not run the method as published, and the critics could argue the method did not beat a strong baseline, and both could be right about their own environment. The environment, not the algorithm, was the ambiguity.
...
[^fn-systems-mlops]: In this chapter, the operating discipline for deployed learning systems that motivates similar versioning, monitoring, drift, rollback, and ownership records in Architecture 2.0 loops.
...
completely by the tenth. Operating discipline, the architecture counterpart of
MLOps, is what versions workloads and tools, preserves provenance and negative
traces, monitors drift, and keeps architect-owned rejection gates alive.
...
- **Carry forward:** An environment is a contract, the legal actions, the feedback they return, and what can reject a result, so another method can act in the same harness.
