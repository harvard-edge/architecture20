...
![**The Architecture 2.0 design loop:** State, a bounded action, tool feedback, an evidence ledger, and a human commitment form one turn, with rejected candidates fed back as negative traces. The evidence ledger and human commitment are what a generic reinforcement-learning loop lacks. They replace a scalar reward, so a rejection authority, not a number, decides what advances. As fidelity, cost, and commitment rise, the autonomy the loop can safely grant narrows.](images/F3-design-loop){#fig-design-loop width="100%" fig-alt="A left-to-right Architecture 2.0 design loop with boxes for represented state, a bounded action, tool feedback, an evidence ledger, and a human commitment, and a dashed return path where rejected candidates become negative traces, above an axis showing that as fidelity, cost, and commitment rise, acceptable autonomy narrows."}
...
a claim can be compared, reproduced, and contested by someone who never ran the
loop. The design-loop card is simply that object made routine.
