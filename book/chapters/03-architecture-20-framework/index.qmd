...
Both entities are critical, but while the world model focuses on prediction and belief, the evidence ledger records what was observed, under which assumptions and fidelity regime, and how the result was accepted, rejected, or escalated. In this chapter, the ledger's job is concrete: tie a candidate to the feedback that evaluated it, the constraints it faced, and the decision that accepted or rejected it. To flatten the learning curve, consider these terms as the automated equivalent of a scientific peer-review loop or a Design Space Exploration (DSE) review board. A *rejection gate* is analogous to a reviewer rejecting a paper for a methodological flaw, and an *evidence ledger* is a multidimensional record capturing the Pareto tradeoffs and justifying exactly *why* alternative architectures were rejected during exploration.
...
::: {.callout-war-story title="A claim with empty card fields"}
**The claim.** Read AlphaChip through the design-loop card and the public review questions become clearer. The generation fields are strong. The intent and task are clear, and the method proposed macro placements at reported expert quality [@MirhoseiniEtAl2021GraphPlacement].
...
**The lesson.** A card with those gaps does not say the work is wrong. It says the public claim needs narrower wording or stronger shared evidence, and it names exactly what to ask for: a shared baseline, a runnable environment, higher-fidelity evidence, and an owner who can reject. State the claim as a review object and the argument stops being about reputation and starts being about the missing fields.
:::
