...
measurement, feedback, and judgment. The discontinuity is its *automation*. Historically, humans drove this loop; in Architecture 2.0, AI systems can execute steps of it programmatically, acting inside the loop while the architect owns the commitment. That automation can change the failure mode. When verification and rejection mechanisms collapse, plausible errors can propagate faster and reach higher commitment levels before humans catch them.
...
The loop itself must be formalized as a five-part execution state, comprising what state it saw, what actions it allowed, what alternatives it rejected, what evidence supports the result, and who owns the commitment if the claim is wrong
...
2.0 makes the same move for computing-system design. The alternative is not creative
...
The tuple becomes a reader checklist for the lighthouse prompt (@tbl-architectural-claim-schema). The important point is that the prompt's
...
In practice, the loop has at least the following elements:

- **State:** what is known about the workload, design, tools, constraints, and prior evidence.
- **Actions:** what can be changed, generated, queried, tuned, or tested.
- **Observations:** what the loop can see after an action.
- **Objectives and constraints:** what counts as progress and what is not allowed.
- **Feedback path:** the measurement, simulation, synthesis report, trace, review, or deployment signal returned by the environment.
- **Stopping and escalation rules.**
- **Decisions:** accept, reject, revise, or request stronger evidence.
...
loop is a Markov decision process[^fn-rl-mdp]. It is a bookkeeping discipline. If a loop
...
model and still leave the important state invisible, such as what task was bounded, what
...
leans on them. These are the world model the loop reasons with, and the evidence ledger it
reasons from.

> **World model.** Borrowing the AI/control term for a system's predictive model
> of action consequences [@HaSchmidhuber2018WorldModels; @SuttonBarto2018ReinforcementLearning],
> a world model is the loop's architecture-scoped belief about how actions change
> outcomes, whether that belief lives in a simulator, a learned surrogate, a cost
> model, or design rules. This chapter uses the term only at that working level;
> @sec-data-representations-world-models gives the canonical definition and shows
> what such a model must encode, scope, and keep credible.

Both entities are critical, but they do different work. The world model carries
the loop's prediction about what an action will do. The evidence
ledger records what was actually observed, under which assumptions and fidelity
regime, and how the result was accepted, rejected, or escalated. In this
chapter, the ledger's job is concrete. It ties a candidate to the feedback that
evaluated it, the constraints it faced, and the decision that accepted or
rejected it. A rejection gate is the mechanism that can stop the candidate; the
evidence ledger is the record that explains why the gate fired, why it did not,
or why the claim had to be narrowed.
...
a paper uses an LLM[^fn-ml-llm], reinforcement learning[^fn-rl-method-label], Bayesian optimization, a surrogate
...
Several things must become explicit before an AI method can participate in architecture work (@fig-ontology-chain). Read it left to right as a chain of
...
The change is that the loop, not the model, becomes the review object.

For practical use, this book compresses the ontology into *five framework
elements*. They group the same twelve fields the design-loop card records,
including intent, task, design space, representation plus world model, environment,
method role, feedback budget, evidence, negative traces, rejection authority,
commitment boundary, and human decision. These elements name what a loop is
*made of*, distinct from the *five-part execution state* of @sec-moonshot, which
names what one turn *records*. There is one reader-facing artifact, the
twelve-field design-loop card. Every other list in this book, the five framework
elements, the five-part execution state, the nine-field claim tuple, and the
seven-question loop contract, is a view of it
(@sec-appendix-b-design-loop-card), not a competing schema.
...
the encoded design state, including specifications, workload traces, architecture
...
unit is rarely a single model. It is a composition of roles, such as generator,
...
decision. For example, a power prediction is just a number until it is tied to the specific simulator version, PDK assumptions, and workload traces that produced it. The decision is where the architect accepts, rejects, revises, or
escalates the result.

The checklist version is the question a reader should be able to ask of a
paper, benchmark, tool, or internal loop before trusting an Architecture 2.0
claim (@tbl-compact-framework-checklist).
...
(@tbl-architecture-translation-matrix).
...
Allowed loop authority rises through four stages (@fig-autonomy-ladder-human-agent). The point is not that the automated system gradually replaces the human
...
subtasks, such as to search a design space, tune a configuration, generate a benchmark
...
It must be translated into bounded work, such as to characterize the workload, choose a
...
To ground this translation of intent into tasks, this book treats several task families as recurring, including design-space exploration,
...
compound systems, including simulators, models, scripts, profilers, spreadsheets,
...
- Which result or check would invalidate the candidate?
- Who owns rejection authority at this gate?
...
baseline, represented state, environment, method roles, evidence ledger,
rejection authority, and commitment boundary. A strong contribution would turn
those fields into a claim packet that reviewers can inspect without sharing the
...
environment it acts in, the method roles that produced it, the feedback turned
into an evidence ledger, the authority that can reject it, and the commitment
boundary that stays human-owned. Stated as a review object rather than a result,
...
- **Up next:** The next chapter asks what representations and world models must encode before an AI-assisted loop can act on architecture state credibly.
:::
