...
commitment boundary are visible. In card terms, $W$ and $B$ anchor the task and evidence comparison, $\mathcal{D}$
...
The ontology and framework proposed here map the known requirements for Architecture 2.0. The next research step is to turn those requirements into checkable objects that reviewers, tools, and authors can use without pretending that all labs share the same infrastructure.

1. **Which parts of the design-loop card can become an executable claim packet?**
The design-loop card (@sec-appendix-b-design-loop-card) is a review object, but a paper-quality research question asks which fields can be checked automatically and which must remain human judgment. A strong contribution would define an executable schema for tasks, baselines, constraints, evidence ledgers, and rejection gates, then evaluate whether independent reviewers can verify a claim packet without access to the original lab's full toolchain. Cryptographic proofs, including zero-knowledge proofs that verify a statement without revealing the underlying private data, are useful only if they help this concrete audit problem.

2. **What are the limits of turning legacy tools into formal environments?**
Decades of simulators, compilers, and EDA flows were not designed as clean reinforcement-learning environments. A thesis here would build an environment contract for one or more legacy tools, define legal actions and invalid-action semantics, and then stress-test whether an optimizer can exploit abstraction gaps to fabricate false Pareto frontiers. The contribution would be strongest if it reports both the useful boundary of the wrapper and the failure cases that still require human review (see @sec-architecture-environments-tool-interfaces).

3. **How can rejected candidates become causal architecture state?**
Current AI methods often discard the failure traces that teach human architects the boundaries of a design space. A strong paper would define a representation for rejected candidates, failed timing closures, proxy-fidelity mismatches, and repaired alternatives, then test whether that representation improves prediction of invalid regions or reduces repeated failures. The thesis-level version would connect those negative paths to a causal world model of architecture state rather than a bag of error messages (see @sec-arch2-ontology).

4. **Can mechanically checked commit gates preserve human authority at high loop velocity?**
As automated loops scale toward bounded autonomous ecosystems (@fig-autonomy-ladder-human-agent), a policy statement is not enough to keep generation separate from commitment. A concrete systems contribution would implement commit-gate protocols that bind a candidate's evidence level to the maximum claim it may authorize. The evaluation should include adversarial or buggy agents attempting to cross fidelity boundaries without explicit human approval, so the paper measures whether the protocol actually prevents unauthorized commitments.
