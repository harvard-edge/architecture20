...
![**The Architecture 2.0 loop tuple:** The design-loop update $s,a,o,e,d$ extends a generic reinforcement-learning loop's state, action, and observation with an evidence ledger and a human decision path, so a scalar reward or objective can guide search without becoming the final authority for commitment.](images/F3-architecture-tuple){#fig-architecture-tuple width="100%" fig-alt="Block diagram: state to action to observation to an evidence ledger to a human decision, with a loop back to state. State, action, and observation are marked as shared with a generic reinforcement-learning or Gym loop; the evidence ledger and human decision are marked as the Architecture 2.0 extension that prevents a scalar reward from becoming final commitment authority."}
