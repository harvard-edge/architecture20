...
Both entities are critical, but while the world model focuses on prediction and belief, the evidence ledger records what was observed, under which assumptions and fidelity regime, and how the result was accepted, rejected, or escalated.
...
**The claim.** Read AlphaChip through the design-loop card and the review does itself. The generation fields are strong. The intent and task are clear, and the method proposed macro placements at reported expert quality [@MirhoseiniEtAl2021GraphPlacement].

**The gap.** The fields that decide trust were the ones left blank. The baseline others could run was disputed, the environment and testcases were not shared, the public, independently runnable evidence did not carry the contested comparison through shared post-route metrics, and no independent rejection authority could clear the result [@ChengEtAl2023AssessmentRL].

**The lesson.** A card with those blanks does not say the work is wrong. It says the claim cannot be accepted as it stands, and it names exactly what to ask for: a shared baseline, a runnable environment, higher-fidelity evidence, and an owner who can reject. State the claim as a review object and the argument stops being about reputation and starts being about the missing fields.
:::
