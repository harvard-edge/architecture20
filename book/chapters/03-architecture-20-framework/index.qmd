...
::: {.callout-war-story}
**The claim as a card.** Read AlphaChip through the design-loop card and the review does itself. The generation fields are strong. The intent and task are clear, and the method proposed macro placements at reported expert quality [@MirhoseiniEtAl2021GraphPlacement].

**The empty fields.** The fields that decide trust were the ones left blank. The baseline others could run was disputed, the environment and testcases were not shared, the public, independently runnable evidence did not carry the contested comparison through shared post-route metrics, and no independent rejection authority could clear the result [@ChengEtAl2023AssessmentRL]. Each blank is a review outcome, not a formatting gap.

**The verdict the card forces.** A card with those blanks does not say the work is wrong. It says the claim cannot be accepted as it stands, and it names exactly what to ask for: a shared baseline, a runnable environment, higher-fidelity evidence, and an owner who can reject. State the claim as a review object and the argument stops being about reputation and starts being about the missing fields.
:::
