...
failure modes require different remedies and can reinforce one another.

The lighthouse prompt makes that difficulty concrete. It asks an architect to
...
The lighthouse prompt expands a choice among a CPU, accelerator, or SoC block
...
The public AlphaChip debate provides a neutral case for a narrower question:
does faster candidate production reveal what is slowing architecture work? We
do not use the case here to judge whether learned placement is effective.
...
evaluation, review, and rejection. Equal proposal and simulator budgets are
needed to separate an architectural result from the question of whether AI
helped produce it.
...
| Proxy metrics are enough | Proxy wins can vanish under stronger checks or different workloads. | State which conclusions a proxy cannot settle. |
