...
**The lesson.** This is the scissors gap in a single result. A loop is rejection-bound, not generation-bound, and faster generation without matching rejection capacity widens the gap rather than closing it. A loop earns trust through what it can independently reject, not through how much it can produce.
:::
...
reject only widens the gap. Generation has become cheap while rejection remains
expensive. Once that becomes true, the
...
Iron Law but Amdahl's. @sec-loop-patterns-across-stack shows that a design
loop's throughput is set by how cheaply it can reject, not by how many
candidates it proposes, and casts that bottleneck as an Amdahl-like bound with
trusted rejection as the serial fraction. Architecture 2.0 elevates the
...
trusted feedback can keep up widens the scissors gap rather than closing it.
Amdahl's Law offers the useful analogy. A loop's speedup is limited by the part
that cannot be parallelized or automated. Here, the limiting serial fraction is
trusted, independent rejection capacity (see @fig-rejection-bottleneck). That
capacity has two parts, and they age differently. How *fast* the loop can reject
is a throughput limit, and cheap, independent, increasingly automated rejectors
keep raising it. Who is *accountable* for the commitment if a claim is wrong is
not a speed limit at all; it is a governance floor that stays human even as
verification gets faster. @sec-loop-patterns-across-stack makes the throughput
bound precise and shows how much of it an automated rejector can discharge
before it reaches what only accountable judgment can clear.

![**The Rejection Bottleneck:** No matter how many candidates an AI loop can generate (millions) or how fast surrogate models can screen them (thousands), the total throughput of an architectural exploration loop is bottlenecked by the serial capacity of trusted, independent rejection, illustrated here by the human-review floor that only accountable judgment can clear.](images/F2-rejection-bottleneck){#fig-rejection-bottleneck width="100%" fig-alt="A funnel diagram showing 1,000,000 AI generated candidates hitting a bottleneck of 5 manual human reviews per day."}
