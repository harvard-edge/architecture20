...
The challenge is that composition adds a dense layer of coupled choices. A chiplet system is more than a larger board-level system inside a package; the package alters latency, bandwidth, energy, thermal coupling, and physical constraints. Furthermore, a modern SoC integrates dozens to hundreds of heterogeneous IP blocks from different vendors. Coherence-protocol mismatches, unstated timing constraints, and system-level deadlocks often surface only when the full SoC is assembled.[^fn-soc-deadlocks-c02] Each added block narrows the set of valid designs. Moving a function across a chiplet boundary changes the available interfaces, software path, and tools needed to evaluate it.
...
Even when a hardware candidate clears physical constraints and evaluation budgets, software must still be able to use it. This is the executable software path: the programmers, compilers, runtimes, libraries, and deployment systems needed to use a hardware mechanism without losing its benefit to data movement or overhead. Historical shifts like RISC and CUDA succeeded because they provided the necessary compiler support and programming models to make the hardware accessible.
