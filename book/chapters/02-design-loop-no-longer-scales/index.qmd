...
. What matters here is how the comparison is bounded. It
...
| RISC | Workload, compiler, Very Large-Scale Integration (VLSI), and the quantitative cost/performance case became the architectural argument . | A complete comparison includes the workload, compiler, and implementation cost. |
| SPEC-style benchmarking | Workload selection, run rules, reporting conventions, and comparability became community infrastructure . | Benchmarks govern workload choice, execution, and comparison. |
| Logic synthesis and timing closure | Hardware Description Language (HDL), libraries, constraints, and timing reports let downstream checks reject upstream choices . Synthesis tools are not clean API calls; they require massive license servers, runtime budgets, and navigation of brittle failure logs. | Implementation checks can rule out architecture candidates before the project commits to them. |
...
| TPU-block floorplanning | The AlphaChip paper reported reducing placement generation from months of human effort to hours of machine time . | Routed physical quality and strong baselines still require separate evaluation. |
| Tensor-program tuning | AutoTVM, a machine learning-based tensor program optimizer, describes tensor-operator search spaces on the order of billions of possible implementations for a single GPU operator . | Compiler schedules create a hardware-dependent search problem of their own. |
...
#   qualitatively (order-of-magnitude, hollow marker) .
...
### PDN and Thermomechanical Modeling
...
parallelism . Intel's Larrabee moved specific graphics
...
, the architect must still decide which traces, model
...
difference . A compute-only comparison can therefore rank
...
#| fig-cap: |
#|   **Data movement can dominate arithmetic energy.** Rough Horowitz 45\ nm energy values show why architecture comparisons must represent locality, buffering, precision, scheduling, and memory hierarchy rather than counting arithmetic alone . The values are transcribed order-of-magnitude anchors, including the source's 1.3--2.6\ nJ DRAM range, not current-node device estimates.
#| out-width: "92%"
...
. Multifidelity methods use the same basic strategy
...
macro placements in hours , and a later public
