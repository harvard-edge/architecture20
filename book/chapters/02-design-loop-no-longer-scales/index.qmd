...
The strain on classical design loops reveals that AI in architecture is not merely a generation problem. It is a systems-design challenge created by scarce feedback, fast-moving software, and design spaces too large to exhaustively inspect. The questions in this chapter should be read as paper seeds for the strained-loop regime.

1. **What is the minimum inspectable loop state?**
How can we formally define the smallest state record, spanning task intent, legal actions, candidate provenance, evidence spent, and rejected alternatives, that makes an AI-generated architecture reviewable? A strong paper would propose a schema and then test whether independent architects can reconstruct the decision path under a fixed high-fidelity sample budget. The result should make the black box of AI generation small enough to audit, not merely better documented.

2. **How can failure and uncertainty become reusable evidence?**
In the absence of abundant open architecture data, loops need ways to learn from proprietary constraints, failed candidates, and tool-version drift without exposing everything. A thesis-level artifact could be a redacted negative-trace format plus an evaluation showing that later searches avoid previously invalid regions while preserving the boundary between cheap warning signals and high-fidelity evidence. The question is especially sharp when physical constraints move into architecture (see @sec-physical-constraints-move-into-architecture).

3. **What rejection hierarchy is safe enough for the scissors gap?**
Cheap triage proxies are necessary because validation is the bottleneck, but they become dangerous when they silently pass invalid candidates or reject valuable ones. A top architecture or systems paper would construct a hierarchy of rejection tests, estimate its false-accept and false-reject behavior, and report the strongest commitment that each level is allowed to authorize. This keeps the focus on feedback scarcity rather than treating faster search as progress by itself (see @sec-feedback-and-verification-become-the-bottleneck).

4. **How should architectural claims carry a validity horizon?**
Claims tied to workloads, models, compilers, and runtimes can expire before a chip decision reaches maturity. A concrete contribution would package an architecture claim with versioned workload snapshots, software assumptions, and re-evaluation triggers that fire when the support for the claim drifts. The evaluation could replay historical software or workload changes to show when a claim should be maintained, weakened, or withdrawn (see @sec-software-changes-faster-than-silicon).
