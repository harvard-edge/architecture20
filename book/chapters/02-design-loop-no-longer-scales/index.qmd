...
reject only widens the gap. The shorthand is simple: generation has become
cheap while rejection remains expensive. Once that becomes true, the
...
**The claim.** Learned placement was reported to compress a months-long floorplanning loop to hours, and generating strong-looking placements only got faster [@MirhoseiniEtAl2021GraphPlacement]. That is the half of AlphaChip everyone remembers.
...
The pressure is structural, not a matter of taste. Two walls converge on the
classical loop. One is physical, built from specialization, heterogeneous
integration, and data movement. The other is verification, built from software
velocity and validation burden. Between them opens a scissors gap, in which
generative methods raise the rate of plausible proposals far faster than any
tool raises the rate of trusted rejection.
...
- **Up next:** AI helps only when the loop records state, failures, evidence, rejection authority, and each commitment's human owner; the discussion on "Architectural Claims and Design Loops" (@sec-architecture-20-ontology) names the minimum loop state that makes that pressure inspectable.
:::
