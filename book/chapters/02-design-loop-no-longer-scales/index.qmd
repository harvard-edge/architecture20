...
::: {.callout-war-story}
**Generation was the easy part.** Learned placement compressed a months-long floorplanning loop to hours, and generating strong-looking placements only got faster [@MirhoseiniEtAl2021GraphPlacement]. That is the half of AlphaChip everyone remembers.

**Rejection was not.** What did not keep pace was trusted, independent rejection. A candidate that looked excellent on the method's fast proxy still had to be checked against the metrics that decide a real chip, and an independent reassessment that tried to run that check reported baselines the result had not cleared in that setup [@ChengEtAl2023AssessmentRL]. The generation side sped up while the accountable rejection side did not, and the space between them is where the dispute lives.

**The bound holds.** This is the scissors gap in a single result. A loop is rejection-bound, not generation-bound, and faster generation without matching rejection capacity widens the gap rather than closing it. A loop earns trust through what it can independently reject, not through how much it can produce.
:::
