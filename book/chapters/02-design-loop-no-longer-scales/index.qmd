...
hindsight. Architecture 2.0 makes the compiler-ILP evidence a gate the bet must
clear before commit, so an architecture can no longer be committed on a software
capability that was asserted rather than measured.
:::
...
entries were wrong, and no cheap check caught it before silicon. The lesson
sharpens under Architecture 2.0. The missing rejection, an automated gate that
could have caught the error early, not the missing entries, set the price.
:::
