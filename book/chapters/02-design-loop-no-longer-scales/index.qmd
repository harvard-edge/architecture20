...
Addressing this crux requires us to examine the specific bottlenecks in the modern Architecture Design Loop, separating the generation of candidates from the rigorous evaluation they demand.
:::
...
Mastering these objectives will provide a concrete foundation for evaluating AI tools not just as novelties, but as practical solutions to architectural bottlenecks.
:::
...
Because AI can generate a vast number of candidate designs, the true bottleneck becomes the finite capacity of high-fidelity simulators and human experts required to verify them. To close this gap, AI can act as more than just a simple filter. Drawing on the economic principle of the *Value of Information*, AI can dynamically route candidates between fast proxies and cycle-accurate simulators, only spending expensive compute if the result is likely to change the architect's final decision. In software engineering, for example, running every continuous integration test on every commit bankrupts infrastructure; instead, systems can predict which tests are likely to fail and skip the rest [@MachalicaEtAl2019PredictiveTestSelection]. Neural architecture search applies a similar principle: Hyperband treats evaluation strictly as a budget-allocation problem, aggressively culling weak candidates using cheap proxies and saving expensive evaluations only for the survivors [@LiEtAl2018Hyperband]. By adopting this approach, architects can optimally allocate their cycle-accurate simulation budgets to evaluate an otherwise unmanageable search space based on the value of the information gained.
...
The ISA defines the static software contract, but modern software stacks move fast. In distributed systems, teams manage rapid changes using "Consumer-Driven Contracts"—executable expectations published by the consumer. Similarly, AI workloads should publish living, executable contracts that hardware models must test against in a CI/CD environment. Compiler and runtime integration, generated code, and executable tests provide the evidence that software can use the proposed subsystem as claimed under these dynamic conditions.

Recognizing this living software contract forces the design loop to evaluate hardware and software dependencies concurrently rather than in isolation.
:::
...
Lessons from the machine learning and broader science communities emphasize how shared datasets and rigorous benchmarks steer entire fields. ImageNet drove a decade of progress in computer vision by standardizing the evaluation task [@RussakovskyEtAl2015ImageNet], while AlphaFold succeeded by combining deep learning with the physical constraints of structural biology to predict protein folding [@JumperEtAl2021AlphaFold]. Architecture faces a similar imperative: AI methods must be grounded in community-maintained datasets and benchmarks that reflect actual design constraints, lest they optimize for the wrong target.
...
Physical effects create tight, non-linear coupling between architectural choices—similar to *epistasis* in biology, where genes interact, or drug discovery, where early candidates fail late due to toxicity. Data movement and thermals are the "toxicity" of architecture. They must be shifted left in the evaluation cycle to avoid late-stage physical failure.
...
The motivation for integrating AI into computer architecture arises not from the appeal of new technology, but from the crushing complexity of the modern design loop. Architecture has always relied on feedback, yet today's projects must synchronize unprecedented amounts of information. Coupled specialization and chiplet choices, evolving software stacks, and stringent physical constraints generate immense search spaces. More critically, they generate evidence requirements that easily outpace a team's simulation capacity and expert review bandwidth. We need AI because manual coordination and traditional heuristics can no longer keep up with the scale of validation and evaluation required to confidently close a design decision.
...
Ultimately, the goal of AI in architecture is not to automate the architect away, but to restore the efficacy of the design loop. Producing more candidates is useless if the checks remain unaffordable or the next action remains unclear. Identifying the limiting engineering pressure is the necessary first step; @sec-architecture-20-ontology will define the bounded study that keeps this AI-assisted work coherent and actionable.
...
By carrying these concepts forward, you will be prepared to integrate AI responsibly, ensuring that each new capability directly targets a verified bottleneck in the design process.
:::
