...
AI can accelerate parts of that work. An agent can collect design information, propose alternatives, and predict PPA (Power, Performance, Area) costs, serving as a powerful explorer and searcher. The first task is to identify what is actually slowing the work, delegating rapid search to the AI so the architect can focus on orchestration and final judgment.
...
downstream check has authority to reject a proposal. For instance, providing an AI with a 500-page ISA manual does not inherently teach it that a specific register-file sizing rule cannot be violated to save area. AI assistance therefore
...
This explosion in specialization implies that the traditional "general purpose" core is increasingly relegated to control plane tasks, while the bulk of the computational heavy lifting is done by a heterogeneous mosaic of Domain Specific Architectures (DSAs). Each new DSA introduces its own bespoke programming model, instruction set, and toolchain. For an AI agent attempting to navigate this space, the challenge is not just proposing a new MAC array, but proposing a complete hardware-software co-design that a real compiler can actually target. If the agent generates a custom MAC array with irregular register strides that no existing LLVM backend can map to, the proposed 'optimal' hardware is effectively useless.
...
results before expert review. By using acquisition functions from Bayesian optimization to measure model uncertainty, the agent can recommend expensive EDA runs only for designs that either push the Pareto frontier or resolve critical unknowns. Used well, that assistance can save compute,
