...
The driver of verification cost is state space explosion. When a designer adds a single state-holding element (like a flip-flop) to an integrated circuit, the size of the design increases linearly, but the number of possible states the system can inhabit doubles ($2^n$). As designs scale, the state space that must be verified grows exponentially, far outpacing the linear growth of the design effort itself.
