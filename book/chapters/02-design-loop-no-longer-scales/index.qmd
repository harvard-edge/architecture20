...
When human-driven heuristic search reaches its mathematical breaking point, we are forced to reevaluate the economics and mechanisms of architectural exploration. The fundamental research challenges in this domain lie in formally defining the inflection points where autonomous agents become necessary, and establishing rigorous bounds to dictate when an agent should trust a fast proxy versus when it must invoke a costly, cycle-accurate oracle.


**Theme 1: Identifying the AI Inflection Point**
* At what exact threshold of structural complexity does heuristic, human-driven search mathematically break down and necessitate autonomous agentic traversal?
* How can we quantify the "entropy" of a microarchitectural design space to predict whether an RL agent will outperform a traditional solver?
* What metrics define when the dimensionality of cross-layer co-design (e.g., compiler + RTL + layout) exceeds the cognitive limits of a human engineering team?

**Theme 2: The Economics of Exploration**
* How do we mathematically formalize the tradeoff between exploring novel, paradigm-shifting architectures and the immense compute cost of simulating them?
* At what point does the compute cost of training a reinforcement learning agent for architectural search exceed the non-recurring engineering (NRE) savings of human labor?
* Can we develop predictive cost models that dynamically halt an agent's search when the marginal gain in PPA no longer justifies the required simulation compute?

**Theme 3: The Simulation vs. Prediction Tradeoff**
* Under what formal bounds should an autonomous agent safely escalate from fast analytical proxies to expensive, cycle-accurate oracles to prevent rank-inversions?
* How can surrogate search algorithms be formally constrained to proactively prune physically unroutable architectural states without requiring any cycle-accurate oracle invocation?
* What are the theoretical limits of multi-fidelity optimization when the highest-fidelity oracle requires days of execution time?

## Summarize the Scale Challenge

::: {.callout-design-principle title="The Bottleneck Theorem of Automation"}
Accelerating candidate generation actively harms project velocity if the limiting constraint is simulation capacity or expert review. AI assistance must target the specific bottleneck—whether search, tool execution, telemetry parsing, or evidence synthesis—rather than uniformly increasing the volume of unverified artifacts.
:::
