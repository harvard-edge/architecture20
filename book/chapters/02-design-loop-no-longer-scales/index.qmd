...
::: {.column-margin}
**Author's Note:** Norbert Wiener, the founding father of cybernetics, established that true control requires feedback. In this chapter, the pressure is that trusted feedback and rejection no longer keep pace with the number and complexity of architectural choices.
:::

```{=latex}
\abstract*{Architecture 2.0 is motivated by pressure inside computer architecture, not by enthusiasm for AI. The classical architecture loop remains valuable, but it strains as specialization, heterogeneous integration, memory movement, physical constraints, software velocity, and verification burden converge. The central bottleneck is not idea generation. It is trusted feedback, the ability to evaluate, reject, revise, and commit design choices at the speed and fidelity the problem now requires. That bottleneck makes AI-assisted architecture a loop-design problem because generative models can increase proposal volume faster than they increase trusted rejection capacity.}
```
...
**The lesson.** This is the scissors gap in a single result. When proposal arrival outpaces trusted disposition capacity, the workflow is rejection-bound, and faster generation widens the gap rather than closing it. Its claims earn trust through relevant evidence and checks capable of independent rejection, not through output volume.
:::

The delegation rule follows directly from that story. Automation is safe only
where rejection capacity has been designed first.
...
| Rejection-bound | Comparable proposals enter an adjudication stage faster than trusted dispositions can settle them, so its queue grows and weak candidates spill into expensive stages. | Lacking an early gate, an accelerator that leads a cheap proxy ranking is not rejected until an expensive check exposes its data-movement cost. | Proposal-arrival and disposition rates, queue behavior, rejection gates, and the negative-trace log. | Control admission, add validated early gates, or raise trusted disposition capacity, then record every disposition and its reason. |
...
The five bounds are working labels here, sharpened in
@sec-architecture-20-ontology into fields of the design-loop card.
@sec-the-rejection-bound turns the rejection bound into a scoped adjudication model,
but the pressure starts here. A stage widens the gap only when comparable
proposals enter faster than its trusted disposition path can settle them. Cheap
generation matters insofar as it raises admitted proposal arrival or increases
the evidence demand on that path. Once that becomes true, the
architecture problem is no longer how many candidates an AI system can imagine,
but how many unsupported claims the loop can dispose of before they consume
scarce evidence or human commitment.

Because this failure mode is the central problem, the remainder of this chapter is not a general technology-trend survey. The pressures matter here because they create one common failure mode. The architecture team can imagine more candidates than it can evaluate, reject, and justify.
...
explain why the loop itself, not just the artifact, becomes an object of
design.
...
organization reacts when feedback latency changes. In other words, the cadence
changed because the feedback and commitment costs of the physical process
changed.
...
later in this chapter make the pattern concrete. Autotuning[^fn-eda-autotuning] treats measurements
...
[^fn-eda-autotuning]: Autotuning is the process of automatically searching for optimal program or system parameters. It belongs in this loop discussion because it treats every measurement as a costly sample, the same feedback-scarcity discipline the design loop needs when it decides which candidates are worth evaluating and which to reject on cheaper evidence.
...
explicit. This could mean defining the interface, the rules, the workload, the tool contract, the
...
scale that makes the loop state itself the bottleneck.
...
Architecture 2.0 does not break the Iron Law. It treats AI-assisted search,
prediction, and critique as bounded method roles inside a loop that still
negotiates the hardware-software contract through compiler, ISA,
microarchitectural, and evidence constraints. The loop-level analogue is not the
Iron Law but an Amdahl-like cost model. @sec-loop-patterns-across-stack shows
that an adjudication stage becomes rejection-bound when proposals arrive faster
than trusted dispositions, then states the assumptions under which cheap checks
can reduce disposition cost. Architecture 2.0 elevates the
...
CPU/accelerator/System-on-Chip (SoC) choice also fixes interfaces, memory paths,
compiler/runtime assumptions, physical constraints, and an evidence schedule
that defines which states get cheap proxies, which get simulation, which escalate, and which
...
machine [@SchlanskerRau2000EPIC]. Written as a design-loop card, that bet is a
rejection gate waiting to fire.

- **State seen:** a wide static-issue target whose payoff needs sustained ILP
  close to the hardware's issue width, running general-purpose code with
  unpredictable control flow and memory aliasing.
- **Action allowed:** widen the machine and delegate parallelism extraction to
  the compiler.
- **Evidence required to commit:** compiler-*achieved* ILP measured on
  representative branchy workloads, not the ILP the machine could retire in
  principle.
- **Rejection rule:** reject the wide-static bet if achieved ILP stays far below
  the issue width. The machine was built to issue on the order of six operations
  per cycle, against sustained integer ILP that real compilers struggled to push
  past one or two.
- **Commitment owner:** the architects who signed the tapeout.

The rejecting evidence was gatherable before commit by building the compiler,
running real control flow, and measuring the ILP it actually extracts. A loop
with that gate would have forced compiler-achieved ILP into the commitment
review before tapeout. If measured ILP stayed far below the issue width on
representative workloads, the gate would require narrowing scope, changing the
organization, or explicitly accepting the risk. Instead the loop committed
silicon on the assumed capability, and the part underdelivered against simpler
rivals for a decade.
Intel's Larrabee made the same shape of bet a decade later, that software
...
software would let a simple core match x86 [@DehnertEtAl2003Transmeta]. Both
rested on a software capability asserted rather than measured.

That is the whole difference. Architecture 1.0 can name this failure in
hindsight.

**Lesson:** Architecture 2.0 makes the compiler-ILP evidence a gate the bet must clear before commit, so an architecture can no longer be committed on a software capability that was asserted rather than measured.
:::
...
move quickly. AI models change. Precision formats (e.g., FP8, INT4, block-scale formats[^fn-hw-block-scale-formats]) and sparsity patterns (the structured or unstructured zeros in weights or activations that hardware can skip) mutate wildly, meaning the hardware must place bets on data types years in advance. Compiler passes change. Kernel libraries
...
volume and churn rather than simply making the same workload cheaper. The
shift is already visible in practice. AI coding assistants had been adopted at
scale by the mid-2020s. The 2024 Stack Overflow Developer Survey reported that a
...
to repay the silicon. That repayment includes the embodied carbon manufactured into
the part up front, an irreversible commitment the holding workload must amortize, so
a candidate that is low-energy in operation can still fail a carbon gate if the
workload it bet on moves. The artifact remains the commitment target; the loop
...
about what evidence should count for a class of systems.[^fn-inst-nist] The loop artifact is a
...
reality. It is the layer where software intent, hardware mechanisms, and
physical constraints must be represented together before a commitment, without
replacing the specialist EDA and implementation flows that verify them.
...
overhead, or leakage has not necessarily improved the system, an instance of the *waterbed effect* [@Bode1945NetworkAnalysis], where pushing down one energy term inevitably causes another to bulge upward, just as pressing on a waterbed displaces water to the sides (@fig-waterbed-effect).
...
evidence. It is that each commitment boundary needs evidence appropriate to its
risk; otherwise the apparent speedup from AI-generated candidates is paid back
later as discarded work.
...
line, a scissors gap that @fig-scissors-gap makes explicit. On one blade is not
only candidate count but the coupled burden of choices, constraints, evidence,
software paths, and physical feasibility. On the other is not the ability to
think but the slower-growing capacity to evaluate, review, reject, and commit
with confidence. The first blade rises quickly. The second does not.
...
engineering capacity and commitment risk, not just compute cycles. This is the
economic reason rejection, not generation, must pace automated search.
Generated candidates are cheap only until they consume scarce verification,
tool, and review capacity.
...
**Lesson:** The missing rejection, an automated gate that could have caught the error early, not the missing entries, set the price.
:::
...
This bottleneck is not only a diagnosis. It has a service-capacity model. The
diagnostic table earlier in this chapter asked which part of one loop saturates
first; @sec-loop-patterns-across-stack makes the rejection-bound case precise
for a fixed adjudication stream. Generating proposals faster than trusted
feedback can keep up widens the scissors gap rather than closing it. An
Amdahl-like cost model captures the cases that cheap checks cannot settle and
that must escalate to trusted, higher-cost rejection capacity (see
@fig-rejection-bottleneck). That
capacity has two parts, and they age differently. Trusted disposition capacity
is a throughput limit, and validated, increasingly automated checks can keep
raising it. Who is *accountable* for the commitment if a claim is wrong is
not a speed limit at all; it is a governance floor that stays human even as
verification gets faster. @sec-loop-patterns-across-stack states the model with
explicit routing, audit, and cost assumptions; it does not present it as a
general law of search or useful design progress.
...
Their value is not only that they judge the past decision. Post-silicon
bring-up and field telemetry can recalibrate world models (a loop's internal predictive model of how the system behaves, developed in @sec-data-representations-world-models), invalidate stale
workload assumptions, and create evidence for the next loop, provided the loop
records the population, hardware stepping, software stack, sampling policy,
privacy filter, rollback trigger, and decision owner.
...
Designing a loop around the rate of trusted feedback is difficult exactly because architecture is not a standard machine-learning problem. Introductory supervised-learning pipelines and many benchmarked AI tasks often assume
...
@MattsonEtAl2020MLPerf]. Architecture 2.0 needs that kind of discipline, but
for loop state, feedback cost, invalid actions, rejected candidates, and human
commitment, not only for final scores.
...
provenance, drift, rejected alternatives, and a commitment boundary. The checklist version of this mismatch appears in @tbl-ai-assumptions-architecture-violations.
...
If the environment can enforce those boundaries, bounded AI roles become useful because the classical loop is under pressure. AI can help
...
loops that learn, record, reject, and justify architecture work.
...
This chapter asked what the loop must record, when architecture choices grow
faster than trusted feedback, for AI to help rather than merely add candidates.
The pressure is structural, not a matter of taste. Specialization,
heterogeneous integration, data movement, software velocity, and validation
burden converge on the classical loop. Between them opens a scissors gap, in
which generative methods raise the rate of plausible proposals far faster than
any tool raises the rate of trusted rejection.

That reframes the problem. The scarce resource in modern architecture is not
ideas but trusted feedback, the ability to evaluate, reject, revise, and commit
at the fidelity a decision demands. An AI that widens the design space without
widening rejection capacity makes the gap worse, not better. This is why
feedback, not generation, sets the pace of progress, and why the chapter treats
it as the bottleneck to design around.

AI assistance earns its place only inside a loop built to absorb it, one that
records state, preserves failed and rejected candidates as evidence, names what
can say no, and keeps a human accountable for each commitment. The goal is not a
larger search space but a loop disciplined enough that more proposals become
more progress instead of more noise.

::: {.callout-carry-forward}
- **Carry forward:** When plausible candidates are cheap, trusted rejection, not generation, is what bounds the design loop.
...
- **Up next:** AI helps only when the loop records state, failures, evidence, rejection authority, and each commitment's human owner; @sec-architecture-20-ontology names the minimum loop state that makes that pressure inspectable.
:::
