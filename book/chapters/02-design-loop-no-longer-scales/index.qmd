...
chapter climbs them in turn. Microarchitecture and multicore execution add
...
The climb starts with the levers already in every architect's toolkit. A deeper pipeline, a more aggressive branch predictor, a speculative scheduler, or a larger cache hierarchy each recovers performance, and each adds corner cases a fair comparison must cover. A prefetcher that helps one access pattern can pollute a cache under another, and a speculation policy that raises throughput can raise energy and expose a side channel.

Multicore execution adds another layer on top of that. Coherence protocols, memory-ordering rules, synchronization costs, and workload partitioning all shape how a candidate behaves under a real workload, and none of them shows up in a single-thread proxy. Even at this familiar rung, before any specialized block is added, the information a comparison must carry has already outgrown a single number. The newer levers only widen that gap.

## Specialization Multiplies the Choices

Specialization is the next lever, and it changes the climb. Where microarchitecture and multicore mechanisms tune a general-purpose machine, specialization tailors the hardware to a particular workload, which changes both the metrics that matter and the number of choices the architect must evaluate. A battery- and thermally limited mobile part and a warehouse-scale system bounded by power delivery and total cost of ownership (TCO) demand different measures of efficiency [@BarrosoHolzleRanganathan2019DatacenterAsComputer], and a specialized block is only worth judging against one such target.
...
## Composition: Systems-on-Chip and Chiplets
...
Software moves faster than the hardware built to run it. New models, precision formats, and software stacks appear within the span of a single silicon program, while a chip design cycle still runs for years. Apple's release intervals have compressed from about 19 to 7 months, while NVIDIA's datacenter GPUs hold a steady two-year cadence (@fig-apple-silicon-cadence). Even the faster pace still trails the software the hardware must run.

```{python}
#| label: fig-apple-silicon-cadence
#| fig-cap: |
#|   **Hardware release cadence still trails software.** Apple's M-series intervals compress from 19 to 7 months while NVIDIA's datacenter GPUs hold a steady two-year cadence, both slower than the software stacks they run. Labels give the month gap between consecutive announced parts.
#| out-width: "100%"
...
The fit reads the mismatch directly. Training compute has climbed more than fourfold a year for over a decade, far outpacing the one- to two-year cadence at which the silicon underneath it turns over, so a design tuned to one model scale meets a different workload by the time it reaches silicon.
