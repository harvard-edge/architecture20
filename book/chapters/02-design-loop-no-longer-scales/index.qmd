...
**The claim.** In AlphaChip, learned placement was reported to compress a months-long floorplanning loop to hours, and generating strong-looking placements only got faster [@MirhoseiniEtAl2021GraphPlacement]. That is the half of the story everyone remembers.

**The gap.** What did not keep pace was trusted, independent rejection. A candidate that looked excellent on the method's fast proxy still had to be checked against the metrics that decide a real chip, and an independent reassessment that tried to run that check reported baselines the result had not cleared in that setup [@ChengEtAl2023AssessmentRL]. The generation side sped up while the accountable rejection side did not, and the space between them is where the dispute lives.
...
Itanium's EPIC architecture rested on one claim, that a compiler could
...
overhead, or leakage has not necessarily improved the system, an instance of the *waterbed effect* [@Bode1945NetworkAnalysis], in which suppressing one energy term forces another to rise (@fig-waterbed-effect).
...
timing analysis, design-rule checking, layout-versus-schematic checking, EM/IR,
