...
::: {.callout-field-note title="Field Note: Predictive Test Selection and Hyperband"}
**Observation:** The "Scissors Gap." AI can generate an infinite number of candidate designs, but the high-fidelity simulators and human experts required to verify them are strictly finite. The bottleneck is evaluation capacity, not generation.

**Context:** In Software Engineering (Meta/Facebook, ICSE 2019), running every CI test on every commit bankrupts the infrastructure. Meta used AI to predict which tests were likely to fail and skipped the rest (Predictive Test Selection). Similarly, in Neural Architecture Search (Li et al., 2017), Hyperband treats evaluation strictly as a budget-allocation problem, aggressively culling weak candidates using cheap proxies and saving expensive evaluations only for the survivors.

**Lesson:** This proves that using AI as an "intelligent filter" to triage expensive evaluations yields massive throughput gains, showing architects exactly how to optimally allocate their cycle-accurate simulation budgets.
:::
...
Recognizing the ISA as a comprehensive software contract forces the design loop to evaluate hardware and software dependencies concurrently rather than in isolation.
:::
