...
Architecture work is getting harder to finish, not because good ideas are scarcer but because each new design must hold more of the system at once. This imposes severe cognitive limits on the architect. The explosion of parameters across microarchitecture, power delivery, and software optimization outpaces the human capacity to hold the system state in a single mind, explaining why traditional heuristics fail. A modern project spans more design information, more software and physical constraints, more possible designs, and more expensive forms of evaluation than an earlier one, and it must keep all of that consistent long enough to defend a single decision. Much of the complexity an architect once inherited from the next process node now has to be built and managed by hand.
...
As those layers accumulate, the difficulty is not simply that modern projects can generate more candidates.
...
Experienced architects also carry important context implicitly. They know which workloads were excluded, which tool settings are trusted, why a previous candidate failed, and what decision is actually under review. Machine learning models, however, are highly vulnerable to "catastrophic forgetting" [@McCloskeyCohen1989Catastrophic]—the tendency to abruptly forget previously learned information upon learning new constraints. In an agentic loop, an AI system that lacks explicit memory management may "forget" a critical thermal constraint while obsessively optimizing a newly introduced timing violation. It may produce a coherent analysis of a different comparison from the one the architect intended. The system and reviewer therefore need access to the explicit workload and interface assumptions, tool settings, failed alternatives, and decision under review.
...
Each of those questions widens the space of specialized options an AI system might explore, but the width is not the hard part. For the XR subsystem the trap is concrete. A vector unit sized for the throughput of one SLAM matrix-multiplication kernel can post the best performance per watt in the search and still be the wrong block, because the perception model beside it has a different tensor shape and reuse pattern, and because the display holds a fixed per-frame deadline that a throughput-optimal design can miss while a slower design meets it. Specialization ties a candidate's worth to one workload's efficiency measure, so a method that optimizes performance per watt for the wrong workload, or that saves area while missing the display deadline, can rank candidates confidently in a way that means nothing for the target. AI can enumerate and score specialized options quickly; it helps only when the measure it optimizes is the one the deployment rewards and the architect has fixed which constraint a candidate may not violate. A larger search aimed at the wrong measure produces more candidates tuned for a workload no one will deploy.

## Composing Systems-on-Chip and Chiplets
...
the power limits that ended Dennard scaling. As the cost per transistor stopped
falling below roughly the 28\ nm generation, large monolithic dies at the
...
blocks that do not need the leading edge. But a chiplet system is not simply a bigger board-level system
...
The difficulty of composing systems-on-chip extends far beyond partitioning and package constraints. Modern SoCs integrate dozens to hundreds of heterogeneous IP blocks—CPU clusters, GPUs, NPUs, memory controllers, and Network-on-Chip (NoC) interconnects—often sourced from different vendors with varying interface standards and incomplete documentation. This creates a severe integration bottleneck where architectural validation breaks down. Subtle mismatches in coherence protocols, unspoken timing constraints, and system-level deadlocks frequently emerge only when the full SoC is assembled. An AI method that optimizes a single accelerator block but cannot synthesize correct-by-construction glue logic or verify cross-domain coherence protocols fails to solve the actual labor bottleneck scaling modern system design.
...
The binding limit often becomes the finite capacity of high-fidelity simulators and human experts needed to check the many candidate designs that AI can generate. AI's most defensible role here is triage, screening weak candidates with cheap proxies and reserving expensive cycle-accurate simulation for designs that might change the final decision.

### The Emulation and Prototyping Bottleneck

Even when a candidate survives simulation, software bring-up requires running trillions of cycles—a scale that software-based simulation cannot reach. Currently, hardware emulation and FPGA prototyping are the only viable platforms for this scale of evaluation. However, these platforms introduce their own severe bottlenecks. They are notoriously difficult to compile to, often requiring days for a single build, and offer incredibly low visibility for debugging compared to software simulators. The architect is trapped between slow, high-visibility simulation and fast, low-visibility emulation. An AI-assisted loop must help bridge this gap, perhaps by predicting which traces to capture or by steering emulation runs, rather than blindly generating hardware that crashes opaquely on an FPGA.

### Advanced Packaging and "Known Good Die" Testing

The composition of systems into chiplets introduces physical and economic penalties that narrow the viable design space. In 2.5D and 3D integration, such as TSMC CoWoS or Intel Foveros, discovering a defective chiplet only after packaging ruins the entire expensive assembly, including the silicon interposer and pristine High Bandwidth Memory (HBM) stacks. This economic penalty shifts the burden of testability leftward, demanding high-coverage, pre-bond test patterns at the bare-die stage. An AI agent proposing a novel chiplet partition cannot merely optimize for interconnect bandwidth or power; it must also ensure that the topology does not obscure test access, treating Known Good Die (KGD) constraints as a strict filter on valid architectures.

### Reliability, Fault Tolerance, and Safety

As physical constraints multiply, a candidate's viability extends beyond performance and power to encompass soft errors, silicon aging phenomena such as Negative Bias Temperature Instability (NBTI) and Hot Carrier Injection (HCI), and manufacturing variations. In specialized domains, such as automotive systems governed by ISO 26262 or aerospace applications, reliability is a primary evaluation bottleneck. Evaluating these metrics requires statistical fault injection and aging simulations, rather than deterministic cycle counts. A generative AI that hill-climbs solely for peak instructions per cycle will produce brittle designs. It must instead reason over statistical proofs of reliability, acknowledging that peak performance is irrelevant if the system fails in the field.

### Hardware Security and Isolation Verification

Security evaluation adds a further dimension that cheap performance proxies cannot capture. Systemic security involves verifying information flow and isolation across complex privilege levels and untrusted third-party intellectual property (IP). While a functional bug causes a crash, a security bug causes a breach, fundamentally altering the stakes of the design loop. When an AI optimizer aggressively shares speculative state buffers or execution units between secure and non-secure domains to save power, it risks inadvertently opening side-channel vulnerabilities. Because proxies cannot measure security guarantees, the evaluation loop must incorporate expensive formal methods or extensive penetration testing to verify that a structurally optimal candidate does not violate information flow.

### Power Delivery Networks and Thermomechanical Stress

Beneath the architectural organization lies the stark physical reality of dark silicon. Pushing hundreds of amps into a tightly packed chiplet package creates localized thermal hotspots and severe Power Delivery Network (PDN) stress. Moving data does not just consume average energy; it can fail signoff due to thermomechanical warping or localized voltage droop even if the aggregate energy budget appears acceptable. An AI cost model must represent spatial power distribution and layout-aware congestion. If an agent relies purely on aggregate power proxies, it will confidently hallucinate feasible-looking designs that would physically warp the substrate, wasting expensive downstream synthesis cycles.

## Hardware Proposals Include an Executable Software Path

Triage decides which hardware candidates deserve expensive evaluation, but not whether software can use them at all. The available choices extend beyond hardware. An accelerator, vector unit,
...
AI workloads provide an unusually visible example of changing scale and shifting computation. Traditional convolutional networks performed fixed, deterministic computation, but modern generative models perform dynamic, iterative computation where memory access patterns change constantly due to growing key-value caches, variable sequence lengths, sparse mixture-of-experts routing, and the split between prefill and decode phases that serving systems now place on separate hardware. An analysis of Epoch AI's model database estimated roughly four- to fivefold annual growth in training compute for recent notable and frontier models through May 2024 (@fig-training-compute-growth), with results that depend on model class, outliers, and the form of the fit [@SevillaRoldan2024ComputeGrowth]. Compute growth is not itself
...
**The claim.** AMD's Bulldozer generation (Zambezi, 2011) bet that many simpler integer cores sharing a front-end and a floating-point unit, pushed to high clock speeds, would win on the numbers buyers and reviewers compared most directly, core count and clock frequency.
...
**The lesson.** Rank a candidate on the metric its workload rewards, not the number that is easiest to count. In silicon that gap is expensive to discover, because it appears only after the design has shipped. Restoring competitive per-thread performance took a clean-sheet microarchitecture, Zen, several years later.
:::
...
and downstream physical-design checks [@ChengEtAl2023AssessmentRL;
@GoldieEtAl2024ThatChipHasSailed]. None of those comparisons measured
...
### Open Source Hardware and IP Integration

A final layer of complexity in composition is the reliance on third-party IP. Modern SoCs are not built entirely from scratch; they integrate proprietary blocks such as ARM cores, PCIe PHYs, and DDR controllers. The design loop stalls when architects, and by extension AI agents, must treat these IPs as black boxes, relying on static vendor datasheets and opaque models rather than fully executable, open-source RTL. An AI agent cannot co-optimize the internals of proprietary IP; it must instead navigate interface boundaries and synthesize glue logic based on incomplete, non-executable data. This opaque boundary forces the loop to rely on integration-level testing rather than cycle-accurate internal metrics, significantly complicating the verification of timing and power constraints across the SoC.
