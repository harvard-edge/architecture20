...
Because AI can generate a vast number of candidate designs, the true bottleneck becomes the finite capacity of high-fidelity simulators and human experts required to verify them. AI must route candidates between fast proxies and cycle-accurate simulators to save compute for designs that might actually change the final decision. In software engineering, for example, running every continuous integration test on every commit bankrupts infrastructure; instead, systems can predict which tests are likely to fail and skip the rest [@MachalicaEtAl2019PredictiveTestSelection]. Neural architecture search applies a similar principle: Hyperband treats evaluation strictly as a budget-allocation problem, aggressively culling weak candidates using cheap proxies and saving expensive evaluations only for the survivors [@LiEtAl2018Hyperband].
...
Physical effects create tight, non-linear coupling. Data movement and thermals act as hard physical walls; if they are not modeled early, candidates that look great on compute will fail at implementation.
...
Architecture has always relied on feedback, yet today's projects must synchronize unprecedented amounts of information. Coupled specialization and chiplet choices, evolving software stacks, and stringent physical constraints generate immense search spaces. More critically, they generate evidence requirements that easily outpace a team's simulation capacity and expert review bandwidth. We need AI because manual coordination and traditional heuristics can no longer keep up with the scale of validation and evaluation required to confidently close a design decision.
