...
Because AI can generate a vast number of candidate designs, the true bottleneck becomes the finite capacity of high-fidelity simulators and human experts required to verify them. To close this gap, AI can act as an intelligent filter that yields massive throughput gains by triaging expensive evaluations. In software engineering, for example, running every continuous integration test on every commit bankrupts infrastructure; instead, systems can predict which tests are likely to fail and skip the rest [@MachalicaEtAl2019PredictiveTestSelection]. Neural architecture search applies a similar principle: Hyperband treats evaluation strictly as a budget-allocation problem, aggressively culling weak candidates using cheap proxies and saving expensive evaluations only for the survivors [@LiEtAl2018Hyperband]. By adopting this approach, architects can optimally allocate their cycle-accurate simulation budgets to evaluate an otherwise unmanageable search space.
