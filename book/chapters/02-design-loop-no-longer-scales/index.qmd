...
Itanium's EPIC architecture bet that a compiler could expose enough
instruction-level parallelism statically to feed a very wide machine
[@SchlanskerRau2000EPIC]. The hardware shipped; the compiler evidence did not.
Extracting that parallelism across real control flow and memory aliasing proved
harder than the design assumed, and the part underdelivered against simpler
out-of-order rivals for a decade. The architectural claim rested on a
software-loop capability that was asserted, not demonstrated. Read through
Architecture 2.0, the loop committed silicon on feedback it never gathered. The
compiler was a rejection authority the design should have consulted before the
bet, not after it.
:::
...
trusted feedback cannot keep up widens the scissors gap rather than closing it. Just as Amdahl's Law dictates that sequential code bottlenecks multicore speedups, the Architecture 2.0 loop has its own Amdahl's Law. No matter how fast the generative AI proposes candidates, the throughput of the design loop is strictly bounded by the serial fraction of trusted, independent rejection capacity, of which accountable human judgment is the irreducible floor (see @fig-rejection-bottleneck). @sec-loop-patterns-across-stack makes that bound precise and shows how much of it a cheap, independent, often-automated rejector can discharge before it reaches the human floor.
