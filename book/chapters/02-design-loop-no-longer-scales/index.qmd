...
For decades, computing advanced through a layered stack of technology, architecture, and optimization, often abbreviated TAO. Device scaling supplied faster transistors, microarchitecture organized them, and compilers mapped software onto the result. The end of Dennard scaling added a fourth layer, specialization, extending TAO to TAOS, because physical limits at the device level now shape every layer above rather than receding beneath them (@fig-tao-vs-taos).
...
The binding limit becomes the finite capacity of high-fidelity simulators and human experts needed to check the many candidate designs that AI can generate. AI has to act as triage, screening weak candidates with cheap proxies and reserving expensive cycle-accurate simulation for designs that might change the final decision.
...
Because hardware and software must be explored together, AI can help with the work between them. Earlier hardware-software co-design relied on negotiation between separate teams and often settled on compromised interfaces. A joint search can vary compiler schedules alongside hardware parameters, generating the intermediate representation and the hardware RTL together, so a strong combination is not missed because the two were tuned in isolation. Separate AI methods might propose the hardware organization, compiler schedule, runtime configuration, and test plan, provided their assumptions are reconciled.
...
Software moves faster than the hardware built to run it. New models, precision formats, and software stacks appear within the span of a single silicon program, while a chip design cycle still runs for years. Apple's and NVIDIA's compressing release intervals (@fig-apple-silicon-cadence) show hardware teams shortening that gap where they can.

![**Silicon release intervals are compressing.** Apple and NVIDIA increasingly ship year-over-year architecture and platform updates, shortening the hardware cycle relative to software change.](images/ch2_apple_silicon_cadence.png){#fig-apple-silicon-cadence width="100%" fig-alt="Accelerating Pace of Apple Silicon and NVIDIA Releases timeline"}
...
The public AlphaChip debate illustrates a narrower point about bottlenecks. We
use it here only to ask whether faster candidate production reveals what is
slowing architecture work, not to judge learned placement.

::: {.callout-war-story title="Candidate speed did not reveal the bottleneck"}
**The claim.** The 2021 AlphaChip paper reported that a reinforcement-learning method could produce macro placements in hours [@MirhoseiniEtAl2021GraphPlacement].

**The gap.** The later public disagreement turns on differences in cases, compute budgets, implementations, and downstream physical-design checks. None of those comparisons measured candidate-arrival rates, review queues, or why candidates were rejected [@ChengEtAl2023AssessmentRL; @GoldieEtAl2024ThatChipHasSailed].

**The lesson.** Faster placement demonstrates candidate-production speed. It does not establish that reviewer capacity is the bottleneck or identify which part of the project limits progress.
:::

The technical disagreement remains open, and the full evidence dispute belongs to @sec-proxy-mismatch-metric-gaming-and-calibration. The diagnostic point is that a fair
