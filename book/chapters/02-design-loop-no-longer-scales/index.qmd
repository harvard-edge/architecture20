...
Every architecture project follows a fundamental design loop where an architect frames a question, proposes a candidate, evaluates it with workloads and tools, and revises or rejects it in light of the results. When designing a new cache replacement policy, the architect simulates it against standardized benchmarks, analyzes the miss rate and latency, and tweaks the policy until the performance targets are met in a process that remains fundamentally sound.
...
The mobile XR subsystem example makes that difficulty concrete by asking an architect to compare compute organizations for a real-time mobile XR workload within a 3\ W, 3\ nm-class mobile envelope. A useful answer must connect the workload and
...
For decades, computing progressed through the predictable "TAO" stack of Technology, Architecture, and Optimization. Device scaling provided the foundation, architecture supplied the organization, and compilers optimized the software, but the end of Dennard scaling forced a transition to "TAOS" by adding an 'S' for Specialization (@fig-tao-vs-taos).
...
Specialization also changes the range of choices that an architect must consider, as a battery- and thermally limited mobile part and a warehouse-scale system bounded by power delivery and total cost of ownership (TCO) demand different measures of efficiency [@BarrosoHolzleRanganathan2019DatacenterAsComputer]. This specialization changes both the metrics that matter and the number of choices the architect must evaluate.
...
Even a narrow comparison can contain many combinations, and the following arithmetic is illustrative rather than measured. Suppose an architect explores an accelerator and memory subsystem for one XRBench workload family [@KwonEtAl2023XRBench] and considers five compute organizations, four vector or accelerator interface choices, six memory hierarchy choices, four interconnect choices, three voltage/frequency policies, and three compiler/runtime policies. The product is
...
or take far longer before silicon measurements are available. A simple processor with just 300 latches has $2^{300}$ possible states, exceeding the estimated number of atoms in the observable universe, while a modern processor has billions of state elements. This state space explosion prevents brute-force search of the architectural space.
...
This distinction is especially critical when transitioning from **Parametric Grid Search** to **Open-Topology Generation**. Traditional Design Space Exploration (DSE) tweaks predefined numerical knobs (e.g., cache sizes, queue depths), which is fast but bounded by the existing architecture. AI models promise a shift toward generative open-topology search—dynamically re-architecting bus connections or injecting new IP blocks entirely. While vastly more powerful, open-topology generation produces an explosive number of illegal, untestable states. The challenge shifts from merely generating novel designs to tightly constraining the generative model so that it only proposes valid, verifiable RTL topologies.
...
The true bottleneck becomes the finite capacity of high-fidelity simulators and human experts required to verify the vast number of candidate designs that AI can generate. AI must act as a triage system that aggressively culls weak candidates using cheap proxies and saves expensive cycle-accurate simulations for designs that might actually change the final decision.
...
Because hardware and software must be explored together, AI can help with the work between them. Earlier hardware-software co-design relied on human-driven negotiation between isolated teams and often resulted in compromised interfaces. A joint AI optimization space can explore compiler schedules alongside hardware parameters, generating the intermediate representation and the hardware RTL concurrently to find global optima that human teams would miss. Separate AI methods might propose the hardware organization, compiler schedule, runtime configuration, and test plan under the direction of a central coordinator.
...
The sheer velocity of the software ecosystem is forcing a profound shift in hardware design cadence. While historical chip design cycles spanned several years, the rapid emergence of new models and software architectures has accelerated the need for faster silicon turnouts. Accelerated roadmaps like NVIDIA's annual architecture targets and Apple Silicon's consistent generation upgrades (@fig-apple-silicon-cadence) demonstrate that traditional slow design loops have become a major liability for keeping up with software needs.
...
AI workloads provide an unusually visible example of changing scale and shifting computation. Traditional convolutional networks performed fixed, deterministic computation, but modern generative models perform dynamic, iterative computation where memory access patterns change constantly due to growing key-value caches and variable sequence lengths. This shift from static to dynamic workloads demands adaptive hardware and software systems that can reconfigure on the fly. An analysis of Epoch AI's model database estimated roughly four- to fivefold annual growth in training compute for recent notable and frontier models through May 2024, with results that depend on model class, outliers, and the form of the fit [@SevillaRoldan2024ComputeGrowth]. Compute growth is not itself
...
Benchmarks must evolve to keep changing workloads comparable because AI methods require community-maintained datasets that reflect actual design constraints. Without them, AI systems will confidently optimize for obsolete targets.
...
Physical effects like data movement and thermals act as hard walls that cause candidates to fail at implementation if they are not modeled early, even if those candidates look great on compute proxies.
...
Hardware is fundamentally irrevocable because bugs become permanent once silicon tapes out. The 1994 Pentium FDIV flaw shows why early checks matter, as a defect arising from five omitted lookup table entries [@Edelman1997PentiumDivision] led Intel to report a \$475M pretax charge for replacement and associated costs [@Intel1995PentiumCharge]. More recently, the Meltdown and Spectre vulnerabilities exposed deep architectural flaws in speculative execution that could not be fully patched in software without severe performance penalties, while AMD's Bulldozer microarchitecture made a heavy bet on integer core sharing that failed to deliver expected single-thread performance and required years to recover with the completely new Zen architecture. These episodes show how architectural flaws become permanent, expensive liabilities after a design has shipped, leaving only replacement, support, and public remediation rather than a local software revision.
...
Architecture has always relied on feedback, but today's projects must coordinate much more information. Coupled choices, evolving software, and strict physical constraints create evidence requirements that outpace a team's simulation capacity and review bandwidth. AI is increasingly necessary because manual coordination can no longer keep up with the scale of validation required to confidently close a design decision.

To meaningfully accelerate this process, AI-assisted architecture must directly address five practical bottlenecks. Relevant information must be available and current, meaning AI must ingest and reconcile workloads, software assumptions, interfaces, and physical constraints to prevent stale data from invalidating results. The design space must contain valid choices, requiring AI to navigate the coupled dependencies of hardware, software, packaging, and physical limits to propose genuinely implementable designs. Because decisive feedback may remain expensive, AI surrogates and predictors must intelligently triage candidates and reserve costly, high-fidelity simulation and physical implementation for designs most likely to succeed. Architects and reviewers have finite time, so AI must help reviewers by isolating disagreements, filtering weak candidates early, and summarizing trade-offs rather than merely flooding the queue with unverified options. Finally, the evidence must support a defined decision, requiring AI to map its outputs directly to the architect's required actions and organize the exact evidence needed to move from exploration to implementation.
