...
AI methods provide a powerful way to manage this explicit and implicit context. Language models and retrieval systems can suggest cache configurations, prepare simulator runs, group failures, and compare new results with earlier experiments [@Lin2023LLMAssistedDesign]. This allows an automated assistant to handle the repetitive mechanics of the design loop. We need to think about how we structure explicit engineering artifacts so that an automated optimizer can reliably execute and track these comparisons, keeping the loop moving efficiently.
...
::: {.callout-war-story title="The silent SoC deadlock"}
**The claim.** Independently verified IP blocks can be seamlessly integrated into a working System-on-Chip.

**The gap.** In a notorious multi-vendor IP integration failure, two blocks with perfectly verified individual behavior were combined on the same interconnect. A mismatched Network-on-Chip (NoC) credit-return protocol between the two individually verified IP blocks created an unresolvable cycle of dependencies.

**The lesson.** This undocumented interface quirk caused a silent system hang that eluded simulation and took months to debug in post-silicon bring-up, proving that system-level deadlocks often only emerge when the full SoC is assembled.
:::
...
::: {.callout-war-story title="The AVX-512 frequency penalty"}
**The claim.** Adding ultra-wide 512-bit vector units to general-purpose CPU cores would deliver massive leaps in peak floating-point throughput.

**The gap.** When these dense vector units woke up and began executing heavy matrix workloads, they drew so much current so quickly that the resulting voltage droop (di/dt) threatened to crash the processor.

**The lesson.** To survive these transients, early AVX-512 implementations were forced to implement aggressive, hard-coded frequency downclocking specifically to survive dense matrix workloads, erasing much of the promised peak performance gains.
:::
...
Beyond the initial design costs, physical and manufacturing limiters have become first-class constraints. Modern systems are increasingly bottlenecked not by logic density, but by power delivery networks (PDN), thermal dissipation ceilings, and the complex yield curves of 2.5D and 3D packaging. A chiplet-based architecture might look optimal on a spreadsheet, but if the interposer yields at 50% or the concentrated logic creates uncoolable "hot spots," the design will fail economically or physically in the real world. For the XR Lighthouse, the 3 W limit is not just a battery constraint; it is a strict skin-temperature constraint for a face-worn device. A dense MAC array might pass average TDP checks but create a localized hotspot that makes the headset physically unwearable.

We must also face the final physical constraints: Design Rule Checks (DRC). At 3nm, there are over 10,000 highly complex geometrical rules (multi-patterning coloring, via enclosure, min-metal spacing) [@ASML2022AdvancedPatterning]. If a floorplanner shifts a macro block, it might instantly create thousands of discrete DRC violations. If the automated loop cannot parse these spatial markers and fix them, the output requires manual cleanup.

::: {.callout-war-story title="The DRC whack-a-mole"}
**The claim.** A minor late-stage macro block shift to improve timing should be a quick fix.

**The gap.** In a 3nm tapeout trench, a physical design team attempting to resolve a single via-enclosure violation found that the layout tool rippled the changes outward, instantly spawning three new violations in adjacent polygons.

**The lesson.** The team became stuck in a multi-week loop of fixing one error only to spawn three more, illustrating exactly why human-guided feedback loops break at this scale.
:::
