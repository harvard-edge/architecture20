...
The human architect remains the final judge, orchestrating the Evaluation Harness to validate these proposals against physical silicon constraints like STA and DRC. The first task is to identify what is actually slowing the work, delegating rapid search to the AI while reserving rigorous evaluation for the architect.
...
## Frame Architecture State and Evaluation Results
...
Machine learning models, however, are highly vulnerable to "catastrophic forgetting" [@McCloskeyCohen1989Catastrophic]—the tendency to abruptly forget previously learned information upon learning new constraints. In an agentic loop, an AI searcher may forget a critical thermal budget or DRC rule while obsessively optimizing a newly introduced STA timing violation. It may produce a coherent analysis of a different comparison from the one the architect intended. The human architect must orchestrate an Evaluation Harness that explicitly tracks workload assumptions, interface constraints, tool settings, and physical limitations to maintain the epistemic trust boundary.

### Make Inputs and Checks Explicit
...
### Connect More Design State for Modern Comparisons
...
## Manage Microarchitecture and Multicore Complexity
...
## Control the Specialization Explosion
...
## Compose Systems-on-Chip and Chiplets
...
### Bridge the Emulation and Prototyping Bottleneck
...
### Enforce "Known Good Die" Testing Constraints
...
### Evaluate Reliability, Fault Tolerance, and Safety
...
A generative AI acting as a searcher that hill-climbs solely for peak instructions per cycle will produce brittle designs. Instead of expecting the AI to independently reason over reliability, the human architect must orchestrate an Evaluation Harness. This harness enforces the epistemic trust boundary, rejecting brittle candidates based on rigorous physical constraints, thermal budgets, and statistical proofs before any silicon commitment is made.

### Verify Hardware Security and Isolation
...
### Model Power Delivery Networks and Thermomechanical Stress
...
## Bind Hardware Proposals to an Executable Software Path
...
## Track Software Changes Before and After Release {#sec-software-changes-faster-than-silicon}
...
## Anchor Early Results to Stronger Evidence {#sec-physical-constraints-move-into-architecture}
...
### Budget for Expensive Stronger Evaluation {#sec-stronger-evaluation-expensive}
...
### Mitigate the Verification Bottleneck (State Space Explosion)
...
## Diagnose Combined Pressure on the Design Loop
...
## Apply AI Assistance at Architecture Bottlenecks {#sec-feedback-and-verification-become-the-bottleneck}
...
### Target AI at the Architecture Bottleneck
...
### Integrate Open Source Hardware and IP
