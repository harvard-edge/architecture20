...
```{=latex}
\abstract*{This chapter explains why familiar architecture work has become harder to complete and where AI may help. More information and feedback must stay synchronized, specialization and chiplets create coupled choices, a hardware proposal needs an executable software path, software keeps moving before and after release, and stronger evidence can reverse an early result. These pressures compound into missing information, unhelpful choices, scarce evaluation, overloaded review, and unclear decisions. AI is useful only when it reduces one of those burdens without weakening the comparison or moving the work elsewhere.}
```
...
- **Explain** why a hardware proposal needs an executable software path and why that path can change before and after release;
...
### Design State Required for Modern Comparisons
...
AMD's MI300 family makes the coupling concrete. MI300A combines CPU and GPU
compute dies with shared high-bandwidth memory, while MI300X uses additional GPU
compute dies in place of the CPU dies. The product variants change compute,
memory, communication, packaging, and the software-visible machine together.
...
## A Hardware Proposal Includes an Executable Software Path
...
Intel's Larrabee moved rasterization, interpolation, and post-shader blending
into software on many x86 cores while retaining fixed-function texture
filtering where software was too slow [@SeilerEtAl2008Larrabee]. Transmeta's
...
::: {.callout-design-principle title="Reduce the work that limits the decision"}
Assign AI to a specific architecture task, name the engineering work it should
save, and retain an independent check for the claim. Then measure the whole
comparison again. A local speedup is not an improvement if it creates more
evaluation or review work elsewhere.
:::
...
explicit. Modern projects must synchronize more of those artifacts, make
coupled specialization and chiplet choices, establish an executable software
path, revisit assumptions as software changes, and reserve scarce engineering
capacity for evidence strong enough to reverse an early result. Together, these
pressures can keep the work from reaching a defensible decision in several
different ways.
