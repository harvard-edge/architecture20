...
\usepackage{xcolor}
% Footnote margin-mark styling: a small top-left corner bracket (a little "L")
% around just the sidenote number, which sits in its nook; the note text then
% flows normally. A short top rule and a short left rule, both ~number height,
% meet at the corner. Targets sidenote footnotes (\@sidenotes@placemarginal);
% .column-margin asides route through the \marginnote redefinition and stay
% unbracketed.
\newcommand{\archsnframe}[1]{%
  {\color{archtwoline}\rule[1.15ex]{5pt}{0.4pt}\hspace{-5pt}\rule[-0.15ex]{0.4pt}{1.35ex}}%
  \hspace{2pt}#1%
}
\makeatletter
...
\SetArchTwoPreviewVersion{Preview v0.1.0}
