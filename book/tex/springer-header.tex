...
% Let figures float and pack: loosen the placement fractions so a float can
% fill up to 90%% of a text page and only 7%% running text is required, and
% allow more floats per page. Reduces float-induced page whitespace.
...
\newcommand{\archsnframe}[1]{%
  {\color{archtwoblue}\rule[-0.14ex]{0.5pt}{0.78ex}\rlap{\rule[-0.14ex]{6pt}{0.5pt}}}%
  \kern1.8pt#1%
}
\makeatletter
...
% Release identity: the publish pipeline injects \ArchTwoReleaseVersion (from
% ARCH2_VERSION, via cli/arch2.py header-includes) so the PDF matches the site;
% local builds fall back to the static default below. \SetArchTwoEditionLabel
...
\providecommand{\ArchTwoReleaseVersion}{Development build}
\SetArchTwoEditionLabel{\ArchTwoReleaseVersion}
...
\newcommand{\ArchTwoRunningFooter}{%
  \vbox{%
    ...
    \vskip 3.4pt%
    \hbox to\textwidth{%
      \normalfont\sffamily\tiny\color{archtwomuted}Architecture 2.0 | \ArchTwoEditionLabel%
      \hfil
      \ArchTwoSiteURL%
    }%
  }%
...
\def\@maketitle{%
  ...
  \begingroup
    ...
    {\large\@author\par}%
    \vspace{0.10in}%
    {\small\bfseries\color{archtwoblue}\ArchTwoEditionLabel\par}%
    \vspace{0.62in}%
    ...
    \vspace{0.14in}%
    {\small\color{archtwomuted}\ArchTwoSiteURL\par}%
    \ifx\@date\@empty\else
      \vspace{0.10in}%
...
% Center part titles instead of using the class default
\makeatletter
\patchcmd{\@part}{\raggedleft}{\centering}{}{}
\patchcmd{\@spart}{\raggedleft}{\centering}{}{}
\makeatother
