...
if __name__ == "__main__":
    app(prog_name="arch2")def preview(
    chapter: str = typer.Argument(..., help="Chapter name (e.g. 01-moonshot) or path to preview."),
    html: bool = typer.Option(False, "--html", help="Build HTML."),
    pdf: bool = typer.Option(False, "--pdf", help="Build PDF."),
    epub: bool = typer.Option(False, "--epub", help="Build EPUB."),
) -> None:
    ...
    chosen = [t for t, on in (("html", html), ("pdf", pdf), ("epub", epub)) if on]
    if len(chosen) > 1:
        console.print("[yellow]Warning: Quarto preview only supports one format at a time well. Using the first one.[/yellow]")
    ...
    try:
        ...
        content = re.sub(r'\n\s*chapters:\n(?:.*?)(?=\n\s*appendices:)', chapters_replacement, content, flags=re.DOTALL)
        
        appendices_replacement = "\n  appendices: []"
        content = re.sub(r'\n\s*appendices:\n(?:.*?)(?=\nwebsite:)', appendices_replacement, content, flags=re.DOTALL)
        ...
        if backup_yml.exists():
            import shutil
            shutil.move(backup_yml, quarto_yml)_ import annotations
